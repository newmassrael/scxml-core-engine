...
| `script_engine_language` | optional string, `"lua"` \| `"ecmascript"` | **Which kind** of engine the host must supply. Present when `needs_script_engine` is `true` **and** the run targeted a single backend; absent on a multi-backend run, because `check` sweeps every language and the answer is per-backend. Rust, Go, Python and C11 receive expressions SCE lowered to Lua at generation time. C++ and Kotlin receive the author's ECMAScript source and hand it to the injected engine as it stands, so their engine must evaluate ECMAScript — and both default to one (`SCE_SCRIPT_ENGINE=quickjs`, `W3CTestBase.DEFAULT_ENGINE=rhino`). Until 2026-08-27 this field was the constant `"lua"` for every target, so a C++ or Kotlin host obeying it built the wrong engine; `docs/SCE_LUA_TRANSLATION_SEAM.md` carries the per-backend table and `sce-build/tests/script_engine_language_parity.rs` holds the value to what the artifact actually carries. |
...
| `unreadable_variables` | optional array of objects | The typed `<data>` declarations the run emitted **no typed reader** for, each `{var, reason}` plus `language`, `spelling`, `with` and `location` where they apply. Omitted when every typed declaration got one. A reader that is not there looks, from the host's side, like one the generator forgot, so the absence carries its cause: `type-disagreement`, `not-an-expression` (a `json` reader names the variable in an expression), or a spelling some backend cannot use — `keyword` (no escape reaches it: C++, Rust `self`/`Self`/`super`/`crate`), `not-an-identifier`, `member-collision` (the generated type already carries the name), `duplicate-spelling` (another variable folds to it). Where a language can escape a keyword it does — Rust `r#`, Kotlin backticks, a trailing `_` in Python — and the reader exists. Existence is decided once for every backend by `sce-build/src/reader_names.rs`, so a reader never exists in one backend's output and not another's; `reason` is an open discriminator. |
