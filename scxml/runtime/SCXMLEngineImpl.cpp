...
void SCXMLEngineImpl::shutdown() {
    if (initialized_) {
        Runtime::JSEngine::instance().shutdown();
        initialized_ = false;
    }
}

std::string SCXMLEngineImpl::getEngineInfo() const {
    return Runtime::JSEngine::instance().getEngineInfo() + " (SCXML C++ API v1.0)";
}

bool SCXMLEngineImpl::createSession(const std::string& sessionId,
                                   const std::string& parentSessionId) {
    return Runtime::JSEngine::instance().createSession(sessionId, parentSessionId);
}

bool SCXMLEngineImpl::destroySession(const std::string& sessionId) {
    return Runtime::JSEngine::instance().destroySession(sessionId);
}

bool SCXMLEngineImpl::hasSession(const std::string& sessionId) const {
    return Runtime::JSEngine::instance().hasSession(sessionId);
}

std::vector<SessionInfo> SCXMLEngineImpl::getActiveSessions() const {
    auto sessionIds = Runtime::JSEngine::instance().getActiveSessions();
...
std::future<ExecutionResult> SCXMLEngineImpl::executeScript(const std::string& sessionId,
    ...
    auto jsFuture = Runtime::JSEngine::instance().executeScript(sessionId, script);
...
std::future<ExecutionResult> SCXMLEngineImpl::evaluateExpression(const std::string& sessionId,
    ...
    auto jsFuture = Runtime::JSEngine::instance().evaluateExpression(sessionId, expression);
...
std::future<ExecutionResult> SCXMLEngineImpl::setVariable(const std::string& sessionId,
    ...
    auto jsFuture = Runtime::JSEngine::instance().setVariable(sessionId, name, value);
...
std::future<ExecutionResult> SCXMLEngineImpl::getVariable(const std::string& sessionId,
                                                         const std::string& name) {
    auto jsFuture = Runtime::JSEngine::instance().getVariable(sessionId, name);
...
std::future<ExecutionResult> SCXMLEngineImpl::setCurrentEvent(const std::string& sessionId,
    ...
    auto internalEvent = convertEvent(event);
    auto jsFuture = Runtime::JSEngine::instance().setCurrentEvent(sessionId, internalEvent);
...
std::future<ExecutionResult> SCXMLEngineImpl::setupSystemVariables(const std::string& sessionId,
    ...
    auto jsFuture = Runtime::JSEngine::instance().setupSystemVariables(sessionId, sessionName, ioProcessors);
...
size_t SCXMLEngineImpl::getMemoryUsage() const {
    return Runtime::JSEngine::instance().getMemoryUsage();
}

void SCXMLEngineImpl::collectGarbage() {
    Runtime::JSEngine::instance().collectGarbage();
}

ExecutionResult SCXMLEngineImpl::convertResult(const Runtime::JSResult& jsResult) const {
...
std::shared_ptr<Runtime::Event> SCXMLEngineImpl::convertEvent(std::shared_ptr<Event> publicEvent) const {
    if (!publicEvent) {
        return nullptr;
    }

    auto internalEvent = std::make_shared<Runtime::Event>(publicEvent->getName(), publicEvent->getType());
