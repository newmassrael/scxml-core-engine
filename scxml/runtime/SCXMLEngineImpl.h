...
public:
    ...
    bool createSession(const std::string& sessionId,
                      const std::string& parentSessionId = "") override;
    ...
    std::future<ExecutionResult> executeScript(const std::string& sessionId,
                                               const std::string& script) override;
    std::future<ExecutionResult> evaluateExpression(const std::string& sessionId,
                                                    const std::string& expression) override;

    // === Variable Management ===
    std::future<ExecutionResult> setVariable(const std::string& sessionId,
                                             const std::string& name,
                                             const ScriptValue& value) override;
    std::future<ExecutionResult> getVariable(const std::string& sessionId,
                                             const std::string& name) override;

    // === SCXML Event System ===
    std::future<ExecutionResult> setCurrentEvent(const std::string& sessionId,
                                                 std::shared_ptr<Event> event) override;
    std::future<ExecutionResult> setupSystemVariables(const std::string& sessionId,
...
private:
    ...
    ExecutionResult convertResult(const Runtime::JSResult& jsResult) const;

    // Convert public Event to internal Event
    std::shared_ptr<Runtime::Event> convertEvent(std::shared_ptr<Event> publicEvent) const;
