...
- §16.4 region-partition liveness signalling — **Zenoh path landed** via the §16.7 row 13 `REGION_PARTITIONED` reason code + per-partition token at `sce/live/<machine>/<partition>` + segment-count-discriminating subscriber (§16.4 reference implementation). Codegen rejects a partitioned machine that declares `liveliness:` without an `error.communication` handler in the SCXML (`reject_region_liveliness_without_handler`). The **SOME/IP `register_availability_handler` per-partition app** variant remains open — it needs a per-partition service-id allocation scheme and a §10.4 transport-contract micro-revision, tracked as a separate E2 sub-landing.
