...
**SomeIP scxml-invoke service-ID collision validator (Session 4c of §9.6 cross-device rollout, 2026-04-25)**: the 256-ID birthday-paradox boundary called out in Session 4b's closing paragraph (above) becomes a build-time invariant here. A new deploy-parse validator `validate_someip_scxml_invoke_service_id_collisions` lives in `sce-build/src/mesh/deploy.rs` alongside the eight other `parse_deploy_str` validators (`validate_machine_name_uniqueness`, `validate_synth_invoke_infix`, …): all four entry points that load a deploy.yaml (`inject_partition_context_for`, `inject_server_model_mutations`, `compile_mesh_codegen`, `validate_mesh_event_coverage`) route through `parse_deploy_str` and therefore inherit the check without per-call-site duplication. **Why `parse_deploy_str` and not the per-machine site (`validate_scxml_invoke_transport`)**: a service-ID collision is a deploy-wide invariant — it requires looking at every §9.6 someip participant in the deployment, not at one parent's outbound peer list. Co-locating with a per-machine validator that takes `&resolved_name` would mix concerns and re-run the deploy-wide work N times per build. Walking the deploy structurally (BTreeMap iteration order, deterministic golden hash) the validator collects participants — every machine M whose `bindings["#X"].transport == "someip"` for a peer-shape `#X` declared in `topology.*.machines`, plus the named peer X — and groups by `service_id_for_machine(name)`; any group of size ≥ 2 is rejected with `mesh/deploy-someip-scxml-invoke-service-id-collision` carrying the colliding service ID and the lex-sorted machine names. **Adversarial fixture vs hard-coded pair**: the validator's CI test (`someip_service_id_collision_rejected_via_adversarial_pair`) does not pin a magic colliding pair — it asks `crate::mesh::transport::someip::find_colliding_pair` at runtime for any pair the current FNV-1a low-byte projection collapses to the same slot, then drives the deploy.yaml synthesis from that result. A future drift in the FNV constants therefore re-discovers a fresh colliding pair instead of silently de-colliding (which would mask the validator drift); the pinned-hash drift on the hash function itself stays caught by `transport::someip::tests::known_names_have_pinned_hashes`. **Conservative single-domain assumption**: today every §9.6 someip participant in the deploy is treated as one collision domain. SCE does not yet model multi-OEM `vsomeip.json` `network:` boundaries that could host disjoint SCE-reserved ID spaces; until that federation lands, the single-domain assumption is the conservative trade — false-positive on truly-disjoint OEM networks costs a rename, false-negative on shared infrastructure costs an on-device debugging session. A second carry-out — a machine that has only OEM `<send>` someip bindings (no §9.6 `<invoke type="scxml">`) is still counted as a participant — exists for the same reason: deploy.yaml structure cannot tell the two shapes apart, and the false-positive cost is operator-actionable while a false-negative would silently mis-route at runtime.
