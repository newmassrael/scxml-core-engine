...
2. **`transport: someip` binding/server AND ≥2 sibling partitions of this machine in the `partitions:` block** (RFC F.X-3) — vsomeip's `register_availability_handler` per-partition fires row 13 `REGION_PARTITIONED`. The ≥2-partition requirement is load-bearing: vsomeip's discovery primitive is keyed on `(service, instance)` not on a hierarchical name, so a single-process SOMEIP machine has no orthogonal axis for "sibling partition death" and the row-13 handler would never fire.

Row 8 (`PEER_PARTITIONED`) for SOME/IP is **transport-conditional and currently deferred** (RFC F.X-4 candidate, §16.9 Session F). The deferral is documented at the row-8 entry in §16.7 itself; until F.X-4 lands, the validator gate above keeps the silent-broken window closed by rejecting SOME/IP-only machines that declare `liveliness:` with fewer than 2 sibling partitions.
...
| 8 | Peer machine's liveness signal observed lost (transport-conditional — currently Zenoh only via `sce/live/<machine>` 2-segment token DELETE; SOME/IP path deferred to RFC F.X-4) | `PEER_PARTITIONED` | `target: string`, `last_seen_ms_ago: int` |
...
- §16.4 region-partition liveness signalling — **Zenoh and SOME/IP row 13 paths landed**. Zenoh emits row 8 + row 13 from a single `sce/live/**` subscriber via segment-count discrimination (§16.4 reference implementation). SOME/IP row 13 (`REGION_PARTITIONED`) lands per RFC F.X-3 (`claudedocs/rfc-someip-region-liveness.md`) on the F.X-2 consolidated SCE app `<machine>_<partition>_sce_app_` using counter+pin-assigned service IDs from the F.X-1-reserved liveness sub-range `[0x8180, 0x81FF]`; vsomeip's `register_availability_handler` fires `available=false` when a sibling partition's SCE app exits over Service Discovery, and the codegen-emitted handler raises `error.communication` with `reason="REGION_PARTITIONED"`. Deploy-validator accepts SOMEIP machines that declare `liveliness:` AND appear in the `partitions:` block with ≥2 sibling partitions (single-process SOMEIP has no orthogonal axis for sibling-partition death). Codegen rejects any machine that declares `liveliness:` without an `error.communication` handler (`reject_liveliness_without_handler`), covering both row 8 and row 13 under a single symmetric gate. The **SOME/IP row 8 (`PEER_PARTITIONED`) path** remains deferred (RFC F.X-4 candidate) — vsomeip's `(service, instance)` keying has no segment-count axis to disambiguate, so machine-level death emission needs an independent design pass; the validator gate keeps the silent-broken window closed for that path.
