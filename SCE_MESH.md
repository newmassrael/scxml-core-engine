...
12. **Parallel root partition designation**: Every distributed `<parallel>` — a `<parallel>` whose regions span two or more partitions — must have exactly one partition claiming that `<parallel>`'s root via `partitions.<name>.hosts_parallel_roots: [{machine, parallel}]`. The root partition owns the `ParallelCompletionTracker` (§16.5) and is the site where `done.state.<parallel_id>` is raised into the local external queue. A `<parallel>` whose regions live entirely in a single partition has that partition as implicit root; the field may be omitted in that case. Rule 8's unit enumeration is unchanged — a `<parallel>` container id is **not** an orthogonal unit (the container runs wherever any of its regions run); rule 12 is a layered, orthogonal obligation on top of rules 1/8. A claimant partition must co-host at least one region of the claimed parallel: the tracker aggregates local region completions plus inter-partition `ParallelRegionDone` envelopes (§16.5 wire 21), and a root that co-hosts no region would force gratuitous inter-partition traffic for its own region updates. Violations:
    ...
    **Status (2026-04-20)**: the `hosts_parallel_roots:` serde field and the rule 12 validator are **not yet implemented**. `sce-build` parses `partitions:` per rules 1-11; a deploy.yaml including `hosts_parallel_roots:` today is rejected by `PartitionDecl`'s `deny_unknown_fields`, and the runtime root selection has no consumer — no `ParallelCompletionTracker` exists (§16.5 runtime is also pending). Rule 12 enforcement lands atomically with three obligations held as a single bundle: the §16.5 `ParallelCompletionTracker` runtime, the validator, and the `tools/codegen/templates/mesh/cpp/parallel_final.jinja2` template body — a file not present in the current tree; the name is pinned in this banner so the implementation commit need not re-decide naming against the atomicity claim below — emitting the `<parallel>`-final branch (root-hosting partition emits `ParallelCompletionTracker` + `done.state.<parallel_id>` raise into the local external queue; non-root partition emits wire 21 `ParallelRegionDone` only, with no local tracker and no local `done.state` raise). The field designates a runtime owner, so a validator without the tracker would accept well-formed designations that no code reads; a tracker without the validator would silently pick a root by whatever convention the runtime happened to implement; and an AOT template branch missing either half would emit the wrong shape (local `done.state` from every partition, or wire 21 emission without a local tracker to aggregate) — any two-of-three landing alone creates the "built but unconsumed" or "silent selection" anti-patterns. **Scaffolding carve-out.** Creating the extension hook in core `tools/codegen/templates/entry_exit_actions.jinja2` that delegates `<parallel>`-final emission to `mesh/cpp/parallel_final.jinja2` when partition context is present, establishing the `sce-build`→`sce-codegen` partition-context pipeline, and creating `parallel_final.jinja2` itself with a body whose partition-absent output is byte-identical to pre-mesh single-process AOT codegen are **not** rule 12 landings and may proceed in separate earlier commits. They expose no author-visible semantics — a deploy.yaml without `hosts_parallel_roots:` produces unchanged codegen output — so the atomic 3-bundle anti-pattern reasoning above does not bind them. The atomic constraint applies only to the **semantic payload**: validator enforcement of rule 12, `ParallelCompletionTracker` runtime, and the `parallel_final.jinja2` partition-aware branch body that reads partition context. A scaffolding commit that wires `hosts_parallel_roots:` into codegen context without the simultaneous validator rejection of missing bundle pieces is excluded from this carve-out — it would re-introduce the "built but unconsumed" anti-pattern. Until all three semantic pieces land, `barrier_timeout_ms:` is accepted as a non-zero range check only (`sce-build/src/mesh/deploy.rs` L1164), its root-hosting-only semantics stated at §14 L2731+ as cross-reference without parse-time enforcement.
...
**Status (2026-04-20)**: §14 partition schema (rules 6-10, parse-time) and §14 cross-reference rules 1/2/5/11 (coverage / default-discipline / synth-infix / nested-parallel) landed in `sce-build`. The R1/R2/R3/R4 distributability analyzer described above (shared-write detection, cross-region transition detection, snapshot reads, script opacity) is **not yet implemented**: `sce-build` parses `partitions:` and enforces coverage, but it does not walk the SCXML to detect R1-R4 violations against the parallel/region graph. The analyzer lands atomically — R1/R2/R3/R4 share author-visible semantics (a document with a shared-write bug plus a cross-region transition must get one diagnostic covering both, not a sequence that depends on which rule landed first), and a partial landing (e.g., R2-only) would accept R1/R3/R4 violations silently and mislead authors into believing their partition plan was analyzed when only one rule was. The `distributability: strict | permissive` knob is correspondingly not parsed today; the grammar above is the target shape for the atomic R1-R4 commit. Current in-tree behaviour: every `partitions:` declaration that passes coverage is accepted as-authored, so a document whose regions share writable ancestor data (R1 violation) or whose transitions cross region boundaries (R2 violation) runs distributed with whatever semantics the transport layer happens to produce. No `mesh` ctest fixture declares `partitions:` today, so this silent-accept path is not exercised on the current suite; it would surface the moment a distributed harness (§16.8) or a first-party mesh fixture opts into partition-per-region execution.
