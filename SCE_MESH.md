...
**Phase 5 scope**: only the "transport-native runtime target selection" subset of dynamic mode is landed. Priority-based cross-transport resolution (`resolution.strategy: priority`), per-instance dedup (`dedup.key: instance_id`), external registries (Consul, etcd, mDNS), and an SCE-level peer table are all deferred or rejected. What IS landed is the binding-value-field placeholder mechanism described in §3.3 and formalised in §14.4 — a deploy.yaml binding may carry `{name}` tokens resolved at `<send>` / `<invoke>` time from `<param>` values. Per-transport capability gating is enforced at build time on two orthogonal registry axes (`TransportDescriptor::pool_shape` and `::pool_member_carrier`): today Zenoh supports open placeholder substitution, DDS supports a bounded `members:` list (because a writer built at invoke time has not finished discovery and a VOLATILE writer drops its first sample — see §14.4), SOME/IP supports a bounded `instances:` list (because vsomeip's `request_service(ANY_INSTANCE)` does not actually subscribe to every instance), and custom_tcp / shm / local / can do not support placeholders at all.
...
| `custom_tcp` | `ActiveError` | The same `MeshEnvelope`, framed by SCE and written back on the stream the request arrived on (§10.4.4) — the stashed `pending_server_links_` entry, so the notice reaches the connection that asked and no other |
| `dds` | `ActiveError` | The same `MeshEnvelope`, published on the reply topic derived from the request topic (§8.2), after the admitted correlation is erased so a late engine response cannot publish a second answer |
...
| `local`, `shm`, `can` | `Unsupported` | The knob is rejected at parse time (`mesh/deploy-invalid-server-response-deadline`) rather than accepted into a router that would ignore it |

The `ActiveError` / `DropSilently` split is a property of the protocols, not a policy choice. A `zenoh::Query` exposes `reply` and destruction with nothing in between, so dropping it is the strongest signal the query model admits. The three active arms each have somewhere to put an answer, and they get there by different routes: SOME/IP reserves a message type and a return code for exactly this condition; custom_tcp defines its own framing, so the notice is an ordinary frame and what makes it correct is its route rather than a field; DDS has a reply topic paired with the request topic. The consequence is the same in all three and is the only part the author sees — the timeout is *reported* rather than inferred, which is what lets the requester distinguish "the server gave up" from "the server vanished". Two conditions that a silent drop renders identical.

**Where the arms are and are not the same.** The notice envelope is built once — id, `source`, `type = error.rpc.deadline`, both correlation fields, `rpc_status = DeadlineExceeded`, and a `rpc_error_message` naming the abandoned call and the deadline it exceeded — and only its delivery branches per transport. What does differ is *when the abandoned call's name is known*: a SOME/IP method handler is registered per (service, instance, method), so the name is a build-time constant, while custom_tcp and DDS route every inbound request through one callback and copy `env.type` into the timer closure at arm time. The copy is made on the inbound thread, so nothing allocates inside the `noexcept` scheduler callback, and the name survives the envelope being handed to the engine.

**Reference position.** Neither vsomeip nor AUTOSAR `ara::com` arms a server-side deadline at all: both offer only a client-side future timeout, so in both the server's request handle stays parked indefinitely and the caller cannot tell an abandoned call from a lost datagram. The same hole appears wherever a request/reply middleware exposes only the requester's timer. SCE arms the deadline from a deploy.yaml declaration and answers through whatever channel the transport provides — a reserved protocol slot where one exists (SOME/IP), the request's own return path where SCE owns the framing (custom_tcp), the paired reply topic where the reply is a publication (DDS). Only `zenoh`, whose query model has no server-authored failure channel at all, is left with the silent release.
...
The remaining `Unsupported` entries are structural rather than pending work. `local` dispatches in-process, so the reply leg runs on the caller's own stack and there is no parked handle to release; `shm` and `can` carry no `RequestReply` capability, so there is no server arm at all. The dimension continues to state what is realised end-to-end rather than what is reachable, so a new arm moves the registry entry and the fixture together or not at all.
...
**Resolution**: at build time, `sce-build` scans every `<invoke type="scxml">` with inline `<content>` and **captures the inner `<scxml>` document as a structured submodel on the parent invoke**. The captured machine is treated exactly like a named peer:
...
1. Build tool synthesizes a stable machine name: `<parent_machine_id>__sce_synth_invoke__<invoke_id>`. The `__sce_synth_invoke__` infix is reserved: `sce-build` **rejects any author-declared machine id containing this infix** with a build error, so name collisions with synthesized machines are structurally impossible. If a collision is nonetheless detected (e.g., two invokes sharing the same id under the same parent — which is already a W3C violation), `sce-build` fails with:
   ...
   ```
2. The inline content is **parsed in-memory** into `ScxmlInvokeInfo::inline_child` on the parent invoke; the `<invoke>` is rewritten in-memory to `src="#<parent>__sce_synth_invoke__<invoke_id>"` (content field dropped). **No disk side-effect**: the synthesized machine's `.scxml` file does NOT materialise next to the parent on disk — earlier SCE versions wrote a sibling `<parent>__sce_synth_invoke__<invoke_id>.scxml` file during parse, but that polluted tracked fixture directories whenever a downstream caller ran `sce-codegen generate -o <somewhere-else>`. The naming convention applies to the **name** only, not to a file location; codegen emit is the single point of disk materialization (the `_sm.*` artefacts land in `-o`, no intermediate `.scxml` step).
...
4. AOT codegen produces a state machine class for the synthesized machine identical to named peers. The standalone `sce-codegen generate` (and the W3C unified path) iterates the parent's `inline_child` entries and emits each child's `_sm.*` alongside the parent's in the caller-supplied output directory.

Peer enumeration for the synthesized machine follows the reserved infix. The parser rewrites inline `<content>` to `src="#<synth>"` **only in the in-memory model** — both the parent's on-disk SCXML (still carrying inline `<content>` with no `src=` attribute) and the synthesized child (now living as an in-memory submodel on the parent invoke, no sibling file on disk) leave the sibling-file regex that resolves `scxml_remote_inbound_peers` for named peers with nothing to observe. `sce-build` closes this by inverting the `__sce_synth_invoke__` infix: when `resolved_name` matches the infix and the extracted parent id is a declared topology machine with a distinct partition per rule 3, the parent is added to the synthesized machine's inbound peer list. Same-partition synthesized machines stay off the mesh inbound set (per rule 3 default) and the local child-session shape is preserved.
...
**The section needs a binding it can gate.** Buffering is per-target and exists only for transports whose registry entry sets `TransportDescriptor::buffers_outbound` — the ones that report when a peer becomes reachable (SOME/IP's `(service, instance)` availability handler, a Zenoh publisher's matching status). A machine whose bindings are all on other transports gets the `OutboundBuffer.h` include and not one buffer member, so the same `mesh/deploy-invalid-outbound-buffer` rejects the section there too, with the buffering transports named. The test is "at least one buffered binding", not "every binding": a machine that buffers its SOME/IP target and also talks DDS is honoured in full.

A §14.4 pool binding on a buffered transport is the converse case and rejects separately (`mesh/deploy-pool-dispatch-without-member`, §14.4) — readiness is one signal per binding and a pool has one address per member.
...
| DDS | `topic:` | `{name}` token in the value (must resolve to a member of `members:`) | The topic name each member's `Dds::Client` was built on |
...
All three mechanisms converge on "binding references a `<param>` name". Zenoh and DDS embed the reference syntactically inside their address field; SOME/IP names it via an explicit binding field (`instance_from:`) because the instance_id is a typed `uint16_t`, not a string. A referenced `<param>` name that no `<param>` supplies at the call site is a build-time error (`mesh/topology-pool-param-name-missing`). Reserved `_mesh_*` param names (§9.5) never supply placeholders.

**Two independent axes.** A pool is described by the transport registry's `pool_shape` (None / Open / Bounded — must the member set be declared up front?) and `pool_member_carrier` (None / TypedInstanceId / StringSegment — what *is* a member?). They are orthogonal, and all three realised transports occupy distinct cells: Zenoh is Open + StringSegment, DDS is Bounded + StringSegment, SOME/IP is Bounded + TypedInstanceId. The shape decides *whether* an enumeration is required; the carrier decides *which key* carries it (`members:` for string segments, `instances:` for typed instance ids) and *how* the runtime value is selected (`{name}` embed vs `instance_from:`). Folding the two would reject DDS, whose shape matches SOME/IP and whose carrier matches Zenoh.

**Transport capability gating.** A binding may carry placeholders only if its transport declares a non-`None` `pool_shape` in the registry. Today:

- **Zenoh** — Open + StringSegment. The KeyExpr substitution is validated only for placeholder grammar; the resulting string is passed to Zenoh's native routing which delivers to whichever peer has a matching subscriber. Nothing is enumerated, so `members:` on a Zenoh binding is rejected — a declared set nothing reads is indistinguishable from one that took effect.
- **DDS** — Bounded + StringSegment, with a `members: [<string>, ...]` list. The constraint is discovery, not addressing: `Dds::Client` takes its topic as a runtime string, but a writer created at invoke time has not matched when the first sample is written and a VOLATILE writer drops that sample with no error (`Client::waitForServer` exists precisely because of this). Codegen therefore substitutes each member at build time and builds that member's endpoints at `init()`, and the runtime value selects among endpoints that already exist. A value outside the list fails closed exactly as the SOME/IP arm does. Because the member list enumerates the values of one placeholder, a bounded binding embedding more than one is rejected (`mesh/deploy-pool-invalid-placeholder`) — the member set of a multi-placeholder address is a product the list cannot express.
- **SOME/IP** — Bounded + TypedInstanceId, with an `instances: [<int>, ...]` list. vsomeip's `request_service(SERVICE, ANY_INSTANCE)` does not actually subscribe to every instance — it requests instance `0xFFFF` as a specific instance. Codegen emits one `request_service(SERVICE, i)` per declared instance at `init()`; the runtime placeholder value is validated against the list and fails-closed with `error.execution` + `reason=INVOKE_SRC_NOT_FOUND` (§9.5 three-tier error table, §10.7.1) if out of range — no `vsomeip::message->send()` is called, so no wire `RpcStatus` is involved.
- **custom_tcp** / **shm** / **local** / **can** — `pool_shape: None`. These transports have no runtime routing layer; adding one would be middleware-SD reinvention (§3.3). Any binding on these transports carrying a placeholder is rejected at parse time (`mesh/deploy-pool-not-supported-by-transport`).
...
**Transport-compat gate**. The liveliness emit branch fires per-transport — Zenoh on `"zenoh"` in the machine's transport set, SOME/IP on the F.X-3 codegen-derived `someip_liveness_service_id_self_hex` (row 13 region-liveness, RFC F.X-3) and the F.X-4 codegen-derived `someip_machine_liveness_service_id_self_hex` (row 8 machine-liveness, RFC F.X-4). Declaring `liveliness:` on a machine that supports neither path would pass the handler-presence gate yet produce zero transport-layer observer code — the required `error.communication` handler would compile but never fire, matching the `feedback_silently_broken_hooks` anti-pattern exactly. [`sce-build/src/mesh/deploy.rs::validate_liveliness`] therefore rejects any machine that declares `liveliness:` without one of:

1. **`transport: zenoh` binding or server** (any partition shape) — Zenoh emits row 8 + row 13 from a single subscriber via `sce/live/**` segment-count discrimination.
2. **`transport: someip` binding/server** (any partition shape, RFC F.X-4) — every SOMEIP `liveliness:` opt-in implicitly enables row 8 machine-level emission (vsomeip `register_availability_handler` on the F.X-4 machine-level service ID in `[0x8280, 0x82FF]`, codegen-emitted from the consolidated SCE app on every partition binary so multi-partition machines stay alive while any partition's SCE app is offering, per §10.9 invariant 4). Row 13 region-liveness additionally fires when the machine appears across ≥2 sibling partitions of the `partitions:` block (RFC F.X-3); single-partition machines have row 8 coverage but not row 13 (vsomeip's `(service, instance)` keying admits no sibling-partition observer for a machine that has no siblings).

After F.X-4 every `error.communication` row in §16.7 is transport-portable across Zenoh + SOMEIP. The previously transport-conditional row 8 (§16.7) is now uniformly observable; the row-13 ≥2-partition condition is a row-13 emission-shape constraint, not a row-8 deferral.
...
- **Scheduler pump**: the fire path relies on the base engine's `PullScheduler`, which is pulled by `tick()` — **not** `step()` alone. The `tick()` short-circuit is gated on `isInFinalState()` (top-level `<final>` only; parent-presence check on `currentState_`), so a Root partition holding a `<parallel>` whose local region has reached a regional `<final>` while sibling regions are still pending continues to pump — which is exactly the window the §16.5 barrier timer needs. The structural "is this state a `<final>` element" question stays on the generated policy as `StatePolicy::isFinalState` and is never used as a "machine is done" guard. `pumpScheduledEvents()` remains public as a fine-grained drain helper (harnesses that interleave scheduler-only pulses with explicit `step()` sequencing), but normal polling loops should call `tick()` alone.
...
| 8 | Peer machine's liveness signal observed lost. Transport-portable manifestation — Zenoh: `sce/live/<machine>` 2-segment token DELETE; SOME/IP: vsomeip `register_availability_handler` fires `available=false` on the machine-level liveness service in `[0x8280, 0x82FF]` (RFC F.X-4); DDS: a matched writer's LIVELINESS lease expires and the notification reader's `on_liveliness_changed` reports a decreased alive count (requires `transports.dds.qos.liveliness_lease_ms`, §8.2); custom_tcp: the stream to the peer closes — a FIN, a reset, or a keepalive timeout — which is the most direct form of the signal, since a connection-oriented transport holds the liveness evidence rather than inferring it. `transports.custom_tcp.keepalive:` is what extends that from "peer exited" to "peer is unreachable", §16.8.3.) | `PEER_PARTITIONED` | `target: string`, `last_seen_ms_ago: int?` |
...
**Status (2026-07-31)**: Session E2 scope per §16.9 — the S4 seed has landed, the full harness has not. Present in the tree: `tests/w3c_distributed_manifest.yaml` classifying test417, the `tests/w3c/dist/` tree holding the partition registry and the shared per-partition entry point, the driver `run_distributed.cpp` (the "or C++ test" form §16.8.4 permits, in place of `run_distributed.py`), and a `w3c_distributed_conformance` ctest label carrying that one test; the driver and several mesh fixtures do spawn per-partition OS processes. Absent: the `irp_runner` sweep over the IRP suite — the manifest carries one entry, and its header defers the remaining `<parallel>`-bearing tests to S5+ — and the single-process-vs-distributed cross-comparison that §16.8.1 makes the pass criterion. The seed driver instead decides its verdict from partition exit codes, under the §16.2 property-1 reduction documented in `run_distributed.cpp`. [`docs/SCE_MESH_CONFORMANCE_MATRIX.md`](docs/SCE_MESH_CONFORMANCE_MATRIX.md) is the day-0 map of transport primitives and mesh-runtime invariants the harness consumes as it grows.
...
**Socket layer.** Because `custom_tcp` is SCE's own implementation, it has no sibling to the escape hatches the other transports carry — there is no `zenoh.json5`, `vsomeip.json` or `cyclonedds.xml` a deployer can reach past the schema with. An option absent from `transports.custom_tcp:` is therefore an option that does not exist. The socket layer is declared there in full:

```yaml
transports:
  custom_tcp:
    listen: "127.0.0.1:9000"
    backlog: 128                    # default 16
    reuse_addr: true                # SO_REUSEADDR on the listen socket
    nodelay: true                   # TCP_NODELAY on every connection
    connect_max_attempts: 20        # dial retries before giving up
    connect_retry_interval_ms: 50   # with the above, ~1 s total
    keepalive: true                 # default false; see below
    keepalive_idle_s: 60            # TCP_KEEPIDLE
    keepalive_interval_s: 10        # TCP_KEEPINTVL
    keepalive_count: 6              # TCP_KEEPCNT
    recv_buffer_bytes: 262144       # SO_RCVBUF; omit for the kernel default
    send_buffer_bytes: 262144       # SO_SNDBUF; omit for the kernel default
```

Every default is the literal the implementation carried before the fields existed, so a deployment that declares none behaves identically to one built before this surface opened. The defaults were chosen for the harness — `SO_REUSEADDR` makes rapid teardowns deterministic, `TCP_NODELAY` removes coalescing jitter from latency measurements, the dial retry covers ctest startup — and each remains a good default while ceasing to be the only obtainable value. `nodelay` in particular is a bandwidth/latency trade a production link may want to decide differently.

`backlog`, `connect_max_attempts`, `connect_retry_interval_ms` and the buffer sizes are rejected at parse time below 1 (`mesh/deploy-invalid-custom-tcp-socket`): each would silently disable what it configures — a zero backlog refuses every connection, zero attempts never dials, a zero interval busy-spins — and omitting the field is how an author takes the default. There are no upper bounds; a backlog ceiling would be a guess at the peer fleet size and a buffer ceiling a guess at the link's bandwidth-delay product, and the kernel clamps what it cannot honour. The buffer fields are named after the Cyclone DDS settings they correspond to (`SocketReceiveBufferSize` / `SocketSendBufferSize`).

Both ends of a link apply the connection-level options through one helper (`detail::apply_connection_options`), so an accepted connection and a dialed one cannot disagree about Nagle or buffer sizing.

**Keepalive is how custom_tcp reaches §16.7 row 8.** A peer that exits cleanly closes its stream, and the client's reader observes that immediately — so `PEER_PARTITIONED` fires without any configuration. A peer that *crashes or is partitioned away* leaves a half-open socket instead: the local end holds a connection nobody is on, and `read()` blocks indefinitely. `keepalive: true` is what turns that case into a bounded detection, with `idle + interval * count` as the latency budget (the defaults give roughly 120 s). It is off by default because enabling it changes when an existing deployment observes a peer disappear.

This makes custom_tcp the fourth transport on that row, and the one that holds the evidence most directly: Zenoh infers liveness from a token, SOME/IP from a service-discovery flag, DDS from a lease it must be told to request — while a connection-oriented transport simply has the connection. The raise is client-side, where the peer's deploy.yaml name is structurally known; a dropped *inbound* connection on the shared `listen:` server carries no peer identity until an envelope names one, so the server side does not raise and does not pretend to.
...
- §16.4 / §16.7 liveness signalling — **Zenoh and SOME/IP row 8 + row 13 paths all landed**. Zenoh emits row 8 + row 13 from a single `sce/live/**` subscriber via segment-count discrimination (§16.4 reference implementation). SOME/IP row 13 (`REGION_PARTITIONED`) lands per RFC F.X-3 (`claudedocs/rfc-someip-region-liveness.md`) on the F.X-2 consolidated SCE app `<machine>_<partition>_sce_app_` using counter+pin-assigned service IDs from the F.X-1-reserved sub-range `[0x8180, 0x81FF]`; SOME/IP row 8 (`PEER_PARTITIONED`) lands per RFC F.X-4 (`claudedocs/rfc-someip-machine-liveness.md`) on the same consolidated SCE app using machine-keyed service IDs from the F.X-4-reserved disjoint sub-range `[0x8280, 0x82FF]`, with multi-partition machines offering the row-8 service from every partition binary (D5 emit-from-every-partition) so peers see `available=false` only when the LAST partition's SCE app exits — matching §10.9 invariant 4. The shared allocator helper `mesh::someip::range_alloc` (extracted at the F.X-4 third-consumer threshold) drives all three SCE-internal SOME/IP service-ID subsystems (invoke `[0x8100, 0x817F]` + region-liveness + machine-liveness). The codegen-emitted handler raises `error.communication` with `reason="PEER_PARTITIONED"` (row 8, populating `target`) or `reason="REGION_PARTITIONED"` (row 13, populating `machine + partition`). Deploy-validator accepts SOMEIP `liveliness:` opt-in for any partition shape (D4-shape-1 implicit row-8 emission); the F.X-3 region-liveness assigner narrows to ≥2-partition machines for row-13 emission scope. Codegen rejects any machine that declares `liveliness:` without an `error.communication` handler (`reject_liveliness_without_handler`), covering both row 8 and row 13 under a single symmetric gate.
