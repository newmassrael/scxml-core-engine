...
5. **Sender-identity keying stays on `env.source`** — containers that key on sender identity (DedupRouter `windows_` §10.5, OrderingBuffer `state_` §10.6, `peer_last_seen_` §16.7 row 9) key on `env.source` (machine_name). This axis is correct under both single-session and §14.4 server pool topologies, by construction:
...
- Pool + `<invoke type="sce:mesh-rpc">` coexistence rejected at codegen with `mesh/codegen-pool-with-mesh-rpc-client-unsupported` (router-scoped correlation table cannot safely alias across sessions)
...
- **Mesh-rpc client coexistence**. Pool + `<invoke type="sce:mesh-rpc">` on the same router is rejected at codegen with `mesh/codegen-pool-with-mesh-rpc-client-unsupported`. `invoke_correlation_` and `active_invokes_` are router-scoped (one table per `TransportRouter`), so two hosted sessions would alias each other's UUID entries under the §9.5 "one live correlation entry per invoke_id" invariant. The rejection is mechanical: either remove the mesh-rpc invoke sites from this machine or reduce `server.instances:` to a single entry. Split across deployments is fine — the rejection is per-router, not per-deployment.
