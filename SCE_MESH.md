...
- Per-session state migration for containers currently keyed on `env.source` (DedupRouter, OrderingBuffer, `peer_last_seen_`) — §10.9 invariant 5 documents the single-session-per-machine keying assumption; lifting it is gated on a concrete multi-session liveliness or ordering requirement
