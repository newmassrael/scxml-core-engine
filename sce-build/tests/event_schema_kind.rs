//! NL→IR Mapping Roadmap Item C1 Path A — EventSchema kind
//! integration tests.
//!
...
//!     enum import / send-typed payload / mesh-matched) compile
...
//! wired in `compile_scxml_with_imports` between pass-2 SCXML parsing
//! and the worker/BC cross-doc validators; the mesh DL-7' validator
//! is wired in the deploy-aware path after
//! `validate_stateless_accept_externs`.
...
// ─── Positive: cross-doc enum import (schema imports an Enum kind) ──
...
// ─── Schemaless fallback (DL-9'): statechart without schema
//     imports is unaffected by the validator ─────────────────────────
