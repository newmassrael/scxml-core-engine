...
/// NL→IR Mapping Roadmap Item C1 (DL-7) — distinguishes the three
/// shapes a cross-machine EventSchema mismatch can take so consumers
/// can match without substring-greping the `Display` prose. The
/// `Display` impl emits the exact phrases spliced into the outer
...
pub enum DeployError {
    ...
    /// NL→IR Mapping Roadmap Item C1 (DL-7 mesh cross-machine
...
fn deploy_fields(e: &DeployError) -> DiagnosticPayload {
    match e {
        ...
        //    EventSchema mismatch. `actual` carries the sender's view
        //    (hash or "absent" tag), `expected` enumerates the
        //    receiver's view; `key_fragments` joins event + both
        //    machine names + reason tag so two distinct mismatches in
        //    the same build collapse to different `fnv1a:...` IDs.
