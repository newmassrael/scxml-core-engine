...
///
/// - compact URI: `sce:provenance="[redacted: customer standard id]@23#4.4.2"`
///   (`doc_id @ rev # section`; trailing `:page` optional after the
///   section to carry a page number, e.g. `[redacted: customer standard id]#4.4.2:118`)
...
mod tests {
    ...
    #[test]
    fn compact_doc_only() {
        let p = SpecProvenance::parse_compact("[redacted: customer standard id]").unwrap();
        assert_eq!(p.doc_id, "[redacted: customer standard id]");
        assert!(p.rev.is_none() && p.section.is_none() && p.page.is_none());
    }

    #[test]
    fn compact_doc_rev_section() {
        let p = SpecProvenance::parse_compact("[redacted: customer standard id]@23#4.4.2").unwrap();
        assert_eq!(p.doc_id, "[redacted: customer standard id]");
    ...
    fn compact_with_page() {
        let p = SpecProvenance::parse_compact("[redacted: customer standard id]@23#4.4.2:118").unwrap();
    ...
    fn json_skips_absent_optionals() {
        let p = SpecProvenance {
            doc_id: "[redacted: customer standard id]".to_string(),
            ..Default::default()
        };
        let json = serde_json::to_string(&p).unwrap();
        assert_eq!(json, r#"{"doc_id":"[redacted: customer standard id]"}"#);
    }
}
