...
mod tests {
            ...
            match self {
                NoAnchor::AwaitingResolver => {
                    "this code has not been demonstrated carrying the \
                     ...
                     the Forge class, which \
                     `a_forge_document_has_no_anchor_for_a_diagnostic_to_carry` \
                     now makes: the Forge AST wire schema, generated \
                     from the Forge IR, declares no provenance field \
                     anywhere, so on a Forge document the resolver's \
                     answer and the empty field coincide by \
                     construction. \
                     \
                     ⚠⚠⚠ So READ THIS ENTRY AS A STATEMENT ABOUT \
                     EVIDENCE, NOT ABOUT WORK. It says only that no \
                     scenario has yet raised this code on an anchored \
                     STATECHART document. It does not claim the \
                     statechart pipeline raises it at all, and for a \
                     code only Forge raises there is nothing here to \
                     do — that contract is already kept. Which codes \
                     those are is exactly the membership no census \
                     answered, so this entry does not pretend to know. \
                     \
    ...
    /// anchor for a diagnostic to carry, so its empty
    /// `spec_provenance` is the true answer and not a pending one.
    ///
    /// # Why this is a guard and not a sentence
    ///
    /// That case is what makes the majority of the roster's
    /// registrations already-satisfied rather than outstanding work.
    /// Asserting it per code would mean deciding, for each of 358,
    /// which pipeline raises it — and that membership was derived
    /// three times on 2026-09-11 by three different source censuses
    /// which disagreed with each other, because the module that
    /// constructs an error variant is not the pipeline that raises it.
    /// So the class is proven ONCE, here, about the thing that is
    /// actually structural.
    ///
    /// # What it reads, and why that file
    ///
    /// `apis/forge-ast.v1.schema.json` is generated by
    /// `schemars::schema_for!(ForgeAstEnvelopeOwned)` over the Forge
    /// IR in `sce-build/src/forge/`, and `ast_export`'s drift test
    /// keeps it in step with those types. So the schema is a faithful
    /// projection of what a Forge document can hold.
    ///
    /// # ⚠ "Forge document" is narrower than "document the Forge
    /// pipeline parses", and the first version of this test got that
    /// wrong
    ///
    /// `ForgeDocument` is a `oneOf` over **eighteen** kinds, and
    /// `ForgeKind::Statechart` is one of them — so a statechart IS a
    /// Forge document in the envelope's sense, and it carries
    /// provenance on `State`, `Transition` and `Action`. Asserting
    /// "the schema mentions no provenance" therefore failed, correctly,
    /// and the failure was the test being wrong rather than the
    /// contract.
    ///
    /// What §2.1.2 means is the NON-statechart kinds — codec, mesh
    /// binding, event schema and the rest — which is what this walks.
    /// The kinds are selected by their own `kind` discriminant
    /// (`{"enum": ["codec"]}` and so on), never by a list kept here: a
    /// hand-kept list of which kinds are "really Forge" is the same
    /// membership guess that three source censuses already got wrong.
    ///
    /// ⚠ The day a non-statechart kind gains `sce:provenance`, its IR
    /// type gains a field, the schema gains the key, and this goes red
    /// — which is the point. A comment saying the same thing would not.
    #[test]
    fn a_forge_document_has_no_anchor_for_a_diagnostic_to_carry() {
        let schema: serde_json::Value =
            serde_json::from_str(include_str!("../../../apis/forge-ast.v1.schema.json"))
                .expect("the Forge AST schema is checked-in JSON");
        ...
        /// Every property name reachable from `node`, following `$ref`
        /// into `defs`. `seen` stops the recursion on the IR's cycles
        /// (a `State` holds `State`s).
        fn reachable_properties(
            ...
            match node {
                serde_json::Value::Object(map) => {
                    if let Some(serde_json::Value::String(reference)) = map.get("$ref") {
                        ...
                        if seen.insert(name.to_string()) {
                            if let Some(target) = defs.get(name) {
                                reachable_properties(target, defs, seen, out);
                            }
                        }
                    }
                    ...
                    for value in map.values() {
                        reachable_properties(value, defs, seen, out);
                    }
                }
                serde_json::Value::Array(items) => {
                    for item in items {
                        reachable_properties(item, defs, seen, out);
                    }
                }
                _ => {}
            }
        ...
        /// The `kind` discriminant a `ForgeDocument` branch declares.
        fn branch_kind(branch: &serde_json::Value) -> Option<String> {
            branch
                .get("properties")?
                .get("kind")?
                .get("enum")?
        ...
        let branches = defs
            .get("ForgeDocument")
            .and_then(|doc| doc.get("oneOf"))
            .and_then(serde_json::Value::as_array)
            .expect("ForgeDocument is a oneOf over the kinds");

        let mut statechart_properties = std::collections::BTreeSet::new();
        let mut forge_kind_properties = std::collections::BTreeSet::new();
        let mut forge_kinds: Vec<String> = Vec::new();
        for branch in branches {
            let kind = branch_kind(branch).expect("every branch declares its kind");
            let mut seen = std::collections::BTreeSet::new();
            if kind == "statechart" {
                reachable_properties(branch, defs, &mut seen, &mut statechart_properties);
            } else {
                forge_kinds.push(kind);
                reachable_properties(branch, defs, &mut seen, &mut forge_kind_properties);
            }
        }

        let carries = |set: &std::collections::BTreeSet<String>| -> Vec<String> {
            set.iter()
                .filter(|name| name.to_ascii_lowercase().contains("provenance"))
                .cloned()
                .collect()
        };

        // ⚠ The walker proves itself before it is believed. A broken
        // `reachable_properties` — one that followed no `$ref`, say —
        // would find provenance nowhere and report the Forge kinds
        // clean while measuring nothing. The statechart branch is the
        // one place provenance is KNOWN to live, so finding it there is
        // what makes the empty answer below evidence rather than
        // silence.
        assert!(
            !carries(&statechart_properties).is_empty(),
            "the walk found no provenance on the statechart branch, \
             where `State`, `Transition` and `Action` carry it. The \
             walk is broken, so its verdict about the other kinds says \
             nothing",
        );
        assert!(
            forge_kinds.len() > 10,
            "only {} non-statechart kinds were walked: {forge_kinds:?}",
            forge_kinds.len(),
        );

        let carrying = carries(&forge_kind_properties);
        assert!(
            carrying.is_empty(),
            "a non-statechart Forge kind now declares {carrying:?}, \
             across the kinds {forge_kinds:?}. The roster registers \
             most of its codes on the grounds that such a document has \
             nowhere to write `sce:provenance`, so that ground has just \
             moved: every code those raises reach has to be re-decided, \
             and §2.1.2's second admissible case has to be rewritten to \
             say when it still applies.",
        );
    }
