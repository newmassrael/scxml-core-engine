...
HistoryValidator::HistoryValidator(std::function<std::shared_ptr<IStateNode>(const std::string &)> stateProvider)
    ...
    SCE_LOG_DEBUG("HistoryValidator: Initialized history validator");
}
...
bool HistoryValidator::validateRegistration(const std::string &historyStateId, const std::string &parentStateId,
    ...
    if (historyStateId.empty() || parentStateId.empty()) {
        SCE_LOG_ERROR("HistoryValidator: History state ID and parent state ID cannot be empty");
        return false;
    }

    // Check for invalid history type
    if (type == HistoryType::NONE) {
        SCE_LOG_ERROR("HistoryValidator: History type cannot be NONE for registration");
        return false;
    }

    // Check if history state is already registered
    if (registeredHistoryStates_.find(historyStateId) != registeredHistoryStates_.end()) {
        SCE_LOG_WARN("History state already registered: {}", historyStateId);
        return false;
    }

    // Check if parent state exists and is a compound state
    if (!isValidCompoundState(parentStateId)) {
        SCE_LOG_ERROR("Parent state is not a valid compound state: {}", parentStateId);
        return false;
    }
    ...
    if (registeredParentTypes_.find(parentTypeKey) != registeredParentTypes_.end()) {
        SCE_LOG_WARN("Parent state {} already has a history state of the specified type", parentStateId);
        return false;
    }

    SCE_LOG_INFO("Registration validation passed for {}", historyStateId);
    return true;
}
...
bool HistoryValidator::validateRegistrationWithDefault(const std::string &historyStateId,
    ...
    if (!defaultStateId.empty()) {
        ...
        if (!defaultState) {
            SCE_LOG_ERROR("Default state does not exist: {}", defaultStateId);
            return false;
        }

        // §scxml-3.10: Check if default state is valid for history type
    ...
    SCE_LOG_INFO("Registration with default validation passed for {}", historyStateId);
    return true;
}

bool HistoryValidator::validateRecording(const std::string &parentStateId,
                                         const std::vector<std::string> &activeStateIds) const {
    SCE_LOG_DEBUG("Validating recording - parent: {}, active states: {}", parentStateId, activeStateIds.size());

    // Check for empty parent ID
    if (parentStateId.empty()) {
        SCE_LOG_ERROR("HistoryValidator: Parent state ID cannot be empty for recording");
        return false;
    }

    // Check if parent state exists
    if (!stateProvider_) {
        SCE_LOG_ERROR("HistoryValidator: No state provider available");
        return false;
    }

    auto parentState = stateProvider_(parentStateId);
    if (!parentState) {
        SCE_LOG_ERROR("Parent state not found: {}", parentStateId);
        return false;
    }

    // Active states can be empty (valid scenario)
    SCE_LOG_INFO("Recording validation passed for {}", parentStateId);
    return true;
}

bool HistoryValidator::validateRestoration(const std::string &historyStateId) const {
    SCE_LOG_DEBUG("Validating restoration - history: {}", historyStateId);

    // Check for empty ID
    if (historyStateId.empty()) {
        SCE_LOG_ERROR("HistoryValidator: History state ID cannot be empty for restoration");
        return false;
    }

    // Check if history state is registered
    if (registeredHistoryStates_.find(historyStateId) == registeredHistoryStates_.end()) {
        SCE_LOG_ERROR("History state not registered: {}", historyStateId);
        return false;
    }

    SCE_LOG_INFO("Restoration validation passed for {}", historyStateId);
    return true;
}
...
void HistoryValidator::registerHistoryStateId(const std::string &historyStateId) {
    ...
    SCE_LOG_DEBUG("Registered history state ID: {}", historyStateId);
}
...
void HistoryValidator::registerParentType(const std::string &parentStateId, HistoryType type) {
    ...
    SCE_LOG_DEBUG("Registered parent-type combination: {}", key);
}
...
bool HistoryValidator::isValidCompoundState(const std::string &stateId) const {
    ...
    if (!state) {
        SCE_LOG_WARN("State not found: {}", stateId);
        return false;
    }
    ...
    SCE_LOG_DEBUG("State {} is {}a compound state", stateId, (isCompound ? "" : "not "));
