...
SCE::Type SCE::StateNodeParser::determineStateType(const std::shared_ptr<IXMLElement> &stateElement) {
    if (!stateElement) {
        return Type::ATOMIC;
    }
    ...
    auto stateChildren = SCE::ParsingCommon::findChildElements(stateElement, "state");
    auto parallelChildren = SCE::ParsingCommon::findChildElements(stateElement, "parallel");
    auto finalChildren = SCE::ParsingCommon::findChildElements(stateElement, "final");
    auto historyChildren = SCE::ParsingCommon::findChildElements(stateElement, "history");
...
void SCE::StateNodeParser::parseTransitions(const std::shared_ptr<IXMLElement> &parentElement,
                                            std::shared_ptr<SCE::IStateNode> state) {
    if (!parentElement || !state || !transitionParser_) {
        return;
    }

    auto transitionElements = SCE::ParsingCommon::findChildElements(parentElement, "transition");
...
void SCE::StateNodeParser::parseInvokeElements(const std::shared_ptr<IXMLElement> &parentElement,
    ...
    auto invokeElements = SCE::ParsingCommon::findChildElements(parentElement, "invoke");
...
void SCE::StateNodeParser::parseInitialElement(const std::shared_ptr<IXMLElement> &initialElement,
                                               std::shared_ptr<SCE::IStateNode> state) {
...
void SCE::StateNodeParser::parseEntryExitActionNodes(const std::shared_ptr<IXMLElement> &parentElement,
                                                     std::shared_ptr<SCE::IStateNode> state) {
...
void SCE::StateNodeParser::parseExecutableContentBlock(const std::shared_ptr<IXMLElement> &parentElement,
                                                       std::vector<std::shared_ptr<SCE::IActionNode>> &actionBlock) {
    if (!parentElement) {
        return;
    }
