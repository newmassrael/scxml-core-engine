...
CborError encodeBytes16Key(CborEncoder *m, int key, const std::array<uint8_t, 16> &v) {
    ...
    if (e == CborNoError) {
        e = cbor_encode_byte_string(m, v.data(), v.size());
    }
    return e;
}
...
CborError encodeStringKey(CborEncoder *m, int key, const std::string &v) {
    CborError e = encodeIntKey(m, key);
    if (e == CborNoError) {
        e = cbor_encode_text_string(m, v.data(), v.size());
    }
    return e;
}
...
CborError encodeBody(CborEncoder *m, const MeshEnvelope &env) {
    ...
    if (e == CborNoError) {
        e = encodeBytes16Key(m, kEnvelopeKeyId, env.id);
    }
    if (e == CborNoError) {
        e = encodeStringKey(m, kEnvelopeKeySource, env.source);
    }
    if (e == CborNoError) {
        e = encodeStringKey(m, kEnvelopeKeyType, env.type);
    }
    if (e == CborNoError) {
        e = encodeUintKey(m, kEnvelopeKeyPattern, static_cast<uint64_t>(env.pattern));
    }
    if (e == CborNoError) {
        e = encodeUintKey(m, kEnvelopeKeyDatacontenttype, static_cast<uint64_t>(env.datacontenttype));
    }
    if (e == CborNoError) {
        e = encodeByteStringKey(m, kEnvelopeKeyData, env.data);
    }
...
CborError tryEncode(const MeshEnvelope &env, uint8_t *buf, size_t cap, size_t &out_used, size_t &out_extra) {
    CborEncoder root;
...
bool readBytes16(CborValue *it, std::array<uint8_t, 16> &out) {
    ...
    size_t buflen = 16;
    if (cbor_value_copy_byte_string(it, out.data(), &buflen, it) != CborNoError) {
        return false;
    }
    return buflen == 16;
}
...
bool readText(CborValue *it, std::string &out) {
    ...
    if (cbor_value_copy_text_string(it, dst, &buflen, it) != CborNoError) {
        return false;
    }
    return buflen == len;
}
...
bool readU64(CborValue *it, uint64_t &out) {
    ...
    if (cbor_value_get_uint64(it, &out) != CborNoError) {
        return false;
    }
...
bool decodeEnvelope(const uint8_t *raw, std::size_t len, MeshEnvelope &out) {
    ...
    CborParser parser;
    CborValue root;
    if (cbor_parser_init(raw, len, 0, &parser, &root) != CborNoError) {
        return false;
    }
    ...
    bool seen_id = false, seen_source = false, seen_type = false, seen_pattern = false, seen_codec = false,
         seen_data = false;
    ...
    while (!cbor_value_at_end(&it)) {
        ...
        int key = 0;
        if (cbor_value_get_int_checked(&it, &key) != CborNoError) {
            return false;
        }
        ...
        switch (key) {
        case kEnvelopeKeyId:
            if (!readBytes16(&it, out.id)) {
                return false;
            }
            seen_id = true;
            break;
        case kEnvelopeKeySource:
            if (!readText(&it, out.source)) {
                return false;
            }
            seen_source = true;
            break;
        case kEnvelopeKeyType:
            if (!readText(&it, out.type)) {
                return false;
            }
            seen_type = true;
            break;
