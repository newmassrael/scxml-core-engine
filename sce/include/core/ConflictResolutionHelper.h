...
struct ConflictResolutionAlgorithms {
    ...
    struct TransitionDescriptor {
        ...
        bool isTargetless = false;  // §scxml-5.9.2: Whether transition has no target attribute
...
public:
    ...
    static std::vector<State> computeExitSet(State source, State target, bool isInternal = false,
        ...
        trans.isTargetless = isTargetless;  // §scxml-5.9.2: Pass targetless transition flag
