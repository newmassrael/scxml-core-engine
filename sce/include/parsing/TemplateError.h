...
// M1 (this header) declares only the base class and the
// `TemplateNotImplemented` sentinel — every non-passthrough
// shape the M1 skeleton encounters raises
// `TemplateNotImplemented` naming the milestone that will close
// the case. M2-M4 replace those sentinel throws with proper
// named subtypes mirroring `sce-build/src/template.rs::TemplateError`:
//
...
// M1-only sentinel raised for any `<sce:use>` shape the M1
// skeleton cannot safely expand. Carries a message naming the
// milestone that introduces proper handling, so an operator
// hitting the exception sees a pointed diagnostic rather than
// a silent mis-expansion or wrong-output bug.
//
