...
TEST_F(InternalChainIsBoundedTest, ADequeueThatSelectedNothingSpendsNoBudget) {
    sm_->processEvent("unanswered");

    EXPECT_EQ(counter("ignores"), "1000")
    ...
    EXPECT_EQ(sm_->getStatistics().lastTruncatedMacrostepState, "")
        << "and nothing names a state, because nothing was stopped";
    EXPECT_TRUE(sm_->isRunning()) << "the document settled on its own";
    EXPECT_EQ(sm_->getCurrentState(), "ignoring") << "the chain rests where it ended";
}
