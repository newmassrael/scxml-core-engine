...
protected:
    void SetUp() override {
        ...
        stateMachine = std::make_shared<StateMachine>(ScriptEngineProvider::getScriptEngine());

        // Create JSEngine session for this test
        sessionId = "history_integration_test";
        bool sessionCreated = SCE::ScriptEngineProvider::getScriptEngine().createSession(sessionId);
        if (!sessionCreated) {
            throw std::runtime_error("Failed to create script engine session for HistoryStateIntegrationTest");
        }
    }
    ...
    void TearDown() override {
        ...
        if (!sessionId.empty()) {
            SCE::ScriptEngineProvider::getScriptEngine().destroySession(sessionId);
        }
...
TEST_F(HistoryStateIntegrationTest, W3C_HistoryState_DefaultTransition_FirstVisit) {
    ...
    auto initResult = SCE::ScriptEngineProvider::getScriptEngine().executeScript(sessionId, "var result = '';").get();
...
TEST_F(HistoryStateIntegrationTest, W3C_HistoryState_StateRestoration_SubsequentVisit) {
    ...
    auto firstVisit = SCE::ScriptEngineProvider::getScriptEngine()
...
TEST_F(HistoryStateIntegrationTest, W3C_HistoryState_ShallowVsDeep_RestorationDifference) {
    ...
    auto clearResult = SCE::ScriptEngineProvider::getScriptEngine().executeScript(sessionId, "result = '';").get();
...
TEST_F(HistoryStateIntegrationTest, W3C_HistoryState_ExecutionOrder_OnentryBeforeTransition) {
    ...
    auto result = SCE::ScriptEngineProvider::getScriptEngine().evaluateExpression(sessionId, "execution_order").get();
    EXPECT_TRUE(result.isSuccess());
...
TEST_F(HistoryStateIntegrationTest, W3C_HistoryState_ComplexWorkflow_PauseAndResume) {
    ...
    // Step 2: Processing
    auto proceed = SCE::ScriptEngineProvider::getScriptEngine()
    ...
    // Step 3: Validation
    auto validate = SCE::ScriptEngineProvider::getScriptEngine()
                        .executeScript(sessionId, "step_count = step_count + 1; workflow_state = workflow_state + "
    ...
    // Step 4: Completion
    auto complete = SCE::ScriptEngineProvider::getScriptEngine()
                        .executeScript(sessionId, "step_count = step_count + 1; workflow_state = workflow_state + "
    ...
    // Pause workflow
    auto pauseWorkflow = SCE::ScriptEngineProvider::getScriptEngine()
