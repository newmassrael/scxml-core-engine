...
TEST(SendParamPayloadAotTest, SendParamsReachEventDataFromChildAndInternalQueue) {
    ...
    EXPECT_NE(sm.terminalState(), SM::State::FailChildPayload)
    ...
    EXPECT_NE(sm.terminalState(), SM::State::FailInternalPayload)
        << "`loopback` arrived without `_event.data.carried`: a `<send target=\"#_internal\">` must "
           ...
           "boundary.";
    EXPECT_NE(sm.terminalState(), SM::State::FailNumberType)
    ...
    EXPECT_NE(sm.terminalState(), SM::State::FailStringType)
    ...
    EXPECT_NE(sm.terminalState(), SM::State::FailDuplicateParams)
    ...
    EXPECT_NE(sm.terminalState(), SM::State::FailNoParamError)
        << "`withBadParam` arrived with no `error.execution` before it: W3C SCXML 5.7.1 puts that "
           ...
           "first.";
    EXPECT_NE(sm.terminalState(), SM::State::FailBrokenParamDelivered)
