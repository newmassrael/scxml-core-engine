...
impl<'a> CodecZenohQuery<'a> {
    ...
    /// [`CodecZenohQueryOwned`] (alloc). Call at a decode boundary when
    ...
    /// `feature = "alloc"`.
    pub fn into_owned(self) -> CodecZenohQueryOwned {
        CodecZenohQueryOwned {
            header: self.header,
            consolidation: self.consolidation,
            parameters_len: self.parameters_len,
            parameters: self.parameters.map(|_v| _v.to_vec()),
