...
impl<'a> CodecZenohOpenBody<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 158;
...
pub struct CodecZenohOpenBodyOwned {
    pub lease: u64,
    pub initial_sn: u64,
    pub cookie_len: Option<u64>,
    pub cookie: Option<::sce_forge_runtime::heapless::Vec<u8, 128>>,
}

impl<'a> CodecZenohOpenBody<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecZenohOpenBodyOwned`]. Call at a decode boundary when the
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohOpenBodyOwned, CodecError> {
        Ok(CodecZenohOpenBodyOwned {
            ...
            cookie: self.cookie.map(|_v| ::sce_forge_runtime::heapless::Vec::from_slice(_v).map_err(|_| CodecError::TooManyElements)).transpose()?,
        })
    }
}
