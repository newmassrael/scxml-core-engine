...
static inline sce_forge_codec_status_t codec_zenoh_undecl_queryable_decode(sce_forge_cursor_t *cursor, codec_zenoh_undecl_queryable_t *out, uint8_t z) {
 ...
 * Callers either pre-reserve CODEC_ZENOH_UNDECL_QUERYABLE_MAX_BYTES bytes and use
 * `codec_zenoh_undecl_queryable_encode_to_buf` (below), or run the writer themselves
 * for coalesced-send paths. */
static inline sce_forge_codec_status_t codec_zenoh_undecl_queryable_encode(const codec_zenoh_undecl_queryable_t *self, sce_forge_writer_t *w, uint8_t z) {
 ...
 * SCE_FORGE_CODEC_BUFFER_OVERFLOW when `cap < CODEC_ZENOH_UNDECL_QUERYABLE_MAX_BYTES`
 * was insufficient for this codec's wire bytes. Worst-case bound is
 * `CODEC_ZENOH_UNDECL_QUERYABLE_MAX_BYTES` — callers sizing `buf` accordingly never
 * see overflow. */
static inline sce_forge_codec_status_t codec_zenoh_undecl_queryable_encode_to_buf(const codec_zenoh_undecl_queryable_t *self, uint8_t *buf, size_t cap, size_t *out_len, uint8_t z) {
    sce_forge_writer_t _w = sce_forge_writer_init_buf(buf, cap);
    sce_forge_codec_status_t _st = codec_zenoh_undecl_queryable_encode(self, &_w, z);
    *out_len = _w.pos;
    return _st;
}
