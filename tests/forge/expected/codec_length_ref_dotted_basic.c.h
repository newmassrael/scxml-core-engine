...
typedef struct {
    uint8_t bytes[CODEC_LENGTH_REF_DOTTED_BASIC_MAX_BYTES];
    size_t  len;
} codec_length_ref_dotted_basic_encoded_t;
...
static inline codec_length_ref_dotted_basic_encoded_t codec_length_ref_dotted_basic_encode(const codec_length_ref_dotted_basic_t *self) {
    codec_length_ref_dotted_basic_encoded_t r;
    r.len = CODEC_LENGTH_REF_DOTTED_BASIC_MIN_BYTES;
    r.bytes[0] = self->carrier;
    if (self->payload_len <= 15) {
        memcpy(&r.bytes[1], self->payload, self->payload_len);
        r.len = 1 + self->payload_len;
    }
    return r;
}
