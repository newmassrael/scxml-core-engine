// SCE-MAP: codec_zenoh_timestamp:34 :: _forge_body
...
#define CODEC_ZENOH_TIMESTAMP_MIN_BYTES 0
#define CODEC_ZENOH_TIMESTAMP_MAX_BYTES 34
...
static inline sce_forge_codec_status_t codec_zenoh_timestamp_decode(sce_forge_cursor_t *cursor, codec_zenoh_timestamp_t *out) {
    ...
    uint64_t zid_len;
    {
        sce_forge_codec_status_t _vle_st = sce_forge_cursor_read_vle_u64(cursor, &zid_len);
        if (_vle_st != SCE_FORGE_CODEC_OK) return _vle_st;
    }
    out->zid_len = zid_len;
    {
        size_t _n = (size_t)out->zid_len;
...
static inline sce_forge_codec_status_t codec_zenoh_timestamp_encode(const codec_zenoh_timestamp_t *self, sce_forge_writer_t *w) {
    ...
    SCE_FORGE_TRY_WRITE(sce_forge_writer_write_vle_u64(w, (uint64_t)(self->zid_len)));
    {
