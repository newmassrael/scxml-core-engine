...
package codec_zenoh_undecl_queryable
...
// CodecZenohUndeclQueryable represents the codec frame layout.
type CodecZenohUndeclQueryable struct {
	Id uint32
	ExtKeyexpr *codec_zenoh_decl_ext_keyexpr.CodecZenohDeclExtKeyexpr
}

// DecodeCodecZenohUndeclQueryable decodes the next frame from cursor.
...
// Encode writes the CodecZenohUndeclQueryable into the caller-owned sink.
// Returns nil on success; codec.ErrBufferOverflow from a bounded sink
// when the destination has insufficient remaining capacity; growable
// sinks (e.g. BytesSink) are effectively infallible.
func (s *CodecZenohUndeclQueryable) Encode(w codec.SceSink, Z byte) error {
...
func (s *CodecZenohUndeclQueryable) EncodeToBytes(Z byte) []byte {
