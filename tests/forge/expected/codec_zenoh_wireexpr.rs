#![doc = "SCE-MAP: codec_zenoh_wireexpr:53 :: _forge_body"]
...
// build-configuration time: `CodecZenohWireexprOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohWireexprOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohWireexprOwned` alone is the build's
...
// `let v: CodecZenohWireexprOwned = CodecZenohWireexprOwned { .. };`
...
pub struct CodecZenohWireexprOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub id: u64,
    pub suffix_len: Option<u64>,
    pub suffix: Option<S::Str<128>>,
}

#[allow(dead_code)]
impl<'a> CodecZenohWireexpr<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecZenohWireexprOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohWireexprOwned<S>, CodecError> {
        Ok(CodecZenohWireexprOwned {
            id: self.id,
            suffix_len: self.suffix_len,
            suffix: self.suffix.map(<S::Str<128> as ::sce_forge_runtime::codec::SceStr>::from_view).transpose()?,
        })
    }
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohWireexprOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohWireexpr<'_> {
        CodecZenohWireexpr {
            id: self.id,
            suffix_len: self.suffix_len,
            suffix: self.suffix.as_ref().map(::sce_forge_runtime::codec::SceStr::as_str),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohWireexprOwned<D>, CodecError> {
