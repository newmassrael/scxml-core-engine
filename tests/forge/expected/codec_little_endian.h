...
struct CodecLittleEndian {
    ...
    [[nodiscard]] std::optional<::SCE::Forge::CodecError> encode(::SCE::Forge::SceSink& w) const noexcept {
        if (auto _e = w.write_u8(sensorId); _e) return _e;
        if (auto _e = w.write_u8(static_cast<uint8_t>(value & 0xFF)); _e) return _e;
        if (auto _e = w.write_u8(static_cast<uint8_t>((value >> 8) & 0xFF)); _e) return _e;
        if (auto _e = w.write_u8(status); _e) return _e;
        return std::nullopt;
    }
