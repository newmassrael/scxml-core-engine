#![doc = "SCE-MAP: codec_present_if_tail:8 :: _forge_body"]
...
pub struct CodecPresentIfTail<'a> {
    pub flags: u8,
    pub payload: Option<&'a [u8]>,
}

#[allow(dead_code)]
impl<'a> CodecPresentIfTail<'a> {
...
// `CodecPresentIfTail<'a>` above is a zero-copy view borrowing the decode
...
// `CodecPresentIfTailOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecPresentIfTailOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecPresentIfTailOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecPresentIfTailOwned` alone is the build's
...
// `let v: CodecPresentIfTailOwned = CodecPresentIfTailOwned { .. };`
...
pub struct CodecPresentIfTailOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub flags: u8,
    pub payload: Option<S::Bytes<64>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecPresentIfTailOwned<S> {
...
impl<'a> CodecPresentIfTail<'a> {
    ...
    /// [`CodecPresentIfTailOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecPresentIfTailOwned<S>, CodecError> {
        Ok(CodecPresentIfTailOwned {
            flags: self.flags,
            payload: self.payload.map(<S::Bytes<64> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice).transpose()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecPresentIfTailOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecPresentIfTailOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecPresentIfTail<'_> {
        CodecPresentIfTail {
            flags: self.flags,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecPresentIfTailOwned<D>, CodecError> {
