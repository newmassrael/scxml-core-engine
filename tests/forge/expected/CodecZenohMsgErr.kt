...
// `CodecZenohMsgErr()` before any encode()/decode() call. Defaults
...
data class CodecZenohMsgErr(
    ...
    fun X(): Boolean = (this.header.toInt() and 0x20) != 0
    ...
    fun E(): Boolean = (this.header.toInt() and 0x40) != 0
    ...
    fun encode(): ByteArray {
        ...
        run {
            var _w: ULong = (payload_len).toULong()
            while (_w >= 0x80UL) {
    ...
    companion object {
        ...
        /// (RFC §5.B L494-519).
        fun decode(cursor: SceCursor): CodecZenohMsgErr? {
            ...
            val payload_len = cursor.readVleU64() ?: return null
            val payload = run {
