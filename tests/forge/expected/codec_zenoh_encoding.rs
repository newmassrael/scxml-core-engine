...
impl<'a> CodecZenohEncoding<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 142;
...
// build-configuration time: `CodecZenohEncodingOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohEncodingOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohEncodingOwned` alone is the build's
...
// `let v: CodecZenohEncodingOwned = CodecZenohEncodingOwned { .. };`
...
pub struct CodecZenohEncodingOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub packed_id: u32,
    pub schema_len: Option<u64>,
    pub schema: Option<S::Str<128>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohEncodingOwned<S> {
...
impl<'a> CodecZenohEncoding<'a> {
    ...
    /// [`CodecZenohEncodingOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohEncodingOwned<S>, CodecError> {
        Ok(CodecZenohEncodingOwned {
            packed_id: self.packed_id,
            schema_len: self.schema_len,
            schema: self.schema.map(<S::Str<128> as ::sce_forge_runtime::codec::SceStr>::from_view).transpose()?,
        })
    }
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohEncodingOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohEncoding<'_> {
        CodecZenohEncoding {
            packed_id: self.packed_id,
            schema_len: self.schema_len,
            schema: self.schema.as_ref().map(::sce_forge_runtime::codec::SceStr::as_str),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohEncodingOwned<D>, CodecError> {
