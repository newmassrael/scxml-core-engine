...
impl<'a> CodecZenohTimestampExt<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let ts = {
            ...
            let _v = CodecZenohTimestamp::decode(&mut _inner)?;
            cursor.advance(_len)?;
            _v
        };
        Ok(Self {
...
// `CodecZenohTimestampExt<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohTimestampExtOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohTimestampExtOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohTimestampExtOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohTimestampExtOwned` alone is the build's
...
// `let v: CodecZenohTimestampExtOwned = CodecZenohTimestampExtOwned { .. };`
...
pub struct CodecZenohTimestampExtOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub ext_size: u64,
    pub ts: CodecZenohTimestampOwned<S>,
}

#[allow(dead_code)]
impl<'a> CodecZenohTimestampExt<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecZenohTimestampExtOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohTimestampExtOwned<S>, CodecError> {
        Ok(CodecZenohTimestampExtOwned {
            ext_size: self.ext_size,
            ts: self.ts.try_into_owned_in::<S>()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohTimestampExtOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohTimestampExtOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohTimestampExt<'_> {
        CodecZenohTimestampExt {
            ext_size: self.ext_size,
            ts: self.ts.as_borrowed(),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohTimestampExtOwned<D>, CodecError> {
