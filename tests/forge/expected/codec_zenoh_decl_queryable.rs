#![doc = "SCE-MAP: codec_zenoh_decl_queryable:46 :: _forge_body"]
...
pub struct CodecZenohDeclQueryable<'a> {
    pub id: u32,
...
#[allow(dead_code)]
impl<'a> CodecZenohDeclQueryable<'a> {
    ...
    pub fn encode<S: SceSink>(&self, w: &mut S, n: u8, z: u8) -> Result<(), CodecError> {
    ...
    /// primary `encode`.
    #[cfg(feature = "alloc")]
    pub fn encode_to_vec(&self, n: u8, z: u8) -> Vec<u8> {
        let mut _sce_v: Vec<u8> = Vec::with_capacity(Self::MAX_ENCODED_BYTES);
        let mut _sce_sink = VecSink::new(&mut _sce_v);
        self.encode(&mut _sce_sink, n, z)
...
// `CodecZenohDeclQueryable<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohDeclQueryableOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohDeclQueryableOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohDeclQueryableOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohDeclQueryableOwned` alone is the build's
...
// `let v: CodecZenohDeclQueryableOwned = CodecZenohDeclQueryableOwned { .. };`
...
pub struct CodecZenohDeclQueryableOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub id: u32,
    pub wireexpr: CodecZenohWireexprOwned<S>,
    pub ext_type: Option<u8>,
    pub ext_value: Option<u64>,
}
...
impl<'a> CodecZenohDeclQueryable<'a> {
    ...
    /// [`CodecZenohDeclQueryableOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohDeclQueryableOwned<S>, CodecError> {
        Ok(CodecZenohDeclQueryableOwned {
            id: self.id,
            wireexpr: self.wireexpr.try_into_owned_in::<S>()?,
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohDeclQueryableOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohDeclQueryableOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohDeclQueryable<'_> {
        CodecZenohDeclQueryable {
            id: self.id,
            wireexpr: self.wireexpr.as_borrowed(),
            ext_type: self.ext_type,
            ext_value: self.ext_value,
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohDeclQueryableOwned<D>, CodecError> {
