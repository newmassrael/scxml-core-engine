...
#define CODEC_VLE_ZINT_U64_MAX_BYTES 9
 ...
 * Callers either pre-reserve CODEC_VLE_ZINT_U64_MAX_BYTES bytes and use
 ...
 * SCE_FORGE_CODEC_BUFFER_OVERFLOW when `cap < CODEC_VLE_ZINT_U64_MAX_BYTES`
 * was insufficient for this codec's wire bytes. Worst-case bound is
 * `CODEC_VLE_ZINT_U64_MAX_BYTES` — callers sizing `buf` accordingly never
 * see overflow. */
static inline sce_forge_codec_status_t codec_vle_zint_u64_encode_to_buf(const codec_vle_zint_u64_t *self, uint8_t *buf, size_t cap, size_t *out_len) {
    sce_forge_writer_t _w = sce_forge_writer_init_buf(buf, cap);
    sce_forge_codec_status_t _st = codec_vle_zint_u64_encode(self, &_w);
    *out_len = _w.pos;
    return _st;
}
