#![doc = "SCE-MAP: codec_zenoh_interest_body:56 :: _forge_body"]
...
// build-configuration time: `CodecZenohInterestBodyOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohInterestBodyOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohInterestBodyOwned` alone is the build's
...
// `let v: CodecZenohInterestBodyOwned = CodecZenohInterestBodyOwned { .. };`
...
pub struct CodecZenohInterestBodyOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub keyexpr: Option<CodecZenohWireexprOwned<S>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohInterestBodyOwned<S> {
...
impl<'a> CodecZenohInterestBody<'a> {
    ...
    /// [`CodecZenohInterestBodyOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohInterestBodyOwned<S>, CodecError> {
        Ok(CodecZenohInterestBodyOwned {
            header: self.header,
            keyexpr: self.keyexpr.map(|_v| _v.try_into_owned_in::<S>()).transpose()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohInterestBodyOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohInterestBodyOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohInterestBody<'_> {
        CodecZenohInterestBody {
            header: self.header,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohInterestBodyOwned<D>, CodecError> {
