...
#define CODEC_ZENOH_LOCATOR_MAX_BYTES 138
...
typedef struct {
    uint8_t bytes[CODEC_ZENOH_LOCATOR_MAX_BYTES];
    size_t  len;
} codec_zenoh_locator_encoded_t;
...
static inline codec_zenoh_locator_encoded_t codec_zenoh_locator_encode(const codec_zenoh_locator_t *self) {
    codec_zenoh_locator_encoded_t r;
    ...
    r.len = 0;
    {
        uint64_t _w = (uint64_t)(self->locator_len);
        while (_w >= 0x80u) {
    ...
    for (size_t _bi = 0; _bi < self->locator_len; ++_bi) r.bytes[r.len++] = (uint8_t)self->locator[_bi];
    return r;
}
