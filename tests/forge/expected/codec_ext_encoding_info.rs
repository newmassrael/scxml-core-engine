...
// build-configuration time: `CodecExtEncodingInfoOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecExtEncodingInfoOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecExtEncodingInfoOwned` alone is the build's
...
// `let v: CodecExtEncodingInfoOwned = CodecExtEncodingInfoOwned { .. };`
...
pub struct CodecExtEncodingInfoOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub combined_id: u32,
    pub schema_size: u8,
    pub schema: Option<S::Bytes<64>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecExtEncodingInfoOwned<S> {
...
impl<'a> CodecExtEncodingInfo<'a> {
    ...
    /// [`CodecExtEncodingInfoOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecExtEncodingInfoOwned<S>, CodecError> {
        Ok(CodecExtEncodingInfoOwned {
            combined_id: self.combined_id,
            schema_size: self.schema_size,
            schema: self.schema.map(<S::Bytes<64> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice).transpose()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecExtEncodingInfoOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecExtEncodingInfoOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecExtEncodingInfo<'_> {
        CodecExtEncodingInfo {
            combined_id: self.combined_id,
            schema_size: self.schema_size,
            schema: self.schema.as_ref().map(::sce_forge_runtime::codec::SceByteBuf::as_slice),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecExtEncodingInfoOwned<D>, CodecError> {
