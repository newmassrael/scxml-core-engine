...
pub struct CodecExtEncodingInfoOwned {
    pub combined_id: u32,
    pub schema_size: u8,
    pub schema: Option<::sce_forge_runtime::heapless::Vec<u8, 64>>,
}
...
impl<'a> CodecExtEncodingInfo<'a> {
    ...
    /// [`CodecExtEncodingInfoOwned`]. Call at a decode boundary when the
    ...
    pub fn try_into_owned(self) -> Result<CodecExtEncodingInfoOwned, CodecError> {
        Ok(CodecExtEncodingInfoOwned {
            ...
            schema: self.schema.map(|_v| ::sce_forge_runtime::heapless::Vec::from_slice(_v).map_err(|_| CodecError::TooManyElements)).transpose()?,
        })
    }
}
