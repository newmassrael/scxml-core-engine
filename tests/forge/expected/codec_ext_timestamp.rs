#![doc = "SCE-MAP: codec_ext_timestamp:24 :: _forge_body"]
// SCE-MAP: codec_ext_timestamp:24 :: _forge_body
...
pub struct CodecExtTimestamp<'a> {
    pub time: u64,
    pub zid_size: u8,
    pub zid: &'a [u8],
}
...
// build-configuration time: `CodecExtTimestampOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecExtTimestampOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecExtTimestampOwned` alone is the build's
...
// `let v: CodecExtTimestampOwned = CodecExtTimestampOwned { .. };`
...
pub struct CodecExtTimestampOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub time: u64,
    pub zid_size: u8,
    pub zid: S::Bytes<16>,
}
...
impl<'a> CodecExtTimestamp<'a> {
    ...
    /// [`CodecExtTimestampOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecExtTimestampOwned<S>, CodecError> {
        Ok(CodecExtTimestampOwned {
            time: self.time,
            zid_size: self.zid_size,
            zid: <S::Bytes<16> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.zid)?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecExtTimestampOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecExtTimestampOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecExtTimestamp<'_> {
        CodecExtTimestamp {
            time: self.time,
            zid_size: self.zid_size,
            zid: ::sce_forge_runtime::codec::SceByteBuf::as_slice(&self.zid),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecExtTimestampOwned<D>, CodecError> {
