...
// `CodecVariantSessionOpen()` before any encode()/decode() call. Each default
...
data class CodecVariantSessionOpen(
    ...
    fun encode(w: SceSink): CodecError? {
        w.writeU8((version.toInt() ushr 8 and 0xFF).toByte())?.let { return it }
        w.writeU8((version.toInt() and 0xFF).toByte())?.let { return it }
        return null
    }
