...
pub struct CodecTlvChainPresentIfBasicOwned {
    pub carrier: u8,
    pub entries: Option<Vec<CodecTlvEntryOwned>>,
}
...
impl<'a> CodecTlvChainPresentIfBasic<'a> {
    ...
    /// [`CodecTlvChainPresentIfBasicOwned`]. Call at a decode boundary when the
    ...
    pub fn try_into_owned(self) -> Result<CodecTlvChainPresentIfBasicOwned, CodecError> {
        Ok(CodecTlvChainPresentIfBasicOwned {
            ...
            entries: self.entries.map(|_v| _v.into_iter().map(|_e| _e.try_into_owned()).collect::<Result<_, _>>()).transpose()?,
        })
    }
}
