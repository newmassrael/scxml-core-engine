...
typedef struct {
    uint8_t bytes[CODEC_INIT_COOKIE_BODY_MAX_BYTES];
    size_t  len;
} codec_init_cookie_body_encoded_t;
...
static inline sce_forge_codec_status_t codec_init_cookie_body_decode(sce_forge_cursor_t *cursor, codec_init_cookie_body_t *out, uint8_t parent_flags) {
...
static inline codec_init_cookie_body_encoded_t codec_init_cookie_body_encode(const codec_init_cookie_body_t *self, uint8_t parent_flags) {
    /* RFC §5.B B5-γ: see decode — same parameter, same suppress. */
    (void)parent_flags;
    codec_init_cookie_body_encoded_t r;
    ...
    if ((parent_flags & 0x20) != 0) {
    {
        uint64_t _w = (uint64_t)(self->cookie_size);
        while (_w >= 0x80u) {
    ...
    if ((parent_flags & 0x20) != 0) {
        for (size_t _bi = 0; _bi < self->cookie_len && _bi < self->cookie_size; ++_bi) r.bytes[r.len++] = self->cookie[_bi];
    }
    return r;
}
