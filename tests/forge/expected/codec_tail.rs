#![doc = "SCE-MAP: codec_tail:3 :: _forge_body"]
...
// `CodecTail<'a>` above is a zero-copy view borrowing the decode
...
// build-configuration time: `CodecTailOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecTailOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecTailOwned` alone is the build's
...
// `let v: CodecTailOwned = CodecTailOwned { .. };`
...
pub struct CodecTailOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub msg_id: u8,
    pub status: u8,
...
impl<'a> CodecTail<'a> {
    ...
    /// [`CodecTailOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecTailOwned<S>, CodecError> {
        Ok(CodecTailOwned {
            msg_id: self.msg_id,
            status: self.status,
    ...
    pub fn try_into_owned(self) -> Result<CodecTailOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecTailOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecTail<'_> {
        CodecTail {
            msg_id: self.msg_id,
            status: self.status,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecTailOwned<D>, CodecError> {
