...
typedef struct {
    uint8_t bytes[CODEC_ZENOH_RESPONSE_MAX_BYTES];
    size_t  len;
} codec_zenoh_response_encoded_t;
...
static inline codec_zenoh_response_encoded_t codec_zenoh_response_encode(const codec_zenoh_response_t *self) {
    codec_zenoh_response_encoded_t r;
    /* RFC §5.B Y3 atomic 2b-ii peek-byte / 2b-iv streaming-prefix:
    ...
    {
        uint64_t _w = (uint64_t)(self->key_id);
        while (_w >= 0x80u) {
            r.bytes[r.len++] = (uint8_t)((_w & 0x7Fu) | 0x80u);
            _w >>= 7;
        }
