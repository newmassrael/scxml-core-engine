...
typedef struct {
    uint8_t bytes[CODEC_PRESENT_IF_STRING_MAX_BYTES];
    size_t  len;
} codec_present_if_string_encoded_t;
...
static inline codec_present_if_string_encoded_t codec_present_if_string_encode(const codec_present_if_string_t *self) {
    codec_present_if_string_encoded_t r;
    ...
    if ((self->carrier & 0x01) != 0) {
        r.bytes[r.len++] = self->text_len;
    }
    if ((out->carrier & 0x01) != 0) {
        for (size_t _bi = 0; _bi < self->text_len; ++_bi) r.bytes[r.len++] = (uint8_t)self->text[_bi];
    }
    return r;
}
