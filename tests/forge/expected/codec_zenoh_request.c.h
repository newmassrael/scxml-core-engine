...
#define CODEC_ZENOH_REQUEST_MAX_BYTES 1380
 ...
 * trait — round-trip safety (`codec_zenoh_request_t x = CODEC_ZENOH_REQUEST_DEFAULT_INIT;
 * codec_zenoh_request_t_encode(&x)` decodes back to the same arm)
...
static inline sce_forge_codec_status_t codec_zenoh_request_encode(const codec_zenoh_request_t *self, sce_forge_writer_t *w) {
    ...
    switch (self->body.kind) {
        case CODEC_ZENOH_REQUEST_BODY_KIND_CODEC_ZENOH_MSG_PUT:
            SCE_FORGE_TRY_WRITE(codec_zenoh_msg_put_encode(&self->body.arm.codec_zenoh_msg_put, w));
            break;
        case CODEC_ZENOH_REQUEST_BODY_KIND_CODEC_ZENOH_MSG_DEL:
            SCE_FORGE_TRY_WRITE(codec_zenoh_msg_del_encode(&self->body.arm.codec_zenoh_msg_del, w));
            break;
        case CODEC_ZENOH_REQUEST_BODY_KIND_CODEC_ZENOH_QUERY:
            SCE_FORGE_TRY_WRITE(codec_zenoh_query_encode(&self->body.arm.codec_zenoh_query, w));
            break;
        case CODEC_ZENOH_REQUEST_BODY_KIND_DEFAULT:
            SCE_FORGE_TRY_WRITE(codec_zenoh_query_encode(&self->body.arm.default_body, w));
            break;
    }
    return SCE_FORGE_CODEC_OK;
}
 ...
 * SCE_FORGE_CODEC_BUFFER_OVERFLOW when `cap < CODEC_ZENOH_REQUEST_MAX_BYTES`
 * was insufficient for this codec's wire bytes. Worst-case bound is
 * `CODEC_ZENOH_REQUEST_MAX_BYTES` — callers sizing `buf` accordingly never
 * see overflow. */
static inline sce_forge_codec_status_t codec_zenoh_request_encode_to_buf(const codec_zenoh_request_t *self, uint8_t *buf, size_t cap, size_t *out_len) {
    sce_forge_writer_t _w = sce_forge_writer_init_buf(buf, cap);
    sce_forge_codec_status_t _st = codec_zenoh_request_encode(self, &_w);
    *out_len = _w.pos;
    return _st;
}
