// SCE-MAP: codec_length_ref:3 :: _forge_body
 ...
 * `codec_length_ref_encode_to_buf` (below), or run the writer themselves
 * for coalesced-send paths. */
static inline sce_forge_codec_status_t codec_length_ref_encode(const codec_length_ref_t *self, sce_forge_writer_t *w) {
 ...
 * SCE_FORGE_CODEC_BUFFER_OVERFLOW when `cap < CODEC_LENGTH_REF_MAX_BYTES`
 * was insufficient for this codec's wire bytes. Worst-case bound is
 * `CODEC_LENGTH_REF_MAX_BYTES` — callers sizing `buf` accordingly never
 * see overflow. */
static inline sce_forge_codec_status_t codec_length_ref_encode_to_buf(const codec_length_ref_t *self, uint8_t *buf, size_t cap, size_t *out_len) {
    sce_forge_writer_t _w = sce_forge_writer_init_buf(buf, cap);
    sce_forge_codec_status_t _st = codec_length_ref_encode(self, &_w);
    *out_len = _w.pos;
    return _st;
}
