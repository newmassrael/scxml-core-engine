...
namespace SCE::Generated::CodecLengthRefUint16Le {

struct CodecLengthRefUint16Le {
    ...
    /// `cursor.last_error()`.
    static std::optional<CodecLengthRefUint16Le> decode(::SCE::Forge::SceCursor& cursor) {
        ...
        uint16_t payload_len = static_cast<uint16_t>(raw[0] | (static_cast<uint16_t>(raw[1]) << 8));
        std::vector<uint8_t> payload = std::vector<uint8_t>(raw + 2, raw + 2 + payload_len);
        CodecLengthRefUint16Le _decoded{
    ...
    static constexpr std::size_t MAX_ENCODED_BYTES = 1026;
