...
static inline codec_peek_arm_b_encoded_t codec_peek_arm_b_encode(const codec_peek_arm_b_t *self) {
    codec_peek_arm_b_encoded_t r;
    r.len = CODEC_PEEK_ARM_B_MIN_BYTES;
    r.bytes[0] = self->header;
    r.bytes[1] = (uint8_t)((self->payload >> 8) & 0xFF);
    r.bytes[2] = (uint8_t)(self->payload & 0xFF);
    return r;
}
