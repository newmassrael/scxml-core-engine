...
package codec_zenoh_decl_kexpr
...
// CodecZenohDeclKexpr represents the codec frame layout.
type CodecZenohDeclKexpr struct {
	Id uint16
	Wireexpr codec_zenoh_wireexpr.CodecZenohWireexpr
}

// DecodeCodecZenohDeclKexpr decodes the next frame from cursor.
// On success the cursor advances past the consumed bytes; returns
// `codec.ErrNeedMoreBytes` (without advancing) when the cursor's tail
// is shorter than the declared minimum frame (RFC §synth-5-B L494-519).
// VLE codecs may also return `codec.ErrVLEWidthOverflow`.
func DecodeCodecZenohDeclKexpr(cursor *codec.SceCursor, N byte) (*CodecZenohDeclKexpr, error) {
...
// Encode writes the CodecZenohDeclKexpr into the caller-owned sink.
// Returns nil on success; codec.ErrBufferOverflow from a bounded sink
// when the destination has insufficient remaining capacity; growable
// sinks (e.g. BytesSink) are effectively infallible.
func (s *CodecZenohDeclKexpr) Encode(w codec.SceSink, N byte) error {
...
func (s *CodecZenohDeclKexpr) EncodeToBytes(N byte) []byte {
	_dst := make([]byte, 0, 259)
