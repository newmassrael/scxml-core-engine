...
struct CodecRepeatElem {
    ...
    [[nodiscard]] std::optional<::SCE::Forge::CodecError> encode(::SCE::Forge::SceSink& w) const noexcept {
        if (auto _e = w.write_u8(static_cast<uint8_t>((seq >> 8) & 0xFF)); _e) return _e;
        if (auto _e = w.write_u8(static_cast<uint8_t>(seq & 0xFF)); _e) return _e;
        return std::nullopt;
    }
