#![doc = "SCE-MAP: codec_zenoh_init_body:42 :: _forge_body"]
...
pub struct CodecZenohInitBody<'a> {
    pub version: u8,
    pub cbyte: u8,
    pub zid: &'a [u8],
...
// `CodecZenohInitBody<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohInitBodyOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohInitBodyOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohInitBodyOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohInitBodyOwned` alone is the build's
...
// `let v: CodecZenohInitBodyOwned = CodecZenohInitBodyOwned { .. };`
...
pub struct CodecZenohInitBodyOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub version: u8,
    pub cbyte: u8,
    pub zid: S::Bytes<16>,
    pub sn_res: Option<u8>,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohInitBodyOwned<S> {
...
impl<'a> CodecZenohInitBody<'a> {
    ...
    /// [`CodecZenohInitBodyOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohInitBodyOwned<S>, CodecError> {
        Ok(CodecZenohInitBodyOwned {
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohInitBodyOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohInitBodyOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohInitBody<'_> {
        CodecZenohInitBody {
            version: self.version,
            cbyte: self.cbyte,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohInitBodyOwned<D>, CodecError> {
