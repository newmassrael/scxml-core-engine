#![doc = "SCE-MAP: codec_length_ref_uint32_le:13 :: _forge_body"]
...
impl<'a> CodecLengthRefUint32Le<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        // lower bound.
        let mut _consumed: usize = 4;
        {
            let _end = 4usize + value.payload.len();
...
// `CodecLengthRefUint32Le<'a>` above is a zero-copy view borrowing the decode
...
// `CodecLengthRefUint32LeOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecLengthRefUint32LeOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecLengthRefUint32LeOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecLengthRefUint32LeOwned` alone is the build's
...
// `let v: CodecLengthRefUint32LeOwned = CodecLengthRefUint32LeOwned { .. };`
...
pub struct CodecLengthRefUint32LeOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub payload_len: u32,
...
impl<'a> CodecLengthRefUint32Le<'a> {
    ...
    /// [`CodecLengthRefUint32LeOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecLengthRefUint32LeOwned<S>, CodecError> {
        Ok(CodecLengthRefUint32LeOwned {
    ...
    pub fn try_into_owned(self) -> Result<CodecLengthRefUint32LeOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecLengthRefUint32LeOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecLengthRefUint32Le<'_> {
        CodecLengthRefUint32Le {
            payload_len: self.payload_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecLengthRefUint32LeOwned<D>, CodecError> {
