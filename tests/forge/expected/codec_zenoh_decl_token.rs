#![doc = "SCE-MAP: codec_zenoh_decl_token:28 :: _forge_body"]
// SCE-MAP: codec_zenoh_decl_token:28 :: _forge_body
...
pub struct CodecZenohDeclToken<'a> {
    pub id: u32,
    pub wireexpr: CodecZenohWireexpr<'a>,
}

#[allow(dead_code)]
impl<'a> CodecZenohDeclToken<'a> {
...
// `CodecZenohDeclToken<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohDeclTokenOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohDeclTokenOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohDeclTokenOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohDeclTokenOwned` alone is the build's
...
// `let v: CodecZenohDeclTokenOwned = CodecZenohDeclTokenOwned { .. };`
...
pub struct CodecZenohDeclTokenOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub id: u32,
...
impl<'a> CodecZenohDeclToken<'a> {
    ...
    /// [`CodecZenohDeclTokenOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohDeclTokenOwned<S>, CodecError> {
        Ok(CodecZenohDeclTokenOwned {
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohDeclTokenOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohDeclTokenOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohDeclToken<'_> {
        CodecZenohDeclToken {
            id: self.id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohDeclTokenOwned<D>, CodecError> {
