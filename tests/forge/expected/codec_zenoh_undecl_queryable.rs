#![doc = "SCE-MAP: codec_zenoh_undecl_queryable:23 :: _forge_body"]
...
pub struct CodecZenohUndeclQueryable<'a> {
    pub id: u32,
    pub ext_keyexpr: Option<CodecZenohDeclExtKeyexpr<'a>>,
}

#[allow(dead_code)]
impl<'a> CodecZenohUndeclQueryable<'a> {
...
// `CodecZenohUndeclQueryable<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohUndeclQueryableOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohUndeclQueryableOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohUndeclQueryableOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohUndeclQueryableOwned` alone is the build's
...
// `let v: CodecZenohUndeclQueryableOwned = CodecZenohUndeclQueryableOwned { .. };`
...
pub struct CodecZenohUndeclQueryableOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub id: u32,
    pub ext_keyexpr: Option<CodecZenohDeclExtKeyexprOwned<S>>,
}

#[allow(dead_code)]
impl<'a> CodecZenohUndeclQueryable<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecZenohUndeclQueryableOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohUndeclQueryableOwned<S>, CodecError> {
        Ok(CodecZenohUndeclQueryableOwned {
            id: self.id,
            ext_keyexpr: self.ext_keyexpr.map(|_v| _v.try_into_owned_in::<S>()).transpose()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohUndeclQueryableOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohUndeclQueryableOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohUndeclQueryable<'_> {
        CodecZenohUndeclQueryable {
            id: self.id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohUndeclQueryableOwned<D>, CodecError> {
