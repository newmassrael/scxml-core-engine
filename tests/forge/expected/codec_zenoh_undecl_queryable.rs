...
pub struct CodecZenohUndeclQueryableOwned {
    pub id: u32,
...
impl<'a> CodecZenohUndeclQueryable<'a> {
    ...
    /// [`CodecZenohUndeclQueryableOwned`] (alloc). Call at a decode boundary when
    ...
    /// `feature = "alloc"`.
    pub fn into_owned(self) -> CodecZenohUndeclQueryableOwned {
        CodecZenohUndeclQueryableOwned {
            id: self.id,
            ext_keyexpr: self.ext_keyexpr.map(|_v| _v.into_owned()),
        }
    }
}

#[cfg(feature = "alloc")]
impl CodecZenohUndeclQueryableOwned {
