#![doc = "SCE-MAP: codec_zenoh_decl_ext_keyexpr_inner:64 :: _forge_body"]
// SCE-MAP: codec_zenoh_decl_ext_keyexpr_inner:64 :: _forge_body
...
pub struct CodecZenohDeclExtKeyexprInner<'a> {
    pub inner_header: u8,
    pub id: u64,
...
#[allow(dead_code)]
impl<'a> CodecZenohDeclExtKeyexprInner<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let suffix = if (inner_header & 0x01u8) != 0 {
    ...
    pub fn n(&self) -> bool {
        (self.inner_header & 0x01) != 0
    }

    pub fn set_n(&mut self, v: bool) {
        if v {
            self.inner_header |= 0x01;
        } else {
            self.inner_header &= !0x01;
        }
    }
    ...
    pub fn m(&self) -> bool {
        (self.inner_header & 0x02) != 0
    }

    pub fn set_m(&mut self, v: bool) {
        if v {
            self.inner_header |= 0x02;
        } else {
            self.inner_header &= !0x02;
        }
    }
...
// `CodecZenohDeclExtKeyexprInner<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohDeclExtKeyexprInnerOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohDeclExtKeyexprInnerOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohDeclExtKeyexprInnerOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohDeclExtKeyexprInnerOwned` alone is the build's
...
// `let v: CodecZenohDeclExtKeyexprInnerOwned = CodecZenohDeclExtKeyexprInnerOwned { .. };`
...
pub struct CodecZenohDeclExtKeyexprInnerOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub inner_header: u8,
    pub id: u64,
    pub suffix: Option<S::Bytes<128>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohDeclExtKeyexprInnerOwned<S> {
...
impl<'a> CodecZenohDeclExtKeyexprInner<'a> {
    ...
    /// [`CodecZenohDeclExtKeyexprInnerOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohDeclExtKeyexprInnerOwned<S>, CodecError> {
        Ok(CodecZenohDeclExtKeyexprInnerOwned {
            inner_header: self.inner_header,
            id: self.id,
            suffix: self.suffix.map(<S::Bytes<128> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice).transpose()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohDeclExtKeyexprInnerOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohDeclExtKeyexprInnerOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohDeclExtKeyexprInner<'_> {
        CodecZenohDeclExtKeyexprInner {
            inner_header: self.inner_header,
            id: self.id,
            suffix: self.suffix.as_ref().map(::sce_forge_runtime::codec::SceByteBuf::as_slice),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohDeclExtKeyexprInnerOwned<D>, CodecError> {
