#![doc = "SCE-MAP: codec_length_ref_uint16_le:12 :: _forge_body"]
// SCE-MAP: codec_length_ref_uint16_le:12 :: _forge_body
...
pub struct CodecLengthRefUint16Le<'a> {
    pub payload_len: u16,
    pub payload: &'a [u8],
}

#[allow(dead_code)]
impl<'a> CodecLengthRefUint16Le<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 1026;
...
// `CodecLengthRefUint16Le<'a>` above is a zero-copy view borrowing the decode
...
// `CodecLengthRefUint16LeOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecLengthRefUint16LeOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecLengthRefUint16LeOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecLengthRefUint16LeOwned` alone is the build's
...
// `let v: CodecLengthRefUint16LeOwned = CodecLengthRefUint16LeOwned { .. };`
...
pub struct CodecLengthRefUint16LeOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub payload_len: u16,
    pub payload: S::Bytes<1024>,
}

#[allow(dead_code)]
impl<'a> CodecLengthRefUint16Le<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecLengthRefUint16LeOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecLengthRefUint16LeOwned<S>, CodecError> {
        Ok(CodecLengthRefUint16LeOwned {
            payload_len: self.payload_len,
            payload: <S::Bytes<1024> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.payload)?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecLengthRefUint16LeOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecLengthRefUint16LeOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecLengthRefUint16Le<'_> {
        CodecLengthRefUint16Le {
            payload_len: self.payload_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecLengthRefUint16LeOwned<D>, CodecError> {
