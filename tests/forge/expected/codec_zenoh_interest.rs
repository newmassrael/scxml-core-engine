...
#[cfg(feature = "alloc")]
#[derive(Debug, Clone, PartialEq)]
pub struct CodecZenohInterestOwned {
    pub header: u8,
    pub id: u64,
    pub body: Option<CodecZenohInterestBodyOwned>,
    pub extensions: Option<Vec<CodecZenohExtEntryOwned>>,
}

#[cfg(feature = "alloc")]
#[allow(dead_code)]
impl CodecZenohInterestOwned {
...
impl<'a> CodecZenohInterest<'a> {
    ...
    /// [`CodecZenohInterestOwned`]. Call at a decode boundary when the
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohInterestOwned, CodecError> {
        Ok(CodecZenohInterestOwned {
            header: self.header,
            id: self.id,
            body: self.body.map(|_v| _v.try_into_owned()).transpose()?,
