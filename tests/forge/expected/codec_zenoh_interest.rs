...
impl<'a> CodecZenohInterest<'a> {
    ...
    /// [`CodecZenohInterestOwned`] (alloc). Call at a decode boundary when
    ...
    /// `feature = "alloc"`.
    pub fn into_owned(self) -> CodecZenohInterestOwned {
        CodecZenohInterestOwned {
            header: self.header,
            id: self.id,
            body: self.body.map(|_v| _v.into_owned()),
