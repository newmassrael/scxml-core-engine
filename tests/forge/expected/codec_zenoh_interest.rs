#![doc = "SCE-MAP: codec_zenoh_interest:73 :: _forge_body"]
// SCE-MAP: codec_zenoh_interest:73 :: _forge_body
...
pub struct CodecZenohInterest<'a> {
    pub header: u8,
    pub id: u64,
    pub body: Option<CodecZenohInterestBody<'a>>,
...
impl<'a> Default for CodecZenohInterest<'a> {
    fn default() -> Self {
        Self {
            header: 0x19u8,
            id: Default::default(),
...
// `CodecZenohInterest<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohInterestOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohInterestOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohInterestOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohInterestOwned` alone is the build's
...
// `let v: CodecZenohInterestOwned = CodecZenohInterestOwned { .. };`
...
use super::codec_zenoh_interest_body::CodecZenohInterestBodyOwned;
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct CodecZenohInterestOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub id: u64,
    pub body: Option<CodecZenohInterestBodyOwned<S>>,
    pub extensions: Option<S::List<CodecZenohExtEntryOwned<S>, 16>>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohInterestOwned<S> {
...
impl<'a> CodecZenohInterest<'a> {
    ...
    /// [`CodecZenohInterestOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohInterestOwned<S>, CodecError> {
        Ok(CodecZenohInterestOwned {
            header: self.header,
            id: self.id,
            body: self.body.map(|_v| _v.try_into_owned_in::<S>()).transpose()?,
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohInterestOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohInterestOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<CodecZenohInterest<'_>, CodecError> {
        Ok(CodecZenohInterest {
            header: self.header,
            id: self.id,
            body: self.body.as_ref().map(|_v| _v.as_borrowed()),
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohInterestOwned<D>, CodecError> {
