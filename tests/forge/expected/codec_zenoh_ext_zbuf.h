...
namespace SCE::Generated::CodecZenohExtZbuf {

struct CodecZenohExtZbuf {
    uint64_t value_len;
    ...
    /// `cursor.last_error()`.
    static std::optional<CodecZenohExtZbuf> decode(::SCE::Forge::SceCursor& cursor) {
    ...
    static constexpr std::size_t MAX_ENCODED_BYTES = 41;
    ...
    [[nodiscard]] std::optional<::SCE::Forge::CodecError> encode(::SCE::Forge::SceSink& w) const noexcept {
        ...
        {
            std::uint64_t _w = static_cast<std::uint64_t>(value_len);
            std::uint32_t _vn = 0;
            while (_w >= 0x80 && _vn < 8) {
