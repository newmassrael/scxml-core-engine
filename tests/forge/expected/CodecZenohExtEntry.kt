...
// `CodecZenohExtEntry()` before any encode()/decode() call. Each default
...
data class CodecZenohExtEntry(
    ...
    fun encode(w: SceSink): CodecError? {
        ...
        when (val _b = this.body) {
            is CodecZenohExtEntryVariant.CodecZenohExtUnit -> _b.body.encode(w)?.let { return it }
            is CodecZenohExtEntryVariant.CodecZenohExtZint -> _b.body.encode(w)?.let { return it }
            is CodecZenohExtEntryVariant.CodecZenohExtZbuf -> _b.body.encode(w)?.let { return it }
            is CodecZenohExtEntryVariant.Default -> _b.body.encode(w)?.let { return it }
        }
        return null
    }
