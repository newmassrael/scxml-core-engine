...
// build-configuration time: `CodecDmaAlignedBasicOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecDmaAlignedBasicOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecDmaAlignedBasicOwned` alone is the build's
...
// `let v: CodecDmaAlignedBasicOwned = CodecDmaAlignedBasicOwned { .. };`
...
pub struct CodecDmaAlignedBasicOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub msg_id: u8,
    pub reserved: u8,
    pub aligned_payload: S::Bytes<64>,
}

#[allow(dead_code)]
impl<'a> CodecDmaAlignedBasic<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecDmaAlignedBasicOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecDmaAlignedBasicOwned<S>, CodecError> {
        Ok(CodecDmaAlignedBasicOwned {
            msg_id: self.msg_id,
            reserved: self.reserved,
            aligned_payload: <S::Bytes<64> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.aligned_payload)?,
        })
    }
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecDmaAlignedBasicOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecDmaAlignedBasic<'_> {
        CodecDmaAlignedBasic {
            msg_id: self.msg_id,
            reserved: self.reserved,
            aligned_payload: ::sce_forge_runtime::codec::SceByteBuf::as_slice(&self.aligned_payload),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecDmaAlignedBasicOwned<D>, CodecError> {
