#![doc = "SCE-MAP: codec_zenoh_reply:54 :: _forge_body"]
...
impl<'a> CodecZenohReply<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 426;
...
// `CodecZenohReply<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohReplyOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohReplyOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohReplyOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohReplyOwned` alone is the build's
...
// `let v: CodecZenohReplyOwned = CodecZenohReplyOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct CodecZenohReplyOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub consolidation: Option<u8>,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohReplyOwned<S> {
...
impl<'a> CodecZenohReply<'a> {
    ...
    /// [`CodecZenohReplyOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohReplyOwned<S>, CodecError> {
        Ok(CodecZenohReplyOwned {
            header: self.header,
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohReplyOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohReplyOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<CodecZenohReply<'_>, CodecError> {
        Ok(CodecZenohReply {
            header: self.header,
            consolidation: self.consolidation,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohReplyOwned<D>, CodecError> {
