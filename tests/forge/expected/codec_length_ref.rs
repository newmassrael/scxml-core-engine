#![doc = "SCE-MAP: codec_length_ref:3 :: _forge_body"]
...
impl<'a> CodecLengthRef<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let msg_id = raw[0];
        let len = raw[1];
        let payload = &raw[2..2 + len as usize];
        let value = Self {
            msg_id,
            len,
            payload,
...
// `CodecLengthRef<'a>` above is a zero-copy view borrowing the decode
...
// `CodecLengthRefOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecLengthRefOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecLengthRefOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecLengthRefOwned` alone is the build's
...
// `let v: CodecLengthRefOwned = CodecLengthRefOwned { .. };`
...
pub struct CodecLengthRefOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub msg_id: u8,
    pub len: u8,
    pub payload: S::Bytes<32>,
}

#[allow(dead_code)]
impl<'a> CodecLengthRef<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecLengthRefOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecLengthRefOwned<S>, CodecError> {
        Ok(CodecLengthRefOwned {
            msg_id: self.msg_id,
            len: self.len,
            payload: <S::Bytes<32> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.payload)?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecLengthRefOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecLengthRefOwned<S> {
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecLengthRefOwned<D>, CodecError> {
