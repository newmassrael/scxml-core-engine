...
typedef struct {
    uint8_t header;
    uint64_t id;
    /* RFC §5.B Y0c embed: nested codec_zenoh_interest_body_t struct (no length prefix on the wire) */
...
typedef struct {
    uint8_t bytes[CODEC_ZENOH_INTEREST_MAX_BYTES];
    size_t  len;
} codec_zenoh_interest_encoded_t;
...
static inline codec_zenoh_interest_encoded_t codec_zenoh_interest_encode(const codec_zenoh_interest_t *self) {
    codec_zenoh_interest_encoded_t r;
    ...
    if ((self->header & 0x20) != 0 || (self->header & 0x40) != 0) {
        codec_zenoh_interest_body_encoded_t _sub = codec_zenoh_interest_body_encode(&self->body);
