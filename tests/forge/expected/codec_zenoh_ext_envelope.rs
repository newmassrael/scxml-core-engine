#![doc = "SCE-MAP: codec_zenoh_ext_envelope:35 :: _forge_body"]
// SCE-MAP: codec_zenoh_ext_envelope:35 :: _forge_body
...
#[allow(dead_code)]
impl<'a> CodecZenohExtEnvelope<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 337;
...
// `CodecZenohExtEnvelope<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohExtEnvelopeOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohExtEnvelopeOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohExtEnvelopeOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohExtEnvelopeOwned` alone is the build's
...
// `let v: CodecZenohExtEnvelopeOwned = CodecZenohExtEnvelopeOwned { .. };`
...
pub struct CodecZenohExtEnvelopeOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header_flags: u8,
    pub extensions: S::List<CodecZenohExtEntryOwned<S>, 16>,
}

#[allow(dead_code)]
impl<'a> CodecZenohExtEnvelope<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecZenohExtEnvelopeOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohExtEnvelopeOwned<S>, CodecError> {
        Ok(CodecZenohExtEnvelopeOwned {
            header_flags: self.header_flags,
            extensions: ::sce_forge_runtime::codec::try_collect_list(self.extensions, |_e| _e.try_into_owned_in::<S>())?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohExtEnvelopeOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohExtEnvelopeOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<CodecZenohExtEnvelope<'_>, CodecError> {
        Ok(CodecZenohExtEnvelope {
            header_flags: self.header_flags,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohExtEnvelopeOwned<D>, CodecError> {
