...
// Encode writes the CodecZenohPushBody into the caller-owned sink.
// Returns nil on success; codec.ErrBufferOverflow from a bounded sink
// when the destination has insufficient remaining capacity; growable
// sinks (e.g. BytesSink) are effectively infallible.
func (s *CodecZenohPushBody) Encode(w codec.SceSink) error {
	...
	case s.Body.CodecZenohPut != nil:
		if err := s.Body.CodecZenohPut.Encode(w); err != nil {
			return err
		}
	case s.Body.CodecZenohDel != nil:
		if err := s.Body.CodecZenohDel.Encode(w); err != nil {
			return err
		}
...
func (s *CodecZenohPushBody) EncodeToBytes() []byte {
	_dst := make([]byte, 0, 2)
