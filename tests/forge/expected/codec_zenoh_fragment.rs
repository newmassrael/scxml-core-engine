#![doc = "SCE-MAP: codec_zenoh_fragment:23 :: _forge_body"]
...
// `CodecZenohFragment<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohFragmentOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohFragmentOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohFragmentOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohFragmentOwned` alone is the build's
...
// `let v: CodecZenohFragmentOwned = CodecZenohFragmentOwned { .. };`
...
pub struct CodecZenohFragmentOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub sn: u64,
...
impl<'a> CodecZenohFragment<'a> {
    ...
    /// [`CodecZenohFragmentOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohFragmentOwned<S>, CodecError> {
        Ok(CodecZenohFragmentOwned {
            sn: self.sn,
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohFragmentOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohFragmentOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohFragment<'_> {
        CodecZenohFragment {
            sn: self.sn,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohFragmentOwned<D>, CodecError> {
