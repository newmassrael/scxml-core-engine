...
// build-configuration time: `CodecExtAttachmentOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecExtAttachmentOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecExtAttachmentOwned` alone is the build's
...
// `let v: CodecExtAttachmentOwned = CodecExtAttachmentOwned { .. };`
...
pub struct CodecExtAttachmentOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub length: u8,
    pub body: S::Bytes<64>,
}
...
impl<'a> CodecExtAttachment<'a> {
    ...
    /// [`CodecExtAttachmentOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecExtAttachmentOwned<S>, CodecError> {
        Ok(CodecExtAttachmentOwned {
            length: self.length,
            body: <S::Bytes<64> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.body)?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecExtAttachmentOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecExtAttachmentOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecExtAttachment<'_> {
        CodecExtAttachment {
            length: self.length,
            body: ::sce_forge_runtime::codec::SceByteBuf::as_slice(&self.body),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecExtAttachmentOwned<D>, CodecError> {
