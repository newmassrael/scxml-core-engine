...
// build-configuration time: `CodecZenohMsgPutOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohMsgPutOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohMsgPutOwned` alone is the build's
...
// `let v: CodecZenohMsgPutOwned = CodecZenohMsgPutOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct CodecZenohMsgPutOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub timestamp: Option<CodecZenohTimestampOwned<S>>,
    pub encoding: Option<CodecZenohEncodingOwned<S>>,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohMsgPutOwned<S> {
...
impl<'a> CodecZenohMsgPut<'a> {
    ...
    /// [`CodecZenohMsgPutOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohMsgPutOwned<S>, CodecError> {
        Ok(CodecZenohMsgPutOwned {
            header: self.header,
            timestamp: self.timestamp.map(|_v| _v.try_into_owned_in::<S>()).transpose()?,
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohMsgPutOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohMsgPutOwned<S> {
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohMsgPutOwned<D>, CodecError> {
