...
pub struct CodecInitCookieBody<'a> {
    pub version: u8,
...
impl<'a> CodecInitCookieBody<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 68;
...
// build-configuration time: `CodecInitCookieBodyOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecInitCookieBodyOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecInitCookieBodyOwned` alone is the build's
...
// `let v: CodecInitCookieBodyOwned = CodecInitCookieBodyOwned { .. };`
...
pub struct CodecInitCookieBodyOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub version: u8,
    pub cookie_size: Option<u16>,
    pub cookie: Option<S::Bytes<64>>,
}

#[allow(dead_code)]
impl<'a> CodecInitCookieBody<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecInitCookieBodyOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecInitCookieBodyOwned<S>, CodecError> {
        Ok(CodecInitCookieBodyOwned {
            version: self.version,
            cookie_size: self.cookie_size,
            cookie: self.cookie.map(<S::Bytes<64> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice).transpose()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecInitCookieBodyOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecInitCookieBodyOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecInitCookieBody<'_> {
        CodecInitCookieBody {
            version: self.version,
            cookie_size: self.cookie_size,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecInitCookieBodyOwned<D>, CodecError> {
