#![doc = "SCE-MAP: codec_zenoh_timestamp:34 :: _forge_body"]
...
pub struct CodecZenohTimestamp<'a> {
    pub time: u64,
    pub zid_len: u64,
    pub zid: &'a [u8],
}
...
// `CodecZenohTimestamp<'a>` above is a zero-copy view borrowing the decode
...
// build-configuration time: `CodecZenohTimestampOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohTimestampOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohTimestampOwned` alone is the build's
...
// `let v: CodecZenohTimestampOwned = CodecZenohTimestampOwned { .. };`
...
pub struct CodecZenohTimestampOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub time: u64,
    pub zid_len: u64,
    pub zid: S::Bytes<16>,
}
...
impl<'a> CodecZenohTimestamp<'a> {
    ...
    /// [`CodecZenohTimestampOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohTimestampOwned<S>, CodecError> {
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohTimestampOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohTimestampOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohTimestamp<'_> {
        CodecZenohTimestamp {
            time: self.time,
            zid_len: self.zid_len,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohTimestampOwned<D>, CodecError> {
