...
pub struct CodecPresentIfStringOwned {
    pub carrier: u8,
    pub text_len: Option<u8>,
    pub text: Option<::sce_forge_runtime::heapless::String<32>>,
}

#[allow(dead_code)]
impl CodecPresentIfStringOwned {
...
impl<'a> CodecPresentIfString<'a> {
    ...
    /// [`CodecPresentIfStringOwned`]. Call at a decode boundary when the
    ...
    pub fn try_into_owned(self) -> Result<CodecPresentIfStringOwned, CodecError> {
        Ok(CodecPresentIfStringOwned {
            ...
            text: self.text.map(|_v| ::sce_forge_runtime::heapless::String::try_from(_v).map_err(|_| CodecError::TooManyElements)).transpose()?,
        })
    }
}
