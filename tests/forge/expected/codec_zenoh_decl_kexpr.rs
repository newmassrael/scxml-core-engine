#![doc = "SCE-MAP: codec_zenoh_decl_kexpr:47 :: _forge_body"]
...
// `CodecZenohDeclKexpr<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohDeclKexprOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohDeclKexprOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohDeclKexprOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohDeclKexprOwned` alone is the build's
...
// `let v: CodecZenohDeclKexprOwned = CodecZenohDeclKexprOwned { .. };`
...
pub struct CodecZenohDeclKexprOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub id: u16,
...
impl<'a> CodecZenohDeclKexpr<'a> {
    ...
    /// [`CodecZenohDeclKexprOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohDeclKexprOwned<S>, CodecError> {
        Ok(CodecZenohDeclKexprOwned {
            id: self.id,
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohDeclKexprOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohDeclKexprOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohDeclKexpr<'_> {
        CodecZenohDeclKexpr {
            id: self.id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohDeclKexprOwned<D>, CodecError> {
