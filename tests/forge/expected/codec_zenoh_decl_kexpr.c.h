...
typedef struct {
    uint8_t bytes[CODEC_ZENOH_DECL_KEXPR_MAX_BYTES];
    size_t  len;
} codec_zenoh_decl_kexpr_encoded_t;
...
static inline sce_forge_codec_status_t codec_zenoh_decl_kexpr_decode(sce_forge_cursor_t *cursor, codec_zenoh_decl_kexpr_t *out, uint8_t parent_flags) {
...
static inline codec_zenoh_decl_kexpr_encoded_t codec_zenoh_decl_kexpr_encode(const codec_zenoh_decl_kexpr_t *self, uint8_t parent_flags) {
    /* RFC §5.B B5-γ: see decode — same parameter, same suppress. */
    (void)parent_flags;
    codec_zenoh_decl_kexpr_encoded_t r;
