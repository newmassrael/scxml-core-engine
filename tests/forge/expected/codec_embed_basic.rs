#![doc = "SCE-MAP: codec_embed_basic:43 :: _forge_body"]
...
// build-configuration time: `CodecEmbedBasicOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecEmbedBasicOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecEmbedBasicOwned` alone is the build's
...
// `let v: CodecEmbedBasicOwned = CodecEmbedBasicOwned { .. };`
...
pub struct CodecEmbedBasicOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub tag: u8,
    pub locator: CodecZenohLocatorOwned<S>,
}

#[allow(dead_code)]
impl<'a> CodecEmbedBasic<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecEmbedBasicOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecEmbedBasicOwned<S>, CodecError> {
        Ok(CodecEmbedBasicOwned {
            tag: self.tag,
            locator: self.locator.try_into_owned_in::<S>()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecEmbedBasicOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecEmbedBasicOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecEmbedBasic<'_> {
        CodecEmbedBasic {
            tag: self.tag,
            locator: self.locator.as_borrowed(),
        }
    }
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecEmbedBasicOwned<D>, CodecError> {
