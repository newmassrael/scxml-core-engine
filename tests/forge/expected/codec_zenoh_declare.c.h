...
#ifndef SCE_FORGE_CODEC_ZENOH_DECLARE_H
#define SCE_FORGE_CODEC_ZENOH_DECLARE_H
...
typedef struct {
    ...
    /* RFC §synth-5-B embed: nested codec_zenoh_declaration_t struct (no length prefix on the wire) */
    codec_zenoh_declaration_t declaration;
} codec_zenoh_declare_t;
 ...
 * trait — round-trip safety (`codec_zenoh_declare_t x = CODEC_ZENOH_DECLARE_DEFAULT_INIT;
 * codec_zenoh_declare_t_encode(&x)` decodes back to the same arm)
...
#define CODEC_ZENOH_DECLARE_DEFAULT_INIT { \
    .header = 0x1eu, \
}
...
static inline sce_forge_codec_status_t codec_zenoh_declare_decode(sce_forge_cursor_t *cursor, codec_zenoh_declare_t *out) {
    ...
    {
        sce_forge_codec_status_t _st = codec_zenoh_declaration_decode(cursor, &out->declaration);
        if (_st != SCE_FORGE_CODEC_OK) return _st;
    }
 ...
 * Callers either pre-reserve CODEC_ZENOH_DECLARE_MAX_BYTES bytes and use
 * `codec_zenoh_declare_encode_to_buf` (below), or run the writer themselves
 * for coalesced-send paths. */
static inline sce_forge_codec_status_t codec_zenoh_declare_encode(const codec_zenoh_declare_t *self, sce_forge_writer_t *w) {
    ...
    SCE_FORGE_TRY_WRITE(codec_zenoh_declaration_encode(&self->declaration, w));
    return SCE_FORGE_CODEC_OK;
}
 ...
 * SCE_FORGE_CODEC_BUFFER_OVERFLOW when `cap < CODEC_ZENOH_DECLARE_MAX_BYTES`
 * was insufficient for this codec's wire bytes. Worst-case bound is
 * `CODEC_ZENOH_DECLARE_MAX_BYTES` — callers sizing `buf` accordingly never
 * see overflow. */
static inline sce_forge_codec_status_t codec_zenoh_declare_encode_to_buf(const codec_zenoh_declare_t *self, uint8_t *buf, size_t cap, size_t *out_len) {
    sce_forge_writer_t _w = sce_forge_writer_init_buf(buf, cap);
    sce_forge_codec_status_t _st = codec_zenoh_declare_encode(self, &_w);
    *out_len = _w.pos;
    return _st;
}
...
static inline uint8_t codec_zenoh_declare_mid(const codec_zenoh_declare_t *self) {
    return (uint8_t)((self->header >> 0) & (uint8_t)0x1F);
}

static inline void codec_zenoh_declare_set_mid(codec_zenoh_declare_t *self, uint8_t v) {
...
static inline bool codec_zenoh_declare_i(const codec_zenoh_declare_t *self) {
    return (self->header & 0x20) != 0;
}

static inline void codec_zenoh_declare_set_i(codec_zenoh_declare_t *self, bool v) {
    if (v) {
...
static inline bool codec_zenoh_declare_z(const codec_zenoh_declare_t *self) {
    return (self->header & 0x80) != 0;
}

static inline void codec_zenoh_declare_set_z(codec_zenoh_declare_t *self, bool v) {
    if (v) {
...
#endif  /* SCE_FORGE_CODEC_ZENOH_DECLARE_H */
