...
typedef struct {
    uint8_t bytes[CODEC_ZENOH_DECLARE_MAX_BYTES];
    size_t  len;
} codec_zenoh_declare_encoded_t;
...
static inline codec_zenoh_declare_encoded_t codec_zenoh_declare_encode(const codec_zenoh_declare_t *self) {
    codec_zenoh_declare_encoded_t r;
    ...
    {
        uint64_t _w = (uint64_t)(self->interest_id);
        while (_w >= 0x80u) {
    ...
    {
        codec_zenoh_declaration_encoded_t _sub = codec_zenoh_declaration_encode(&self->declaration);
        if (r.len + _sub.len <= sizeof(r.bytes)) {
