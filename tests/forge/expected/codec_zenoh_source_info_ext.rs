#![doc = "SCE-MAP: codec_zenoh_source_info_ext:49 :: _forge_body"]
// SCE-MAP: codec_zenoh_source_info_ext:49 :: _forge_body
...
use super::codec_zenoh_source_info::CodecZenohSourceInfo;
...
pub struct CodecZenohSourceInfoExt<'a> {
    pub ext_size: u64,
    pub info: CodecZenohSourceInfo<'a>,
}
...
impl<'a> CodecZenohSourceInfoExt<'a> {
    ...
    pub fn decode(cursor: &mut SceCursor<'a>) -> Result<Self, CodecError> {
        ...
        let ext_size = cursor.read_vle_u64()?;
        let info = {
            let _len = ext_size as usize;
            let _raw = cursor.peek_slice(_len)?;
            let mut _inner = SceCursor::new(_raw);
            let _v = CodecZenohSourceInfo::decode(&mut _inner)?;
            cursor.advance(_len)?;
            _v
        };
        Ok(Self {
    ...
    pub const MAX_ENCODED_BYTES: usize = 265;
...
// build-configuration time: `CodecZenohSourceInfoExtOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohSourceInfoExtOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohSourceInfoExtOwned` alone is the build's
...
// `let v: CodecZenohSourceInfoExtOwned = CodecZenohSourceInfoExtOwned { .. };`
...
pub struct CodecZenohSourceInfoExtOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub ext_size: u64,
    pub info: CodecZenohSourceInfoOwned<S>,
}

#[allow(dead_code)]
impl<'a> CodecZenohSourceInfoExt<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecZenohSourceInfoExtOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohSourceInfoExtOwned<S>, CodecError> {
        Ok(CodecZenohSourceInfoExtOwned {
            ext_size: self.ext_size,
            info: self.info.try_into_owned_in::<S>()?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohSourceInfoExtOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohSourceInfoExtOwned<S> {
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohSourceInfoExtOwned<D>, CodecError> {
