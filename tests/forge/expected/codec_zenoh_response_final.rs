#![doc = "SCE-MAP: codec_zenoh_response_final:41 :: _forge_body"]
...
pub struct CodecZenohResponseFinal<'a> {
    pub header: u8,
    pub request_id: u64,
...
impl<'a> Default for CodecZenohResponseFinal<'a> {
    fn default() -> Self {
        Self {
            header: 0x1au8,
...
#[allow(dead_code)]
impl<'a> CodecZenohResponseFinal<'a> {
...
// `CodecZenohResponseFinal<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohResponseFinalOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohResponseFinalOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohResponseFinalOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohResponseFinalOwned` alone is the build's
...
// `let v: CodecZenohResponseFinalOwned = CodecZenohResponseFinalOwned { .. };`
...
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq)]
pub struct CodecZenohResponseFinalOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub header: u8,
    pub request_id: u64,
...
#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohResponseFinalOwned<S> {
...
impl<'a> CodecZenohResponseFinal<'a> {
    ...
    /// [`CodecZenohResponseFinalOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohResponseFinalOwned<S>, CodecError> {
        Ok(CodecZenohResponseFinalOwned {
            header: self.header,
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohResponseFinalOwned, CodecError> {
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohResponseFinalOwned<S> {
    ...
    pub fn try_as_borrowed(&self) -> Result<CodecZenohResponseFinal<'_>, CodecError> {
        Ok(CodecZenohResponseFinal {
            header: self.header,
            request_id: self.request_id,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohResponseFinalOwned<D>, CodecError> {
