...
// build-configuration time: `CodecNestedParentOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecNestedParentOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecNestedParentOwned` alone is the build's
...
// `let v: CodecNestedParentOwned = CodecNestedParentOwned { .. };`
...
pub struct CodecNestedParentOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub hdr: u8,
    pub m: u8,
    pub required_body: CodecNestedBodyOwned<S>,
    pub optional_body: Option<CodecNestedBodyOwned<S>>,
    pub body_list: S::List<CodecNestedBodyOwned<S>, 4>,
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecNestedParentOwned<S> {
...
impl<'a> CodecNestedParent<'a> {
    ...
    /// [`CodecNestedParentOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecNestedParentOwned<S>, CodecError> {
        Ok(CodecNestedParentOwned {
            hdr: self.hdr,
            m: self.m,
            required_body: self.required_body.try_into_owned_in::<S>()?,
            optional_body: self.optional_body.map(|_v| _v.try_into_owned_in::<S>()).transpose()?,
            body_list: ::sce_forge_runtime::codec::try_collect_list(self.body_list, |_e| _e.try_into_owned_in::<S>())?,
        })
    }
...
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecNestedParentOwned<S> {
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecNestedParentOwned<D>, CodecError> {
