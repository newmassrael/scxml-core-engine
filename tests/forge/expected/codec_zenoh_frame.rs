#![doc = "SCE-MAP: codec_zenoh_frame:25 :: _forge_body"]
// SCE-MAP: codec_zenoh_frame:25 :: _forge_body
...
pub struct CodecZenohFrame<'a> {
    pub sn: u64,
    pub payload: &'a [u8],
}

#[allow(dead_code)]
impl<'a> CodecZenohFrame<'a> {
    ...
    pub const MAX_ENCODED_BYTES: usize = 65545;
...
// `CodecZenohFrame<'a>` above is a zero-copy view borrowing the decode
...
// `CodecZenohFrameOwned`. The rkyv-style Archived(borrowed) ↔ native
...
// build-configuration time: `CodecZenohFrameOwned<Heap>` holds growable
// containers with the declared capacities advisory, `CodecZenohFrameOwned<Inline>`
// holds every field inline at its declared capacity and never allocates, and
// both exist in the same binary. `CodecZenohFrameOwned` alone is the build's
...
// `let v: CodecZenohFrameOwned = CodecZenohFrameOwned { .. };`
...
pub struct CodecZenohFrameOwned<S: ::sce_forge_runtime::codec::CodecStorage = ::sce_forge_runtime::codec::DefaultStorage> {
    pub sn: u64,
    pub payload: S::Bytes<65536>,
}

#[allow(dead_code)]
impl<'a> CodecZenohFrame<'a> {
    /// Deep-copy this borrowed zero-copy view into an owned, lifetime-free
    /// [`CodecZenohFrameOwned`] held in the given storage profile. Call at
    ...
    /// way.
    pub fn try_into_owned_in<S: ::sce_forge_runtime::codec::CodecStorage>(self) -> Result<CodecZenohFrameOwned<S>, CodecError> {
        Ok(CodecZenohFrameOwned {
            sn: self.sn,
            payload: <S::Bytes<65536> as ::sce_forge_runtime::codec::SceByteBuf>::from_slice(self.payload)?,
        })
    }
    ...
    pub fn try_into_owned(self) -> Result<CodecZenohFrameOwned, CodecError> {
        self.try_into_owned_in()
    }
}

#[allow(dead_code)]
impl<S: ::sce_forge_runtime::codec::CodecStorage> CodecZenohFrameOwned<S> {
    ...
    pub fn as_borrowed(&self) -> CodecZenohFrame<'_> {
        CodecZenohFrame {
            sn: self.sn,
    ...
    /// than truncated.
    pub fn transcode_in<D: ::sce_forge_runtime::codec::CodecStorage>(&self) -> Result<CodecZenohFrameOwned<D>, CodecError> {
