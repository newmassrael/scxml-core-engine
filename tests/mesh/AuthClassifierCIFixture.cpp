...
void runOpenssl(const std::string &command) {
    ...
    if (rc != 0) {
        throw std::runtime_error("openssl invocation failed (rc=" + std::to_string(rc) + "): " + command);
    }
}
...
void generateCertMaterial(const fs::path &d) {
    ...
    runOpenssl("openssl req -x509 -new -nodes -key " + p("ca1.key") + " -sha256 -days 1 -out " + p("ca1.cert.pem") +
               " -subj '/CN=SCE-Test-CA1' >/dev/null 2>&1");
    runOpenssl("openssl genrsa -out " + p("ca2.key") + " 2048 >/dev/null 2>&1");
    runOpenssl("openssl req -x509 -new -nodes -key " + p("ca2.key") + " -sha256 -days 1 -out " + p("ca2.cert.pem") +
    ...
    runOpenssl("openssl x509 -req -in " + p("server.csr") + " -CA " + p("ca1.cert.pem") + " -CAkey " + p("ca1.key") +
               " -CAcreateserial -out " + p("server.cert.pem") + " -days 1 -sha256 -extfile " + p("server.ext") +
               " >/dev/null 2>&1");
    ...
    runOpenssl("openssl x509 -req -in " + p("client.csr") + " -CA " + p("ca2.cert.pem") + " -CAkey " + p("ca2.key") +
               " -CAcreateserial -out " + p("client.cert.pem") + " -days 1 -sha256 -extfile " + p("client.ext") +
               " >/dev/null 2>&1");
}
...
void applyRouterConfig(zenoh::Config &config, const fs::path &d, int port) {
    ...
    config.insert_json5("listen/endpoints", "[" + jstr("tls/127.0.0.1:" + std::to_string(port)) + "]");
    config.insert_json5("transport/link/tls/root_ca_certificate", jstr(p + "/ca1.cert.pem"));
    config.insert_json5("transport/link/tls/listen_certificate", jstr(p + "/server.cert.pem"));
    config.insert_json5("transport/link/tls/listen_private_key", jstr(p + "/server.key"));
...
void applyClientConfig(zenoh::Config &config, const fs::path &d, int port) {
    ...
    config.insert_json5("scouting/multicast/enabled", "false");
    config.insert_json5("connect/endpoints", "[" + jstr("tls/127.0.0.1:" + std::to_string(port)) + "]");
    config.insert_json5("transport/link/tls/root_ca_certificate", jstr(p + "/ca1.cert.pem"));
    config.insert_json5("transport/link/tls/connect_certificate", jstr(p + "/client.cert.pem"));
    config.insert_json5("transport/link/tls/connect_private_key", jstr(p + "/client.key"));
    config.insert_json5("transport/link/tls/enable_mtls", "true");
    config.insert_json5("transport/link/tls/verify_name_on_connect", "false");
}
...
protected:
    static void SetUpTestSuite() {
        ...
        cert_env_ =
            std::make_unique<TempCertEnv>(fs::temp_directory_path() / ("sce_auth_ci_" + std::to_string(::getpid())));
...
std::string captureZenohMTLSHandshakeFailMessage(const fs::path &cert_dir) {
    ...
    auto server_session =
        zenoh::Session::open(std::move(server_config), zenoh::Session::SessionOptions::create_default(), &server_err);
    if (server_err != Z_OK) {
        throw std::runtime_error("server Session::open failed (likely zenoh-cpp build missing "
                                 "tls feature or port " +
                                 std::to_string(port) + " unbindable) — ZResult=" + std::to_string(server_err));
    }
...
TEST_F(AuthClassifierCIFixture, MTLSHandshakeFailureSurfacesAsZException) {
    ...
    ASSERT_FALSE(msg.empty()) << "zenoh client Session::open did NOT throw on a CA-mismatched mTLS "
