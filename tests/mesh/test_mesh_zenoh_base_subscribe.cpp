...
constexpr const char *kListen = SCE::Generated::brake_zenoh_base_subscribe::ZENOH_CONNECT_ENDPOINTS[0];
...
int run_test() {
    ...
    // wire path.
    auto sub_env = make_envelope("event.subscribe.status", SCE::Mesh::PatternKind::EventSubscribe);
    MESH_TEST_REQUIRE(router.send_zenoh(sub_env, kMotorKey, "#motor"), "send_zenoh EventSubscribe returned false");
