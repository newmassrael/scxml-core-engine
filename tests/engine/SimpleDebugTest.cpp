...
TEST(DebugTest, BasicJavaScriptExecution) {
  ...
  auto &engine = RSM::JSEngine::instance();
  ...
  ASSERT_TRUE(result.isSuccess())
      << "Failed to evaluate expression: " << result.errorMessage;
...
TEST(DebugTest, VariableAssignmentAndRetrieval) {
  ...
  // Assign variable
  auto assignResult =
      engine.executeScript(sessionId, "var testVar = 'Hello World'; testVar")
          .get();
...
TEST(DebugTest, SystemVariables) {
  ...
  auto sessionIdTypeResult =
      engine.evaluateExpression(sessionId, "typeof _sessionid").get();
  ...
  auto nameTypeResult =
      engine.evaluateExpression(sessionId, "typeof _name").get();
  ...
  auto ioTypeResult =
      engine.evaluateExpression(sessionId, "typeof _ioprocessors").get();
  ASSERT_TRUE(ioTypeResult.isSuccess());
  EXPECT_EQ(ioTypeResult.getValue<std::string>(), "object");

  // Test _event exists and is object
  auto eventTypeResult =
      engine.evaluateExpression(sessionId, "typeof _event").get();
...
TEST(DebugTest, ErrorHandling) {
  ...
  auto syntaxErrorResult =
      engine.evaluateExpression(sessionId, "var x = ;").get();
  EXPECT_FALSE(syntaxErrorResult.isSuccess())
      << "Syntax error should be caught";

  // Test reference error handling
  auto refErrorResult =
      engine.evaluateExpression(sessionId, "undefinedVariable").get();
  EXPECT_FALSE(refErrorResult.isSuccess())
      << "Reference error should be caught";

  // Test that engine continues to work after errors
  auto workingResult = engine.evaluateExpression(sessionId, "1 + 1").get();
  ASSERT_TRUE(workingResult.isSuccess())
      << "Engine should continue working after errors";
...
TEST(DebugTest, ComplexExpressions) {
  ...
  auto complexResult =
      engine
          .evaluateExpression(sessionId,
                              "_name.length > 0 && typeof _sessionid === "
  ...
  auto functionResult =
      engine
          .executeScript(sessionId, "function factorial(n) { return n <= 1 ? 1 "
  ...
  auto objectResult =
      engine
          .executeScript(sessionId,
                         "var obj = {a: 1, b: {c: 2}}; obj.b.c + obj.a")
          .get();
...
TEST(DebugTest, ConsoleLogging) {
  ...
  auto logResult =
      engine
          .executeScript(sessionId,
                         "console.log('Debug test message'); 'completed'")
          .get();
  ...
  auto multiLogResult =
      engine
          .executeScript(sessionId, "console.log('Multiple', 'arguments', 123, "
