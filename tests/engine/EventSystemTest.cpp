...
TEST_F(EventSystemTest, EventObjectStructure) {
  // Test _event exists
  auto eventResult =
      engine_->evaluateExpression(sessionId_, "typeof _event").get();
  ...
  for (const auto &prop : requiredProps) {
    ...
    EXPECT_TRUE(propResult.getValue<bool>())
        << "_event should have property: " << prop;
  }
}
...
TEST_F(EventSystemTest, EventObjectModification) {
  ...
  auto modifyResult =
      engine_
          ->executeScript(sessionId_, "_event.name = 'modified'; _event.name")
          .get();
...
TEST_F(EventSystemTest, EventDataHandling) {
  ...
  auto arrayDataResult =
      engine_
          ->executeScript(sessionId_,
                          "_event.data = [1, 2, 3]; _event.data.length")
          .get();
...
TEST_F(EventSystemTest, EventNameAndType) {
  ...
  auto nameResult =
      engine_
          ->executeScript(sessionId_, "_event.name = 'user.login'; _event.name")
          .get();
  ASSERT_TRUE(nameResult.isSuccess());
  ...
  auto typeResult =
      engine_
          ->executeScript(sessionId_, "_event.type = 'platform'; _event.type")
          .get();
  ASSERT_TRUE(typeResult.isSuccess());
  ...
  auto complexNameResult =
      engine_
          ->executeScript(
              sessionId_,
              "_event.name = 'error.execution.timeout'; _event.name")
          .get();
  ASSERT_TRUE(complexNameResult.isSuccess());
  EXPECT_EQ(complexNameResult.getValue<std::string>(),
...
TEST_F(EventSystemTest, EventOriginProperties) {
  ...
  auto originResult =
      engine_
          ->executeScript(sessionId_,
                          "_event.origin = '#_internal'; _event.origin")
          .get();
  ...
  EXPECT_EQ(origintypeResult.getValue<std::string>(),
            "http://www.w3.org/TR/scxml/#SCXMLEventProcessor");
  ...
  auto invokeidResult =
      engine_
          ->executeScript(sessionId_,
                          "_event.invokeid = 'invoke_123'; _event.invokeid")
          .get();
  ...
  auto sendidResult =
      engine_
          ->executeScript(sessionId_,
                          "_event.sendid = 'send_456'; _event.sendid")
          .get();
...
TEST_F(EventSystemTest, EventInExpressions) {
  ...
  auto setupResult =
      engine_
          ->executeScript(sessionId_,
                          "_event.name = 'user.action'; _event.data = {userId: "
  ...
  auto conditionalResult =
      engine_
          ->evaluateExpression(
              sessionId_,
              "_event.name === 'user.action' && _event.data.userId === 123")
          .get();
  ...
  auto nestedResult =
      engine_->evaluateExpression(sessionId_, "_event.data.action").get();
  ASSERT_TRUE(nestedResult.isSuccess());
  EXPECT_EQ(nestedResult.getValue<std::string>(), "click");

  // Test using event data in calculations
  auto calcResult =
      engine_->evaluateExpression(sessionId_, "_event.data.userId * 2").get();
...
TEST_F(EventSystemTest, EventSerialization) {
  ...
  auto serializeResult =
      engine_->evaluateExpression(sessionId_, "JSON.stringify(_event.data)")
          .get();
  ASSERT_TRUE(serializeResult.isSuccess());

  std::string expected =
      "{\"user\":{\"id\":1,\"name\":\"test\"},\"items\":[1,2,3]}";
  ...
  auto fullSerializeResult =
      engine_
          ->evaluateExpression(
              sessionId_,
              "JSON.stringify({name: _event.name, data: _event.data})")
          .get();
...
TEST_F(EventSystemTest, EventPersistence) {
  ...
  auto modifyResult =
      engine_
          ->executeScript(sessionId_,
                          "_event.data = 'modified_data'; _event.data")
          .get();
  ASSERT_TRUE(modifyResult.isSuccess());
