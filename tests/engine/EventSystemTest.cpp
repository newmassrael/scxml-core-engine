...
TEST_F(EventSystemTest, EventObjectModification) {
    ...
    // This documents current behavior - in full SCXML implementation, _event should be read-only
...
TEST_F(EventSystemTest, EventDataHandling) {
    ...
    auto simpleDataResult = engine_->executeScript(sessionId_, 
        "_event.data = 'simple_data'; _event.data").get();
    ASSERT_TRUE(simpleDataResult.isSuccess());
    EXPECT_EQ(simpleDataResult.getValue<std::string>(), "simple_data");

    // Test setting object data
    auto objectDataResult = engine_->executeScript(sessionId_, 
        "_event.data = {key: 'value', number: 42}; _event.data.key + '_' + _event.data.number").get();
    ASSERT_TRUE(objectDataResult.isSuccess());
    EXPECT_EQ(objectDataResult.getValue<std::string>(), "value_42");

    // Test setting array data
    auto arrayDataResult = engine_->executeScript(sessionId_, 
        "_event.data = [1, 2, 3]; _event.data.length").get();
...
TEST_F(EventSystemTest, EventNameAndType) {
    ...
    auto nameResult = engine_->executeScript(sessionId_, 
        "_event.name = 'user.login'; _event.name").get();
    ASSERT_TRUE(nameResult.isSuccess());
    EXPECT_EQ(nameResult.getValue<std::string>(), "user.login");

    // Test setting event type
    auto typeResult = engine_->executeScript(sessionId_, 
        "_event.type = 'platform'; _event.type").get();
    ASSERT_TRUE(typeResult.isSuccess());
    EXPECT_EQ(typeResult.getValue<std::string>(), "platform");

    // Test complex event names with dots
    auto complexNameResult = engine_->executeScript(sessionId_, 
        "_event.name = 'error.execution.timeout'; _event.name").get();
...
TEST_F(EventSystemTest, EventOriginProperties) {
    // Test setting origin
    auto originResult = engine_->executeScript(sessionId_, 
        "_event.origin = '#_internal'; _event.origin").get();
    ASSERT_TRUE(originResult.isSuccess());
    EXPECT_EQ(originResult.getValue<std::string>(), "#_internal");

    // Test setting origintype
    auto origintypeResult = engine_->executeScript(sessionId_, 
        "_event.origintype = 'http://www.w3.org/TR/scxml/#SCXMLEventProcessor'; _event.origintype").get();
    ...
    auto invokeidResult = engine_->executeScript(sessionId_, 
        "_event.invokeid = 'invoke_123'; _event.invokeid").get();
    ASSERT_TRUE(invokeidResult.isSuccess());
    EXPECT_EQ(invokeidResult.getValue<std::string>(), "invoke_123");

    // Test setting sendid
    auto sendidResult = engine_->executeScript(sessionId_, 
        "_event.sendid = 'send_456'; _event.sendid").get();
...
TEST_F(EventSystemTest, EventInExpressions) {
    ...
    auto setupResult = engine_->executeScript(sessionId_, 
        "_event.name = 'user.action'; _event.data = {userId: 123, action: 'click'}; true").get();
    ASSERT_TRUE(setupResult.isSuccess());

    // Test using event in conditional expressions
    auto conditionalResult = engine_->evaluateExpression(sessionId_, 
        "_event.name === 'user.action' && _event.data.userId === 123").get();
    ASSERT_TRUE(conditionalResult.isSuccess());
    EXPECT_TRUE(conditionalResult.getValue<bool>());

    // Test accessing nested event data
    auto nestedResult = engine_->evaluateExpression(sessionId_, 
        "_event.data.action").get();
    ASSERT_TRUE(nestedResult.isSuccess());
    EXPECT_EQ(nestedResult.getValue<std::string>(), "click");

    // Test using event data in calculations
    auto calcResult = engine_->evaluateExpression(sessionId_, 
        "_event.data.userId * 2").get();
...
TEST_F(EventSystemTest, EventSerialization) {
    ...
    auto setupResult = engine_->executeScript(sessionId_, 
        "_event.name = 'complex.event'; "
        "_event.data = {user: {id: 1, name: 'test'}, items: [1, 2, 3]}; true").get();
    ASSERT_TRUE(setupResult.isSuccess());

    // Test JSON serialization of event data
    auto serializeResult = engine_->evaluateExpression(sessionId_, 
        "JSON.stringify(_event.data)").get();
    ...
    auto fullSerializeResult = engine_->evaluateExpression(sessionId_, 
        "JSON.stringify({name: _event.name, data: _event.data})").get();
    ASSERT_TRUE(fullSerializeResult.isSuccess());
    
    std::string expectedFull = "{\"name\":\"complex.event\",\"data\":{\"user\":{\"id\":1,\"name\":\"test\"},\"items\":[1,2,3]}}";
...
TEST_F(EventSystemTest, EventPersistence) {
    ...
    auto setResult = engine_->executeScript(sessionId_, 
        "_event.name = 'persistent.event'; _event.data = 'persistent_data'; true").get();
    ...
    auto modifyResult = engine_->executeScript(sessionId_, 
        "_event.data = 'modified_data'; _event.data").get();
    ASSERT_TRUE(modifyResult.isSuccess());
