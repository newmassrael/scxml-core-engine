...
class Build:
    ...
    def unreadable(self, var: str) -> dict | None:
        """Why the build emitted no typed reader for `var`, if it said.

        W3C SCXML 5.3: the generator publishes each typed `<data>` it left
        without a reader in `unreadable_variables`, with the reason — a
        keyword no backend's escape reaches, a member the generated type
        already carries, two ids that fold together. Quoting it is the
        difference between "this output cannot be read" and why.
        """
        for record in self.manifest.get("unreadable_variables") or ():
            if record.get("var") == var:
                return record
        return None
...
class StatechartRun:
    ...
    def declared_value(self, name: str):
        ...
        reader = getattr(self.engine.policy, method, None) if method else None
        if reader is None:
            refused = self.build.unreadable(name)
            if refused is not None:
                where = (f" in {refused['language']}" if refused.get("language")
                         else "")
                raise VerifyError(
                    f"output {name!r}: the document declares it an output and "
                    f"the generator gave it no reader -- "
                    f"{refused.get('reason', 'no reason recorded')}{where}. "
                    f"A reader exists in every backend or in none, so rename "
                    f"the variable")
            raise VerifyError(
                f"output {name!r}: the document declares it an output and the "
                f"generated machine offers no accessor for it. One is emitted "
                f"for a variable whose type the document settles, so this "
                f"one's type is not settled")
        value = reader()
        if value is None:
            raise VerifyError(
...
def _verify(pack: Pack, binding_path: pathlib.Path, codegen: pathlib.Path | None,
    ...
    # The names the module gives this document's inputs, output functions and
    # record fields, read from the module rather than re-derived -- see
    # `_host_names`.
    parameters = _host_names(module, "parameters")
    functions = _host_names(module, "functions")
    fields = _host_names(module, "fields")

    def argument(name: str) -> str:
        spelled = parameters.get(name)
        if spelled is None:
            raise VerifyError(
                f"input {name!r}: the binding names it and the document "
                f"declares no input of that name, so there is no parameter to "
                f"pass it as")
        return spelled
    ...
    for case, owner, judged in rounds_of(examples.cases):
        ...
        try:
            ...
            continue
        kwargs = {argument(n): v for n, v in values.items()}
        ...
        if worlds is not None:
            ...
            try:
                records = [getattr(w, holder["update"])(
                               **kwargs, **{argument(k): v for k, v in a.items()})
        ...
        for name, rule in outputs.items():
            ...
            if name in withheld:
                ...
                continue
            fn_name, field_name = functions.get(name), fields.get(name)
            fn = (None if records is not None or fn_name is None else
                  getattr(module, fn_name, None))
            produces = ((field_name is not None and hasattr(records[0], field_name))
                        if records is not None else fn is not None)
            if not produces:
            ...
            try:
                ...
                runs = ([getattr(r, field_name) for r in records]
                        if records is not None else
                        [fn(**kwargs, **{argument(k): v for k, v in a.items()})
