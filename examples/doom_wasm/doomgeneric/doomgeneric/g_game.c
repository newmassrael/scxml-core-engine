...
void G_DoLoadLevel (void)
{
    ...
    SCE_SecretLevelChange();  // Scan targets for hint system    
...
{ 
	...
	if (AM_Responder (ev)) 
        ...
        // H key: Toggle secret hint system
        if (ev->type == ev_keydown && ev->data1 == 'h') {
            SCE_SecretHintToggle();
            return true;
        }
...
void G_DoWorldDone (void) 
{        
    // SCE: Moving to next level after intermission
    SCE_GameWorldDone();
    gamestate = GS_LEVEL; 
...
void G_DoLoadGame (void) 
{
    ...
    // SCE: Clear enemy tracking before restoring (mobj pointers will change)
    SCE_EnemyClearAll();
    ...
    // SCE: Re-register all live monsters after load
    SCE_EnemyRescanAll();
...
{ 
    ...
    P_ArchivePlayers ();
    Secret_PrepareForSave();  // Remove hint sprites before archive
    P_ArchiveWorld (); 
    P_ArchiveThinkers (); 
    P_ArchiveSpecials ();
    Secret_RestoreAfterSave();  // Respawn hint sprites after archive
    P_WriteSaveGameEOF();
