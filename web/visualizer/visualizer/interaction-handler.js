...
class InteractionHandler {
    ...
    async toggleCompoundState(stateId) {
        ...
        // Whether a compound is collapsed is an INPUT to the layout, so the
        // layout is run again against it — the geometry is not patched here
        // to match.
        //
        // ⚠ Everything below this call used to do the patching: set the
        // toggled node's size, assign default positions to children that had
        // none, push neighbours out of the way, recompute the parent's
        // bounds, then re-derive every route. Measured on
        // `ancestor_entry_is_not_default_entry`, collapsing `outer` left
        // FIVE edges not touching the states they join — and it did so
        // before any of this round's changes, because patched geometry and
        // the routes computed against it drift apart by construction.
        //
        // `buildELKGraph` already reads `collapsed`: `getVisibleNodes()`
