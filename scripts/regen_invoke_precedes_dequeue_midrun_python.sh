...
#
# Regenerate backends/python/tests/integration/invoke_precedes_dequeue_midrun/*_sm.py
...
# files. The pybind11 channel test at `backends/python/bindings/tests/test_invoke_precedes_dequeue_midrun.py`
# stays untouched (dual-channel: pybind11 → C++ Interpreter +
# Python AOT codegen).
#
# Usage (from repo root):
#   scripts/regen_invoke_precedes_dequeue_midrun_python.sh
#
# Requires:
...
GENERATED_DIR="backends/python/tests/integration/invoke_precedes_dequeue_midrun"
