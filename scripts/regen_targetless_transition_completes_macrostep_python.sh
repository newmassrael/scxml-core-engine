...
#
# Regenerate backends/python/tests/integration/targetless_transition_completes_macrostep/*_sm.py
...
#
# Mirrors scripts/regen_targetless_transition_completes_macrostep.sh (Rust). Only `*_sm.py`
# is copied back, so the hand-authored
# `test_targetless_transition_completes_macrostep_aot.py` next to it is never touched.
#
...
GENERATED_DIR="backends/python/tests/integration/targetless_transition_completes_macrostep"
