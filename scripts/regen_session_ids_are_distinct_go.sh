...
#
# Regenerate backends/go/tests/integration/session_ids_are_distinct/{session_ids_are_distinct,
# session_ids_are_distinct__sce_synth_invoke__inv_*}_sm.go from the
# canonical fixture at
# integration_resources/session_ids_are_distinct/session_ids_are_distinct.scxml.
#
# Mirrors scripts/regen_session_ids_are_distinct.sh (Rust). The TMP-staging
...
# hand-authored `session_ids_are_distinct_test.go` next to the generated
...
GENERATED_DIR="backends/go/tests/integration/session_ids_are_distinct"
...
#
# `--input-root` overrides the default synth RFC 6.2.6 source-hash root (the
