...
#
# Regenerate backends/python/tests/integration/send_param_payload/*_sm.py
# from the canonical fixture at
# integration_resources/send_param_payload/send_param_payload.scxml.
#
# Mirrors scripts/regen_send_param_payload.sh (Rust). The TMP-staging
...
# files. The pybind11 channel test at `backends/python/bindings/tests/test_send_param_payload.py`
# stays untouched (dual-channel: pybind11 → C++ Interpreter +
# Python AOT codegen).
#
# Usage (from repo root):
#   scripts/regen_send_param_payload_python.sh
#
# Requires:
...
GENERATED_DIR="backends/python/tests/integration/send_param_payload"
