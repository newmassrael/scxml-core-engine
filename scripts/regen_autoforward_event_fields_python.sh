...
#
# Regenerate backends/python/tests/integration/autoforward_event_fields/*_sm.py
# from the canonical fixture at
# integration_resources/autoforward_event_fields/autoforward_event_fields.scxml.
#
# Mirrors scripts/regen_autoforward_event_fields.sh (Rust). The TMP-staging
...
# files. The pybind11 channel test at `backends/python/bindings/tests/test_autoforward_event_fields.py`
# stays untouched (dual-channel: pybind11 → C++ Interpreter +
# Python AOT codegen).
#
# Usage (from repo root):
#   scripts/regen_autoforward_event_fields_python.sh
#
# Requires:
...
GENERATED_DIR="backends/python/tests/integration/autoforward_event_fields"
