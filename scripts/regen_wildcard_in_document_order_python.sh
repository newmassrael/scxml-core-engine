...
#
# Regenerate backends/python/tests/integration/wildcard_in_document_order/*_sm.py
# from the canonical fixture at
# integration_resources/wildcard_in_document_order/wildcard_in_document_order.scxml.
#
# Mirrors scripts/regen_wildcard_in_document_order.sh (Rust). The TMP-staging
...
# files. This stem has no pybind11 channel test: its seven channels are
# the ones `sce-build/tests/integration_stem_registration.rs` requires,
# and the pybind11 surface is not one of them.
#
# Usage (from repo root):
#   scripts/regen_wildcard_in_document_order_python.sh
#
# Requires:
...
GENERATED_DIR="backends/python/tests/integration/wildcard_in_document_order"
STEM="wildcard_in_document_order"
INPUT_ROOT="integration_resources/wildcard_in_document_order"
