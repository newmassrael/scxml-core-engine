...
#
# Regenerate backends/python/tests/integration/autoforward_internal_queue/*_sm.py
# from the canonical fixture at
# integration_resources/autoforward_internal_queue/autoforward_internal_queue.scxml.
#
# Mirrors scripts/regen_autoforward_internal_queue.sh (Rust). The TMP-staging
...
# files. The pybind11 channel test at `backends/python/bindings/tests/test_autoforward_internal_queue.py`
# stays untouched (dual-channel: pybind11 → C++ Interpreter +
# Python AOT codegen).
#
# Usage (from repo root):
#   scripts/regen_autoforward_internal_queue_python.sh
#
# Requires:
...
GENERATED_DIR="backends/python/tests/integration/autoforward_internal_queue"
