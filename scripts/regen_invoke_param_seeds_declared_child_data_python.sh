...
#
# Regenerate backends/python/tests/integration/invoke_param_seeds_declared_child_data/*_sm.py
...
# files. The pybind11 channel test at
# `backends/python/bindings/tests/test_invoke_param_seeds_declared_child_data.py`
# stays untouched (dual-channel: pybind11 → C++ Interpreter +
# Python AOT codegen).
#
# Usage (from repo root):
#   scripts/regen_invoke_param_seeds_declared_child_data_python.sh
#
# Requires:
...
GENERATED_DIR="backends/python/tests/integration/invoke_param_seeds_declared_child_data"
