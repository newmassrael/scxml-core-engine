...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --with-spdlog)
            ...
            ;;
        --skip-manifest)
            # Skip the clang-AST manifest emit (~1 min). MANIFEST.json
            # is independently regenerated by verify_embed_manifest.sh,
            # so verification flows that only need the file payload can
            # bypass it; the canonical packaged tree (committed embed/)
            # still includes the manifest via the unflagged invocation.
            SKIP_MANIFEST=true
            shift
            ;;
        -h|--help)
            echo "Usage: $0 [-o OUTPUT_DIR] [--with-spdlog] [--skip-manifest]"
            echo "  -o DIR           Output directory (default: ./embed/)"
            echo "  --with-spdlog    Include bundled spdlog headers"
            echo "  --skip-manifest  Skip MANIFEST.json emit (faster verification builds)"
            exit 0
            ;;
        *)
...
# synchronised. Embed mirrors the find_package install layout: SCE-self
...
# ============================================================================
LICENSES_CMAKE="${SCE_ROOT}/sce/sce_licenses.cmake"
if [ ! -f "${LICENSES_CMAKE}" ]; then
    echo "ERROR: ${LICENSES_CMAKE} not found" >&2
    exit 1
fi

# Parse a single set(<NAME> ...) block from sce_licenses.cmake. Mirrors
# the SCE_BASE_INCLUDE_DIRS extraction above (sed range + strip).
parse_license_group() {
    local var_name="$1"
    sed -n "/set(${var_name}/,/^)/{
        s/^[[:space:]]*//
...
LICENSE_FILES_SCE=$(parse_license_group SCE_LICENSE_FILES_SCE)
LICENSE_FILES_TP_CORE=$(parse_license_group SCE_LICENSE_FILES_THIRD_PARTY_CORE)
LICENSE_FILES_TP_SPDLOG=$(parse_license_group SCE_LICENSE_FILES_THIRD_PARTY_SPDLOG)

if [ -z "${LICENSE_FILES_SCE}" ] || [ -z "${LICENSE_FILES_TP_CORE}" ]; then
    echo "ERROR: Failed to parse license SSOT ${LICENSES_CMAKE}" >&2
    echo "       SCE=[${LICENSE_FILES_SCE}]" >&2
    echo "       TP_CORE=[${LICENSE_FILES_TP_CORE}]" >&2
    exit 1
fi
...
for f in ${LICENSE_FILES_SCE}; do
    ...
    cp "${SCE_ROOT}/${f}" "${OUTPUT_DIR}/${f}"
done

# Third-party LICENSE files preserve their repo-relative path so the
# embed tree's third_party/ layout matches the source repo verbatim.
...
copy_third_party_license ${LICENSE_FILES_TP_CORE}
if ${WITH_SPDLOG}; then
    copy_third_party_license ${LICENSE_FILES_TP_SPDLOG}
fi
...
if ${SKIP_MANIFEST}; then
    echo "Skipping public-header manifest emit (--skip-manifest)."
elif command -v "${CLANG_FOR_MANIFEST}" >/dev/null 2>&1; then
