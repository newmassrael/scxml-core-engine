...
XIncludeProcessor::XIncludeProcessor()
    : isProcessing_(false), maxRecursionDepth_(10), currentRecursionDepth_(0)
{
    Logger::debug("XIncludeProcessor::Constructor - Creating XInclude processor");
}

XIncludeProcessor::~XIncludeProcessor()
{
    Logger::debug("XIncludeProcessor::Destructor - Destroying XInclude processor");
}
...
bool XIncludeProcessor::process(xmlpp::Document *doc)
{
    ...
    try
    {
        ...
        Logger::debug("XIncludeProcessor::process() - Starting XInclude processing");
        ...
        if (rootElement)
        {
            ...
            Logger::debug("XIncludeProcessor::process() - Processed " + std::to_string(processedCount) + " XInclude directives");
        }
        else
        {
        ...
        try
        {
            ...
            Logger::debug("XIncludeProcessor::process() - Native XInclude processing successful");
        }
...
{
    basePath_ = basePath;
    Logger::debug("XIncludeProcessor::setBasePath() - Base path set to: " + basePath);
}

void XIncludeProcessor::addSearchPath(const std::string &searchPath)
{
    searchPaths_.push_back(searchPath);
    Logger::debug("XIncludeProcessor::addSearchPath() - Added search path: " + searchPath);
}
...
bool XIncludeProcessor::processXIncludeElement(xmlpp::Element *xincludeElement, const std::string &baseDir)
{
    ...
    Logger::debug("XIncludeProcessor::processXIncludeElement() - Processing XInclude element");
...
bool XIncludeProcessor::loadAndMergeFile(const std::string &href, xmlpp::Element *xincludeElement, const std::string &baseDir)
{
    ...
    Logger::debug("XIncludeProcessor::loadAndMergeFile() - Loading: " + fullPath);
    ...
    try
    {
        ...
        Logger::debug("XIncludeProcessor::loadAndMergeFile() - Successfully merged: " + fullPath);
        return true;
    }
    catch (const std::exception &ex)
    {
...
{
    Logger::warning("XIncludeProcessor - " + message);
