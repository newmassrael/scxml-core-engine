...
JSResult JSEngine::executeScriptInternal(const std::string &sessionId,
                                         const std::string &script) {
  Logger::debug("JSEngine: executeScriptInternal - sessionId: " + sessionId +
                ", script: " + script);
  ...
  ::JSValue result = JS_Eval(ctx, script.c_str(), script.length(), "<script>",
                             JS_EVAL_TYPE_GLOBAL);

  Logger::debug("JSEngine: JS_Eval completed, checking result...");
  Logger::debug("JSEngine: JS_IsException(result): " +
                std::string(JS_IsException(result) ? "true" : "false"));
  Logger::debug("JSEngine: JS_IsUndefined(result): " +
                std::string(JS_IsUndefined(result) ? "true" : "false"));
  Logger::debug("JSEngine: JS_IsNull(result): " +
                std::string(JS_IsNull(result) ? "true" : "false"));
  Logger::debug("JSEngine: JS_IsNumber(result): " +
                std::string(JS_IsNumber(result) ? "true" : "false"));
...
JSResult JSEngine::evaluateExpressionInternal(const std::string &sessionId,
  ...
  JSContext *ctx = session->jsContext;
  ::JSValue result = JS_Eval(ctx, expression.c_str(), expression.length(),
...
JSResult JSEngine::setVariableInternal(const std::string &sessionId,
...
JSResult JSEngine::getVariableInternal(const std::string &sessionId,
                                       const std::string &name) {
  SessionContext *session = getSession(sessionId);
  if (!session || !session->jsContext) {
    return JSResult::createError("Session not found: " + sessionId);
...
JSResult
JSEngine::setCurrentEventInternal(const std::string &sessionId,
  ...
  if (event) {
    ...
    JS_SetPropertyStr(ctx, eventObj, "name",
                      JS_NewString(ctx, event->getName().c_str()));
    JS_SetPropertyStr(ctx, eventObj, "type",
                      JS_NewString(ctx, event->getType().c_str()));
    JS_SetPropertyStr(ctx, eventObj, "sendid",
                      JS_NewString(ctx, event->getSendId().c_str()));
    JS_SetPropertyStr(ctx, eventObj, "origin",
                      JS_NewString(ctx, event->getOrigin().c_str()));
    JS_SetPropertyStr(ctx, eventObj, "origintype",
                      JS_NewString(ctx, event->getOriginType().c_str()));
    JS_SetPropertyStr(ctx, eventObj, "invokeid",
                      JS_NewString(ctx, event->getInvokeId().c_str()));

    // Set event data
    if (event->hasData()) {
      std::string dataStr = event->getDataAsString();
      ::JSValue dataValue =
          JS_ParseJSON(ctx, dataStr.c_str(), dataStr.length(), "<event-data>");
...
JSResult JSEngine::setupSystemVariablesInternal(
    const std::string &sessionId, const std::string &sessionName,
  ...
  // Set _sessionid
  JS_SetPropertyStr(ctx, global, "_sessionid",
                    JS_NewString(ctx, sessionId.c_str()));

  // Set _name
  JS_SetPropertyStr(ctx, global, "_name",
                    JS_NewString(ctx, sessionName.c_str()));
