...
RSM::ActionParser::ActionParser(std::shared_ptr<RSM::INodeFactory> nodeFactory)
...
RSM::ActionParser::parseActionNode(const xmlpp::Element *actionNode) {
  if (!actionNode) {
  ...
  if (typeAttr) {
    ...
    Logger::debug("RSM::ActionParser::parseActionNode() - Type: " +
                  typeAttr->get_value());
  } else {
  ...
  for (auto attr : attributes) {
    ...
    if (xmlAttr) {
      ...
      if (name != "name" && name != "id" && name != "type") {
        ...
        Logger::debug(
            "RSM::ActionParser::parseActionNode() - Added attribute: " + name +
            " = " + value);
      }
    }
  }
  ...
  for (auto child : children) {
    ...
    if (textNode && !textNode->is_white_space()) {
      ...
      Logger::debug(
          "RSM::ActionParser::parseActionNode() - Added text content");
    } else {
      ...
      if (elementNode && isActionNode(elementNode)) {
        ...
        if (childAction) {
          ...
          Logger::debug(
              "RSM::ActionParser::parseActionNode() - Added child action: " +
              childAction->getId());
        }
      }
    }
  ...
  if (!childActions.empty()) {
    ...
    Logger::debug("RSM::ActionParser::parseActionNode() - Added " +
                  std::to_string(childActions.size()) + " child actions");
  }

  Logger::debug(
      "RSM::ActionParser::parseActionNode() - Action parsed successfully");
  return action;
}

std::shared_ptr<RSM::IActionNode> RSM::ActionParser::parseExternalActionNode(
    const xmlpp::Element *externalActionNode) {
  ...
  if (delayAttr) {
    ...
    Logger::debug("RSM::ActionParser::parseExternalActionNode() - Delay: " +
                  delayAttr->get_value());
  }

  // 외부 구현 요소 파싱
  auto implNode =
      externalActionNode->get_first_child("code:external-implementation");
  if (!implNode) {
    // 네임스페이스 없이 시도
  ...
  for (auto attr : attributes) {
    ...
    if (xmlAttr) {
      ...
      if (name != "name" && name != "id" && name != "delay") {
        action->setAttribute(name, value);
        Logger::debug(
            "RSM::ActionParser::parseExternalActionNode() - Added attribute: " +
            name + " = " + value);
      }
    }
  }
...
RSM::ActionParser::parseActionsInElement(const xmlpp::Element *parentElement) {
  ...
  if (!parentElement) {
    Logger::warn(
        "RSM::ActionParser::parseActionsInElement() - Null parent element");
    return actions;
  }
  ...
  Logger::debug("RSM::ActionParser::parseActionsInElement() - Found " +
                std::to_string(actions.size()) + " actions");
  return actions;
}

void RSM::ActionParser::parseSpecialExecutableContent(
  ...
  for (auto child : children) {
    ...
    if (elementNode) {
      // 자식 요소가 액션 노드인 경우 파싱
      if (isActionNode(elementNode) ||
          isSpecialExecutableContent(elementNode)) {
        auto childAction = parseActionNode(elementNode);
        if (childAction) {
          childActions.push_back(childAction);
        }
  ...
  if (!childActions.empty()) {
    ...
    Logger::debug(
        "RSM::ActionParser::parseSpecialExecutableContent() - Added " +
...
bool RSM::ActionParser::isActionNode(const xmlpp::Element *element) const {
  ...
  // 커스텀 액션 태그
  if (matchNodeName(nodeName, "action") ||
      matchNodeName(nodeName, "code:action")) {
    return true;
  }

  // 표준 SCXML 실행 가능 콘텐츠 태그
  return matchNodeName(nodeName, "raise") ||
         matchNodeName(nodeName, "assign") ||
         matchNodeName(nodeName, "script") || matchNodeName(nodeName, "log") ||
         matchNodeName(nodeName, "send") || matchNodeName(nodeName, "cancel");
}

bool RSM::ActionParser::isSpecialExecutableContent(
  ...
  return matchNodeName(nodeName, "if") || matchNodeName(nodeName, "elseif") ||
         matchNodeName(nodeName, "else") ||
         matchNodeName(nodeName, "foreach") ||
         matchNodeName(nodeName, "invoke") ||
         matchNodeName(nodeName, "finalize");
}

bool RSM::ActionParser::isExternalActionNode(
    const xmlpp::Element *element) const {
  if (!element) {
    return false;
  }
  ...
  std::string nodeName = element->get_name();
  return matchNodeName(nodeName, "external-action") ||
         matchNodeName(nodeName, "code:external-action");
}

void RSM::ActionParser::parseExternalImplementation(
    const xmlpp::Element *element, std::shared_ptr<IActionNode> actionNode) {
  ...
  if (classAttr) {
    ...
    Logger::debug(
        "RSM::ActionParser::parseExternalImplementation() - External class: " +
        className);
  }
...
bool RSM::ActionParser::matchNodeName(const std::string &nodeName,
