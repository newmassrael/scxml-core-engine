...
RSM::InvokeParser::InvokeParser(std::shared_ptr<RSM::INodeFactory> nodeFactory)
...
RSM::InvokeParser::parseInvokeNode(const xmlpp::Element *invokeElement) {
  if (!invokeElement) {
  ...
  if (typeAttr) {
    ...
    Logger::debug(
        "RSM::InvokeParser::parseInvokeNode() - typeexpr attribute present: " +
  ...
  if (srcAttr) {
    ...
    Logger::debug(
        "RSM::InvokeParser::parseInvokeNode() - srcexpr attribute present: " +
  ...
  // finalize 요소 파싱
  auto finalizeElement =
      ParsingCommon::findFirstChildElement(invokeElement, "finalize");
  if (finalizeElement) {
...
RSM::InvokeParser::parseInvokesInState(const xmlpp::Element *stateElement) {
  ...
  if (!stateElement) {
    Logger::warn(
        "RSM::InvokeParser::parseInvokesInState() - Null state element");
    return invokeNodes;
  }

  auto invokeElements =
      ParsingCommon::findChildElements(stateElement, "invoke");
  Logger::debug("RSM::InvokeParser::parseInvokesInState() - Found " +
...
void RSM::InvokeParser::parseFinalizeElement(
    const xmlpp::Element *finalizeElement,
...
void RSM::InvokeParser::parseParamElements(
  ...
  for (auto paramElement : paramElements) {
    ...
    Logger::debug(
        "RSM::InvokeParser::parseParamElements() - Param parsed: name=" + name);
  }
}

std::vector<std::shared_ptr<RSM::IDataModelItem>>
RSM::InvokeParser::parseParamElementsAndCreateDataItems(
    const xmlpp::Element *invokeElement,
    std::shared_ptr<IInvokeNode> invokeNode) {
  ...
  for (auto paramElement : paramElements) {
    ...
    if (!name.empty() && (!expr.empty() || !location.empty())) {
      auto dataItem = nodeFactory_->createDataModelItem(
          name, expr.empty() ? location : expr);
      if (dataItem) {
...
void RSM::InvokeParser::parseContentElement(
    const xmlpp::Element *invokeElement,
    std::shared_ptr<IInvokeNode> invokeNode) {
  if (!invokeElement || !invokeNode) {
    return;
  ...
  auto contentElement =
      ParsingCommon::findFirstChildElement(invokeElement, "content");
  if (contentElement) {
    std::string content;
    ...
    if (exprAttr) {
      ...
      for (auto child : children) {
        ...
        } else if (auto textNode =
                       dynamic_cast<const xmlpp::TextNode *>(child)) {
          // 텍스트 노드는 그대로 추가
