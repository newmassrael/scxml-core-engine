...
DoneDataParser::DoneDataParser(std::shared_ptr<INodeFactory> factory)
    : factory_(factory)
{
    Logger::debug("DoneDataParser::Constructor - Creating DoneData parser");
}


bool DoneDataParser::parseDoneData(const xmlpp::Element *doneDataElement, IStateNode *stateNode)
{
    if (!doneDataElement || !stateNode)
    {
        Logger::error("DoneDataParser::parseDoneData - Null doneData element or state node");
        return false;
    }


    Logger::debug("DoneDataParser::parseDoneData - Parsing <donedata> for state " + stateNode->getId());
    ...
    if (contentElement)
    {
        ...
        Logger::debug(std::string("DoneDataParser::parseDoneData - Found <content> element: ") +
    ...
    Logger::debug("DoneDataParser::parseDoneData - Found " + std::to_string(paramElements.size()) +
    ...
    {
        Logger::error("DoneDataParser::parseDoneData - <content> and <param> cannot be used together in <donedata>");
...
bool DoneDataParser::parseContent(const xmlpp::Element *contentElement, IStateNode *stateNode)
{
    if (!contentElement || !stateNode)
    {
        Logger::error("DoneDataParser::parseContent - Null content element or state node");
        return false;
    }
    ...
    if (exprAttr)
    {
        ...
        Logger::debug("DoneDataParser::parseContent - Found 'expr' attribute: " + exprValue);
    }
    ...
    if (childNode)
    {
        ...
        if (textNode)
        {
            ...
            Logger::debug("DoneDataParser::parseContent - Found text content: " +
    ...
    {
        Logger::error("DoneDataParser::parseContent - <content> cannot have both 'expr' attribute and child content");
        return false;
    }
...
bool DoneDataParser::parseParam(const xmlpp::Element *paramElement, IStateNode *stateNode)
{
    if (!paramElement || !stateNode)
    {
        Logger::error("DoneDataParser::parseParam - Null param element or state node");
        return false;
    }
    ...
    if (!nameAttr)
    {
        Logger::error("DoneDataParser::parseParam - <param> element must have 'name' attribute");
        return false;
    }
    ...
    {
        Logger::error("DoneDataParser::parseParam - <param> cannot have both 'expr' and 'location' attributes");
        return false;
    }
    ...
    if (locationAttr)
    {
        ...
        Logger::debug("DoneDataParser::parseParam - Added param: " + nameValue + " with location: " + locationValue);
        return true;
    }
    ...
    if (exprAttr)
    {
        ...
        Logger::debug("DoneDataParser::parseParam - Added param: " + nameValue + " with expr: " + exprValue);
        return true;
    }


    Logger::error("DoneDataParser::parseParam - <param> must have either 'expr' or 'location' attribute");
    return false;
}
