...
RSM::StateNodeParser::StateNodeParser(
  ...
  Logger::debug(
      "RSM::StateNodeParser::Constructor - Creating state node parser");
}

RSM::StateNodeParser::~StateNodeParser() {
  Logger::debug(
      "RSM::StateNodeParser::Destructor - Destroying state node parser");
}

void RSM::StateNodeParser::setRelatedParsers(
  ...
  Logger::debug(
      "RSM::StateNodeParser::setRelatedParsers() - Related parsers set");
}

std::shared_ptr<RSM::IStateNode> RSM::StateNodeParser::parseStateNode(
  ...
  if (idAttr) {
    ...
    // ID가 없는 경우 자동 생성
    stateId =
        "state_" + std::to_string(reinterpret_cast<uintptr_t>(stateElement));
  ...
  Logger::debug("RSM::StateNodeParser::parseStateNode() - Parsing state: " +
                stateId + " (" +
  ...
  if (!stateNode) {
    Logger::error(
        "RSM::StateNodeParser::parseStateNode() - Failed to create state node");
    return nullptr;
  }
  ...
  if (!parentState) {
    Logger::debug(
        "RSM::StateNodeParser::parseStateNode() - No parent state (root)");
  }
  ...
  if (dataModelParser_) {
    auto dataItems =
        dataModelParser_->parseDataModelInState(stateElement, context);
    for (const auto &item : dataItems) {
      stateNode->addDataItem(item);
      Logger::debug(
          "RSM::StateNodeParser::parseStateNode() - Added data item: " +
          item->getId());
    }
  } else {
  ...
  if (stateType == Type::FINAL && doneDataParser_) {
    const xmlpp::Element *doneDataElement =
        ParsingCommon::findFirstChildElement(stateElement, "donedata");
    if (doneDataElement) {
      bool success =
          doneDataParser_->parseDoneData(doneDataElement, stateNode.get());
      if (!success) {
  ...
  if (stateType == Type::COMPOUND && !stateNode->getChildren().empty()) {
    if (stateType == Type::COMPOUND && !stateNode->getChildren().empty()) {
      // <initial> 요소 확인
      auto initialElement =
          ParsingCommon::findFirstChildElement(stateElement, "initial");
      if (initialElement) {
        // <initial> 요소 파싱
  ...
  Logger::debug("RSM::StateNodeParser::parseStateNode() - State " + stateId +
                " parsed successfully with " +
                std::to_string(stateNode->getChildren().size()) +
                " child states");
  return stateNode;
}

RSM::Type RSM::StateNodeParser::determineStateType(
    const xmlpp::Element *stateElement) {
  if (!stateElement) {
    return Type::ATOMIC;
  }
...
void RSM::StateNodeParser::parseTransitions(
  ...
  auto transitionElements =
      ParsingCommon::findChildElements(parentElement, "transition");
  for (auto *transitionElement : transitionElements) {
    auto transition =
        transitionParser_->parseTransitionNode(transitionElement, state.get());
    if (transition) {
  ...
  Logger::debug("RSM::StateNodeParser::parseStateNode() - Parsed " +
                std::to_string(state->getTransitions().size()) +
                " transitions");
}

void RSM::StateNodeParser::parseEntryExitElements(
    const xmlpp::Element *parentElement,
    std::shared_ptr<RSM::IStateNode> state) {
  ...
  // onentry 요소 처리
  auto onentryElements =
      ParsingCommon::findChildElements(parentElement, "onentry");
  ...
  // onexit 요소 처리
  auto onexitElements =
      ParsingCommon::findChildElements(parentElement, "onexit");
...
void RSM::StateNodeParser::parseChildStates(
    const xmlpp::Element *stateElement,
    std::shared_ptr<RSM::IStateNode> parentState,
    const RSM::SCXMLContext &context) {
  Logger::debug(
      "RSM::StateNodeParser::parseChildStates() - Parsing child states");
  ...
  childStateElements.insert(childStateElements.end(), stateElements.begin(),
                            stateElements.end());

  auto parallelElements =
      ParsingCommon::findChildElements(stateElement, "parallel");
  childStateElements.insert(childStateElements.end(), parallelElements.begin(),
                            parallelElements.end());

  auto finalElements = ParsingCommon::findChildElements(stateElement, "final");
  childStateElements.insert(childStateElements.end(), finalElements.begin(),
                            finalElements.end());

  auto historyElements =
      ParsingCommon::findChildElements(stateElement, "history");
  childStateElements.insert(childStateElements.end(), historyElements.begin(),
                            historyElements.end());
  ...
  Logger::debug("RSM::StateNodeParser::parseChildStates() - Found " +
                std::to_string(childStateElements.size()) + " child states");
}

void RSM::StateNodeParser::parseInvokeElements(
    const xmlpp::Element *parentElement,
    std::shared_ptr<RSM::IStateNode> state) {
  ...
  auto invokeElements =
      ParsingCommon::findChildElements(parentElement, "invoke");
  for (auto *invokeElement : invokeElements) {
    ...
    if (invokeNode) {
      ...
      Logger::debug(
          "RSM::StateNodeParser::parseInvokeElements() - Added invoke: " +
          invokeNode->getId());

      // Param 요소들로부터 데이터 모델 아이템 생성 및 추가
      auto dataItems = invokeParser_->parseParamElementsAndCreateDataItems(
          invokeElement, invokeNode);
  ...
  Logger::debug("RSM::StateNodeParser::parseInvokeElements() - Parsed " +
                std::to_string(state->getInvoke().size()) + " invoke elements");
}

void RSM::StateNodeParser::parseHistoryType(
  ...
  Logger::debug("RSM::StateNodeParser::parseHistoryType() - History state " +
                state->getId() + " type: " + (isDeep ? "deep" : "shallow"));
...
void RSM::StateNodeParser::parseReactiveGuards(
    const xmlpp::Element *parentElement,
    std::shared_ptr<RSM::IStateNode> state) {
  ...
  Logger::debug("RSM::StateNodeParser::parseReactiveGuards() - Parsed " +
                std::to_string(reactiveGuardElements.size()) +
                " reactive guards");
}

void RSM::StateNodeParser::parseInitialElement(
  ...
  const xmlpp::Element *transitionElement =
      ParsingCommon::findFirstChildElement(initialElement, "transition");
