...
GuardParser::GuardParser(std::shared_ptr<INodeFactory> nodeFactory)
...
std::shared_ptr<IGuardNode> GuardParser::parseGuardNode(const xmlpp::Element *guardNode)
{
    if (!guardNode)
    {
        Logger::warning("GuardParser::parseGuardNode() - Null guard node");
        return nullptr;
    }
    ...
    {
        Logger::warning("GuardParser::parseGuardNode() - Guard node missing required attributes");
        ...
        for (auto *attr : attrs)
        {
            ...
            if (xmlAttr)
            {
                Logger::debug("GuardParser::parseGuardNode() - Attribute: " + xmlAttr->get_name() + " = " + xmlAttr->get_value());
            }
        }
        return nullptr;
    }
    ...
    {
        // <code:condition> 또는 <condition> 요소 처리
        auto conditionElement = ParsingCommon::findFirstChildElement(guardNode, "condition");
        if (conditionElement)
        {
            ...
            std::string conditionText = ParsingCommon::extractTextContent(conditionElement, true);
...
std::shared_ptr<IGuardNode> GuardParser::parseGuardFromTransition(const xmlpp::Element *transitionNode, const std::string &targetState)
{
    if (!transitionNode)
    {
        Logger::warning("GuardParser::parseGuardFromTransition() - Null transition node");
        return nullptr;
    }
    ...
    Logger::debug("GuardParser::parseGuardFromTransition() - Parsing guard from transition: " + guardId + " for state: " + targetState);
    ...
    if (condAttr)
    {
        ...
        guard->setCondition(condition);
        Logger::debug("GuardParser::parseGuardFromTransition() - Set condition from cond attribute: " + condition);
    }


    Logger::debug("GuardParser::parseGuardFromTransition() - Guard from transition parsed successfully");
    return guard;
}


std::shared_ptr<IGuardNode> GuardParser::parseReactiveGuard(const xmlpp::Element *reactiveGuardNode)
{
    if (!reactiveGuardNode)
    {
        Logger::warning("GuardParser::parseReactiveGuard() - Null reactive guard node");
        return nullptr;
    }
    ...
    if (!idAttr || (!targetAttr && !conditionAttr))
    {
        Logger::warning("GuardParser::parseReactiveGuard() - Reactive guard node missing required attributes");
        return nullptr;
    }
...
std::vector<std::shared_ptr<IGuardNode>> GuardParser::parseGuardsElement(const xmlpp::Element *guardsNode)
{
    std::vector<std::shared_ptr<IGuardNode>> guards;


    if (!guardsNode)
    {
        Logger::warning("GuardParser::parseGuardsElement() - Null guards node");
        return guards;
    }
...
std::vector<std::shared_ptr<IGuardNode>> GuardParser::parseAllGuards(const xmlpp::Element *scxmlNode)
{
    std::vector<std::shared_ptr<IGuardNode>> allGuards;


    if (!scxmlNode)
    {
        Logger::warning("GuardParser::parseAllGuards() - Null SCXML node");
        return allGuards;
    }
        ...
        if (stateElement)
        {
                ...
                if (transElement)
                {
                    ...
                    if (targetAttr)
                    {
                        ...
                        if (guard)
                        {
                            ...
                            Logger::debug("GuardParser::parseAllGuards() - Added guard from transition in state " + stateId);
                        }
                    }
                }
            }
            ...
            for (auto *node : reactiveGuardNodes)
            {
                ...
                if (guardElement)
                {
                    ...
                    {
                        allGuards.push_back(guard);
                        Logger::debug("GuardParser::parseAllGuards() - Added reactive guard from state " + stateId);
                    }
                }
            }
        }
    ...
    std::sort(allGuards.begin(), allGuards.end(),
              [](const std::shared_ptr<IGuardNode> &a, const std::shared_ptr<IGuardNode> &b)
              {
...
void GuardParser::parseDependencies(const xmlpp::Element *guardNode, std::shared_ptr<IGuardNode> guardObject)
{
...
void GuardParser::parseExternalImplementation(const xmlpp::Element *guardNode, std::shared_ptr<IGuardNode> guardObject)
{
    if (!guardNode || !guardObject)
    {
        return;
