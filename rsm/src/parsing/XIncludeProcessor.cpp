...
RSM::XIncludeProcessor::XIncludeProcessor()
  ...
  Logger::debug(
      "RSM::XIncludeProcessor::Constructor - Creating XInclude processor");
}

RSM::XIncludeProcessor::~XIncludeProcessor() {
  Logger::debug(
      "RSM::XIncludeProcessor::Destructor - Destroying XInclude processor");
}
...
bool RSM::XIncludeProcessor::process(xmlpp::Document *doc) {
  ...
  try {
    ...
    Logger::debug(
        "RSM::XIncludeProcessor::process() - Starting XInclude processing");
    ...
    if (rootElement) {
      ...
      Logger::debug("RSM::XIncludeProcessor::process() - Processed " +
                    std::to_string(processedCount) + " XInclude directives");
    } else {
    ...
    try {
      ...
      Logger::debug(
          "RSM::XIncludeProcessor::process() - Native XInclude processing "
          "successful");
    } catch (const std::exception &ex) {
      addWarning("Native XInclude processing failed: " +
                 std::string(ex.what()));
    }
...
void RSM::XIncludeProcessor::setBasePath(const std::string &basePath) {
  basePath_ = basePath;
  Logger::debug("RSM::XIncludeProcessor::setBasePath() - Base path set to: " +
                basePath);
}
...
void RSM::XIncludeProcessor::addSearchPath(const std::string &searchPath) {
  ...
  Logger::debug(
      "RSM::XIncludeProcessor::addSearchPath() - Added search path: " +
      searchPath);
}

const std::vector<std::string> &
RSM::XIncludeProcessor::getErrorMessages() const {
  return errorMessages_;
}

const std::vector<std::string> &
RSM::XIncludeProcessor::getWarningMessages() const {
  return warningMessages_;
}

const std::unordered_map<std::string, int> &
RSM::XIncludeProcessor::getProcessedFiles() const {
  return processedFiles_;
}

int RSM::XIncludeProcessor::findAndProcessXIncludes(
    xmlpp::Element *element, const std::string &baseDir) {
  if (!element) {
    return 0;
  }
...
bool RSM::XIncludeProcessor::processXIncludeElement(
    xmlpp::Element *xincludeElement, const std::string &baseDir) {
  if (!xincludeElement) {
    return false;
  }
  ...
  // XML 모드만 지원
  if (parseMode != "xml") {
    addWarning("XInclude parse mode '" + parseMode +
               "' not supported, only 'xml' is supported");
    return false;
  }
...
bool RSM::XIncludeProcessor::loadAndMergeFile(const std::string &href,
                                              xmlpp::Element *xincludeElement,
  ...
  Logger::debug("RSM::XIncludeProcessor::loadAndMergeFile() - Loading: " +
                fullPath);
  ...
  try {
    ...
    for (auto *child : children) {
      try {
        ...
        addError("Exception while importing node from " + fullPath + ": " +
                 std::string(ex.what()));
      }
    }
    ...
    try {
      ...
      addWarning("Exception while removing XInclude element: " +
                 std::string(ex.what()));
      // 계속 진행
    }
    ...
    Logger::debug(
        "RSM::XIncludeProcessor::loadAndMergeFile() - Successfully merged: " +
        fullPath);
    return true;
  } catch (const std::exception &ex) {
    addError("Exception while processing included file " + fullPath + ": " +
             std::string(ex.what()));
    return false;
  }
}
...
std::string
RSM::XIncludeProcessor::resolveFilePath(const std::string &href,
                                        const std::string &baseDir) {
  // 절대 경로인 경우 그대로 사용
