...
RSM::SCXMLParser::SCXMLParser(
    std::shared_ptr<RSM::INodeFactory> nodeFactory,
    std::shared_ptr<RSM::IXIncludeProcessor> xincludeProcessor)
    : nodeFactory_(nodeFactory) {
  Logger::debug("RSM::SCXMLParser::Constructor - Creating SCXML parser");
  ...
  // 관련 파서들을 연결
  stateNodeParser_->setRelatedParsers(transitionParser_, actionParser_,
                                      dataModelParser_, invokeParser_,
                                      doneDataParser_);
...
RSM::SCXMLParser::~SCXMLParser() {
  Logger::debug("RSM::SCXMLParser::Destructor - Destroying SCXML parser");
}

std::shared_ptr<RSM::SCXMLModel>
RSM::SCXMLParser::parseFile(const std::string &filename) {
  try {
    // 파싱 상태 초기화
    initParsing();
    ...
    Logger::info("RSM::SCXMLParser::parseFile() - Parsing SCXML file: " +
                 filename);
    ...
    // XInclude 처리
    Logger::debug("RSM::SCXMLParser::parseFile() - Processing XIncludes");
...
std::shared_ptr<RSM::SCXMLModel>
RSM::SCXMLParser::parseContent(const std::string &content) {
  try {
    // 파싱 상태 초기화
    initParsing();

    Logger::info("RSM::SCXMLParser::parseContent() - Parsing SCXML content");
    ...
    // XInclude 처리
    Logger::debug("RSM::SCXMLParser::parseContent() - Processing XIncludes");
    xincludeProcessor_->process(parser.get_document());
...
std::shared_ptr<RSM::SCXMLModel>
RSM::SCXMLParser::parseDocument(xmlpp::Document *doc) {
  if (!doc) {
  ...
  Logger::info("RSM::SCXMLParser::parseDocument() - Valid SCXML document "
  ...
  if (result) {
    Logger::info("RSM::SCXMLParser::parseDocument() - SCXML document parsed "
                 "successfully");
    ...
    if (validateModel(model)) {
      ...
      Logger::error(
          "RSM::SCXMLParser::parseDocument() - SCXML model validation failed");
      return nullptr;
    }
  } else {
    Logger::error(
        "RSM::SCXMLParser::parseDocument() - Failed to parse SCXML document");
    return nullptr;
  }
}

bool RSM::SCXMLParser::parseScxmlNode(const xmlpp::Element *scxmlNode,
  ...
  Logger::debug("RSM::SCXMLParser::parseScxmlNode() - Parsing SCXML root node");
  ...
  if (nameAttr) {
    ...
    Logger::debug("RSM::SCXMLParser::parseScxmlNode() - Name: " +
                  nameAttr->get_value());
  }
  ...
  if (initialAttr) {
    ...
    Logger::debug("RSM::SCXMLParser::parseScxmlNode() - Initial state: " +
  ...
  if (datamodelAttr) {
    ...
    Logger::debug("RSM::SCXMLParser::parseScxmlNode() - Datamodel: " +
                  datamodelType);
  }
  ...
  if (bindingAttr) {
    ...
    Logger::debug("RSM::SCXMLParser::parseScxmlNode() - Binding mode: " +
                  binding);
  }
  ...
  // 가드 조건 파싱
  Logger::debug("RSM::SCXMLParser::parseScxmlNode() - Parsing guards");
  auto guards = guardParser_->parseAllGuards(scxmlNode);
  for (const auto &guard : guards) {
    model->addGuard(guard);

    std::string logMessage =
        "RSM::SCXMLParser::parseScxmlNode() - Added guard: " + guard->getId();
  ...
  Logger::debug("RSM::SCXMLParser::parseScxmlNode() - Parsing root datamodel");
  auto datamodelNode =
      ParsingCommon::findFirstChildElement(scxmlNode, "datamodel");
  if (datamodelNode) {
    auto dataItems =
        dataModelParser_->parseDataModelNode(datamodelNode, context);
    for (const auto &item : dataItems) {
      model->addDataModelItem(item);
      Logger::debug(
          "RSM::SCXMLParser::parseScxmlNode() - Added data model item: " +
          item->getId());
    }
  }
  ...
  Logger::debug(
      "RSM::SCXMLParser::parseScxmlNode() - Looking for root state nodes");

  // 모든 타입의 상태 노드 수집
  std::vector<const xmlpp::Element *> rootStateElements;
  auto stateElements = ParsingCommon::findChildElements(scxmlNode, "state");
  rootStateElements.insert(rootStateElements.end(), stateElements.begin(),
                           stateElements.end());

  auto parallelElements =
      ParsingCommon::findChildElements(scxmlNode, "parallel");
  rootStateElements.insert(rootStateElements.end(), parallelElements.begin(),
                           parallelElements.end());

  auto finalElements = ParsingCommon::findChildElements(scxmlNode, "final");
  rootStateElements.insert(rootStateElements.end(), finalElements.begin(),
                           finalElements.end());
  ...
  Logger::info("RSM::SCXMLParser::parseScxmlNode() - Found " +
               std::to_string(rootStateElements.size()) + " root state nodes");

  // 모든 루트 상태 노드 파싱
  for (auto *stateElement : rootStateElements) {
    Logger::info("RSM::SCXMLParser::parseScxmlNode() - Parsing root state");
    ...
    if (state) {
      ...
      Logger::info("RSM::SCXMLParser::parseScxmlNode() - Root state parsed: " +
                   state->getId());
    } else {
...
void RSM::SCXMLParser::parseContextProperties(
    const xmlpp::Element *scxmlNode, std::shared_ptr<SCXMLModel> model) {
  if (!scxmlNode || !model) {
    return;
  }

  Logger::debug("RSM::SCXMLParser::parseContextProperties() - Parsing context "
                "properties");
  ...
  for (auto *propElement : ctxProps) {
    ...
    if (nameAttr && typeAttr) {
      ...
      Logger::debug(
          "RSM::SCXMLParser::parseContextProperties() - Added property: " +
          name + " (" + type + ")");
    } else {
  ...
  Logger::debug("RSM::SCXMLParser::parseContextProperties() - Found " +
                std::to_string(model->getContextProperties().size()) +
                " context properties");
}

void RSM::SCXMLParser::parseInjectPoints(const xmlpp::Element *scxmlNode,
                                         std::shared_ptr<SCXMLModel> model) {
  if (!scxmlNode || !model) {
    return;
  }
  ...
  Logger::debug(
      "RSM::SCXMLParser::parseInjectPoints() - Parsing injection points");

  // 의존성 주입 지점 파싱 (여러 가능한 이름으로 시도)
  std::vector<std::string> injectNodeNames = {
      "inject-point", "inject_point", "injectpoint", "inject", "dependency"};
  ...
  for (const auto &nodeName : injectNodeNames) {
    ...
    for (auto *injectElement : injectElements) {
      ...
      if (nameAttr && typeAttr) {
        ...
        Logger::debug(
            "RSM::SCXMLParser::parseInjectPoints() - Added inject point: " +
            name + " (" + type + ")");
  ...
  Logger::debug("RSM::SCXMLParser::parseInjectPoints() - Found " +
                std::to_string(model->getInjectPoints().size()) +
                " injection points");
}

bool RSM::SCXMLParser::hasErrors() const { return !errorMessages_.empty(); }
...
void RSM::SCXMLParser::addWarning(const std::string &message) {
  Logger::warn("SCXMLParser - " + message);
...
bool RSM::SCXMLParser::validateModel(std::shared_ptr<SCXMLModel> model) {
  ...
  Logger::info("RSM::SCXMLParser::validateModel() - Validating SCXML model");
  ...
  for (const auto &state : model->getAllStates()) {
    ...
    if (parent) {
      ...
      if (!isChild) {
        addError("State '" + state->getId() + "' has parent '" +
  ...
  if (isValid) {
    Logger::info(
        "RSM::SCXMLParser::validateModel() - Model validation successful");
  } else {
    Logger::info("RSM::SCXMLParser::validateModel() - Model validation "
...
void RSM::SCXMLParser::addSystemVariables(std::shared_ptr<SCXMLModel> model) {
  if (!model) {
    Logger::warn("RSM::SCXMLParser::addSystemVariables() - Null model");
    return;
  }

  Logger::debug("RSM::SCXMLParser::addSystemVariables() - Adding system "
                "variables to data model");

  std::string datamodelType = model->getDatamodel();
  // 시스템 변수가 정의된 데이터 모델에만 적용
  if (datamodelType.empty() || datamodelType == "null") {
    Logger::debug("RSM::SCXMLParser::addSystemVariables() - Skipping system "
                  "variables for null datamodel");
    return;
  }
  ...
  Logger::debug(
      "RSM::SCXMLParser::addSystemVariables() - Added system variable: _name");

  // _sessionid 시스템 변수 추가
  auto sessionIdItem =
      nodeFactory_->createDataModelItem("_sessionid", datamodelType);
  ...
  Logger::debug("RSM::SCXMLParser::addSystemVariables() - Added system "
                "variable: _sessionid");

  // _ioprocessors 시스템 변수 추가
  auto ioProcessorsItem =
      nodeFactory_->createDataModelItem("_ioprocessors", datamodelType);
  ...
  Logger::debug(
      "RSM::SCXMLParser::addSystemVariables() - Added system variable: _event");
}
