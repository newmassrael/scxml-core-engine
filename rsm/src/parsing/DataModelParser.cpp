...
RSM::DataModelParser::DataModelParser(std::shared_ptr<INodeFactory> nodeFactory)
    : nodeFactory_(nodeFactory) {
  Logger::debug(
      "RSM::DataModelParser::Constructor - Creating data model parser");
}

RSM::DataModelParser::~DataModelParser() {
  Logger::debug(
      "RSM::DataModelParser::Destructor - Destroying data model parser");
}

std::vector<std::shared_ptr<RSM::IDataModelItem>>
RSM::DataModelParser::parseDataModelNode(const xmlpp::Element *datamodelNode,
                                         const RSM::SCXMLContext &context) {
  std::vector<std::shared_ptr<IDataModelItem>> items;

  if (!datamodelNode) {
    Logger::warn(
        "RSM::DataModelParser::parseDataModelNode() - Null datamodel node");
    return items;
  }

  Logger::debug(
      "RSM::DataModelParser::parseDataModelNode() - Parsing datamodel node");
  ...
  Logger::debug("RSM::DataModelParser::parseDataModelNode() - Parsed " +
                std::to_string(items.size()) + " data model items");
  return items;
}

std::shared_ptr<RSM::IDataModelItem>
RSM::DataModelParser::parseDataModelItem(const xmlpp::Element *dataNode,
                                         const RSM::SCXMLContext &context) {
  if (!dataNode) {
  ...
  Logger::debug(
      "RSM::DataModelParser::parseDataModelItem() - Parsing data model item: " +
      id);
  ...
  if (srcAttr) {
    ...
    dataItem->setSrc(src);
    Logger::debug("RSM::DataModelParser::parseDataModelItem() - Source URL: " +
                  src);
  ...
  if (typeAttr) {
    ...
    Logger::debug("RSM::DataModelParser::parseDataModelItem() - Type: " +
                  typeAttr->get_value());
  ...
  if (scopeAttr) {
    ...
    Logger::debug("RSM::DataModelParser::parseDataModelItem() - Scope: " +
                  scopeAttr->get_value());
  }
  ...
  for (auto *attr : attributes) {
    ...
    if (xmlAttr) {
      ...
      // 이미 처리한 속성은 건너뜀
      if (name != "id" && name != "expr" && name != "type" &&
          name != "code:scope" && name != "scope" && name != "src") // src 추가
      {
        dataItem->setAttribute(name, value);
        Logger::debug(
            "RSM::DataModelParser::parseDataModelItem() - Added attribute: " +
            name + " = " + value);
      }
    }
  }
...
void RSM::DataModelParser::parseDataContent(
    const xmlpp::Element *dataNode, std::shared_ptr<IDataModelItem> dataItem) {
  ...
  for (auto *child : children) {
    ...
    if (textNode) {
      ...
      if (!content.empty()) {
        ...
        if (!onlyWhitespace) {
          ...
          Logger::debug(
              "RSM::DataModelParser::parseDataContent() - Added text content");
        }
      }
    ...
    if (cdataNode) {
      ...
      dataItem->setContent(content);
      Logger::debug(
          "RSM::DataModelParser::parseDataContent() - Added CDATA content: " +
          content);
    ...
    if (elementNode) {
      ...
      dataItem->setContent("<" + elementNode->get_name() + ">...</" +
...
std::vector<std::shared_ptr<RSM::IDataModelItem>>
RSM::DataModelParser::parseDataModelInState(const xmlpp::Element *stateNode,
                                            const RSM::SCXMLContext &context) {
  std::vector<std::shared_ptr<IDataModelItem>> items;

  if (!stateNode) {
    Logger::warn(
        "RSM::DataModelParser::parseDataModelInState() - Null state node");
    return items;
  }
  ...
  Logger::debug("RSM::DataModelParser::parseDataModelInState() - Found " +
                std::to_string(items.size()) + " data items in state");
  return items;
}

std::string RSM::DataModelParser::extractDataModelType(
    const xmlpp::Element *datamodelNode) const {
  if (!datamodelNode) {
    return "";
  }
...
bool RSM::DataModelParser::isDataModelItem(
...
bool RSM::DataModelParser::matchNodeName(const std::string &nodeName,
...
void RSM::DataModelParser::loadExternalContent(
    const std::string &src, std::shared_ptr<IDataModelItem> dataItem) {
  // 이 메서드는 실제 구현에서 외부 URL에서 데이터를 로드하는 작업을 담당
  // 예: 파일 시스템, HTTP 요청 등

  Logger::debug(
      "RSM::DataModelParser::loadExternalContent() - Loading content from: " +
      src);
