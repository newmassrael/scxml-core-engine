...
RSM::TransitionParser::TransitionParser(
  ...
  Logger::debug(
      "RSM::TransitionParser::Constructor - Creating transition parser");
}

RSM::TransitionParser::~TransitionParser() {
  Logger::debug(
      "RSM::TransitionParser::Destructor - Destroying transition parser");
}

void RSM::TransitionParser::setActionParser(
    std::shared_ptr<RSM::ActionParser> actionParser) {
...
RSM::TransitionParser::parseTransitionNode(const xmlpp::Element *transElement,
  ...
  Logger::debug(
      "RSM::TransitionParser::parseTransitionNode() - Parsing transition: " +
  ...
  if (isInternal) {
    ...
    while (ss >> targetId) {
      if (!targetId.empty()) {
        ...
        Logger::debug(
            "RSM::TransitionParser::parseTransitionNode() - Added target: " +
            targetId);
      }
    }
  }
  ...
  if (typeAttr) {
    ...
    Logger::debug("RSM::TransitionParser::parseTransitionNode() - Type: " +
                  type);
  ...
  if (condAttr) {
    ...
    Logger::debug("RSM::TransitionParser::parseTransitionNode() - Condition: " +
                  cond);
  }
  ...
  if (!guard.empty()) {
    ...
    Logger::debug("RSM::TransitionParser::parseTransitionNode() - Guard: " +
                  guard);
  }
  ...
  if (!event.empty()) {
    ...
    for (const auto &eventName : events) {
      ...
      Logger::debug(
          "RSM::TransitionParser::parseTransitionNode() - Added event: " +
          eventName);
    }
  }
...
std::shared_ptr<RSM::ITransitionNode>
RSM::TransitionParser::parseInitialTransition(
...
RSM::TransitionParser::parseTransitionsInState(
    const xmlpp::Element *stateElement, RSM::IStateNode *stateNode) {
  ...
  // 모든 transition 요소 찾기
  auto transElements =
      ParsingCommon::findChildElements(stateElement, "transition");
  ...
  Logger::debug("RSM::TransitionParser::parseTransitionsInState() - Found " +
                std::to_string(transitions.size()) + " transitions");
  return transitions;
}

bool RSM::TransitionParser::isTransitionNode(
...
void RSM::TransitionParser::parseActions(
    const xmlpp::Element *transElement,
    std::shared_ptr<RSM::ITransitionNode> transition) {
  ...
  if (!actionParser_) {
    ...
    auto actionElements =
        ParsingCommon::findChildElements(transElement, "action");
...
RSM::TransitionParser::parseEventList(const std::string &eventStr) const {
...
bool RSM::TransitionParser::matchNodeName(const std::string &nodeName,
                                          const std::string &searchName) const {
  return ParsingCommon::matchNodeName(nodeName, searchName);
}

std::vector<std::string>
RSM::TransitionParser::parseTargetList(const std::string &targetStr) const {
