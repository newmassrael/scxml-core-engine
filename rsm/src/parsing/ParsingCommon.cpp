...
// 상수 정의
const std::string RSM::ParsingCommon::Constants::SCXML_NAMESPACE =
    "http://www.w3.org/2005/07/scxml";
const std::string RSM::ParsingCommon::Constants::CODE_NAMESPACE =
    "http://www.example.org/code-extensions";
const std::string RSM::ParsingCommon::Constants::CTX_NAMESPACE =
    "http://www.example.org/context-extensions";
const std::string RSM::ParsingCommon::Constants::DI_NAMESPACE =
    "http://www.example.org/dependency-injection";

bool RSM::ParsingCommon::matchNodeName(const std::string &nodeName,
...
RSM::ParsingCommon::findChildElements(const xmlpp::Element *element,
...
const xmlpp::Element *
RSM::ParsingCommon::findFirstChildElement(const xmlpp::Element *element,
                                          const std::string &childName) {
  if (!element) {
    Logger::warn(
        "ParsingCommon::findFirstChildElement() - Null parent element");
    return nullptr;
  }
...
std::string RSM::ParsingCommon::getAttributeValue(
    const xmlpp::Element *element, const std::vector<std::string> &attrNames) {
  if (!element) {
  ...
  for (const auto &attrName : attrNames) {
    Logger::debug("ParsingCommon::getAttributeValue() - Checking attribute: " +
                  attrName);

    auto attr = element->get_attribute(attrName);
    if (attr) {
      Logger::debug("ParsingCommon::getAttributeValue() - Found attribute: " +
                    attrName + " = " + attr->get_value());
...
RSM::ParsingCommon::collectAttributes(
...
std::string
RSM::ParsingCommon::resolveRelativePath(const std::string &basePath,
...
std::string
RSM::ParsingCommon::extractTextContent(const xmlpp::Element *element,
                                       bool trimWhitespace) {
  if (!element) {
    return "";
  }
...
std::vector<const xmlpp::Element *>
RSM::ParsingCommon::findChildElementsWithNamespace(
    const xmlpp::Element *parent, const std::string &elementName,
  ...
  for (auto child : children) {
    ...
    if (element) {
      ...
      if (ns == namespaceURI) {
        size_t colonPos = fullName.find(':');
        std::string localName = (colonPos != std::string::npos)
                                    ? fullName.substr(colonPos + 1)
                                    : fullName;
