...
RSM::TransitionNode::TransitionNode(const std::string &event,
                                    const std::string &target)
    : event_(event), target_(target), guard_(""), reactive_(false),
      internal_(false), targetsDirty_(true) {
  Logger::debug(
      "RSM::TransitionNode::Constructor - Creating transition node: " +
...
RSM::TransitionNode::~TransitionNode() {
  Logger::debug(
      "RSM::TransitionNode::Destructor - Destroying transition node: " +
      (event_.empty() ? "<no event>" : event_) + " -> " + target_);
}

const std::string &RSM::TransitionNode::getEvent() const { return event_; }
...
void RSM::TransitionNode::addTarget(const std::string &target) {
  Logger::debug(
      "RSM::TransitionNode::addTarget() - Adding target to transition " +
...
void RSM::TransitionNode::clearTargets() {
  Logger::debug(
      "RSM::TransitionNode::clearTargets() - Clearing targets for transition " +
...
void RSM::TransitionNode::setGuard(const std::string &guard) {
  Logger::debug(
      "RSM::TransitionNode::setGuard() - Setting guard for transition " +
      (event_.empty() ? "<no event>" : event_) + " -> " + target_ + ": " +
      guard);
  guard_ = guard;
}

const std::string &RSM::TransitionNode::getGuard() const { return guard_; }

void RSM::TransitionNode::addAction(const std::string &action) {
  Logger::debug(
      "RSM::TransitionNode::addAction() - Adding action to transition " +
      (event_.empty() ? "<no event>" : event_) + " -> " + target_ + ": " +
      action);
...
bool RSM::TransitionNode::isReactive() const { return reactive_; }
...
bool RSM::TransitionNode::isInternal() const { return internal_; }

void RSM::TransitionNode::setAttribute(const std::string &name,
...
void RSM::TransitionNode::addEvent(const std::string &event) {
  if (std::find(events_.begin(), events_.end(), event) == events_.end()) {
    Logger::debug(
        "RSM::TransitionNode::addEvent() - Adding event to transition: " +
        event);
    events_.push_back(event);
  }
}
