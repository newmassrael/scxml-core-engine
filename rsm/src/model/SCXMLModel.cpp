...
void RSM::SCXMLModel::setRootState(std::shared_ptr<RSM::IStateNode> rootState) {
  Logger::debug("RSM::SCXMLModel::setRootState() - Setting root state: " +
...
void RSM::SCXMLModel::setName(const std::string &name) { name_ = name; }

const std::string &RSM::SCXMLModel::getName() const { return name_; }

void RSM::SCXMLModel::setInitialState(const std::string &initialState) {
  Logger::debug("RSM::SCXMLModel::setInitialState() - Setting initial state: " +
                initialState);
...
void RSM::SCXMLModel::setDatamodel(const std::string &datamodel) {
  Logger::debug("RSM::SCXMLModel::setDatamodel() - Setting datamodel: " +
                datamodel);
  datamodel_ = datamodel;
}

const std::string &RSM::SCXMLModel::getDatamodel() const { return datamodel_; }

void RSM::SCXMLModel::addContextProperty(const std::string &name,
                                         const std::string &type) {
  Logger::debug(
      "RSM::SCXMLModel::addContextProperty() - Adding context property: " +
      name + " (" + type + ")");
  contextProperties_[name] = type;
}

const std::unordered_map<std::string, std::string> &
RSM::SCXMLModel::getContextProperties() const {
  return contextProperties_;
}

void RSM::SCXMLModel::addInjectPoint(const std::string &name,
                                     const std::string &type) {
  Logger::debug("RSM::SCXMLModel::addInjectPoint() - Adding inject point: " +
                name + " (" + type + ")");
  injectPoints_[name] = type;
}

const std::unordered_map<std::string, std::string> &
RSM::SCXMLModel::getInjectPoints() const {
  return injectPoints_;
}

void RSM::SCXMLModel::addGuard(std::shared_ptr<RSM::IGuardNode> guard) {
  if (guard) {
    Logger::debug("RSM::SCXMLModel::addGuard() - Adding guard: " +
                  guard->getId());
    guards_.push_back(guard);
  }
}

const std::vector<std::shared_ptr<RSM::IGuardNode>> &
RSM::SCXMLModel::getGuards() const {
  return guards_;
}

void RSM::SCXMLModel::addState(std::shared_ptr<RSM::IStateNode> state) {
  if (state) {
    Logger::debug("RSM::SCXMLModel::addState() - Adding state: " +
                  state->getId());
...
RSM::SCXMLModel::getAllStates() const {
  return allStates_;
}
...
RSM::IStateNode *RSM::SCXMLModel::findStateById(const std::string &id) const {
  ...
  for (const auto &state : allStates_) {
    ...
    RSM::IStateNode *result =
        findStateByIdRecursive(state.get(), id, visitedStates);
    if (result)
      return result;
  }
...
RSM::IStateNode *RSM::SCXMLModel::findStateByIdRecursive(
    RSM::IStateNode *state, const std::string &id,
  ...
  for (const auto &child : state->getChildren()) {
    RSM::IStateNode *result =
        findStateByIdRecursive(child.get(), id, visitedStates);
    if (result) {
      return result;
    }
  }
...
void RSM::SCXMLModel::addDataModelItem(
    std::shared_ptr<RSM::IDataModelItem> dataItem) {
  if (dataItem) {
    Logger::debug(
        "RSM::SCXMLModel::addDataModelItem() - Adding data model item: " +
        dataItem->getId());
    dataModelItems_.push_back(dataItem);
  }
}

const std::vector<std::shared_ptr<RSM::IDataModelItem>> &
RSM::SCXMLModel::getDataModelItems() const {
  return dataModelItems_;
}

bool RSM::SCXMLModel::validateStateRelationships() const {
  Logger::info(
      "RSM::SCXMLModel::validateStateRelationships() - Validating state "
      "relationships");
  ...
  for (const auto &state : allStates_) {
    ...
    if (parent) {
      ...
      if (!foundAsChild) {
        Logger::error(
            "RSM::SCXMLModel::validateStateRelationships() - State '" +
    ...
    for (const auto &transition : state->getTransitions()) {
      ...
      for (const auto &target : targets) {
        ...
        if (!targetState) {
          Logger::error(
              "RSM::SCXMLModel::validateStateRelationships() - Transition "
              "in state '" +
    ...
    if (!state->getInitialState().empty()) {
      if (state->getChildren().empty()) {
        Logger::warn("RSM::SCXMLModel::validateStateRelationships() - State '" +
...
std::vector<std::string> RSM::SCXMLModel::findMissingStateIds() const {
  Logger::info(
      "RSM::SCXMLModel::findMissingStateIds() - Looking for missing state IDs");
    ...
    for (const auto &transition : state->getTransitions()) {
      ...
      for (const auto &target : targets) {
        if (!target.empty() && existingIds.find(target) == existingIds.end()) {
          ...
          Logger::warn(
              "RSM::SCXMLModel::findMissingStateIds() - Missing state ID "
  ...
  Logger::info("RSM::SCXMLModel::findMissingStateIds() - Found " +
...
void RSM::SCXMLModel::printModelStructure() const {
  Logger::info(
      "RSM::SCXMLModel::printModelStructure() - Printing model structure");
  ...
  Logger::info(
      "RSM::SCXMLModel::printModelStructure() - Model structure printed");
}

void RSM::SCXMLModel::printStateHierarchy(RSM::IStateNode *state,
                                          int depth) const {
  if (!state)
    return;
...
void RSM::SCXMLModel::setBinding(const std::string &binding) {
  Logger::debug("RSM::SCXMLModel::setBinding() - Setting binding mode: " +
                binding);
  binding_ = binding;
}

const std::string &RSM::SCXMLModel::getBinding() const { return binding_; }

void RSM::SCXMLModel::addSystemVariable(
    std::shared_ptr<RSM::IDataModelItem> systemVar) {
  if (systemVar) {
    Logger::debug(
        "RSM::SCXMLModel::addSystemVariable() - Adding system variable: " +
        systemVar->getId());
...
const std::vector<std::shared_ptr<RSM::IDataModelItem>> &
RSM::SCXMLModel::getSystemVariables() const {
