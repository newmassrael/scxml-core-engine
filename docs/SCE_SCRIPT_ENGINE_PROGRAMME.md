...
|---|---|---|---|
| `transition-guard` | 210 | 135 | **25** |
| `datamodel-variable-init` | 236 | 118 | **5** |
...
`datamodel + guard` (20), then `assign + guard` (10), then
`assign + datamodel` (9). A document is freed from the engine only when
*every* cause it carries is gone.

So the payoff is cumulative, and the order decides what arrives first:

| Order | After §1 | After §2 | After §3 |
|---|---|---|---|
| **guard → datamodel → assign** | **25** | **50** | **109** |
| guard → assign → datamodel | 25 | 43 | 109 |
...
## §1 `transition-guard` -- 210 records, 135 documents, 25 freed alone
...
## §2 `datamodel-variable-init` -- 236 records, 118 documents, 5 freed alone
...
reaches 50 documents where pairing §1 with §3 reaches only 43.
...
## A known defect in the instrument

**State-scoped `<data>` initialisers produce no cause at all.** `analyze()`
in `sce-build/src/script_engine_analyzer.rs` passes only `model.variables`
(top level) to `collect_datamodel_causes`, and nothing in that file reads
`state.datamodel` -- while `classify_variables` in `analyzer.rs` does walk
it, and codegen initialises it into the engine session.

Measured on `integration_resources/send_param_payload/send_param_payload.scxml`:
`<data id="tag" expr="'kept'"/>` inside `<state id="typedPhase">` produces
**none** of that document's 16 causes. There are 9 such initialisers in the
corpus.

This under-reports, which is the dangerous direction: a document whose only
engine need is a state-scoped initialiser would read as needing none. Fixing
it raises the census count, so the ledger must be edited in the same commit.

⚠ **Unexplained, recorded rather than rounded away**: top-level initialisers
in judged documents count 240 (223 `expr` + 4 `src` + 13 `content`) against
the census's 236. Four are unaccounted for. Duplicate ids, multiple
`<datamodel>` blocks, blank `expr` and `id`-less `<data>` were each checked
and are all zero.
