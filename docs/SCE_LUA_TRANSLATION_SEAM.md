...
| `error-channel` | CLOSED | counting | **16** distinguishable failures against **0** — so the FFI carries a code plus a string, and the code already exists (re-derived 2026-09-21: `UnknownEnumVariant` joined both counts together) | `derive:expression-alphabet=16` | `sce-build/src/forge/diagnostic.rs` |
