...
| `error-channel` | CLOSED | counting | **17** distinguishable failures against **0** — so the FFI carries a code plus a string, and the code already exists (re-derived 2026-09-21: `UnknownEnumVariant` and then `MemberOfNonRecord` joined both counts together) | `derive:expression-alphabet=17` | `sce-build/src/forge/diagnostic.rs` |
