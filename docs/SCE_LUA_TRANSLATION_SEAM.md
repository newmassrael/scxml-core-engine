...
| `error-channel` | CLOSED | counting | **20** distinguishable failures against **0** — so the FFI carries a code plus a string, and the code already exists (re-derived 2026-09-21: `UnknownEnumVariant`, then `MemberOfNonRecord`, then `UnknownMember` joined both counts together; 2026-09-24: `TypeMismatch` and `ArgumentCount` did the same) | `derive:expression-alphabet=20` | `sce-build/src/forge/diagnostic.rs` |
