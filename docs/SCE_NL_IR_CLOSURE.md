...
| G2 | closed | The visualizer renders no annotation family, so the diagram a reviewer reads cannot show which elements a requirement claims and which are unclaimed. | The visualizer renders the requirement annotations, drawing the same dependency closure the acceptance report derives rather than a second walk of its own. |
| G3 | closed | The trace table covers the statechart family only. The other kind families have no review artefact carrying the requirement column. | A review artefact with the requirement column exists for at least one non-statechart kind family. |
