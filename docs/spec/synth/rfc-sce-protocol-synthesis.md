...
**GCC ecosystem fallback (auto-on defensive layer).** The ARM
embedded ecosystem default toolchain is `arm-none-eabi-gcc`, which
does **not** support Clang's `consumable` typestate or
Thread-Safety attributes. Neither does Clang when compiling C, so
on this backend the typestate attributes are inert on every build
regardless of compiler. Silently-inert hooks are the failure mode
§2.4 invariant 1 and the kind contracts forbid, so something has
to take their place — and it has to do so without the operator
opting in, because an opt-in that is skipped is the same silence
by another route.

Clang-Tidy is not that something. It reads the same source through
the same Clang frontend, and the typestate macros expand to
nothing on a C build: no annotation survives preprocessing for it
to parse. Mandating it as a parallel verification stage would
impose a build dependency that cannot report the violations it was
mandated for. SCE therefore emits no Clang-Tidy CMake fragment and
defines no `clang-tidy-not-configured` diagnostic. Clang-Tidy
remains worth running for its own checks; it is simply not a
substitute for this contract.

What does take their place is the defensive layer, on by default
exactly when `SCE_OWNERSHIP_ATTRS_AVAILABLE` is 0 — which on C is
always. The runtime boundary is active in a default release build
with no operator action, and its trap does not compile out under
`NDEBUG` (see that layer's section). The silently-inert class is
closed by construction rather than by a configure-time refusal
that the operator could satisfy and still ship nothing.

The accepted release configurations are therefore:
1. Any C build — the defensive layer is on by default and cannot
   degrade to a no-op.
2. A build that turns the defensive layer off **plus** a
   recognized commercial analyzer in CI (PC-lint Plus, Coverity —
   the analyzer layer), declared via `build.static_analyzer` so
   deploy review can see which one.
3. A C++ target on Clang ≥ 9, where the typestate attributes do
   apply.

Turning the defensive layer off without declaring an analyzer is
the one configuration that leaves the contract unenforced. It is
reachable only by explicitly passing `-DSCE_DEFENSIVE_OWNERSHIP=0`,
a deliberate act with its cost stated at the macro, and SCE does
not refuse it: a tightest-loop deployment that has audited its
callback set may want exactly that. What SCE refuses is arriving
there by default.

`build.static_analyzer` is descriptive rather than load-bearing:
it records which analyzer a deployment relies on so deploy review
can confirm CI runs it. SCE does not verify or gate on the claim.

To hedge against bugs that elude static analysis, the **defensive
layer is default-on wherever the typestate attributes do not
apply** — which on the C backend is every build. The default keys
on `SCE_OWNERSHIP_ATTRS_AVAILABLE`, not on the compiler: keying on
the compiler would leave a Clang C build with neither the
compile-time layer nor the runtime one, since the attributes are
just as inert there as under GCC. Authors override:
...
The resulting posture by toolchain:

| Build | Static | Runtime (defensive) | Cost |
|---|---|---|---|
| C target, any compiler | analyzer layer, if one is in CI | on by default | ≈10 cyc/callback |
| C target + recognized commercial analyzer (PC-lint Plus / Coverity) | analyzer layer | on by default | analyzer pass + ≈10 cyc/callback |
| C target, defensive explicitly off, no analyzer | none | off | zero — and the contract is unenforced |
| C++ target on Clang ≥ 9 | typestate + analyzer | off by default | compile-time only |

The matrix matches the reality that most embedded teams ship GCC
binaries: the top row is what a team gets by doing nothing, and it
is already enforced. Adding an analyzer strengthens it statically;
the only way to end up with nothing is to switch the runtime
boundary off by hand, which is visible in the build flags rather
than in a silence nobody notices.
...
Authors ship release builds with the defensive layer left on —
the default on every C target — and, where the budget allows, a
recognized analyzer (PC-lint Plus / Coverity) in CI on top of it.
A C++ target on Clang gets the typestate layer as well. The one
configuration that leaves the contract unenforced is switching the
defensive layer off without an analyzer; SCE does not refuse it,
but it cannot be reached by inaction. The unchecked layer is the
*runtime* row of the table, not a release posture on its own.
RFC §6.2.6 (drift detection) keeps the emitted analyzer comments
in sync with the kind source.
...
There is deliberately no diagnostic for "this build has no static
ownership coverage". Such a check would have to fire on every C
build, since the typestate attributes never apply there, and a
diagnostic that always fires is one operators learn to silence.
The gap it would report is closed instead: the defensive layer
turns itself on in exactly that case, and its trap survives
`NDEBUG`. The remaining uncovered configuration — defensive off,
no analyzer — is reachable only by passing a macro by hand, which
is auditable in the build flags rather than in SCE's output.
