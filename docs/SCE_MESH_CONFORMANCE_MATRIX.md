...
| §16.7 `error.communication` catalog | P | Row 6 (`PARALLEL_BARRIER_TIMEOUT`) via `mesh_barrier_timeout_fires_e2e` / `mesh_barrier_timeout_cancels_e2e` (runtime fire + cancel-wins-race). Row 8 (`PEER_PARTITIONED`) via `mesh_zenoh_liveliness_verification`. Rows 11 / 12 (`MISSING_SEQUENCE` / `ORDERING_GAP`) via `mesh_order_injection_verification`. Row 13 (`REGION_PARTITIONED`) via `mesh_zenoh_region_liveness_verification` (3-segment liveliness DELETE) and `mesh_someip_region_liveness_verification` (vsomeip `register_availability_handler` per RFC F.X-3). Byte-shape pins in `mesh_communication_error_verification` cover those five rows (`BarrierTimeoutShape` / `PeerPartitionedShape` / `MissingSequenceMinimalShape` / `OrderingGapFullShape` / `RegionPartitionedShape`); rows 1 / 2 / 3 / 4 / 5 / 7 / 9 / 10 have no end-to-end transport-fault fixture. |
