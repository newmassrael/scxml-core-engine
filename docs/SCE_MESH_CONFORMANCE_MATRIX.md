...
distributed conformance suite — that suite lives under the
`w3c_distributed_conformance` ctest label and is seeded with a single test.
See "§16.8 status and delta" below for what is built and what is not.
...
and delta"; `B` = verified at build time (Rust unit tests over the analyzer or
validator) with no ctest fixture; `S` = spec-only, no runtime and no build-time
evidence.

`B` exists because the three-letter scale conflated two different states. This
document catalogs *ctest* fixtures, so a section whose entire contract is a
build-time rejection has no fixture to name — and reading `S` for it says
"unverified" when the truth is "verified somewhere this table does not look".
The distinction matters in exactly the direction that costs work: `S` invites a
fixture that would duplicate an existing Rust test, while `B` names the gap as
"no runtime evidence" and leaves the build-time evidence visible.
...
| §9.6 full remote `<invoke type="scxml">` | P | 12 fixtures, not zero — this row read `S | Zero fixtures` until 2026-08-05. Wire coverage: `mesh_session_f_wire_roundtrip` (14/15/18), `mesh_session_f_eventful` (16 + `<finalize>`), `mesh_session_f_autoforward` (17), `mesh_session_f_cancel` (19), `mesh_session_f_peer_drop` (§16.7 row 5), `mesh_session_f_donedata` (three `<donedata>` flavors), `mesh_session_f_not_implemented_verification` (transport-absent local raise). Cross-device: `mesh_session_f_crossdev_lifecycle` + `mesh_session_f_crossdev_donedata` (2P, custom_tcp), `mesh_someip_scxml_invoke_roundtrip`, `mesh_zenoh_scxml_invoke_roundtrip`, plus the build-time rejection `mesh_scxml_invoke_cross_device_missing_rejection`. **Remaining gap**: dds and can carry no scxml-invoke wiring — §9.6 reserves wire values 14-20 so a follow-on transport adds binding wiring only, and neither has been added. That is why this is `P`, not `F`. |
...
| §16.3 / §16.4 Distributability analyzer (R1–R4) | B | No ctest fixture, but not unverified — this row read `S | Zero fixtures` until 2026-08-05. The analyzer is implemented in `sce-build/src/mesh/distributability.rs` and covered by 11 Rust unit tests: R1/R2 strict-reject and permissive-auto-merge, R3 snapshot notice without merge, R4 script-opacity textual match, fixed-point convergence on chained constraints, default mode, and word-bounded identifier matching. The contract is a build-time decision (reject or merge partitions), so it has no runtime surface a ctest could observe — the absence of a fixture is the shape of the feature, not a gap. |
...
| §16.8 IRP distributed harness | P | 1 fixture, not zero — this row read `S | Zero fixtures` until 2026-08-05. `w3c_distributed_conformance_test417` (label `w3c_distributed_conformance`) runs the architecture end to end: `tests/w3c/dist/run_distributed.cpp` fork+execs every partition binary and compares the aggregate verdict against the known-correct single-process W3C outcome. **Remaining gap is scale**: `tests/w3c_distributed_manifest.yaml` classifies exactly one IRP test, so the acid-test bucket has one seed. See "§16.8 status and delta". |
...
**Corrected 2026-08-05.** This bullet list previously named five artifacts as
absent. Four of them now exist, and the fifth landed in a different language —
the list had gone stale without being re-measured:

- `tests/w3c_distributed_manifest.yaml` — **exists**. Carries the closed
  `distributable: {yes, merged_single_partition, no, forbidden}` enum, with
  **one** test classified (`yes`).
- `tests/w3c/dist/` — **exists**, built by CMake: `PartitionRunner.h`,
  `run_distributed.cpp`, `w3c_dist_partition_runner.cpp`, and a `test417/`
  seed.
- ~~`tests/w3c/dist/run_distributed.py`~~ — the driver landed as
  `run_distributed.cpp` instead. It spawns every partition binary via
  fork+exec, collects exit codes, and compares the aggregate verdict against
  the known-correct W3C single-process outcome.
- The CTest label `w3c_distributed_conformance` — **exists**, carrying
  `w3c_distributed_conformance_test417`.
- The R1–R4 distributability analyzer in `sce-build` (§16.3) — **exists** as
  `sce-build/src/mesh/distributability.rs` with 11 unit tests.
...
**What the remaining gap actually is.** The claim this paragraph used to
make — that no fixture spawns per-partition OS processes and none
cross-compares against a single-process reference — is false in both halves.
`mesh_partition_rule12_e2e` forks two children that each `execv` a
partition-specific binary; `mesh_barrier_timeout_{fires,cancels}_e2e` spawn a
NonRoot / Root pair; and `w3c_distributed_conformance_test417` performs the
cross-compare the architecture calls for.

The gap is **scale, not architecture**: the manifest classifies exactly one
W3C IRP test, so the acid-test bucket has a single seed. Closing §16.8 means
classifying the rest of the IRP corpus and wiring the ones marked `yes`
through the same driver — repeating a mechanism that already works, rather
