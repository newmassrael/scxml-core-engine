...
| §16.7 `error.communication` catalog | P | Row 4 (`ENVELOPE_CORRUPT`) raised at every codegen TransportRouter `decodeEnvelope` call site via the `raise_envelope_corrupt` macro in `tools/codegen/templates/mesh/cpp/mesh_transport.h.jinja2` (9 sites: SOME/IP and Zenoh subscribers, queryables, RPC replies, eventgroup notifies). Hand-written endpoints (`ShmChannel::drainWith`, `CustomTcpTransport::read_envelope`, `ZenohScxmlInvokeEndpoint`, `SomeipScxmlInvokeEndpoint`) still silently drop malformed CBOR — extending those interfaces to surface decode failures upward is a separate atomic. Row 6 (`PARALLEL_BARRIER_TIMEOUT`) via `mesh_barrier_timeout_fires_e2e` / `mesh_barrier_timeout_cancels_e2e` (runtime fire + cancel-wins-race). Row 8 (`PEER_PARTITIONED`) via `mesh_zenoh_liveliness_verification`. Row 9 (`BACKPRESSURE_DROP`) via `mesh_outbound_buffer_verification` (admit-time overflow + sustained-overflow no-coalescing). Rows 11 / 12 (`MISSING_SEQUENCE` / `ORDERING_GAP`) via `mesh_order_injection_verification`. Row 13 (`REGION_PARTITIONED`) via `mesh_zenoh_region_liveness_verification` (3-segment liveliness DELETE) and `mesh_someip_region_liveness_verification` (vsomeip `register_availability_handler` per RFC F.X-3). Byte-shape pins in `mesh_communication_error_verification` cover seven rows (`EnvelopeCorruptShape` + `EnvelopeCorruptShapeWithPosition` / `BarrierTimeoutShape` / `PeerPartitionedShape` / `MissingSequenceMinimalShape` / `OrderingGapFullShape` / `RegionPartitionedShape` / `BackpressureDropShape`); rows 1 / 2 / 3 / 5 / 7 / 10 have neither runtime emit nor end-to-end transport-fault fixture. |
