...
| `cli/pseudo-unavailable` | Cli | `sce-codegen pseudo` was asked for a kind `forge::pseudo::render` does not cover. The rendering exists so that a reviewer who approves it has approved the document, which makes it total by contract: every field of the model reaches the output. A kind is therefore rendered in full or refused by name, never abbreviated — a text missing part of the document reads exactly like one missing none of it, and signing it would turn an unreviewed document into a signed one. ⚠ Distinct from `cli/review-table-unavailable`, which is about a kind carrying no `sce:req` site at all; this one is about coverage of the renderer and says nothing about annotation. Not preventable by authoring (the repair is to render the kind in `forge::pseudo`) |
