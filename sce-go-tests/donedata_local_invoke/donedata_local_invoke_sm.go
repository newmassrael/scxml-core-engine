...
// From: /tmp/tmp.Csrh70TjTN/donedata_local_invoke.scxml
//
...
func (p *DonedataLocalInvokePolicy) ExecutePendingInvokes(engine *sce.Engine[DonedataLocalInvokeState, DonedataLocalInvokeEvent]) {
	...
	for _, pending := range invokesToExecute {
		if strings.Contains(pending.InvokeID, ".inv_content") {
			...
			childPolicy := NewDonedataLocalInvokeChild1Policy()
			...
			childEngine := sce.NewEngine[DonedataLocalInvokeChild1State, DonedataLocalInvokeChild1Event](&childPolicy)
		...
		if strings.Contains(pending.InvokeID, ".inv_param") {
			...
			// W3C SCXML 6.4: Create child state machine
			childPolicy := NewDonedataLocalInvokeChild0Policy()
			parentQueue := make(chan sce.ParentEvent, 100)
			childPolicy.ParentExternalQueue = parentQueue
			childPolicy.InvokeID = pending.InvokeID
			childPolicy.ChildSessionID = childSessionID
			...
			childEngine := sce.NewEngine[DonedataLocalInvokeChild0State, DonedataLocalInvokeChild0Event](&childPolicy)
			childEngine.SetCompletionCallback(func() {})
			childEngine.Initialize()
...
type childEngineWrapperInvParam struct {
	engine *sce.Engine[DonedataLocalInvokeChild0State, DonedataLocalInvokeChild0Event]
	policy *DonedataLocalInvokeChild0Policy
}
...
type childEngineWrapperInvContent struct {
	engine *sce.Engine[DonedataLocalInvokeChild1State, DonedataLocalInvokeChild1Event]
	policy *DonedataLocalInvokeChild1Policy
}
