...
| SCXML → IR parsing (`sce-build/src/parser.rs`) | **Drop.** The parser-helper functions (`scxml_child` / `scxml_children`) filter children by **both local element name AND namespace** via `is_scxml_ns`. A foreign-NS element is dropped whether its local name is novel (`<framework:widget>`) or collides with a W3C name (`<framework:onentry>`). Lenient on a missing namespace declaration on the root for legacy fixtures. |
