...
**Typing convention**: the same rule holds for values. An expression's value lands in a place whose type the document declares — an output, a local, a returned value, a condition, a parameter — and stands there only if it is of that kind: a `bool` is not a number, a number is not a `bool`, and a string is not a number (a string does stand as `bytes`). A number stands where another number type is declared when every backend makes the same value of it: an integer as any integer width, wrapped to it, or as a real; a real as a real of either width. Inside an algorithm body the integer contract (§3.4.1) replaces the wrap: an integer stored where a narrower integer type is declared must fit it. A real where an integer is declared is a **build-time error** (`expression/type-mismatch`), because the backends round it differently and disagree on an out-of-range value; `round(x)` or `floor(x)` says which the document means. A call passes as many arguments as its callee has parameters (`expression/argument-count-mismatch`), each held to its parameter's type the same way. An integer literal takes the type of the place it lands in, of the operand it meets, or of the parameter it is passed to, and must fit it: `300` where a `uint8` is declared is a **build-time error** (`expression/literal-out-of-range`) — a literal is the author's own value, so it is not wrapped as a computed one is. A leading minus is part of the literal: `-128` fits an `int8`.
...
**The integer arithmetic contract.** The result of an integer `/` or `%` by zero, a signed `MIN / -1`, an overflow of `+`, `-`, `*` or unary `-` past the declared width, and an integer stored where a narrower integer type is declared — a local, a record field, a list element, the returned value or a call's argument — that the narrower type cannot hold, are not values: the backends do not agree on them (C/C++ leave them undefined, Rust panics in debug builds, Go and Kotlin wrap or throw, Python's integers do not overflow), so no backend may produce one. An algorithm either proves that none of its integer operations can reach them, or says that it can fail:
...
The proof is an interval analysis over the body: each integer value's range, narrowed by the conditions that guard it, with loops widened to their declared type and re-narrowed by their conditions. A conditional `c ? a : b` is judged branch by branch, each under the narrowing its side of `c` gives — so `y >= 0 ? y : y - 399` subtracts from a negative `y` alone — and its value is the union of the two. A quotient by a divisor of one sign keeps the bound its operands give it (`x / 400` of an `int32` stays within `int32 / 400`). A divisor excluded from zero by a `!== 0` guard is known non-zero. One fact is contractual rather than derived — **the length contract**: `len(x)` is at most `2^32 − 1` on every backend, so an index held below a length by `i < len(a)` steps safely in `uint32`.

A `may-fail` algorithm checks **every** integer `+ - * / %` and unary `-` it computes, not only those the analysis flags, at the operation's own width — so `op - 7` on a `uint8` fails before its value widens into an `int32` local — and every store of an integer whose type can hold a value the place's type cannot (an `int64` into an `int32`, any signed value into an unsigned place), which fails `overflow` when the value does not fit. A store whose type fits the place (a `uint8` into an `int32`) is never checked, and a literal that does not fit is a build-time error, as everywhere. In an algorithm that does not declare `may-fail` the analysis must prove each such store fits — the range, not the type, decides, so `(days % 7 + 11) % 7` stores into a `uint8` from an `int64`. An operation in float context (the table above) divides as reals and is not checked. The failures have one name on every backend: `overflow` (a signed `MIN / -1` or `MIN % -1` included), `divide-by-zero`, `capacity-exceeded` for a buffer append past its capacity, and `precondition` for an input outside the algorithm's domain.

**Preconditions.** `<sce:require cond="..."/>` is a statement of a `may-fail` algorithm's body: the body goes on only where `cond` holds, and fails `precondition` where it does not — so an input the algorithm has no answer for (a month 13, a 2026-02-29) is refused rather than answered with a value that reads as one. `cond` is computed as an `<sce:if>`'s is, a failure in it returned first. After it the analysis knows `cond`, as it knows an `<sce:if>`'s in the branch it guards: `<sce:require cond="month >= 1 &amp;&amp; month <= 12"/>` bounds `month` for every statement below it. Only a `may-fail` algorithm has a failure to hand its caller, so a precondition anywhere else is refused (`algorithm/require-without-may-fail`), and a build-time fold refuses one as it refuses a call. The standard documents `sce:std/time/days_in_month.scxml` and `days_from_civil.scxml` are its first users.
...
| C++ | `SCE::Forge::AlgorithmResult<T>` (`ok()` / `value()` / `error()`), no exceptions. Each operation is `SCE::Forge::Checked::<op><width>(sce_failure_, …)`, which records a failure in the body's `AlgorithmFailure` and yields 0 without executing anything the language leaves undefined; every statement is followed by a return of a recorded failure, and every `if` / `while` condition is computed before the branch it decides. Its buffers grow past their capacity (§4.12), so it never reports `capacity-exceeded` |
| C11 | `<symbol>_result_t` with `ok` and `why` (`sce_forge_algorithm_error_t`) — a scalar's `{ value, ok, why }`, and a buffer's result struct gains `why`, set to `SCE_FORGE_ALGORITHM_CAPACITY_EXCEEDED` when an append overflows. Each operation is `sce_forge_checked_<op>_<i|u><bits>(&sce_failure_, …)`, recorded and returned as on C++ |
| Go | `(value, error)` through named results `(sceValue T, sceErr error)`; the error is an `algorithm.Error` whose `Error()` is the contract name. Each operation is `scealgorithm.<Op><Int|Uint><bits>(&sceFailure, …)`, recorded and returned as on C++, and never a panic — a Go integer division by zero panics, so it is refused before it runs |
| Python | Raises `sce_forge_runtime.algorithm.AlgorithmFailure` (its `error.contract_name` names the failure) in place of a value — the exception is Python's failure channel, and it reaches the caller by itself. Each operation is `sce_algorithm.<I\|U><bits>.<op>(…)`, which computes exactly and holds the result to its width: Python's integers never overflow, so without it an `int32` would silently become a wider number |

A backend lowers `may-fail` in the commit that teaches it the checked lowering; until then it refuses the algorithm (`generate/unsupported-feature`), because an unchecked body behind a signature that promises a failure is the silent disagreement this contract exists to remove. **Calls.** An algorithm that declares `may-fail` itself may call one, in an expression or with `<sce:call>`: the call's value is the callee's, and the callee's failure becomes the caller's, unrenamed, before the next statement runs — through the same channel as its own checked arithmetic (Rust `?`; C++ `SCE::Forge::Checked::take(sce_failure_, …)`; C11 the callee header's `<symbol>_take(&sce_failure_, …)`; Go `scealgorithm.Take[T](&sceFailure)(…)`; Kotlin `SceChecked.take(…)`, thrown to the caller's own boundary; Python the exception itself). A condition that calls one is computed before the branch it decides. Everywhere else — an algorithm that does not declare `may-fail`, a statechart, another kind — the call is refused where it is written: nothing there could pass the failure on, and no guard of the caller's can bound what another algorithm does. Only a host calls a `list<T>` or record algorithm (§4.12), `may-fail` or not. Conformance: `algorithm_checked_arith`, `algorithm_checked_call`, `algorithm_checked_narrow`, `algorithm_hlc_tick` and `algorithm_weekly_expand` in `tests/forge/conformance/`, whose cases may name a failure (`"fails"`) in place of a value.

**Enforced.** An algorithm that does not declare `may-fail` is refused at the first integer operation the analysis cannot prove safe (`algorithm/undeclared-integer-failure`, placed at the operation as written), once, before any backend renders — so every backend and `check` refuse the same documents. Two repairs are right and the record proposes neither: declare `may-fail`, or guard the operation so a condition bounds it (`<sce:if cond="prev < 4294967295">` around `prev + 1`). The analysis reads guards on the values an operation computes from — a loop condition bounds its counter inside the body and, narrowed back after widening, after the loop too — but not relations between two values: `q - p` under `q >= p` is not proven, and `algorithm_cobs_encode` counts its run instead of subtracting for that reason. A body with an expression the validator refuses (an undeclared name, a literal its type cannot hold) is not judged; that expression's own refusal is the one reported.

**At build time.** A `<sce:const>` folded at build time follows the same contract, with the values it actually meets rather than a proof: each expression of the fold is typed as the emitters type it, an integer operation that overflows its width or divides as the contract forbids refuses the build (`algorithm/const-integer-failure`, on the `<sce:const>` row, naming the operation and the operands it met — `i * 2` at `128 * 2`), and a value is stored in a local, an element or the iter variable only if its type holds it. A build-time value has no caller to hand a failure to, so `may-fail` does not apply to it. Bitwise operations and shifts are outside the contract and wrap at their width, which is what a CRC table's shifted-out bits rely on. Conformance: `algorithm_const_integer_failure`.
...
**Send operands**: `sce:payload` is **bytes** — a codec's `encode_to_vec()`, or a `bytes` field. Every runtime types the service request's payload as a wire blob, and a scalar has no payload meaning without an endianness and a width, which is the decision a codec makes. A payload naming a declared field that is not `bytes` is refused as `validation/send-operand-type`, on the attribute's own line; an expression whose type the parse cannot establish is left to the generator. `sce:addr` is an **integer**, sent in decimal, or a **string**, sent as written — the service request's address is text in every runtime, and those are the two types all six backends spell alike (a boolean is `True` in Python and `1` through C++'s `std::to_string`; a float takes each language's own digits). The generator decides the form once, from the expression's type, and refuses an address of any other type, or of a type the document does not establish, with the same code on the attribute's line.
...
**`sce:max-size` and the Rust owned form**: `sce:max-size` (default 256, `BYTES_DEFAULT_MAX`) sizes the encode buffer and the inline capacity of that field; it is *not* an on-wire ceiling. The Rust borrowed view is always a zero-copy `&[u8]` / `&str` of any length. The lifetime-free owned mirror is `{Codec}Owned<S>`, parameterised by a storage profile `S: CodecStorage` rather than fixed at build-configuration time: `{Codec}Owned<Heap>` holds growable containers with every declared capacity advisory, `{Codec}Owned<Inline>` holds each field inline at its declared capacity and never allocates, and **both exist in the same binary**. Bare `{Codec}Owned` is `DefaultStorage` — `Heap` where an allocator is available, `Inline` on the heap-free tier — so a heap-capable program can still pin a real-time value to storage that is guaranteed not to allocate, and a `list`- or `embed`-bearing codec has an owned form on the MCU tier as well (the mirror carries no `alloc` gate). `try_into_owned_in::<S>()` picks the profile at a decode boundary and `try_into_owned()` takes the default; on `Inline` a field past its declared capacity raises `CodecError::TooManyElements` (the same bound decode enforces), on `Heap` the copy cannot fail. `transcode_in::<D>()` moves a value between profiles as a checked projection — a value that does not fit the destination is rejected, never truncated. A hand-assembled builder names the profile once, on the binding or the enclosing return type (`let v: {Codec}Owned = {Codec}Owned { .. }`), because an associated container type cannot be run backwards to recover `S` from a value; each field's `N` then infers from the field type via the profile's ctor (`SceBytes::from_slice(&v)?` on the default profile) rather than being hardcoded.
...
```xml
<sce:import kind="algorithm" src="sce:std/time/days_from_civil.scxml" as="civil"/>
```
