...
// From: resources/150/test150.scxml
//
...
//! Generated Rust state machine for `test150`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test150.scxml:6"]
// SCE-MAP: test150.scxml:6
...
impl Test150Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test150State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test150", &io_processors) {
...
impl Default for Test150Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test150Policy {
    type State = Test150State;
    type Event = Test150Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test150State::Fail => true,
            Test150State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test150State::Fail => "fail",
            Test150State::Pass => "pass",
            Test150State::S0 => "s0",
            Test150State::S1 => "s1",
            Test150State::S2 => "s2",
        }
...
{
    // W3C SCXML 5.6: <foreach array="Var3" item="Var1">
    self.ensure_script_engine();
    let sid = self.session_id.as_ref().unwrap().clone();
    let se = sce_rust_runtime::ScriptEngineProvider::get();
        ...
        match se.evaluate_expression(&sid, "Var3") {
            Ok(sce_rust_runtime::ScriptValue::Array(arr)) => {
                ...
                for (_idx, item_val) in arr.into_iter().enumerate() {
                    ...
                    // Set index variable
                    if let Err(e) = se.set_variable(&sid, "Var2", sce_rust_runtime::ScriptValue::Int(_idx as i64)) {
                        log::error!("Foreach: failed to set index 'Var2': {}", e);
                        foreach_success = false;
                        break;
                    }
                }
            ...
            Ok(_) => {
                ...
                log::error!("Foreach: 'Var3' is not an array");
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test150Event::ErrorExecution));
            }
            Err(e) => {
                log::error!("Foreach: failed to evaluate array 'Var3': {}", e);
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test150Event::ErrorExecution));
            }
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test150Event::Foo));
                }
            }
            Test150State::S1 => {
                // W3C SCXML 3.8: onentry block 1/1
...
{
    // W3C SCXML 5.6: <foreach array="Var3" item="Var4">
    self.ensure_script_engine();
    let sid = self.session_id.as_ref().unwrap().clone();
    let se = sce_rust_runtime::ScriptEngineProvider::get();
        ...
        match se.evaluate_expression(&sid, "Var3") {
            Ok(sce_rust_runtime::ScriptValue::Array(arr)) => {
                ...
                for (_idx, item_val) in arr.into_iter().enumerate() {
                    ...
                    // Set index variable
                    if let Err(e) = se.set_variable(&sid, "Var5", sce_rust_runtime::ScriptValue::Int(_idx as i64)) {
                        log::error!("Foreach: failed to set index 'Var5': {}", e);
                        foreach_success = false;
                        break;
                    }
                }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test150Event::Bar));
                }
            }
            _ => {}
        }
...
impl Test150Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test150State,
        event: Test150Event,
        current_state: &mut Test150State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test150State::Fail => false,
            Test150State::Pass => false,
            Test150State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Precomputed prefix match for "error"
                if event == Test150Event::Error || event == Test150Event::ErrorExecution {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test150State::Fail;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test150Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
            ...
            Test150State::S2 => {
                ...
                if event == Test150Event::Null {
if self.safe_evaluate_guard("Var4 ~= nil", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test150State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
