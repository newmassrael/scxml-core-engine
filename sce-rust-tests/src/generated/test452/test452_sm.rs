...
impl Test452Policy {
    ...
    fn ensure_script_engine(&mut self) {
        ...
        match se.evaluate_expression(&sid, "0") {
            Ok(val) => { let _ = se.set_variable(&sid, "foo", val); }
            Err(e) => { log::error!("Failed to evaluate expr for 'foo': {}", e); }
        }


        // W3C SCXML 5.8: Execute global (top-level) scripts at document load time
        if let Err(e) = se.execute_script(&sid, "function testobject()  local self = {} \n    self.bar = 0\n return self end") {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "0") {
            Ok(val) => { let _ = se.set_variable(&sid, "foo", val); }
            Err(e) => {
                log::error!("Failed to evaluate expr for 'foo': {}", e);
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test452Event::ErrorExecution));
            }
        }
...
impl Default for Test452Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test452Event::Event1));
                }
            }
            _ => {}
        }
        ...
        match check_state {
            ...
            Test452State::S0 => {
                ...
                if event == Test452Event::Event1 {
                    ...
                    if self.safe_evaluate_guard("foo.bar == 1", engine) {
                ...
                if event != Test452Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
