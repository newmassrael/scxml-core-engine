...
// From: resources/241/test241_child2.scxml
//
...
//! Generated Rust state machine for `test241_child2`.
//!
...
impl Test241Child2Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test241Child2State::Sub03,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test241_child2", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "0") {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test241Child2Event::ErrorExecution));
            }
        }
...
impl Default for Test241Child2Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test241Child2Policy {
    ...
    fn initial_state() -> Self::State {
        Test241Child2State::Sub03
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test241Child2State::SubFinal3 => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test241Child2State::Sub03 => 0,
            Test241Child2State::SubFinal3 => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test241Child2Event::ErrorExecution => "error.execution",
            Test241Child2Event::Failure => "failure",
            Test241Child2Event::Success => "success",
            Test241Child2Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test241Child2Event::ErrorExecution),
            "failure" => Some(Test241Child2Event::Failure),
            "success" => Some(Test241Child2Event::Success),
            _ => None,
        }
    }
...
impl Test241Child2Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test241Child2State,
        event: Test241Child2Event,
        current_state: &mut Test241Child2State,
        ...
        match check_state {
            Test241Child2State::Sub03 => {
                ...
                if event == Test241Child2Event::Null {
if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test241Child2State::SubFinal3;
            ...
            Test241Child2State::SubFinal3 => false,
            _ => false,
        }
    }
