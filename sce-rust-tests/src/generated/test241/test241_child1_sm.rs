...
// From: resources/241/test241_child1.scxml
//
...
//! Generated Rust state machine for `test241_child1`.
//!
...
pub struct Test241Child1Policy {
    ...
    last_transition_source_state: Test241Child1State,
...
impl Test241Child1Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test241Child1State::Sub02,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test241_child1", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "0") {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test241Child1Event::ErrorExecution));
            }
        }
...
impl Default for Test241Child1Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test241Child1Policy {
    type State = Test241Child1State;
    type Event = Test241Child1Event;
    ...
    fn initial_state() -> Self::State {
        Test241Child1State::Sub02
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test241Child1State::SubFinal2 => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test241Child1State::Sub02 => 0,
            Test241Child1State::SubFinal2 => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test241Child1Event::ErrorExecution => "error.execution",
            Test241Child1Event::Failure => "failure",
            Test241Child1Event::Success => "success",
            Test241Child1Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test241Child1Event::ErrorExecution),
            "failure" => Some(Test241Child1Event::Failure),
            "success" => Some(Test241Child1Event::Success),
            _ => None,
        }
    }
...
impl Test241Child1Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test241Child1State,
        event: Test241Child1Event,
        current_state: &mut Test241Child1State,
        ...
        match check_state {
            Test241Child1State::Sub02 => {
                ...
                if event == Test241Child1Event::Null {
if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test241Child1State::SubFinal2;
            ...
            Test241Child1State::SubFinal2 => false,
            _ => false,
        }
    }
