...
// From: resources/531/test531.scxml
//
...
//! Generated Rust state machine for `test531`.
//!
...
impl Test531Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test531State::S0,
            session_id: None,
...
impl Default for Test531Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test531Policy {
    type State = Test531State;
    type Event = Test531Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test531State::Fail => true,
            Test531State::Pass => true,
            _ => false,
        }
    }
...
}
{
    ...
    // W3C SCXML C.2: BasicHTTP send to HTTP target
    {
        ...
        http_params.entry("_scxmleventname".to_string()).or_default().push("test".to_string());
...
impl Test531Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test531State,
        event: Test531Event,
        current_state: &mut Test531State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test531State::Fail => false,
            Test531State::Pass => false,
            Test531State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test531Event::Test {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test531State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test531Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test531State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
