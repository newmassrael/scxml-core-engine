...
// From: resources/174/test174.scxml
//
...
//! Generated Rust state machine for `test174`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test174Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test174State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test174", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test174Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test174Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test174Policy {
    type State = Test174State;
    type Event = Test174Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test174State::Fail => true,
            Test174State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test174State::Fail => "fail",
            Test174State::Pass => "pass",
            Test174State::S0 => "s0",
        }
    }
...

{
    ...
    let sid = self.session_id.as_ref().unwrap().clone();
    let se = sce_rust_runtime::ScriptEngineProvider::get();
    let expr = "'http://www.w3.org/TR/scxml/#SCXMLEventProcessor'";
...

{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test174Event::Event1);
...
impl Test174Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test174State,
        event: Test174Event,
        current_state: &mut Test174State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test174State::Fail => false,
            Test174State::Pass => false,
            Test174State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test174Event::Event1 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test174State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test174Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test174State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
