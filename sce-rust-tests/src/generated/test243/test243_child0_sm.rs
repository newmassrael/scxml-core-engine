...
// From: resources/243/test243_child0.scxml
//
...
//! Generated Rust state machine for `test243_child0`.
//!
...
impl Test243Child0Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test243Child0State::Sub0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test243_child0", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "0") {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test243Child0Event::ErrorExecution));
            }
        }
...
impl Default for Test243Child0Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test243Child0Policy {
    type State = Test243Child0State;
    type Event = Test243Child0Event;
    ...
    fn initial_state() -> Self::State {
        Test243Child0State::Sub0
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test243Child0State::SubFinal => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test243Child0State::Sub0 => 0,
            Test243Child0State::SubFinal => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test243Child0Event::ErrorExecution => "error.execution",
            Test243Child0Event::Failure => "failure",
            Test243Child0Event::Success => "success",
            Test243Child0Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test243Child0Event::ErrorExecution),
            "failure" => Some(Test243Child0Event::Failure),
            "success" => Some(Test243Child0Event::Success),
            _ => None,
        }
    }
...
impl Test243Child0Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test243Child0State,
        event: Test243Child0Event,
        current_state: &mut Test243Child0State,
        ...
        match check_state {
            Test243Child0State::Sub0 => {
                ...
                if event == Test243Child0Event::Null {
if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test243Child0State::SubFinal;
            ...
            Test243Child0State::SubFinal => false,
            _ => false,
        }
    }
