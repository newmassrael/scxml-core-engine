...
// From: resources/243/test243.scxml
//
...
//! Generated Rust state machine for `test243`.
//!
...
pub struct Test243Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test243State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test243_child0)
    child__invoke_0: Option<Box<sce_rust_runtime::Engine<super::test243_child0_sm::Test243Child0Policy>>>,
...
impl Test243Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test243State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test243", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test243Event::ErrorExecution));
                false
            }
        }
    }
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy = super::test243_child0_sm::Test243Child0Policy::new();
...
impl Default for Test243Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test243Policy {
    type State = Test243State;
    type Event = Test243Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test243State::Fail => true,
            Test243State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test243Event::CancelInvoke => "cancel.invoke",
            Test243Event::DoneInvoke => "done.invoke",
            Test243Event::ErrorExecution => "error.execution",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test243Event::CancelInvoke),
            "done.invoke" => Some(Test243Event::DoneInvoke),
            "error.execution" => Some(Test243Event::ErrorExecution),
...
impl Test243Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test243State,
        event: Test243Event,
        current_state: &mut Test243State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test243State::Fail => false,
            Test243State::Pass => false,
            Test243State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test243Event::Success {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test243State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test243Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test243State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
