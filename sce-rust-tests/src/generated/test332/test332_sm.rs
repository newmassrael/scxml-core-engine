...
// From: resources/332/test332.scxml
//
...
//! Generated Rust state machine for `test332`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test332Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test332State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test332", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test332Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test332Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test332Policy {
    type State = Test332State;
    type Event = Test332Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test332State::Fail => true,
            Test332State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test332State::Fail => "fail",
            Test332State::Pass => "pass",
            Test332State::S0 => "s0",
            Test332State::S1 => "s1",
        }
    }
...

{
    ...
    // W3C SCXML 6.2: Invalid target "!invalid" raises error.execution
    {
        ...
        let mut err_meta = sce_rust_runtime::EventWithMetadata::new(Test332Event::ErrorExecution);
...
impl Test332Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test332State,
        event: Test332Event,
        current_state: &mut Test332State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test332State::Fail => false,
            Test332State::Pass => false,
            Test332State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Precomputed prefix match for "error"
                if event == Test332Event::Error || event == Test332Event::ErrorExecution {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test332State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test332Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test332State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
...
if self.safe_evaluate_guard("Var1 == Var2", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test332State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
