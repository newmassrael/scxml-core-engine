...
// From: resources/278/test278.scxml
//
...
//! Generated Rust state machine for `test278`.
//!
...
impl Test278Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test278State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test278", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "1") {
            log::error!("s1: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test278Event::ErrorExecution));
        }
...
impl Default for Test278Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test278Policy {
    type State = Test278State;
    type Event = Test278Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test278State::Fail => true,
            Test278State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test278Event::ErrorExecution => "error.execution",
            Test278Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test278Event::ErrorExecution),
            _ => None,
        }
    }
...
impl Test278Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test278State,
        event: Test278Event,
        current_state: &mut Test278State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test278State::Fail => false,
            Test278State::Pass => false,
            Test278State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test278Event::Null {
if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test278State::Pass;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test278State::Fail;
                            *transition_taken = true;
                        return true;
                    }
                }
