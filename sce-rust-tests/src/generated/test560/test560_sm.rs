...
// From: resources/560/test560.scxml
//
...
//! Generated Rust state machine for `test560`.
//!
...
impl Test560Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test560State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test560", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test560Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test560Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test560Policy {
    type State = Test560State;
    type Event = Test560Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test560State::Fail => true,
            Test560State::Pass => true,
            _ => false,
        }
    }
                ...
                'action_block: {
{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test560Event::Foo);
...
impl Test560Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test560State,
        event: Test560Event,
        current_state: &mut Test560State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test560State::Fail => false,
            Test560State::Pass => false,
            Test560State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test560Event::Foo {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_event.data.aParam == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test560State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test560Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test560State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
