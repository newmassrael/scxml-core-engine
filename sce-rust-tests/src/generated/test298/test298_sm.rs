...
// From: resources/298/test298.scxml
//
...
//! Generated Rust state machine for `test298`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test298Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test298State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test298", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "0") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test298Event::ErrorExecution));
        }
...
impl Default for Test298Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test298Policy {
    type State = Test298State;
    type Event = Test298Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test298State::Fail => true,
            Test298State::Pass => true,
            Test298State::S02 => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test298State::S01 => Some(Test298State::S0),
            Test298State::S02 => Some(Test298State::S0),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test298State::Fail => "fail",
            Test298State::Pass => "pass",
            Test298State::S0 => "s0",
            Test298State::S01 => "s01",
            Test298State::S02 => "s02",
        }
    }
    ...
    fn get_initial_children(state: Self::State) -> ::sce_rust_runtime::helpers::hierarchy::StateChain<Self::State> {
        match state {
            Test298State::S0 => ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([
                Test298State::S01,
            ]),
    ...
    let _ = event_data;  // suppress unused warning in branches that skip dispatch
            ...
            Test298State::S02 => {
                ...
                let mut done_data_ok = true;
                {
                    ...
                    log::error!("Donedata param 'Var3': empty location (structural error)");
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test298Event::ErrorExecution));
                    done_data_ok = false;
                    if done_data_ok {
                ...
                if done_data_ok {
                    let mut done_meta = sce_rust_runtime::EventWithMetadata::new(Test298Event::DoneStateS0);
...
impl Test298Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test298State,
        event: Test298Event,
        current_state: &mut Test298State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test298State::Fail => false,
            Test298State::Pass => false,
            Test298State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test298Event::ErrorExecution {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test298State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test298Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test298State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test298State::S01 => {
                ...
                if event == Test298Event::Null {
                    // Unconditional eventless transition
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                        *current_state = Test298State::S02;
                        *transition_taken = true;
                    return true;
                }
                false
            }
            Test298State::S02 => false,
            _ => false,
        }
    }
