...
// From: resources/347/test347.scxml
//
...
//! Generated Rust state machine for `test347`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
pub struct Test347Policy {
    ...
    last_transition_source_state: Test347State,
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test347State>>,
    ...
    // W3C SCXML 6.4: Static invoke child 'child' (test347__sce_synth_invoke__child)
    child_child: Option<Box<sce_rust_runtime::Engine<super::test347__sce_synth_invoke__child_sm::Test347SceSynthInvokeChildPolicy>>>,
    pending_done_invoke_child: bool,
...
impl Test347Policy {
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains(".child") {
                ...
                let mut child_policy = super::test347__sce_synth_invoke__child_sm::Test347SceSynthInvokeChildPolicy::new();
                ...
                self.child_child = Some(Box::new(child_engine));
                ...
                sce_rust_runtime::helpers::invoke_processing::drain_and_raise_child_events(
                    &self.child_child.as_ref().unwrap().policy().parent_external_queue,
                    &self.active_invokes,
                    "child",
                    engine,
                );
                ...
                if self.child_child.as_ref().map_or(false, |c| c.is_in_final_state()) {
                    self.pending_done_invoke_child = true;
    ...
    fn do_tick_children(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        if !self.pending_done_invoke_child {
            // W3C SCXML 6.4: Take child out temporarily to avoid split-borrow
            if let Some(mut child) = self.child_child.take() {
                ...
                if child.is_in_final_state() && !self.pending_done_invoke_child {
                    self.pending_done_invoke_child = true;
                    // W3C SCXML 5.5 + 6.3.1: Lift the child's stashed donedata onto
...
impl Default for Test347Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test347Policy {
    type State = Test347State;
    type Event = Test347Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test347State::Fail => true,
            Test347State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test347State::S01 => Some(Test347State::S0),
            Test347State::S02 => Some(Test347State::S0),
            _ => None,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test347Event::CancelInvoke => "cancel.invoke",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test347Event::CancelInvoke),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test347State::Fail => "fail",
            Test347State::Pass => "pass",
            Test347State::S0 => "s0",
            Test347State::S01 => "s01",
            Test347State::S02 => "s02",
        }
    }
    ...
    fn get_initial_children(state: Self::State) -> ::sce_rust_runtime::helpers::hierarchy::StateChain<Self::State> {
        match state {
            Test347State::S0 => ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([
                Test347State::S01,
            ]),
...

{
    ...
    // W3C SCXML 6.2: Delayed send (20000ms)
    engine.schedule_event(
        Test347Event::Timeout,
        core::time::Duration::from_millis(20000),
        &send_id,
        event_data,
    );
    ...
    fn execute_exit_actions(
        ...
        match state {
            Test347State::S0 => {
                ...
                // W3C SCXML 6.4: Cleanup running static child 'child'
                if self.child_child.is_some() {
                    if !self.pending_done_invoke_child {
                        engine.raise(sce_rust_runtime::EventWithMetadata::new(
                            Test347Event::CancelInvoke));
                    }
                    self.child_child = None;
                }
                self.active_invokes.remove("child");
                self.pending_done_invoke_child = false;
            }
            _ => {}
        }
        match state {
...
impl Test347Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test347State,
        event: Test347Event,
        current_state: &mut Test347State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test347State::Fail => false,
            Test347State::Pass => false,
            Test347State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test347Event::Timeout {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test347State::Fail;
            ...
            Test347State::S01 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test347Event::ChildToParent {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test347State::S02;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test347State::S02 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test347Event::DoneInvoke {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test347State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Precomputed prefix match for "error"
