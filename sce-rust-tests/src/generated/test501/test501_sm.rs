...
// From: resources/501/test501.scxml
//
...
//! Generated Rust state machine for `test501`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test501Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test501State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test501", &io_processors) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "_ioprocessors['scxml']['location']") {
            log::error!("global: {}", e);
        }
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "_ioprocessors['scxml']['location']") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test501Event::ErrorExecution));
        }
...
impl Default for Test501Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test501Policy {
    type State = Test501State;
    type Event = Test501Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test501State::Fail => true,
            Test501State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test501State::Fail => "fail",
            Test501State::Pass => "pass",
            Test501State::S0 => "s0",
        }
    }
...

{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test501Event::Foo);
...
impl Test501Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test501State,
        event: Test501Event,
        current_state: &mut Test501State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test501State::Fail => false,
            Test501State::Pass => false,
            Test501State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test501Event::Foo {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test501State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test501Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test501State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
