...
// From: resources/446/test446.scxml
//
...
//! Generated Rust state machine for `test446`.
//!
...
impl Test446Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test446State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        // W3C SCXML 5.2.2: Load variable from external file (matches C++ DataModelInitHelper::initializeVariableFromSrc)
        {
            ...
            let src_raw: &str = "file:test446.txt";
            ...
            let full_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../resources/446/");
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "{1, 2, 3}") {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test446Event::ErrorExecution));
            }
        }
...
impl Default for Test446Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test446Policy {
    type State = Test446State;
    type Event = Test446Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test446State::Fail => true,
            Test446State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test446Event::ErrorExecution => "error.execution",
            Test446Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test446Event::ErrorExecution),
            _ => None,
        }
    }
...
impl Test446Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test446State,
        event: Test446Event,
        current_state: &mut Test446State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test446State::Fail => false,
            Test446State::Pass => false,
            Test446State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test446Event::Null {
if self.safe_evaluate_guard("_isArray(var1)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test446State::Fail;
                            *transition_taken = true;
                        return true;
                    }
                }
            ...
            Test446State::S1 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test446Event::Null {
if self.safe_evaluate_guard("_isArray(var2)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test446State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
