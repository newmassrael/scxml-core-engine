...
// From: resources/343/test343.scxml
//
...
//! Generated Rust state machine for `test343`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test343Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test343State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test343", &io_processors) {
...
impl Default for Test343Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test343Policy {
    type State = Test343State;
    type Event = Test343Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test343State::Fail => true,
            Test343State::Pass => true,
            Test343State::S02 => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test343State::S01 => Some(Test343State::S0),
            Test343State::S02 => Some(Test343State::S0),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test343State::Fail => "fail",
            Test343State::Pass => "pass",
            Test343State::S0 => "s0",
            Test343State::S01 => "s01",
            Test343State::S02 => "s02",
            Test343State::S1 => "s1",
        }
    }
    ...
    fn get_initial_children(state: Self::State) -> ::sce_rust_runtime::helpers::hierarchy::StateChain<Self::State> {
        match state {
            Test343State::S0 => ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([
                Test343State::S01,
            ]),
            ...
            Test343State::S02 => {
                ...
                let mut done_data_ok = true;
                {
                    ...
                    match se.evaluate_expression(&sid, "foo") {
                        ...
                        Err(e) => {
                            log::error!("Donedata param 'someParam' eval failed: {}", e);
                            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test343Event::ErrorExecution));
                        }
                    }
                    if done_data_ok {
                ...
                if done_data_ok {
                    let mut done_meta = sce_rust_runtime::EventWithMetadata::new(Test343Event::DoneStateS0);
...
impl Test343Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test343State,
        event: Test343Event,
        current_state: &mut Test343State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test343State::Fail => false,
            Test343State::Pass => false,
            Test343State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test343Event::ErrorExecution {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test343State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test343Event::DoneStateS0 {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test343State::Fail;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
            ...
            Test343State::S01 => {
                ...
                if event == Test343Event::Null {
                    // Unconditional eventless transition
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                        *current_state = Test343State::S02;
                        *transition_taken = true;
                    return true;
                }
                false
            }
            Test343State::S02 => false,
            Test343State::S1 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test343Event::DoneStateS0 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test343State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test343Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
