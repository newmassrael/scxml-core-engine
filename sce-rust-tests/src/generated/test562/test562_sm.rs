...
// From: resources/562/test562.scxml
//
...
//! Generated Rust state machine for `test562`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test562.scxml:5"]
// SCE-MAP: test562.scxml:5
...
impl Test562Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test562State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test562", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test562Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test562Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test562Policy {
    type State = Test562State;
    type Event = Test562Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test562State::Fail => true,
            Test562State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test562State::Fail => "fail",
            Test562State::Pass => "pass",
            Test562State::S0 => "s0",
        }
    }
...

{
    ...
    let event_data: &str = "this is  a  \nstring";



    // W3C SCXML 6.2: Default send (no target = external event)
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test562Event::Foo);
...
impl Test562Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test562State,
        event: Test562Event,
        current_state: &mut Test562State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test562State::Fail => false,
            Test562State::Pass => false,
            Test562State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test562Event::Foo {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_event.data == 'this is a string'", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test562State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test562Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test562State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
