...
// From: resources/329/test329.scxml
//
...
//! Generated Rust state machine for `test329`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test329Policy {
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test329", &io_processors) {
...
impl StatePolicy for Test329Policy {
    type State = Test329State;
    type Event = Test329Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test329State::Fail => true,
            Test329State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test329State::Fail => "fail",
            Test329State::Pass => "pass",
            Test329State::S0 => "s0",
            Test329State::S1 => "s1",
            Test329State::S2 => "s2",
            Test329State::S3 => "s3",
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test329Event::Foo));
...
impl Test329Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test329State,
        event: Test329Event,
        current_state: &mut Test329State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test329State::Fail => false,
            Test329State::Pass => false,
            Test329State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test329Event::Foo {
                    ...
                    if self.safe_evaluate_guard("Var1 == _sessionid", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test329State::S1;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test329Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test329State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test329State::S1 => {
                ...
                if event == Test329Event::Null {
if self.safe_evaluate_guard("Var2 == _event", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test329State::S2;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
            ...
            Test329State::S2 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test329Event::Null {
if self.safe_evaluate_guard("Var3 == _name", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test329State::S3;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
            ...
            Test329State::S3 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test329Event::Null {
if self.safe_evaluate_guard("Var4 == _ioprocessors", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test329State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
