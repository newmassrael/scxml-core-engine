...
// From: resources/223/test223.scxml
//
...
//! Generated Rust state machine for `test223`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
pub struct Test223Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test223State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test223__sce_synth_invoke__invoke_0)
    child_invoke_0: Option<Box<sce_rust_runtime::Engine<super::test223__sce_synth_invoke__invoke_0_sm::Test223SceSynthInvokeInvoke0Policy>>>,
...
impl Test223Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test223State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test223", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test223Event::ErrorExecution));
                false
            }
        }
    }
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy = super::test223__sce_synth_invoke__invoke_0_sm::Test223SceSynthInvokeInvoke0Policy::new();
...
impl Default for Test223Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test223Policy {
    type State = Test223State;
    type Event = Test223Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test223State::Fail => true,
            Test223State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test223Event::CancelInvoke => "cancel.invoke",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test223Event::CancelInvoke),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test223State::Fail => "fail",
            Test223State::Pass => "pass",
            Test223State::S0 => "s0",
            Test223State::S1 => "s1",
        }
    }
...
impl Test223Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test223State,
        event: Test223Event,
        current_state: &mut Test223State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test223State::Fail => false,
            Test223State::Pass => false,
            Test223State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test223Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
            ...
            Test223State::S1 => {
                ...
                if event == Test223Event::Null {
if self.safe_evaluate_guard("Var1 ~= nil", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test223State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test223State::Fail;
                            *transition_taken = true;
                        return true;
                    }
                }
