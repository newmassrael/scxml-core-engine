...
// From: resources/205/test205.scxml
//
...
//! Generated Rust state machine for `test205`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test205Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test205State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test205", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test205Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test205Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test205Policy {
    type State = Test205State;
    type Event = Test205Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test205State::Fail => true,
            Test205State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test205State::Fail => "fail",
            Test205State::Pass => "pass",
            Test205State::S0 => "s0",
            Test205State::S1 => "s1",
        }
    }
...

{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test205Event::Event1);
...

{
    ...
    // W3C SCXML 6.2: Default send (no target = external event)
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test205Event::Timeout);
        // W3C SCXML 5.10.1: External send — preserve sendid and SCXML event processor origintype
        meta.metadata = sce_rust_runtime::EventMetadata::external(send_id.clone(), String::new());
        meta.metadata.data = event_data.to_string();
        engine.raise_external_with_meta(meta);
...
impl Test205Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test205State,
        event: Test205Event,
        current_state: &mut Test205State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test205State::Fail => false,
            Test205State::Pass => false,
            Test205State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test205Event::Event1 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test205State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test205Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test205State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
...
if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test205State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
