...
// From: resources/148/test148.scxml
//
...
//! Generated Rust state machine for `test148`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test148Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test148State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test148", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "0") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test148Event::ErrorExecution));
        }
...
impl Default for Test148Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test148Policy {
    type State = Test148State;
    type Event = Test148Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test148State::Fail => true,
            Test148State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test148Event::Bar => "bar",
            Test148Event::Bat => "bat",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "bar" => Some(Test148Event::Bar),
            "bat" => Some(Test148Event::Bat),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test148State::Fail => "fail",
            Test148State::Pass => "pass",
            Test148State::S0 => "s0",
        }
    }
...
if self.safe_evaluate_guard("false", engine) {

// W3C SCXML 3.8.1: <raise event="foo">
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test148Event::Foo));
{
...
} else if self.safe_evaluate_guard("false", engine) {

// W3C SCXML 3.8.1: <raise event="bar">
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test148Event::Bar));
{
    // W3C SCXML 5.3: <assign location="Var1">
    self.ensure_script_engine();
    let sid = self.session_id.as_ref().unwrap().clone();
...
// W3C SCXML 3.8.1: <raise event="baz">
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test148Event::Baz));
{
    // W3C SCXML 5.3: <assign location="Var1">
    self.ensure_script_engine();
    let sid = self.session_id.as_ref().unwrap().clone();
...
// W3C SCXML 3.8.1: <raise event="bat">
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test148Event::Bat));
                }
            }
            _ => {}
        }
...
impl Test148Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test148State,
        event: Test148Event,
        current_state: &mut Test148State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test148State::Fail => false,
            Test148State::Pass => false,
            Test148State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test148Event::Baz {
                    ...
                    if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test148State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test148Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test148State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
