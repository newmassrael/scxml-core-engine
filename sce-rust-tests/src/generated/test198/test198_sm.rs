...
// From: resources/198/test198.scxml
//
...
//! Generated Rust state machine for `test198`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test198Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test198State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test198", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test198Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test198Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test198Policy {
    type State = Test198State;
    type Event = Test198Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test198State::Fail => true,
            Test198State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test198State::Fail => "fail",
            Test198State::Pass => "pass",
            Test198State::S0 => "s0",
        }
    }
...

{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test198Event::Event1);
...

{
    ...
    // W3C SCXML 6.2: Default send (no target = external event)
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test198Event::Timeout);
        // W3C SCXML 5.10.1: External send — preserve sendid and SCXML event processor origintype
        meta.metadata = sce_rust_runtime::EventMetadata::external(send_id.clone(), String::new());
        meta.metadata.data = event_data.to_string();
        engine.raise_external_with_meta(meta);
...
impl Test198Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test198State,
        event: Test198Event,
        current_state: &mut Test198State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test198State::Fail => false,
            Test198State::Pass => false,
            Test198State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test198Event::Event1 {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_event.origintype == 'http://www.w3.org/TR/scxml/#SCXMLEventProcessor'", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test198State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test198Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test198State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
