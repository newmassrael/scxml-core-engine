...
// From: resources/250/test250_child0.scxml
//
...
//! Generated Rust state machine for `test250_child0`.
//!
...
impl Test250Child0Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test250Child0State::Sub01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test250_child0", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test250Child0Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test250Child0Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test250Child0Policy {
    type State = Test250Child0State;
    type Event = Test250Child0Event;
    ...
    fn initial_state() -> Self::State {
        Test250Child0State::Sub01
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test250Child0State::SubFinal => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test250Child0State::Sub01 => Some(Test250Child0State::Sub0),
            _ => None,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test250Child0State::Sub0 => 0,
            Test250Child0State::Sub01 => 1,
            Test250Child0State::SubFinal => 2,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test250Child0Event::ErrorExecution => "error.execution",
            Test250Child0Event::Timeout => "timeout",
            Test250Child0Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test250Child0Event::ErrorExecution),
            "timeout" => Some(Test250Child0Event::Timeout),
            _ => None,
        }
    }
...
// W3C SCXML 3.8.8: <log> with script engine expression
{
    ...
    let sid = self.session_id.as_ref().unwrap().clone();
    let se = sce_rust_runtime::ScriptEngineProvider::get();
    match se.evaluate_expression(&sid, "'entering final state, invocation was not cancelled'") {
        Ok(val) => log::info!("{:?}", val),
...
impl Test250Child0Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test250Child0State,
        event: Test250Child0Event,
        current_state: &mut Test250Child0State,
        ...
        match check_state {
            Test250Child0State::Sub0 => {
                ...
                if event == Test250Child0Event::Timeout {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test250Child0State::SubFinal;
                            *transition_taken = true;
                        return true;
                }
                false
            }
            Test250Child0State::Sub01 => false,
            Test250Child0State::SubFinal => false,
            _ => false,
        }
    }
