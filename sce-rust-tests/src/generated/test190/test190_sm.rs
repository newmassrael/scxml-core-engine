...
// From: resources/190/test190.scxml
//
...
//! Generated Rust state machine for `test190`.
//!
...
impl Test190Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test190State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test190", &io_processors) {
            log::error!("Failed to setup system variables: {}", e);
        }

        // W3C SCXML 5.2/5.3: Initialize variable from expr
        match se.evaluate_expression(&sid, "'#_scxml_'") {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test190Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test190Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test190Policy {
    type State = Test190State;
    type Event = Test190Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test190State::Fail => true,
            Test190State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test190Event::ErrorCommunication => "error.communication",
            Test190Event::ErrorExecution => "error.execution",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.communication" => Some(Test190Event::ErrorCommunication),
            "error.execution" => Some(Test190Event::ErrorExecution),
                ...
                'action_block: {
{
    ...
    // W3C SCXML 6.2: Dynamic target evaluation (targetexpr="Var1")
    {
        self.ensure_script_engine();
        let sid = self.session_id.as_ref().unwrap().clone();
        let se = sce_rust_runtime::ScriptEngineProvider::get();
        match se.evaluate_expression(&sid, "Var1") {
            Ok(ref val) if matches!(val, sce_rust_runtime::ScriptValue::Null | sce_rust_runtime::ScriptValue::Undefined) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test190Event::ErrorCommunication));
            }
            Ok(val) => {
                let dynamic_target = val.to_lua_literal();
                let dynamic_target = dynamic_target.trim_matches(|c: char| c == '\'' || c == '"');
                if dynamic_target.starts_with("!") {
                    // W3C SCXML 6.2: Invalid target raises error.execution
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test190Event::ErrorExecution));
                } else {
                    // W3C SCXML 6.2: Dispatch to dynamically resolved target
                    {
                        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test190Event::Event2);
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test190Event::Event1));
{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test190Event::Timeout);
...
impl Test190Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test190State,
        event: Test190Event,
        current_state: &mut Test190State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test190State::Fail => false,
            Test190State::Pass => false,
            Test190State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test190Event::Event1 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test190State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test190Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test190State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test190State::S1 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test190Event::Event2 {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test190State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
