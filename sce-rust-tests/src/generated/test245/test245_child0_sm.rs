...
// From: resources/245/test245_child0.scxml
//
...
//! Generated Rust state machine for `test245_child0`.
//!
...
impl Test245Child0Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test245Child0State::Sub0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test245_child0", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test245Child0Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test245Child0Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test245Child0Policy {
    type State = Test245Child0State;
    type Event = Test245Child0Event;
    ...
    fn initial_state() -> Self::State {
        Test245Child0State::Sub0
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test245Child0State::SubFinal => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test245Child0State::Sub0 => 0,
            Test245Child0State::SubFinal => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test245Child0Event::ErrorExecution => "error.execution",
            Test245Child0Event::Failure => "failure",
            Test245Child0Event::Success => "success",
            Test245Child0Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test245Child0Event::ErrorExecution),
            "failure" => Some(Test245Child0Event::Failure),
            "success" => Some(Test245Child0Event::Success),
            _ => None,
        }
    }
...
impl Test245Child0Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test245Child0State,
        event: Test245Child0Event,
        current_state: &mut Test245Child0State,
        ...
        match check_state {
            Test245Child0State::Sub0 => {
                ...
                if event == Test245Child0Event::Null {
if self.safe_evaluate_guard("Var2 ~= nil", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test245Child0State::SubFinal;
            ...
            Test245Child0State::SubFinal => false,
            _ => false,
        }
    }
