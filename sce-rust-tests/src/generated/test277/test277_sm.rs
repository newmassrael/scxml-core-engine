...
// From: resources/277/test277.scxml
//
...
//! Generated Rust state machine for `test277`.
//!
...
impl Test277Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test277State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test277", &io_processors) {
            log::error!("Failed to setup system variables: {}", e);
        }

        // W3C SCXML 5.2/5.3: Initialize variable from expr
        match se.evaluate_expression(&sid, "nil.invalidProperty") {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "nil.invalidProperty") {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test277Event::ErrorExecution));
            }
        }
...
impl Default for Test277Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test277Policy {
    type State = Test277State;
    type Event = Test277Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test277State::Fail => true,
            Test277State::Pass => true,
            _ => false,
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test277Event::Foo));
                }
            }
            Test277State::S1 => {
                // W3C SCXML 3.8: onentry block 1/1
...
impl Test277Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test277State,
        event: Test277Event,
        current_state: &mut Test277State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test277State::Fail => false,
            Test277State::Pass => false,
            Test277State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test277Event::ErrorExecution {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test277State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test277Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test277State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test277State::S1 => {
                ...
                if event == Test277Event::Null {
if self.safe_evaluate_guard("Var1 == 1", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test277State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
