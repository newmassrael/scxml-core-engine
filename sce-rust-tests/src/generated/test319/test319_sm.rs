...
// From: resources/319/test319.scxml
//
...
//! Generated Rust state machine for `test319`.
//!
...
impl Test319Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test319State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test319", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test319Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test319Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test319Policy {
    type State = Test319State;
    type Event = Test319Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test319State::Fail => true,
            Test319State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test319Event::Bound => "bound",
            Test319Event::ErrorExecution => "error.execution",
            Test319Event::Unbound => "unbound",
            Test319Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "bound" => Some(Test319Event::Bound),
            "error.execution" => Some(Test319Event::ErrorExecution),
            "unbound" => Some(Test319Event::Unbound),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(&mut self, state: Self::State, engine: &mut sce_rust_runtime::Engine<Self>) {
        match state {
            Test319State::S0 => {
                ...
                'action_block: {
// W3C SCXML 5.9: Script engine guard (typeof _event !== 'undefined')
if self.safe_evaluate_guard("_event ~= nil", engine) {
// W3C SCXML 3.8.1: <raise event="bound">
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test319Event::Bound));} else {
// W3C SCXML 3.8.1: <raise event="unbound">
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test319Event::Unbound));}
                }
            }
            _ => {}
        }
...
impl Test319Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test319State,
        event: Test319Event,
        current_state: &mut Test319State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test319State::Fail => false,
            Test319State::Pass => false,
            Test319State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test319Event::Unbound {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test319State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test319Event::Bound {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test319State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
