...
// From: resources/179/test179.scxml
//
...
//! Generated Rust state machine for `test179`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test179.scxml:4"]
// SCE-MAP: test179.scxml:4
...
impl Test179Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test179State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test179", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test179Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test179Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test179Policy {
    type State = Test179State;
    type Event = Test179Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test179State::Fail => true,
            Test179State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test179State::Fail => "fail",
            Test179State::Pass => "pass",
            Test179State::S0 => "s0",
        }
    }
...

{
    ...
    let event_data: &str = "123";



    // W3C SCXML 6.2: Default send (no target = external event)
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test179Event::Event1);
...
impl Test179Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test179State,
        event: Test179Event,
        current_state: &mut Test179State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test179State::Fail => false,
            Test179State::Pass => false,
            Test179State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test179Event::Event1 {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_event.data == 123", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test179State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test179Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test179State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
