...
// From: resources/147/test147.scxml
//
...
//! Generated Rust state machine for `test147`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test147Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test147State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test147", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "0") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test147Event::ErrorExecution));
        }
...
impl Default for Test147Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test147Policy {
    type State = Test147State;
    type Event = Test147Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test147State::Fail => true,
            Test147State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test147Event::Bar => "bar",
            Test147Event::Bat => "bat",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "bar" => Some(Test147Event::Bar),
            "bat" => Some(Test147Event::Bat),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test147State::Fail => "fail",
            Test147State::Pass => "pass",
            Test147State::S0 => "s0",
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test147Event::Foo));
{
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test147Event::Bar));
{
    // W3C SCXML 5.3: <assign location="Var1">
    self.ensure_script_engine();
    let sid = self.session_id.as_ref().unwrap().clone();
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test147Event::Baz));
{
    // W3C SCXML 5.3: <assign location="Var1">
    self.ensure_script_engine();
    let sid = self.session_id.as_ref().unwrap().clone();
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test147Event::Bat));
                }
            }
            _ => {}
        }
...
impl Test147Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test147State,
        event: Test147Event,
        current_state: &mut Test147State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test147State::Fail => false,
            Test147State::Pass => false,
            Test147State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test147Event::Bar {
                    ...
                    if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test147State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test147Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test147State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
