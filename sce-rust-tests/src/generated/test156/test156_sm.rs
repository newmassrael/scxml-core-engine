...
// From: resources/156/test156.scxml
//
...
//! Generated Rust state machine for `test156`.
//!
...
impl Test156Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test156State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test156", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "0") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test156Event::ErrorExecution));
        }
...
impl Default for Test156Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test156Policy {
    type State = Test156State;
    type Event = Test156Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test156State::Fail => true,
            Test156State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test156Event::ErrorExecution => "error.execution",
            Test156Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test156Event::ErrorExecution),
            _ => None,
        }
    }
...
impl Test156Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test156State,
        event: Test156Event,
        current_state: &mut Test156State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test156State::Fail => false,
            Test156State::Pass => false,
            Test156State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test156Event::Null {
if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test156State::Pass;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test156State::Fail;
                            *transition_taken = true;
                        return true;
                    }
                }
