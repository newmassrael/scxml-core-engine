...
// From: resources/149/test149.scxml
//
...
//! Generated Rust state machine for `test149`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test149Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test149State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test149", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "0") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test149Event::ErrorExecution));
        }
...
impl Default for Test149Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test149Policy {
    type State = Test149State;
    type Event = Test149Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test149State::Fail => true,
            Test149State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test149State::Fail => "fail",
            Test149State::Pass => "pass",
            Test149State::S0 => "s0",
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test149Event::Foo));
{
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test149Event::Bar));
{
    // W3C SCXML 5.3: <assign location="Var1">
    self.ensure_script_engine();
    let sid = self.session_id.as_ref().unwrap().clone();
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test149Event::Bat));
                }
            }
            _ => {}
        }
...
impl Test149Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test149State,
        event: Test149Event,
        current_state: &mut Test149State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test149State::Fail => false,
            Test149State::Pass => false,
            Test149State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test149Event::Bat {
                    ...
                    if self.safe_evaluate_guard("Var1 == 0", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test149State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test149Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test149State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
