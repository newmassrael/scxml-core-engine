...
// From: resources/233/test233.scxml
//
...
//! Generated Rust state machine for `test233`.
//!
...
pub struct Test233Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test233State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test233_child0)
    child__invoke_0: Option<Box<sce_rust_runtime::Engine<super::test233_child0_sm::Test233Child0Policy>>>,
...
impl Test233Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test233State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test233", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "1") {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test233Event::ErrorExecution));
            }
        }
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy = super::test233_child0_sm::Test233Child0Policy::new();
    ...
    fn do_execute_finalize_for_child_event(
        &mut self,
        event_with_meta: &sce_rust_runtime::EventWithMetadata<Test233Event>,
...
impl Default for Test233Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test233Policy {
    type State = Test233State;
    type Event = Test233Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test233State::Fail => true,
            Test233State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test233Event::CancelInvoke => "cancel.invoke",
            Test233Event::ChildToParent => "childToParent",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test233Event::CancelInvoke),
            "childToParent" => Some(Test233Event::ChildToParent),
...
impl Test233Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test233State,
        event: Test233Event,
        current_state: &mut Test233State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test233State::Fail => false,
            Test233State::Pass => false,
            Test233State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test233Event::ChildToParent {
                    ...
                    if self.safe_evaluate_guard("Var1 == 2", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test233State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test233Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test233State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
