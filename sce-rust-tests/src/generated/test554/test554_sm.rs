...
// From: resources/554/test554.scxml
//
...
//! Generated Rust state machine for `test554`.
//!
...
pub struct Test554Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test554State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test554_child0)
    child__invoke_0: Option<Box<sce_rust_runtime::Engine<super::test554_child0_sm::Test554Child0Policy>>>,
...
impl Test554Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test554State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test554", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test554Event::ErrorExecution));
                false
            }
        }
    }
            ...
            if pending.invoke_id.contains("._invoke_0") {
                ...
                // 1:1 port of C++ NamelistHelper::evaluateNamelist — validate in parent scope
                {
                    ...
                    if !se.has_variable(&sid, "__undefined_variable_for_error__") {
                        log::error!("Namelist validation failed: '__undefined_variable_for_error__' not declared in parent");
                ...
                let mut child_policy = super::test554_child0_sm::Test554Child0Policy::new();
...
impl Default for Test554Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test554Policy {
    type State = Test554State;
    type Event = Test554Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test554State::Fail => true,
            Test554State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test554Event::CancelInvoke => "cancel.invoke",
            Test554Event::DoneInvoke => "done.invoke",
            Test554Event::ErrorExecution => "error.execution",
            Test554Event::Timer => "timer",
            Test554Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test554Event::CancelInvoke),
            "done.invoke" => Some(Test554Event::DoneInvoke),
            "error.execution" => Some(Test554Event::ErrorExecution),
            "timer" => Some(Test554Event::Timer),
            _ => None,
        }
    }
...
impl Test554Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test554State,
        event: Test554Event,
        current_state: &mut Test554State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test554State::Fail => false,
            Test554State::Pass => false,
            Test554State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test554Event::Timer {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test554State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test554Event::DoneInvoke {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test554State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
