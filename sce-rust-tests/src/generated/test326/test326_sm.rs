...
// From: resources/326/test326.scxml
//
...
//! Generated Rust state machine for `test326`.
//!
...
impl Test326Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test326State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test326", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "_ioprocessors") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test326Event::ErrorExecution));
        }
...
impl Default for Test326Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test326Policy {
    type State = Test326State;
    type Event = Test326Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test326State::Fail => true,
            Test326State::Pass => true,
            _ => false,
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test326Event::Foo));
                }
            }
            Test326State::S2 => {
                // W3C SCXML 3.8: onentry block 1/1
...
impl Test326Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test326State,
        event: Test326Event,
        current_state: &mut Test326State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test326State::Fail => false,
            Test326State::Pass => false,
            Test326State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test326Event::Null {
if self.safe_evaluate_guard("Var1 ~= nil", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test326State::Fail;
                            *transition_taken = true;
                        return true;
                    }
                }
            ...
            Test326State::S1 => {
                ...
                if event == Test326Event::ErrorExecution {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test326State::S2;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test326Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
            ...
            Test326State::S2 => {
                ...
                if event == Test326Event::Null {
                    ...
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                        *current_state = Test326State::Pass;
                        *transition_taken = true;
                    return true;
                }
                false
