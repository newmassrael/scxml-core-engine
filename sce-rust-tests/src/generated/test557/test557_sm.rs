...
impl Test557Policy {
    ...
    fn ensure_script_engine(&mut self) {
        ...
        match se.evaluate_expression(&sid, "<books xmlns=\"\">\n     <book title=\"title1\"/>\n     <book title=\"title2\"/>\n   </books>") {
        ...
        // W3C SCXML 5.2.2: Load variable from external file (matches C++ DataModelInitHelper::initializeVariableFromSrc)
        {
            ...
            let src_raw: &str = "file:test557.txt";
            ...
            let full_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../resources/557/");
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "<books xmlns=\"\">\n     <book title=\"title1\"/>\n     <book title=\"title2\"/>\n   </books>") {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test557Event::ErrorExecution));
            }
        }
...
impl Default for Test557Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
if self.safe_evaluate_guard("var1:getElementsByTagName('book')[1]:getAttribute('title') == 'title1'", engine) {
            ...
            Test557State::S1 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test557Event::Null {
if self.safe_evaluate_guard("var2:getElementsByTagName('book')[2]:getAttribute('title') == 'title2'", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
