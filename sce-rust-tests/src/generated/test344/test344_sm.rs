...
// From: resources/344/test344.scxml
//
...
//! Generated Rust state machine for `test344`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test344Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test344State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test344", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test344Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test344Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test344Policy {
    type State = Test344State;
    type Event = Test344Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test344State::Fail => true,
            Test344State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test344State::Fail => "fail",
            Test344State::Pass => "pass",
            Test344State::S0 => "s0",
            Test344State::S1 => "s1",
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test344Event::Foo));
                }
            }
            _ => {}
        }
...
impl Test344Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test344State,
        event: Test344Event,
        current_state: &mut Test344State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test344State::Fail => false,
            Test344State::Pass => false,
            Test344State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test344Event::Null {
if self.safe_evaluate_guard("_scxml_truthy(return)", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test344State::Fail;
            ...
            Test344State::S1 => {
                ...
                if event == Test344Event::ErrorExecution {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test344State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test344Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
