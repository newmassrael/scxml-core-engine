...
// From: resources/558/test558.scxml
//
...
//! Generated Rust state machine for `test558`.
//!
...
impl Test558Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test558State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test558", &io_processors) {
            log::error!("Failed to setup system variables: {}", e);
        }

        // W3C SCXML B.2: Initialize with inline content
        match se.evaluate_expression(&sid, "this  is \na string") {
            Ok(val) => { let _ = se.set_variable(&sid, "var1", val); }
            Err(e) => { log::error!("Failed to evaluate content for 'var1': {}", e); }
        }
        // W3C SCXML 5.2.2: Load variable from external file (matches C++ DataModelInitHelper::initializeVariableFromSrc)
        {
            ...
            let src_raw: &str = "file:test558.txt";
            ...
            let full_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../resources/558/");
            ...
            match std::fs::read_to_string(&file_path) {
                ...
                Err(e) => {
                    ...
                    log::error!("Failed to read src file '{}' for variable 'var2': {}", file_path, e);
                }
            }
        }
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "this  is \na string") {
            Ok(val) => { let _ = se.set_variable(&sid, "var1", val); }
            Err(e) => {
                log::error!("Failed to evaluate content for 'var1': {}", e);
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test558Event::ErrorExecution));
            }
        }
...
impl Default for Test558Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test558Policy {
    type State = Test558State;
    type Event = Test558Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test558State::Fail => true,
            Test558State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test558Event::ErrorExecution => "error.execution",
            Test558Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test558Event::ErrorExecution),
            _ => None,
        }
    }
...
impl Test558Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test558State,
        event: Test558Event,
        current_state: &mut Test558State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test558State::Fail => false,
            Test558State::Pass => false,
            Test558State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test558Event::Null {
if self.safe_evaluate_guard("var1 == 'this is a string'", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test558State::Fail;
                            *transition_taken = true;
                        return true;
                    }
                }
            ...
            Test558State::S1 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test558Event::Null {
if self.safe_evaluate_guard("var2 == 'this is a string'", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test558State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
