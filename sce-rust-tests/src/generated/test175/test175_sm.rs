...
// From: resources/175/test175.scxml
//
...
//! Generated Rust state machine for `test175`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test175.scxml:6"]
// SCE-MAP: test175.scxml:6
...
impl Test175Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test175State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test175", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "'0s'") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test175Event::ErrorExecution));
        }
...
impl Default for Test175Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test175Policy {
    type State = Test175State;
    type Event = Test175Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test175State::Fail => true,
            Test175State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test175State::Fail => "fail",
            Test175State::Pass => "pass",
            Test175State::S0 => "s0",
            Test175State::S1 => "s1",
        }
    }
...
impl Test175Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test175State,
        event: Test175Event,
        current_state: &mut Test175State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test175State::Fail => false,
            Test175State::Pass => false,
            Test175State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test175Event::Event1 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test175State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test175Event::Event2 {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test175State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test175State::S1 => {
                ...
                if event == Test175Event::Event2 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test175State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test175Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
