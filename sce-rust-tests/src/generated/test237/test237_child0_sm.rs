...
// From: resources/237/test237_child0.scxml
//
...
//! Generated Rust state machine for `test237_child0`.
//!
...
impl Test237Child0Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test237Child0State::Sub0,
            session_id: None,
...
impl Default for Test237Child0Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test237Child0Policy {
    type State = Test237Child0State;
    type Event = Test237Child0Event;
    ...
    fn initial_state() -> Self::State {
        Test237Child0State::Sub0
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test237Child0State::SubFinal => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test237Child0State::Sub0 => 0,
            Test237Child0State::SubFinal => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test237Child0Event::ErrorExecution => "error.execution",
            Test237Child0Event::Timeout => "timeout",
            Test237Child0Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test237Child0Event::ErrorExecution),
            "timeout" => Some(Test237Child0Event::Timeout),
            _ => None,
        }
    }
...
impl Test237Child0Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test237Child0State,
        event: Test237Child0Event,
        current_state: &mut Test237Child0State,
        ...
        match check_state {
            Test237Child0State::Sub0 => {
                ...
                if event == Test237Child0Event::Timeout {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test237Child0State::SubFinal;
                            *transition_taken = true;
                        return true;
                }
                false
            }
            Test237Child0State::SubFinal => false,
            _ => false,
        }
    }
