...
// From: resources/342/test342.scxml
//
...
//! Generated Rust state machine for `test342`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test342Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test342State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test342", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "'foo'") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test342Event::ErrorExecution));
        }
...
impl Default for Test342Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test342Policy {
    type State = Test342State;
    type Event = Test342Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test342State::Fail => true,
            Test342State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test342State::Fail => "fail",
            Test342State::Pass => "pass",
            Test342State::S0 => "s0",
            Test342State::S1 => "s1",
        }
    }
...
impl Test342Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test342State,
        event: Test342Event,
        current_state: &mut Test342State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test342State::Fail => false,
            Test342State::Pass => false,
            Test342State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test342Event::Foo {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test342State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test342Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test342State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
...
if self.safe_evaluate_guard("Var1 == Var2", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test342State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
