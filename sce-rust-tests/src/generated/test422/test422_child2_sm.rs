...
// From: resources/422/test422_child2.scxml
//
...
//! Generated Rust state machine for `test422_child2`.
//!
...
impl Test422Child2Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test422Child2State::Sub2,
            session_id: None,
...
impl Default for Test422Child2Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test422Child2Policy {
    type State = Test422Child2State;
    type Event = Test422Child2Event;
    ...
    fn initial_state() -> Self::State {
        Test422Child2State::Sub2
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test422Child2State::SubFinal2 => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test422Child2State::Sub2 => 0,
            Test422Child2State::SubFinal2 => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test422Child2Event::ErrorExecution => "error.execution",
            Test422Child2Event::InvokeS12 => "invokeS12",
            Test422Child2Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test422Child2Event::ErrorExecution),
            "invokeS12" => Some(Test422Child2Event::InvokeS12),
            _ => None,
        }
    }
...
impl Test422Child2Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test422Child2State,
        event: Test422Child2Event,
        current_state: &mut Test422Child2State,
        ...
        match check_state {
            Test422Child2State::Sub2 => {
                ...
                if event == Test422Child2Event::Null {
                    ...
                    self.last_transition_is_targetless = false;
                        *current_state = Test422Child2State::SubFinal2;
                        *transition_taken = true;
                    return true;
                }
                false
            }
            Test422Child2State::SubFinal2 => false,
            _ => false,
        }
    }
