...
// From: resources/422/test422_child0.scxml
//
...
//! Generated Rust state machine for `test422_child0`.
//!
...
impl Test422Child0Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test422Child0State::Sub0,
            session_id: None,
...
impl Default for Test422Child0Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test422Child0Policy {
    type State = Test422Child0State;
    type Event = Test422Child0Event;
    ...
    fn initial_state() -> Self::State {
        Test422Child0State::Sub0
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test422Child0State::SubFinal0 => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test422Child0State::Sub0 => 0,
            Test422Child0State::SubFinal0 => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test422Child0Event::ErrorExecution => "error.execution",
            Test422Child0Event::InvokeS1 => "invokeS1",
            Test422Child0Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test422Child0Event::ErrorExecution),
            "invokeS1" => Some(Test422Child0Event::InvokeS1),
            _ => None,
        }
    }
...
impl Test422Child0Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test422Child0State,
        event: Test422Child0Event,
        current_state: &mut Test422Child0State,
        ...
        match check_state {
            Test422Child0State::Sub0 => {
                ...
                if event == Test422Child0Event::Null {
                    ...
                    self.last_transition_is_targetless = false;
                        *current_state = Test422Child0State::SubFinal0;
                        *transition_taken = true;
                    return true;
                }
                false
            }
            Test422Child0State::SubFinal0 => false,
            _ => false,
        }
    }
