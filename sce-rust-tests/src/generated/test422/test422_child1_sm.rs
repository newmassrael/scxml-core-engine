...
// From: resources/422/test422_child1.scxml
//
...
//! Generated Rust state machine for `test422_child1`.
//!
...
impl Test422Child1Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test422Child1State::Sub1,
            session_id: None,
...
impl Default for Test422Child1Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test422Child1Policy {
    type State = Test422Child1State;
    type Event = Test422Child1Event;
    ...
    fn initial_state() -> Self::State {
        Test422Child1State::Sub1
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test422Child1State::SubFinal1 => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test422Child1State::Sub1 => 0,
            Test422Child1State::SubFinal1 => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test422Child1Event::ErrorExecution => "error.execution",
            Test422Child1Event::InvokeS11 => "invokeS11",
            Test422Child1Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test422Child1Event::ErrorExecution),
            "invokeS11" => Some(Test422Child1Event::InvokeS11),
            _ => None,
        }
    }
...
impl Test422Child1Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test422Child1State,
        event: Test422Child1Event,
        current_state: &mut Test422Child1State,
        ...
        match check_state {
            Test422Child1State::Sub1 => {
                ...
                if event == Test422Child1Event::Null {
                    ...
                    self.last_transition_is_targetless = false;
                        *current_state = Test422Child1State::SubFinal1;
                        *transition_taken = true;
                    return true;
                }
                false
            }
            Test422Child1State::SubFinal1 => false,
            _ => false,
        }
    }
