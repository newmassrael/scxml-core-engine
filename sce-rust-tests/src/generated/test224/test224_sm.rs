...
// From: resources/224/test224.scxml
//
...
//! Generated Rust state machine for `test224`.
//!
...
pub struct Test224Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test224State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test224_child0)
    child__invoke_0: Option<Box<sce_rust_runtime::Engine<super::test224_child0_sm::Test224Child0Policy>>>,
...
impl Test224Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test224State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test224", &io_processors) {
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy = super::test224_child0_sm::Test224Child0Policy::new();
...
impl Default for Test224Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test224Policy {
    type State = Test224State;
    type Event = Test224Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test224State::Fail => true,
            Test224State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test224Event::CancelInvoke => "cancel.invoke",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test224Event::CancelInvoke),
...
impl Test224Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test224State,
        event: Test224Event,
        current_state: &mut Test224State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test224State::Fail => false,
            Test224State::Pass => false,
            Test224State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test224Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
            ...
            Test224State::S1 => {
                ...
                if event == Test224Event::Null {
if self.safe_evaluate_guard("_indexOf(Var1, Var2) == 0", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test224State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test224State::Fail;
                            *transition_taken = true;
                        return true;
                    }
                }
