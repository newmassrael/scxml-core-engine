...
// From: resources/487/test487.scxml
//
...
//! Generated Rust state machine for `test487`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test487.scxml:5"]
// SCE-MAP: test487.scxml:5
...
impl Test487Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test487State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test487", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test487Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test487Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test487Policy {
    type State = Test487State;
    type Event = Test487Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test487State::Fail => true,
            Test487State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test487State::Fail => "fail",
            Test487State::Pass => "pass",
            Test487State::S0 => "s0",
        }
    }
...
// W3C SCXML 3.8.1: <raise event="event">
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test487Event::Event));
                }
            }
            _ => {}
        }
...
impl Test487Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test487State,
        event: Test487Event,
        current_state: &mut Test487State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test487State::Fail => false,
            Test487State::Pass => false,
            Test487State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test487Event::ErrorExecution {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test487State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test487Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test487State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
