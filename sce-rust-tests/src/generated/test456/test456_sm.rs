...
impl Test456Policy {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "0") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test456Event::ErrorExecution));
        }
...
impl Default for Test456Policy {
    fn default() -> Self {
        Self::new()
    }
}
...

{
    ...
    let sid = self.session_id.as_ref().unwrap().clone();
    let se = sce_rust_runtime::ScriptEngineProvider::get();
    if let Err(e) = se.execute_script(&sid, "\n      Var1 = Var1 + (1)\n      ") {
