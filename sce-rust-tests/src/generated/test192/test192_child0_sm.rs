...
// From: resources/192/test192_child0.scxml
//
...
//! Generated Rust state machine for `test192_child0`.
//!
...
impl Test192Child0Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test192Child0State::Sub0,
...
impl Default for Test192Child0Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test192Child0Policy {
    type State = Test192Child0State;
    type Event = Test192Child0Event;
    ...
    fn initial_state() -> Self::State {
        Test192Child0State::Sub0
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test192Child0State::SubFinal => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test192Child0State::Sub0 => 0,
            Test192Child0State::SubFinal => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test192Child0Event::ChildToParent => "childToParent",
            Test192Child0Event::ErrorExecution => "error.execution",
            Test192Child0Event::EventReceived => "eventReceived",
            Test192Child0Event::ParentToChild => "parentToChild",
            Test192Child0Event::Timeout => "timeout",
            Test192Child0Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "childToParent" => Some(Test192Child0Event::ChildToParent),
            "error.execution" => Some(Test192Child0Event::ErrorExecution),
            "eventReceived" => Some(Test192Child0Event::EventReceived),
            "parentToChild" => Some(Test192Child0Event::ParentToChild),
            "timeout" => Some(Test192Child0Event::Timeout),
            _ => None,
        }
    }
...
impl Test192Child0Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test192Child0State,
        event: Test192Child0Event,
        current_state: &mut Test192Child0State,
        ...
        match check_state {
            Test192Child0State::Sub0 => {
                ...
                if event == Test192Child0Event::ParentToChild {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test192Child0State::SubFinal;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test192Child0Event::Timeout {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
            ...
            Test192Child0State::SubFinal => false,
            _ => false,
        }
    }
