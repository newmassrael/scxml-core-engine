...
// From: resources/158/test158.scxml
//
...
//! Generated Rust state machine for `test158`.
//!
...
impl Test158Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test158State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test158", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test158Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test158Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test158Policy {
    type State = Test158State;
    type Event = Test158Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test158State::Fail => true,
            Test158State::Pass => true,
            _ => false,
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test158Event::Event1));
// W3C SCXML 3.8.1: <raise event="event2">
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test158Event::Event2));
                }
            }
            _ => {}
        }
...
impl Test158Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test158State,
        event: Test158Event,
        current_state: &mut Test158State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test158State::Fail => false,
            Test158State::Pass => false,
            Test158State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test158Event::Event1 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test158State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test158Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test158State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test158State::S1 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test158Event::Event2 {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test158State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
