...
// From: resources/189/test189.scxml
//
...
//! Generated Rust state machine for `test189`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test189Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test189State::S0,
            session_id: None,
...
impl Default for Test189Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test189Policy {
    type State = Test189State;
    type Event = Test189Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test189State::Fail => true,
            Test189State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test189Event::ErrorExecution => "error.execution",
            Test189Event::Event1 => "event1",
            Test189Event::Event2 => "event2",
            Test189Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test189Event::ErrorExecution),
            "event1" => Some(Test189Event::Event1),
            "event2" => Some(Test189Event::Event2),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test189State::Fail => "fail",
            Test189State::Pass => "pass",
            Test189State::S0 => "s0",
        }
    }
...

{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test189Event::Event2);
...

{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test189Event::Event1);
        meta.metadata.data = event_data.to_string();
        engine.raise(meta);
    }
...
impl Test189Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test189State,
        event: Test189Event,
        current_state: &mut Test189State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test189State::Fail => false,
            Test189State::Pass => false,
            Test189State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test189Event::Event1 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test189State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test189Event::Event2 {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test189State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
