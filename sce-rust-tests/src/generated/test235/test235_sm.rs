...
// From: resources/235/test235.scxml
//
...
//! Generated Rust state machine for `test235`.
//!
...
pub struct Test235Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test235State>>,
    ...
    // W3C SCXML 6.4: Static invoke child 'foo' (test235_child0)
    child_foo: Option<Box<sce_rust_runtime::Engine<super::test235_child0_sm::Test235Child0Policy>>>,
...
impl Test235Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test235State::S0,
            session_id: None,
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains(".foo") {
                ...
                let mut child_policy = super::test235_child0_sm::Test235Child0Policy::new();
...
impl Default for Test235Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test235Policy {
    type State = Test235State;
    type Event = Test235Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test235State::Fail => true,
            Test235State::Pass => true,
            _ => false,
        }
    }
...
impl Test235Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test235State,
        event: Test235Event,
        current_state: &mut Test235State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test235State::Fail => false,
            Test235State::Pass => false,
            Test235State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test235Event::DoneInvokeFoo {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test235State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test235Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test235State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
