...
// From: resources/186/test186.scxml
//
...
//! Generated Rust state machine for `test186`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test186Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test186State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test186", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "1") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test186Event::ErrorExecution));
        }
...
impl Default for Test186Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test186Policy {
    type State = Test186State;
    type Event = Test186Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test186State::Fail => true,
            Test186State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test186State::Fail => "fail",
            Test186State::Pass => "pass",
            Test186State::S0 => "s0",
            Test186State::S1 => "s1",
        }
    }
...
impl Test186Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test186State,
        event: Test186Event,
        current_state: &mut Test186State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test186State::Fail => false,
            Test186State::Pass => false,
            Test186State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test186Event::Event1 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test186State::S1;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test186Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test186State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
...
if self.safe_evaluate_guard("Var2 == 1", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test186State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
