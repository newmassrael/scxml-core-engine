...
// From: resources/151/test151.scxml
//
...
//! Generated Rust state machine for `test151`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test151Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test151State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test151", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::eval_or_set_string(
            ...
            "[1,2,3]") {
            log::error!("Failed to init 'Var3' in global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test151Event::ErrorExecution));
        }
...
impl Default for Test151Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test151Policy {
    type State = Test151State;
    type Event = Test151Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test151State::Fail => true,
            Test151State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test151State::Fail => "fail",
            Test151State::Pass => "pass",
            Test151State::S0 => "s0",
            Test151State::S1 => "s1",
            Test151State::S2 => "s2",
        }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test151Event::Foo));
                }
            }
            Test151State::S1 => {
                // W3C SCXML 3.8: onentry block 1/1
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test151Event::Bar));
                }
            }
            _ => {}
        }
...
impl Test151Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test151State,
        event: Test151Event,
        current_state: &mut Test151State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test151State::Fail => false,
            Test151State::Pass => false,
            Test151State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Precomputed prefix match for "error"
                if event == Test151Event::Error || event == Test151Event::ErrorExecution {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test151State::Fail;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test151Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
            ...
            Test151State::S2 => {
                ...
                if event == Test151Event::Null {
if self.safe_evaluate_guard("Var5 ~= nil", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test151State::Pass;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
