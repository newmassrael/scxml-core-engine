...
// From: resources/234/test234_child1.scxml
//
...
//! Generated Rust state machine for `test234_child1`.
//!
...
impl Test234Child1Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test234Child1State::Sub0,
            session_id: None,
...
impl Default for Test234Child1Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test234Child1Policy {
    type State = Test234Child1State;
    type Event = Test234Child1Event;
    ...
    fn initial_state() -> Self::State {
        Test234Child1State::Sub0
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test234Child1State::SubFinal2 => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test234Child1State::Sub0 => 0,
            Test234Child1State::SubFinal2 => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test234Child1Event::ErrorExecution => "error.execution",
            Test234Child1Event::Timeout => "timeout",
            Test234Child1Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test234Child1Event::ErrorExecution),
            "timeout" => Some(Test234Child1Event::Timeout),
            _ => None,
        }
    }
...
impl Test234Child1Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test234Child1State,
        event: Test234Child1Event,
        current_state: &mut Test234Child1State,
        ...
        match check_state {
            Test234Child1State::Sub0 => {
                ...
                if event == Test234Child1Event::Timeout {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test234Child1State::SubFinal2;
                            *transition_taken = true;
                        return true;
                }
                false
            }
            Test234Child1State::SubFinal2 => false,
            _ => false,
        }
    }
