...
// From: resources/335/test335.scxml
//
...
//! Generated Rust state machine for `test335`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Default for Test335Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test335Policy {
    type State = Test335State;
    type Event = Test335Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test335State::Fail => true,
            Test335State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test335State::Fail => "fail",
            Test335State::Pass => "pass",
            Test335State::S0 => "s0",
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test335Event::Foo));
                }
            }
            _ => {}
        }
...
impl Test335Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test335State,
        event: Test335Event,
        current_state: &mut Test335State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test335State::Fail => false,
            Test335State::Pass => false,
            Test335State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test335Event::Foo {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test335State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test335Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test335State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
