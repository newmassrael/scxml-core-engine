...
// From: resources/350/test350.scxml
//
...
//! Generated Rust state machine for `test350`.
//!
...
impl Test350Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test350State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test350", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test350Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test350Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test350Policy {
    type State = Test350State;
    type Event = Test350Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test350State::Fail => true,
            Test350State::Pass => true,
            _ => false,
        }
    }
...
}
{
    ...
    {
        let mut meta = sce_rust_runtime::EventWithMetadata::new(Test350Event::S0Event);
...
impl Test350Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test350State,
        event: Test350Event,
        current_state: &mut Test350State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test350State::Fail => false,
            Test350State::Pass => false,
            Test350State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test350Event::S0Event {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test350State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test350Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test350State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
