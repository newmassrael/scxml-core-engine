...
// From: resources/310/test310.scxml
//
...
//! Generated Rust state machine for `test310`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
pub struct Test310Policy {
    ...
    active_states: ::sce_rust_runtime::helpers::hierarchy::StateChain<Test310State>,
...
impl Test310Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test310State::S0,
...
impl Default for Test310Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test310Policy {
    type State = Test310State;
    type Event = Test310Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test310State::Fail => true,
            Test310State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test310State::S0 => Some(Test310State::P),
            Test310State::S1 => Some(Test310State::P),
            _ => None,
        }
    }
    ...
    fn get_parallel_regions(state: Self::State) -> &'static [Self::State] {
        match state {
            Test310State::P => &[Test310State::S0, Test310State::S1],
            _ => &[],
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test310State::Fail => "fail",
            Test310State::P => "p",
            Test310State::Pass => "pass",
            Test310State::S0 => "s0",
            Test310State::S1 => "s1",
        }
    }
    ...
    fn process_transition(
        ...
        if event == Self::null_event() {
            ...
            let mut seen: ::sce_rust_runtime::SceDedupSet<(Test310State, usize)> =
...
impl Test310Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test310State,
        event: Test310Event,
        current_state: &mut Test310State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
        mut collect_mode: Option<&mut TransitionList>,
    ) -> bool {
        match check_state {
            Test310State::Fail => false,
            Test310State::P => false,
            Test310State::Pass => false,
            Test310State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test310Event::Null {
                    if self.is_state_active("s1") {
                        ...
                        if let Some(ref mut collect) = collect_mode {
                            ...
                            *current_state = Test310State::Pass;
                        ...
                        if let Some(ref mut collect) = collect_mode {
                            ...
                            *current_state = Test310State::Fail;
                            *transition_taken = true;
                        }
                        return true;
                    }
    ...
    fn compute_exit_set(
        ...
        is_internal: bool,
        is_targetless: bool,
    ) -> ::sce_rust_runtime::helpers::hierarchy::StateChain<Test310State> {
    ...
    fn execute_microstep(
        ...
        let mut states_to_exit: ::sce_rust_runtime::helpers::hierarchy::StateChain<Test310State> =
        ...
        for trans in transitions {
            ...
            let domain: Option<Test310State> = if is_internal_to_descendant {
                Some(trans.source)
            } else {
