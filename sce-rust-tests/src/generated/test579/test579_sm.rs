...
// From: resources/579/test579.scxml
//
...
//! Generated Rust state machine for `test579`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
pub struct Test579Policy {
    ...
    history_sh1: Option<Vec<Test579State>>,
...
impl Test579Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test579State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test579", &io_processors) {
            log::error!("Failed to setup system variables: {}", e);
        }

        // W3C SCXML 5.2.2: Initialize global datamodel variables (no error events)

        // W3C SCXML 5.3: Early binding - Initialize state s0 datamodel variables
        // W3C SCXML 5.2/5.3: Initialize 'Var1' from expr (s0)
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "0") {
            log::error!("s0: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test579Event::ErrorExecution));
        }
...
impl Default for Test579Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test579Policy {
    type State = Test579State;
    type Event = Test579Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test579State::Fail => true,
            Test579State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test579State::S01 => Some(Test579State::S0),
            Test579State::S02 => Some(Test579State::S0),
            Test579State::S03 => Some(Test579State::S0),
            _ => None,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test579Event::ErrorExecution => "error.execution",
            Test579Event::Event1 => "event1",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test579Event::ErrorExecution),
            "event1" => Some(Test579Event::Event1),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test579State::Fail => "fail",
            Test579State::Pass => "pass",
            Test579State::S0 => "s0",
            Test579State::S01 => "s01",
            Test579State::S02 => "s02",
            Test579State::S03 => "s03",
            Test579State::S2 => "s2",
            Test579State::S3 => "s3",
        }
    }
    ...
    fn get_initial_children(state: Self::State) -> ::sce_rust_runtime::helpers::hierarchy::StateChain<Self::State> {
        match state {
            Test579State::S0 => ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([
                Test579State::S01,
            ]),
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test579State::S0 => {
                // W3C SCXML 3.11: Check history for sh1
                if let Some(ref history) = self.history_sh1 {
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test579Event::Event1));
                }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test579Event::Event2));
                }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test579Event::Event3));
                }
            }
            _ => {}
        }
    ...
    fn execute_exit_actions(
        ...
        if state == Test579State::S0 {
            ...
            let filtered: Vec<Self::State> = pre_transition_active.iter()
                .filter(|&&s| Self::get_parent(s) == Some(Test579State::S0))
                .copied()
                .collect();
            self.history_sh1 = Some(filtered);
        }
        match state {
            Test579State::S0 => {
...
impl Test579Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test579State,
        event: Test579Event,
        current_state: &mut Test579State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test579State::Fail => false,
            Test579State::Pass => false,
            Test579State::S0 => false,
            Test579State::S01 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test579Event::Event1 {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test579State::S02;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test579Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test579State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test579State::S02 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test579Event::Event2 {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test579State::S03;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
            ...
            Test579State::S3 => {
                ...
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test579Event::Timeout {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;

                            *current_state = Test579State::Pass;
                            *transition_taken = true;
                        return true;
                }
                false
    ...
    // W3C SCXML 3.11: Resolve history target for sh1
    fn resolve_history_target_sh1(&self) -> Test579State {
        if let Some(ref recorded) = self.history_sh1 {
