...
// From: resources/230/test230.scxml
//
...
//! Generated Rust state machine for `test230`.
//!
...
pub struct Test230Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test230State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test230_child0)
    child__invoke_0: Option<Box<sce_rust_runtime::Engine<super::test230_child0_sm::Test230Child0Policy>>>,
...
impl Test230Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test230State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test230", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test230Event::ErrorExecution));
                false
            }
        }
    }
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy = super::test230_child0_sm::Test230Child0Policy::new();
    ...
    fn do_forward_to_autoforward_children(&mut self, event_name: &str, _engine: &mut sce_rust_runtime::Engine<Self>) {
        // Check if child _invoke_0 has autoforward enabled and is active
        {
            ...
            if is_autoforward {
                if let Some(ref mut child) = self.child__invoke_0 {
...
impl Default for Test230Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test230Policy {
    type State = Test230State;
    type Event = Test230Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test230State::Fail => true,
            Test230State::Final => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test230State::S01 => Some(Test230State::S0),
            Test230State::S02 => Some(Test230State::S0),
            _ => None,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test230Event::CancelInvoke => "cancel.invoke",
            Test230Event::ChildToParent => "childToParent",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test230Event::CancelInvoke),
            "childToParent" => Some(Test230Event::ChildToParent),
...
impl Test230Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test230State,
        event: Test230Event,
        current_state: &mut Test230State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test230State::Fail => false,
            Test230State::Final => false,
            Test230State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test230Event::Timeout {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test230State::Final;
            ...
            Test230State::S01 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test230Event::ChildToParent {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test230State::S02;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test230Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), "*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test230State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
            ...
            Test230State::S02 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test230Event::DoneInvoke {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        self.has_transition_actions = false;
