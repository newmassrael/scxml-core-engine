...
// From: resources/230/test230_child0.scxml
//
...
//! Generated Rust state machine for `test230_child0`.
//!
...
impl Test230Child0Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test230Child0State::Sub0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test230_child0", &io_processors) {
    ...
    fn safe_evaluate_guard(&mut self, cond: &str, engine: &mut Engine<Self>) -> bool {
        ...
        match se.evaluate_expression(&sid, cond) {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test230Child0Event::ErrorExecution));
                false
            }
        }
    }
...
impl Default for Test230Child0Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test230Child0Policy {
    type State = Test230Child0State;
    type Event = Test230Child0Event;
    ...
    fn initial_state() -> Self::State {
        Test230Child0State::Sub0
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test230Child0State::SubFinal => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test230Child0State::Sub0 => 0,
            Test230Child0State::SubFinal => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test230Child0Event::ChildToParent => "childToParent",
            Test230Child0Event::ErrorExecution => "error.execution",
            Test230Child0Event::Timeout => "timeout",
            Test230Child0Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "childToParent" => Some(Test230Child0Event::ChildToParent),
            "error.execution" => Some(Test230Child0Event::ErrorExecution),
            "timeout" => Some(Test230Child0Event::Timeout),
            _ => None,
        }
    }
...
impl Test230Child0Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test230Child0State,
        event: Test230Child0Event,
        current_state: &mut Test230Child0State,
        ...
        match check_state {
            Test230Child0State::Sub0 => {
                ...
                if event == Test230Child0Event::ChildToParent {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test230Child0State::SubFinal;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test230Child0Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
            ...
            Test230Child0State::SubFinal => false,
            _ => false,
        }
    }
