...
// From: resources/240/test240_child0.scxml
//
...
//! Generated Rust state machine for `test240_child0`.
//!
...
impl Test240Child0Policy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: Test240Child0State::Sub01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test240_child0", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        match se.evaluate_expression(&sid, "0") {
            ...
            Err(e) => {
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::new(Test240Child0Event::ErrorExecution));
            }
        }
...
impl Default for Test240Child0Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test240Child0Policy {
    type State = Test240Child0State;
    type Event = Test240Child0Event;
    ...
    fn initial_state() -> Self::State {
        Test240Child0State::Sub01
    }

    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test240Child0State::SubFinal1 => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            Test240Child0State::Sub01 => 0,
            Test240Child0State::SubFinal1 => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test240Child0Event::ErrorExecution => "error.execution",
            Test240Child0Event::Failure => "failure",
            Test240Child0Event::Success => "success",
            Test240Child0Event::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test240Child0Event::ErrorExecution),
            "failure" => Some(Test240Child0Event::Failure),
            "success" => Some(Test240Child0Event::Success),
            _ => None,
        }
    }
...
impl Test240Child0Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test240Child0State,
        event: Test240Child0Event,
        current_state: &mut Test240Child0State,
        ...
        match check_state {
            Test240Child0State::Sub01 => {
                ...
                if event == Test240Child0Event::Null {
if self.safe_evaluate_guard("Var1 == 1", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test240Child0State::SubFinal1;
            ...
            Test240Child0State::SubFinal1 => false,
            _ => false,
        }
    }
