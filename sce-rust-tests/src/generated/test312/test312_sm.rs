...
// From: resources/312/test312.scxml
//
...
//! Generated Rust state machine for `test312`.
//!
...
impl Test312Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test312State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test312", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "Var1", "1") {
            log::error!("global: {}", e);
            engine.raise(sce_rust_runtime::EventWithMetadata::new(Test312Event::ErrorExecution));
        }
...
impl Default for Test312Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test312Policy {
    type State = Test312State;
    type Event = Test312Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test312State::Fail => true,
            Test312State::Pass => true,
            _ => false,
        }
    }
...
engine.raise(sce_rust_runtime::EventWithMetadata::new(Test312Event::Foo));
                }
            }
            _ => {}
        }
...
impl Test312Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test312State,
        event: Test312Event,
        current_state: &mut Test312State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test312State::Fail => false,
            Test312State::Pass => false,
            Test312State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test312Event::ErrorExecution {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test312State::Pass;
                            *transition_taken = true;
                        return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test312Event::Null && sce_rust_runtime::helpers::event_matching::matches_event_descriptor(
                    Self::get_event_name(event), ".*") {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test312State::Fail;
                            *transition_taken = true;
                        return true;
                }
                false
