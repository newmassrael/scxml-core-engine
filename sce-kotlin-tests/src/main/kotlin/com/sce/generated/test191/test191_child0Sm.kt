...
class Test191Child0StateMachine(
    ...
    override fun resolveState(stateId: String): Test191Child0State? = when (stateId) {
        "sub0" -> Test191Child0State.Sub0
        "subFinal" -> Test191Child0State.SubFinal
        else -> null
    }

    // W3C SCXML: Get state ID string from State object
    override fun stateIdOf(state: Test191Child0State): String = when (state) {
        is Test191Child0State.Sub0 -> "sub0"
        is Test191Child0State.SubFinal -> "subFinal"
    }

    // W3C SCXML 3.4: Check if state is atomic (leaf — no children)
    override fun isAtomicState(state: Test191Child0State): Boolean = when (state) {
        else -> true
    }


    // W3C SCXML 3.13: Document order for exit ordering
    override fun documentOrderOf(state: Test191Child0State): Int = when (state) {
        is Test191Child0State.Sub0 -> 0
        is Test191Child0State.SubFinal -> 1
    }
    ...
    private fun processNullSub0(
        ...
        else -> TransitionResult.External(Test191Child0State.SubFinal, Test191Child0State.Sub0)
    }
