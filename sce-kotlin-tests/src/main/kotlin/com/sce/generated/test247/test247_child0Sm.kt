...
class Test247Child0StateMachine(
    ...
    override fun resolveState(stateId: String): Test247Child0State? = when (stateId) {
        "subFinal" -> Test247Child0State.SubFinal
        else -> null
    }

    // W3C SCXML: Get state ID string from State object
    override fun stateIdOf(state: Test247Child0State): String = when (state) {
        is Test247Child0State.SubFinal -> "subFinal"
        else -> ""
    }

    // W3C SCXML 3.4: Check if state is atomic (leaf — no children)
    override fun isAtomicState(state: Test247Child0State): Boolean = when (state) {
        else -> true
    }


    // W3C SCXML 3.13: Document order for exit ordering
    override fun documentOrderOf(state: Test247Child0State): Int = when (state) {
        is Test247Child0State.SubFinal -> 0
        else -> 0
    }
