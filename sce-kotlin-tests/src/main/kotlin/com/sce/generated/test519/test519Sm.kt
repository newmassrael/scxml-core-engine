...
class Test519StateMachine(
    ...
    override fun resolveState(stateId: String): Test519State? = when (stateId) {
        "fail" -> Test519State.Fail
        "pass" -> Test519State.Pass
        "s0" -> Test519State.S0
        else -> null
    }

    // W3C SCXML: Get state ID string from State object
    override fun stateIdOf(state: Test519State): String = when (state) {
        is Test519State.Fail -> "fail"
        is Test519State.Pass -> "pass"
        is Test519State.S0 -> "s0"
    }

    // W3C SCXML 3.4: Check if state is atomic (leaf — no children)
    override fun isAtomicState(state: Test519State): Boolean = when (state) {
        else -> true
    }


    // W3C SCXML 3.13: Document order for exit ordering
    override fun documentOrderOf(state: Test519State): Int = when (state) {
        is Test519State.Fail -> 2
        is Test519State.Pass -> 1
        is Test519State.S0 -> 0
    }
    ...
    private fun processS0(
        ...
        event is Test519Event.Test && safeEvaluateGuard("_event.data[\"param1\"] == 1") -> TransitionResult.External(Test519State.Pass, Test519State.S0)
