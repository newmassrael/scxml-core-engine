...
sealed interface Test301Event : Event {
    sealed interface Error : Test301Event {
...
class Test301StateMachine(
...
) : StateMachineEngine<Test301State, Test301Event>(scriptEngine) {

    override val initialState: Test301State = Test301State.S0
    ...
    override fun resolveEventByName(name: String): Test301Event? = when (name) {
        "error.execution" -> Test301Event.Error.Execution
        else -> null
    }

    // W3C SCXML 6.4: Resolve Event object to event name string
    override fun eventNameOf(event: Test301Event): String? = when (event) {
        is Test301Event.Error.Execution -> "error.execution"
        else -> null
    }
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            ...
            raiseInternal(Test301Event.Error.Execution)
        }
    ...
    private fun setCurrentEventInScriptEngine(event: Test301Event) {
        ensureScriptEngine()
        val engine = scriptEngine ?: return
        val sid = scriptSessionId ?: return
    ...
    private fun processNullS0(
        ...
        else -> TransitionResult.External(Test301State.Fail)
    }
