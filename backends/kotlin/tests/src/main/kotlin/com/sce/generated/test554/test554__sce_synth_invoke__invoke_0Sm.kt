...
// SCE-MAP: test554__sce_synth_invoke__invoke_0.scxml:3 :: _machine
...
class Test554SceSynthInvokeInvoke0StateMachine(
    ...
    override fun isFinalState(state: Test554SceSynthInvokeInvoke0State): Boolean = when (state) {
        is Test554SceSynthInvokeInvoke0State.SubFinal -> true
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test554SceSynthInvokeInvoke0State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test554SceSynthInvokeInvoke0State, HistoryId>> =
            listOf(StateTarget(Test554SceSynthInvokeInvoke0State.SubFinal))
    }
    ...
    ): EnabledTransition<Test554SceSynthInvokeInvoke0State, HistoryId>? = when (state) {
        else -> null
    }
    ...
    // SCE-MAP: test554__sce_synth_invoke__invoke_0.scxml:3 :: _machine
    override fun onEntry(state: Test554SceSynthInvokeInvoke0State, isDefaultEntry: Boolean) {
        when (state) {
            is Test554SceSynthInvokeInvoke0State.SubFinal -> {
                // SCE-MAP: test554__sce_synth_invoke__invoke_0.scxml:4 :: subFinal :: _state_body
    ...
    // SCE-MAP: test554__sce_synth_invoke__invoke_0.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test554SceSynthInvokeInvoke0State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
