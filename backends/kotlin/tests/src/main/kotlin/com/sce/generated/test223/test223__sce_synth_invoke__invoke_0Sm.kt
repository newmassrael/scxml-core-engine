...
class Test223SceSynthInvokeInvoke0StateMachine(
    ...
    override fun isFinalState(state: Test223SceSynthInvokeInvoke0State): Boolean = when (state) {
        is Test223SceSynthInvokeInvoke0State.SubFinal -> true
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test223SceSynthInvokeInvoke0State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test223SceSynthInvokeInvoke0State, HistoryId>> =
            listOf(StateTarget(Test223SceSynthInvokeInvoke0State.SubFinal))
    }
    ...
    ): EnabledTransition<Test223SceSynthInvokeInvoke0State, HistoryId>? = when (state) {
        else -> null
    }
    ...
    // SCE-MAP: test223__sce_synth_invoke__invoke_0.scxml:3 :: _machine
    override fun onEntry(state: Test223SceSynthInvokeInvoke0State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test223__sce_synth_invoke__invoke_0.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test223SceSynthInvokeInvoke0State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
