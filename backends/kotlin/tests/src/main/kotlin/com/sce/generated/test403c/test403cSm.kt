...
class Test403cStateMachine(
    ...
    override fun isCompoundState(state: Test403cState): Boolean = when (state) {
        is Test403cState.S0 -> true
        else -> false
    }
    ...
    override fun isFinalState(state: Test403cState): Boolean = when (state) {
        is Test403cState.Fail, is Test403cState.Pass -> true
        else -> false
    }

    // §scxml-D-getChildStates: a state's <state>, <parallel> and <final>
    // children, in document order — for a <parallel>, its regions.
    override fun childStatesOf(state: Test403cState): List<Test403cState> =
        childStates[state] ?: emptyList()

    // W3C SCXML 3.3: a compound state's initial transition target, as written.
    override fun initialTargetsOf(state: Test403cState): List<EntryTarget<Test403cState, HistoryId>> =
        initialTargets[state] ?: emptyList()

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test403cState, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val childStates: Map<Test403cState, List<Test403cState>> = mapOf(
            Test403cState.P0 to listOf(Test403cState.P0s1, Test403cState.P0s2, Test403cState.P0s3, Test403cState.P0s4),
            Test403cState.S0 to listOf(Test403cState.P0),
        )

        val initialTargets: Map<Test403cState, List<EntryTarget<Test403cState, HistoryId>>> = mapOf(
            Test403cState.S0 to listOf(StateTarget(Test403cState.P0)),
        )

        val documentInitialTargetList: List<EntryTarget<Test403cState, HistoryId>> =
            listOf(StateTarget(Test403cState.S0))

        // W3C SCXML 3.13: p0s1's transition 0, as the microstep reads it.
        val transitionP0s1At0 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.P0s1,
            emptyList(),
            0,
            hasActions = false,
        ...
        // W3C SCXML 3.13: p0s1's transition 1, as the microstep reads it.
        val transitionP0s1At1 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.P0s1,
            emptyList(),
            1,
            hasActions = false,
        ...
        // W3C SCXML 3.13: p0s2's transition 0, as the microstep reads it.
        val transitionP0s2At0 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.P0s2,
            listOf(StateTarget(Test403cState.P0s1)),
            0,
            hasActions = true,
            isInternal = false,
        )

        // W3C SCXML 3.13: p0s3's transition 0, as the microstep reads it.
        val transitionP0s3At0 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.P0s3,
            listOf(StateTarget(Test403cState.Fail)),
            0,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: p0s3's transition 1, as the microstep reads it.
        val transitionP0s3At1 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.P0s3,
            listOf(StateTarget(Test403cState.S1)),
            1,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: p0s4's transition 0, as the microstep reads it.
        val transitionP0s4At0 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.P0s4,
            emptyList(),
            0,
            hasActions = true,
        ...
        val transitionS0At0 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.S0,
            listOf(StateTarget(Test403cState.Fail)),
            0,
            hasActions = false,
        ...
        val transitionS0At1 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.S0,
            listOf(StateTarget(Test403cState.Fail)),
            1,
            hasActions = false,
        ...
        val transitionS1At0 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.S1,
            listOf(StateTarget(Test403cState.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test403cState, HistoryId>(
            Test403cState.S1,
            listOf(StateTarget(Test403cState.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test403cEvent) {
    ...
    override fun firstEnabledTransition(
        state: Test403cState,
        event: Test403cEvent?
    ): EnabledTransition<Test403cState, HistoryId>? = when (state) {
        is Test403cState.P0s1 -> when {
            event is Test403cEvent.Event1 -> transitionP0s1At0
            event is Test403cEvent.Event2 -> transitionP0s1At1
            else -> null
        }
        is Test403cState.P0s2 -> when {
            event is Test403cEvent.Event1 -> transitionP0s2At0
            else -> null
        }
        is Test403cState.P0s3 -> when {
            event is Test403cEvent.Event1 -> transitionP0s3At0
            event is Test403cEvent.Event2 -> transitionP0s3At1
            else -> null
        }
        is Test403cState.P0s4 -> when {
            event != null -> transitionP0s4At0
            else -> null
        }
        is Test403cState.S0 -> when {
            event is Test403cEvent.Event2 -> transitionS0At0
            event is Test403cEvent.Timeout -> transitionS0At1
            else -> null
        }
        is Test403cState.S1 -> when {
    ...
    // SCE-MAP: test403c.scxml:5 :: _machine
    override fun onEntry(state: Test403cState, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test403c.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test403cState, transitionIndex: Int) {
        when (source) {
