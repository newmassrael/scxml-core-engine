...
class Test242sub1StateMachine(
    ...
    override fun isFinalState(state: Test242sub1State): Boolean = when (state) {
        is Test242sub1State.Final -> true
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test242sub1State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test242sub1State, HistoryId>> =
            listOf(StateTarget(Test242sub1State.Final))
    }
    ...
    ): EnabledTransition<Test242sub1State, HistoryId>? = when (state) {
        else -> null
    }
    ...
    // SCE-MAP: test242sub1.scxml:5 :: _machine
    override fun onEntry(state: Test242sub1State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test242sub1.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test242sub1State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
