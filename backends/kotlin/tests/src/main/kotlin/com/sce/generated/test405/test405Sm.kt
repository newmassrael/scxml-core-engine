// SCE-GENERATED — DO NOT EDIT
// source-hash: b1edd275a200b2f8553040c83495e98b687c11a97259eaf4d60667291dcb916a

// GENERATED CODE — DO NOT EDIT
// Source: resources/405/test405.scxml
// Generator: SCE Kotlin Code Generator v1.0
// SCE-MAP: test405.scxml:6 :: _machine

package com.sce.generated.test405

import com.sce.runtime.*


// --- States (W3C SCXML 3.2) ---

sealed interface Test405State : State {
    data object Fail : Test405State
    data object Pass : Test405State
    data object S0 : Test405State
    data object S01p : Test405State
    data object S01p1 : Test405State
    data object S01p11 : Test405State
    data object S01p12 : Test405State
    data object S01p2 : Test405State
    data object S01p21 : Test405State
    data object S01p22 : Test405State
    data object S02 : Test405State
    data object S03 : Test405State
    data object S04 : Test405State
}

// --- Events (W3C SCXML 3.12.1) ---

sealed interface Test405Event : Event {
    sealed interface Error : Test405Event {
        data object Execution : Error
    }
    data object Event1 : Test405Event
    data object Event2 : Test405Event
    data object Event3 : Test405Event
    data object Event4 : Test405Event
    data object Timeout : Test405Event
}
// --- State Machine (W3C SCXML) ---

class Test405StateMachine(
) : StateMachineEngine<Test405State, Test405Event>() {

    override val initialState: Test405State = Test405State.S01p11

    // W3C SCXML 6.2: which entry point a host must drive this machine with in
    // the synchronous mode. The same verdict the generate manifest publishes
    // as `needs_event_scheduler`.
    override val needsEventScheduler: Boolean = true

    // --- Document structure (W3C SCXML 3.2-3.4, 3.10) ---
    //
    // What the runtime's Appendix D procedures (com.sce.runtime.Microstep)
    // read of this document. The tables are built once, in the companion
    // object below, because the structure is a fact about the document and
    // not about a run.

    // W3C SCXML 3.3: State hierarchy parent mapping
    override fun parentOf(state: Test405State): Test405State? = when (state) {
        is Test405State.S01p -> Test405State.S0
        is Test405State.S01p1 -> Test405State.S01p
        is Test405State.S01p11 -> Test405State.S01p1
        is Test405State.S01p12 -> Test405State.S01p1
        is Test405State.S01p2 -> Test405State.S01p
        is Test405State.S01p21 -> Test405State.S01p2
        is Test405State.S01p22 -> Test405State.S01p2
        is Test405State.S02 -> Test405State.S0
        is Test405State.S03 -> Test405State.S0
        is Test405State.S04 -> Test405State.S0
        else -> null
    }

    // W3C SCXML 3.3: a <state> with child states — exactly the states that
    // have an initial transition. A <parallel> is not compound.
    override fun isCompoundState(state: Test405State): Boolean = when (state) {
        is Test405State.S0, is Test405State.S01p1, is Test405State.S01p2 -> true
        else -> false
    }

    // W3C SCXML 3.4: Check if state is a parallel state
    override fun isParallelState(state: Test405State): Boolean = when (state) {
        is Test405State.S01p -> true
        else -> false
    }

    // W3C SCXML 3.7: Check if state is a <final> element
    override fun isFinalState(state: Test405State): Boolean = when (state) {
        is Test405State.Fail, is Test405State.Pass -> true
        else -> false
    }

    // §scxml-D-getChildStates: a state's <state>, <parallel> and <final>
    // children, in document order — for a <parallel>, its regions.
    override fun childStatesOf(state: Test405State): List<Test405State> =
        childStates[state] ?: emptyList()

    // W3C SCXML 3.3: a compound state's initial transition target, as written.
    override fun initialTargetsOf(state: Test405State): List<EntryTarget<Test405State, HistoryId>> =
        initialTargets[state] ?: emptyList()

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test405State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val childStates: Map<Test405State, List<Test405State>> = mapOf(
            Test405State.S0 to listOf(Test405State.S01p, Test405State.S02, Test405State.S03, Test405State.S04),
            Test405State.S01p to listOf(Test405State.S01p1, Test405State.S01p2),
            Test405State.S01p1 to listOf(Test405State.S01p11, Test405State.S01p12),
            Test405State.S01p2 to listOf(Test405State.S01p21, Test405State.S01p22),
        )

        val initialTargets: Map<Test405State, List<EntryTarget<Test405State, HistoryId>>> = mapOf(
            Test405State.S0 to listOf(StateTarget(Test405State.S01p)),
            Test405State.S01p1 to listOf(StateTarget(Test405State.S01p11)),
            Test405State.S01p2 to listOf(StateTarget(Test405State.S01p21)),
        )

        val documentInitialTargetList: List<EntryTarget<Test405State, HistoryId>> =
            listOf(StateTarget(Test405State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S0,
            listOf(StateTarget(Test405State.Fail)),
            0,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: s01p's transition 0, as the microstep reads it.
        val transitionS01pAt0 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S01p,
            listOf(StateTarget(Test405State.S02)),
            0,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: s01p11's transition 0, as the microstep reads it.
        val transitionS01p11At0 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S01p11,
            listOf(StateTarget(Test405State.S01p12)),
            0,
            hasActions = true,
            isInternal = false,
        )

        // W3C SCXML 3.13: s01p21's transition 0, as the microstep reads it.
        val transitionS01p21At0 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S01p21,
            listOf(StateTarget(Test405State.S01p22)),
            0,
            hasActions = true,
            isInternal = false,
        )

        // W3C SCXML 3.13: s02's transition 0, as the microstep reads it.
        val transitionS02At0 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S02,
            listOf(StateTarget(Test405State.S03)),
            0,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: s02's transition 1, as the microstep reads it.
        val transitionS02At1 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S02,
            listOf(StateTarget(Test405State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: s03's transition 0, as the microstep reads it.
        val transitionS03At0 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S03,
            listOf(StateTarget(Test405State.S04)),
            0,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: s03's transition 1, as the microstep reads it.
        val transitionS03At1 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S03,
            listOf(StateTarget(Test405State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: s04's transition 0, as the microstep reads it.
        val transitionS04At0 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S04,
            listOf(StateTarget(Test405State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: s04's transition 1, as the microstep reads it.
        val transitionS04At1 = EnabledTransition<Test405State, HistoryId>(
            Test405State.S04,
            listOf(StateTarget(Test405State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    }

    // W3C SCXML: Resolve state ID string to State object
    override fun resolveState(stateId: String): Test405State? = when (stateId) {
        "fail" -> Test405State.Fail
        "pass" -> Test405State.Pass
        "s0" -> Test405State.S0
        "s01p" -> Test405State.S01p
        "s01p1" -> Test405State.S01p1
        "s01p11" -> Test405State.S01p11
        "s01p12" -> Test405State.S01p12
        "s01p2" -> Test405State.S01p2
        "s01p21" -> Test405State.S01p21
        "s01p22" -> Test405State.S01p22
        "s02" -> Test405State.S02
        "s03" -> Test405State.S03
        "s04" -> Test405State.S04
        else -> null
    }

    // W3C SCXML: Get state ID string from State object
    override fun stateIdOf(state: Test405State): String = when (state) {
        is Test405State.Fail -> "fail"
        is Test405State.Pass -> "pass"
        is Test405State.S0 -> "s0"
        is Test405State.S01p -> "s01p"
        is Test405State.S01p1 -> "s01p1"
        is Test405State.S01p11 -> "s01p11"
        is Test405State.S01p12 -> "s01p12"
        is Test405State.S01p2 -> "s01p2"
        is Test405State.S01p21 -> "s01p21"
        is Test405State.S01p22 -> "s01p22"
        is Test405State.S02 -> "s02"
        is Test405State.S03 -> "s03"
        is Test405State.S04 -> "s04"
    }

    // W3C SCXML 3.13: Document order — entry order, and in reverse exit order
    override fun documentOrderOf(state: Test405State): Int = when (state) {
        is Test405State.Fail -> 12
        is Test405State.Pass -> 11
        is Test405State.S0 -> 0
        is Test405State.S01p -> 1
        is Test405State.S01p1 -> 2
        is Test405State.S01p11 -> 3
        is Test405State.S01p12 -> 4
        is Test405State.S01p2 -> 5
        is Test405State.S01p21 -> 6
        is Test405State.S01p22 -> 7
        is Test405State.S02 -> 8
        is Test405State.S03 -> 9
        is Test405State.S04 -> 10
    }






    // W3C SCXML Appendix D selectTransitions, the half only this document can
    // answer: the first of `state`'s own transitions, in document order, that
    // `event` enables and whose guard holds; for `null`, its first eventless
    // transition whose guard holds. The runtime walks the atomic states and
    // their ancestors and keeps the ordered set.
    override fun firstEnabledTransition(
        state: Test405State,
        event: Test405Event?
    ): EnabledTransition<Test405State, HistoryId>? = when (state) {
        is Test405State.S0 -> when {
            event is Test405Event.Timeout -> transitionS0At0
            else -> null
        }
        is Test405State.S01p -> when {
            event is Test405Event.Event1 -> transitionS01pAt0
            else -> null
        }
        is Test405State.S01p11 -> when {
            event == null -> transitionS01p11At0
            else -> null
        }
        is Test405State.S01p21 -> when {
            event == null -> transitionS01p21At0
            else -> null
        }
        is Test405State.S02 -> when {
            event is Test405Event.Event2 -> transitionS02At0
            event != null -> transitionS02At1
            else -> null
        }
        is Test405State.S03 -> when {
            event is Test405Event.Event3 -> transitionS03At0
            event != null -> transitionS03At1
            else -> null
        }
        is Test405State.S04 -> when {
            event is Test405Event.Event4 -> transitionS04At0
            event != null -> transitionS04At1
            else -> null
        }
        else -> null
    }


    // Entry Actions (W3C SCXML 3.8)
    // SCE-MAP: test405.scxml:6 :: _machine
    override fun onEntry(state: Test405State, isDefaultEntry: Boolean) {
        when (state) {
            is Test405State.Fail -> {
                // SCE-MAP: test405.scxml:69 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test405State.Pass -> {
                // SCE-MAP: test405.scxml:68 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test405State.S0 -> {
                // SCE-MAP: test405.scxml:8 :: s0 :: _state_body


            scheduleSend("__send_0", 1000L, Test405Event.Timeout)
            }
            is Test405State.S01p -> {
                // SCE-MAP: test405.scxml:14 :: s01p :: _state_body
            }
            is Test405State.S01p1 -> {
                // SCE-MAP: test405.scxml:18 :: s01p1 :: _state_body
            }
            is Test405State.S01p11 -> {
                // SCE-MAP: test405.scxml:19 :: s01p11 :: _state_body
            }
            is Test405State.S01p12 -> {
                // SCE-MAP: test405.scxml:29 :: s01p12 :: _state_body
            }
            is Test405State.S01p2 -> {
                // SCE-MAP: test405.scxml:32 :: s01p2 :: _state_body
            }
            is Test405State.S01p21 -> {
                // SCE-MAP: test405.scxml:33 :: s01p21 :: _state_body
            }
            is Test405State.S01p22 -> {
                // SCE-MAP: test405.scxml:43 :: s01p22 :: _state_body
            }
            is Test405State.S02 -> {
                // SCE-MAP: test405.scxml:49 :: s02 :: _state_body
            }
            is Test405State.S03 -> {
                // SCE-MAP: test405.scxml:54 :: s03 :: _state_body
            }
            is Test405State.S04 -> {
                // SCE-MAP: test405.scxml:60 :: s04 :: _state_body
            }
        }
    }

    // Exit Actions (W3C SCXML 3.9)
    // SCE-MAP: test405.scxml:6 :: _machine
    override fun onExit(state: Test405State) {
        when (state) {
            is Test405State.Fail -> {
                // SCE-MAP: test405.scxml:69 :: fail :: _state_body
            }
            is Test405State.Pass -> {
                // SCE-MAP: test405.scxml:68 :: pass :: _state_body
            }
            is Test405State.S0 -> {
                // SCE-MAP: test405.scxml:8 :: s0 :: _state_body
            }
            is Test405State.S01p -> {
                // SCE-MAP: test405.scxml:14 :: s01p :: _state_body
            }
            is Test405State.S01p1 -> {
                // SCE-MAP: test405.scxml:18 :: s01p1 :: _state_body
            }
            is Test405State.S01p11 -> {
                // SCE-MAP: test405.scxml:19 :: s01p11 :: _state_body

            raiseInternal(Test405Event.Event2)
            }
            is Test405State.S01p12 -> {
                // SCE-MAP: test405.scxml:29 :: s01p12 :: _state_body
            }
            is Test405State.S01p2 -> {
                // SCE-MAP: test405.scxml:32 :: s01p2 :: _state_body
            }
            is Test405State.S01p21 -> {
                // SCE-MAP: test405.scxml:33 :: s01p21 :: _state_body

            raiseInternal(Test405Event.Event1)
            }
            is Test405State.S01p22 -> {
                // SCE-MAP: test405.scxml:43 :: s01p22 :: _state_body
            }
            is Test405State.S02 -> {
                // SCE-MAP: test405.scxml:49 :: s02 :: _state_body
            }
            is Test405State.S03 -> {
                // SCE-MAP: test405.scxml:54 :: s03 :: _state_body
            }
            is Test405State.S04 -> {
                // SCE-MAP: test405.scxml:60 :: s04 :: _state_body
            }
        }
    }


    // Transition Content (W3C SCXML 3.13)
    // SCE-MAP: test405.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test405State, transitionIndex: Int) {
        when (source) {
        is Test405State.S01p11 -> when (transitionIndex) {
            0 -> {
                // SCE-MAP: test405.scxml:24 :: s01p11 :: _transition_0

            raiseInternal(Test405Event.Event3)
            }
            else -> {}
        }
        is Test405State.S01p21 -> when (transitionIndex) {
            0 -> {
                // SCE-MAP: test405.scxml:38 :: s01p21 :: _transition_0

            raiseInternal(Test405Event.Event4)
            }
            else -> {}
        }
        else -> {}
        }
    }
}
