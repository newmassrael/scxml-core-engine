...
class Test348StateMachine(
    ...
    override fun isFinalState(state: Test348State): Boolean = when (state) {
        is Test348State.Fail, is Test348State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test348State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test348State, HistoryId>> =
            listOf(StateTarget(Test348State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test348State, HistoryId>(
            Test348State.S0,
            listOf(StateTarget(Test348State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test348State, HistoryId>(
            Test348State.S0,
            listOf(StateTarget(Test348State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun firstEnabledTransition(
        state: Test348State,
        event: Test348Event?
    ): EnabledTransition<Test348State, HistoryId>? = when (state) {
        is Test348State.S0 -> when {
            event is Test348Event.S0Event -> transitionS0At0
    ...
    // SCE-MAP: test348.scxml:3 :: _machine
    override fun onEntry(state: Test348State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test348.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test348State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
