...
// SCE-MAP: test318.scxml:5 :: _machine
...
class Test318StateMachine(
    ...
    override fun isFinalState(state: Test318State): Boolean = when (state) {
        is Test318State.Fail, is Test318State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test318State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test318State, HistoryId>> =
            listOf(StateTarget(Test318State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test318State, HistoryId>(
            Test318State.S0,
            listOf(StateTarget(Test318State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test318State, HistoryId>(
            Test318State.S1,
            listOf(StateTarget(Test318State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test318State, HistoryId>(
            Test318State.S1,
            listOf(StateTarget(Test318State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test318Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test318Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test318Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test318Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test318Event) {
    ...
    override fun firstEnabledTransition(
        state: Test318State,
        event: Test318Event?
    ): EnabledTransition<Test318State, HistoryId>? = when (state) {
        is Test318State.S0 -> when {
            event is Test318Event.Foo -> transitionS0At0
            else -> null
        }
        is Test318State.S1 -> when {
    ...
    // SCE-MAP: test318.scxml:5 :: _machine
    override fun onEntry(state: Test318State, isDefaultEntry: Boolean) {
        when (state) {
            is Test318State.Fail -> {
                // SCE-MAP: test318.scxml:29 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test318State.Pass -> {
                // SCE-MAP: test318.scxml:28 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test318State.S0 -> {
                // SCE-MAP: test318.scxml:10 :: s0 :: _state_body

            raiseInternal(Test318Event.Foo)
            }
            is Test318State.S1 -> {
                // SCE-MAP: test318.scxml:17 :: s1 :: _state_body

            raiseInternal(Test318Event.Bar)


            executeAssign(com.sce.runtime.ScriptSource.lua("Var1", "Var1"), com.sce.runtime.ScriptSource.lua("_event.name", "_event.name"))
            }
        }
    }
    ...
    // SCE-MAP: test318.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test318State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
