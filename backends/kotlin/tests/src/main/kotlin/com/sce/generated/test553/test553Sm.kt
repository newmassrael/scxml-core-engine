...
// SCE-MAP: test553.scxml:6 :: _machine
...
class Test553StateMachine(
    ...
    override fun isFinalState(state: Test553State): Boolean = when (state) {
        is Test553State.Fail, is Test553State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test553State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test553State, HistoryId>> =
            listOf(StateTarget(Test553State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test553State, HistoryId>(
            Test553State.S0,
            listOf(StateTarget(Test553State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test553State, HistoryId>(
            Test553State.S0,
            listOf(StateTarget(Test553State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test553Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test553Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    override fun bindCurrentEvent(event: Test553Event) {
    ...
    override fun firstEnabledTransition(
        state: Test553State,
        event: Test553Event?
    ): EnabledTransition<Test553State, HistoryId>? = when (state) {
        is Test553State.S0 -> when {
            event is Test553Event.Timeout -> transitionS0At0
            event is Test553Event.Event1 -> transitionS0At1
            else -> null
        }
        else -> null
    }
    ...
    // SCE-MAP: test553.scxml:6 :: _machine
    override fun onEntry(state: Test553State, isDefaultEntry: Boolean) {
        when (state) {
            is Test553State.Fail -> {
                // SCE-MAP: test553.scxml:25 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test553State.Pass -> {
                // SCE-MAP: test553.scxml:24 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test553State.S0 -> {
    ...
    // SCE-MAP: test553.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test553State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
