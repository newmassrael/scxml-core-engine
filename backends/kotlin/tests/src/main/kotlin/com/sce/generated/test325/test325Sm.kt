...
class Test325StateMachine(
    ...
    override fun isFinalState(state: Test325State): Boolean = when (state) {
        is Test325State.Fail, is Test325State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test325State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test325State, HistoryId>> =
            listOf(StateTarget(Test325State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test325State, HistoryId>(
            Test325State.S0,
            listOf(StateTarget(Test325State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test325State, HistoryId>(
            Test325State.S0,
            listOf(StateTarget(Test325State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            ...
            raisePlatformError(Test325Event.Error.Execution, "<data id='Var1'> expr failed to evaluate")
        }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test325Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test325Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test325Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test325Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test325Event) {
    ...
    override fun firstEnabledTransition(
        state: Test325State,
        event: Test325Event?
    ): EnabledTransition<Test325State, HistoryId>? = when (state) {
        is Test325State.S0 -> when {
    ...
    // SCE-MAP: test325.scxml:7 :: _machine
    override fun onEntry(state: Test325State, isDefaultEntry: Boolean) {
        when (state) {
            is Test325State.Fail -> {
                // SCE-MAP: test325.scxml:20 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test325State.Pass -> {
                // SCE-MAP: test325.scxml:19 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test325State.S0 -> {
                // SCE-MAP: test325.scxml:13 :: s0 :: _state_body
            }
        }
    }
    ...
    // SCE-MAP: test325.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test325State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
