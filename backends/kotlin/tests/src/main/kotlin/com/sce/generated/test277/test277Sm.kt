...
class Test277StateMachine(
    ...
    override fun isFinalState(state: Test277State): Boolean = when (state) {
        is Test277State.Fail, is Test277State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test277State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test277State, HistoryId>> =
            listOf(StateTarget(Test277State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test277State, HistoryId>(
            Test277State.S0,
            listOf(StateTarget(Test277State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test277State, HistoryId>(
            Test277State.S0,
            listOf(StateTarget(Test277State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test277State, HistoryId>(
            Test277State.S1,
            listOf(StateTarget(Test277State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test277State, HistoryId>(
            Test277State.S1,
            listOf(StateTarget(Test277State.Fail)),
            1,
            hasActions = false,
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            val initResult_Var1 = engine.evaluateExpr(sid, com.sce.runtime.ScriptSource.lua("nil.invalidProperty", "undefined.invalidProperty"))
            engine.setVariable(sid, "Var1", initResult_Var1)
        } catch (e: Exception) {
            raisePlatformError(Test277Event.Error.Execution, "<data id='Var1'> expr failed to evaluate")
        }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test277Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test277Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test277Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test277Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test277Event) {
    ...
    override fun firstEnabledTransition(
        state: Test277State,
        event: Test277Event?
    ): EnabledTransition<Test277State, HistoryId>? = when (state) {
        is Test277State.S0 -> when {
            event is Test277Event.Error.Execution -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test277State.S1 -> when {
    ...
    // SCE-MAP: test277.scxml:6 :: _machine
    override fun onEntry(state: Test277State, isDefaultEntry: Boolean) {
        when (state) {
            is Test277State.Fail -> {
                // SCE-MAP: test277.scxml:30 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test277State.Pass -> {
                // SCE-MAP: test277.scxml:29 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test277State.S0 -> {
    ...
    // SCE-MAP: test277.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test277State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
