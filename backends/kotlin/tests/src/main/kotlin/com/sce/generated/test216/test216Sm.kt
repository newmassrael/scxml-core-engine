...
class Test216StateMachine(
    ...
    override fun isFinalState(state: Test216State): Boolean = when (state) {
        is Test216State.Fail, is Test216State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test216State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test216State, HistoryId>> =
            listOf(StateTarget(Test216State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test216State, HistoryId>(
            Test216State.S0,
            listOf(StateTarget(Test216State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test216State, HistoryId>(
            Test216State.S0,
            listOf(StateTarget(Test216State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test216Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test216Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test216Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test216Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test216Event) {
    ...
    override fun firstEnabledTransition(
        state: Test216State,
        event: Test216Event?
    ): EnabledTransition<Test216State, HistoryId>? = when (state) {
        is Test216State.S0 -> when {
            event is Test216Event.Done.Invoke -> transitionS0At0
    ...
    // SCE-MAP: test216.scxml:7 :: _machine
    override fun onEntry(state: Test216State, isDefaultEntry: Boolean) {
        when (state) {
            is Test216State.Fail -> {
                // SCE-MAP: test216.scxml:24 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test216State.Pass -> {
                // SCE-MAP: test216.scxml:23 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test216State.S0 -> {
                ...
                run {
                    ...
                    deferInvoke(state, generatedInvokeId) {
                        ...
                        try {
                            ...
                            raisePlatformError(Test216Event.Error.Execution, "<invoke> could not start a child")
                        }
                    }
                }
            }
    ...
    // SCE-MAP: test216.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test216State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
