...
class Test446StateMachine(
    ...
    override fun isFinalState(state: Test446State): Boolean = when (state) {
        is Test446State.Fail, is Test446State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test446State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test446State, HistoryId>> =
            listOf(StateTarget(Test446State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test446State, HistoryId>(
            Test446State.S0,
            listOf(StateTarget(Test446State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test446State, HistoryId>(
            Test446State.S0,
            listOf(StateTarget(Test446State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test446State, HistoryId>(
            Test446State.S1,
            listOf(StateTarget(Test446State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test446State, HistoryId>(
            Test446State.S1,
            listOf(StateTarget(Test446State.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test446Event) {
    ...
    override fun firstEnabledTransition(
        state: Test446State,
        event: Test446Event?
    ): EnabledTransition<Test446State, HistoryId>? = when (state) {
        is Test446State.S0 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_isArray(var1)", "var1 instanceof Array")) -> transitionS0At0
            event == null -> transitionS0At1
            else -> null
        }
        is Test446State.S1 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_isArray(var2)", "var2 instanceof Array")) -> transitionS1At0
    ...
    // SCE-MAP: test446.scxml:5 :: _machine
    override fun onEntry(state: Test446State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test446.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test446State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
