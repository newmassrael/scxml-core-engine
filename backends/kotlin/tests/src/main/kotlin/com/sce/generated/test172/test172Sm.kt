...
class Test172StateMachine(
    ...
    override fun isFinalState(state: Test172State): Boolean = when (state) {
        is Test172State.Fail, is Test172State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test172State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test172State, HistoryId>> =
            listOf(StateTarget(Test172State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test172State, HistoryId>(
            Test172State.S0,
            listOf(StateTarget(Test172State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test172State, HistoryId>(
            Test172State.S0,
            listOf(StateTarget(Test172State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test172Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test172Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test172Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test172Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test172Event) {
    ...
    override fun firstEnabledTransition(
        state: Test172State,
        event: Test172Event?
    ): EnabledTransition<Test172State, HistoryId>? = when (state) {
        is Test172State.S0 -> when {
            event is Test172Event.Event2 -> transitionS0At0
    ...
    // SCE-MAP: test172.scxml:4 :: _machine
    override fun onEntry(state: Test172State, isDefaultEntry: Boolean) {
        when (state) {
            is Test172State.Fail -> {
                // SCE-MAP: test172.scxml:21 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test172State.Pass -> {
                // SCE-MAP: test172.scxml:20 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test172State.S0 -> {
    ...
    // SCE-MAP: test172.scxml:4 :: _machine
    override fun executeTransitionContent(source: Test172State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
