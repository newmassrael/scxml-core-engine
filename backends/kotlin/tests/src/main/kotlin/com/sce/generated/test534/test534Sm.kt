...
class Test534StateMachine(
    ...
    override fun isFinalState(state: Test534State): Boolean = when (state) {
        is Test534State.Fail, is Test534State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test534State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test534State, HistoryId>> =
            listOf(StateTarget(Test534State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test534State, HistoryId>(
            Test534State.S0,
            listOf(StateTarget(Test534State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test534State, HistoryId>(
            Test534State.S0,
            listOf(StateTarget(Test534State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test534Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test534Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test534Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test534Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test534Event) {
    ...
    override fun firstEnabledTransition(
        state: Test534State,
        event: Test534Event?
    ): EnabledTransition<Test534State, HistoryId>? = when (state) {
        is Test534State.S0 -> when {
            event is Test534Event.Test && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(_event.data._scxmleventname, \"test\")", "_event.data[\"_scxmleventname\"] == \"test\"")) -> transitionS0At0
    ...
    // SCE-MAP: test534.scxml:3 :: _machine
    override fun onEntry(state: Test534State, isDefaultEntry: Boolean) {
        when (state) {
            ...
            run resolveTarget@{
                ...
                try {
                    ...
                    if (target.startsWith("!")) {
                        raisePlatformError(Test534Event.Error.Execution, "<send> targetexpr produced a target this processor cannot address", "__send_1")
                        return@resolveTarget
                    }
                    // W3C SCXML C.1 (test496): Unreachable target (C++ SendHelper::isUnreachableTarget)
                    if (target.isEmpty() || target == "undefined") {
                        raisePlatformError(Test534Event.Error.Communication, "<send> targetexpr evaluated to nothing, so there is no target to reach")
                        return@resolveTarget
                    }
                    _resolvedTarget = target
                } catch (_: Exception) {
                    raisePlatformError(Test534Event.Error.Execution, "<send> targetexpr failed to evaluate")
                }
            }
            _resolvedTarget?.let { _rt ->
            // W3C SCXML C.2: Validate dynamic target is HTTP URL
            if (!_rt.startsWith("http://") && !_rt.startsWith("https://")) {
                raisePlatformError(Test534Event.Error.Communication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL")
            } else {
    ...
    // SCE-MAP: test534.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test534State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
