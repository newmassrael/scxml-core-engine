...
class Test322StateMachine(
    ...
    override fun isFinalState(state: Test322State): Boolean = when (state) {
        is Test322State.Fail, is Test322State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test322State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test322State, HistoryId>> =
            listOf(StateTarget(Test322State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test322State, HistoryId>(
            Test322State.S0,
            listOf(StateTarget(Test322State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test322State, HistoryId>(
            Test322State.S1,
            listOf(StateTarget(Test322State.S2)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test322State, HistoryId>(
            Test322State.S1,
            listOf(StateTarget(Test322State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS2At0 = EnabledTransition<Test322State, HistoryId>(
            Test322State.S2,
            listOf(StateTarget(Test322State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS2At1 = EnabledTransition<Test322State, HistoryId>(
            Test322State.S2,
            listOf(StateTarget(Test322State.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test322Event) {
    ...
    override fun firstEnabledTransition(
        state: Test322State,
        event: Test322Event?
    ): EnabledTransition<Test322State, HistoryId>? = when (state) {
        is Test322State.S0 -> when {
            event == null -> transitionS0At0
            else -> null
        }
        is Test322State.S1 -> when {
            event is Test322Event.Error.Execution -> transitionS1At0
            event != null -> transitionS1At1
            else -> null
        }
        is Test322State.S2 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var1, _sessionid)", "Var1 == _sessionid")) -> transitionS2At0
    ...
    // SCE-MAP: test322.scxml:6 :: _machine
    override fun onEntry(state: Test322State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test322.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test322State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
