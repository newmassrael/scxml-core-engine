...
class Test527StateMachine(
    ...
    override fun isCompoundState(state: Test527State): Boolean = when (state) {
        is Test527State.S0 -> true
        else -> false
    }

    // W3C SCXML 3.7: Check if state is a <final> element
    override fun isFinalState(state: Test527State): Boolean = when (state) {
        is Test527State.Fail, is Test527State.Pass, is Test527State.S02 -> true
        else -> false
    }

    // §scxml-D-getChildStates: a state's <state>, <parallel> and <final>
    // children, in document order — for a <parallel>, its regions.
    override fun childStatesOf(state: Test527State): List<Test527State> =
        childStates[state] ?: emptyList()

    // W3C SCXML 3.3: a compound state's initial transition target, as written.
    override fun initialTargetsOf(state: Test527State): List<EntryTarget<Test527State, HistoryId>> =
        initialTargets[state] ?: emptyList()

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test527State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val childStates: Map<Test527State, List<Test527State>> = mapOf(
            Test527State.S0 to listOf(Test527State.S01, Test527State.S02),
        )

        val initialTargets: Map<Test527State, List<EntryTarget<Test527State, HistoryId>>> = mapOf(
            Test527State.S0 to listOf(StateTarget(Test527State.S01)),
        )

        val documentInitialTargetList: List<EntryTarget<Test527State, HistoryId>> =
            listOf(StateTarget(Test527State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test527State, HistoryId>(
            Test527State.S0,
            listOf(StateTarget(Test527State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test527State, HistoryId>(
            Test527State.S0,
            listOf(StateTarget(Test527State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS01At0 = EnabledTransition<Test527State, HistoryId>(
            Test527State.S01,
            listOf(StateTarget(Test527State.S02)),
            0,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test527Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test527Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test527Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test527Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test527Event) {
    ...
    override fun firstEnabledTransition(
        state: Test527State,
        event: Test527Event?
    ): EnabledTransition<Test527State, HistoryId>? = when (state) {
        is Test527State.S0 -> when {
            event is Test527Event.Done.State.S0 && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(_event.data, \"foo\")", "_event.data == 'foo'")) -> transitionS0At0
            event is Test527Event.Done.State.S0 -> transitionS0At1
            else -> null
        }
        is Test527State.S01 -> when {
    ...
    // SCE-MAP: test527.scxml:4 :: _machine
    override fun onEntry(state: Test527State, isDefaultEntry: Boolean) {
        when (state) {
            ...
            is Test527State.S02 -> {
                ...
                run {
                    ...
                    try {
                        ...
                        raisePlatformError(Test527Event.Error.Execution, "<donedata> <content expr> failed to evaluate")
                    }
    ...
    // SCE-MAP: test527.scxml:4 :: _machine
    override fun executeTransitionContent(source: Test527State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
