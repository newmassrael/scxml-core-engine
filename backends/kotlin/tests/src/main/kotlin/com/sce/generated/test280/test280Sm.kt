...
class Test280StateMachine(
    ...
    override fun isFinalState(state: Test280State): Boolean = when (state) {
        is Test280State.Fail, is Test280State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test280State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test280State, HistoryId>> =
            listOf(StateTarget(Test280State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test280State, HistoryId>(
            Test280State.S0,
            listOf(StateTarget(Test280State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test280State, HistoryId>(
            Test280State.S0,
            listOf(StateTarget(Test280State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test280State, HistoryId>(
            Test280State.S1,
            listOf(StateTarget(Test280State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test280State, HistoryId>(
            Test280State.S1,
            listOf(StateTarget(Test280State.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test280Event) {
    ...
    override fun firstEnabledTransition(
        state: Test280State,
        event: Test280Event?
    ): EnabledTransition<Test280State, HistoryId>? = when (state) {
        is Test280State.S0 -> when {
            event == null -> transitionS0At0
            else -> null
        }
        is Test280State.S1 -> when {
    ...
    // SCE-MAP: test280.scxml:8 :: _machine
    override fun onEntry(state: Test280State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test280.scxml:8 :: _machine
    override fun executeTransitionContent(source: Test280State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
