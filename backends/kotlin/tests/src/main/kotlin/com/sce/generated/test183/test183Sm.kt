...
class Test183StateMachine(
    ...
    override fun isFinalState(state: Test183State): Boolean = when (state) {
        is Test183State.Fail, is Test183State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test183State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test183State, HistoryId>> =
            listOf(StateTarget(Test183State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test183State, HistoryId>(
            Test183State.S0,
            listOf(StateTarget(Test183State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test183State, HistoryId>(
            Test183State.S0,
            listOf(StateTarget(Test183State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test183Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test183Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun storeIdInLocation(location: com.sce.runtime.ScriptSource, id: String, element: String): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test183Event.Error.Execution, "$element idlocation could not take the id")
            false
        }
    }
    ...
    override fun bindCurrentEvent(event: Test183Event) {
    ...
    override fun firstEnabledTransition(
        state: Test183State,
        event: Test183Event?
    ): EnabledTransition<Test183State, HistoryId>? = when (state) {
        is Test183State.S0 -> when {
    ...
    // SCE-MAP: test183.scxml:5 :: _machine
    override fun onEntry(state: Test183State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test183.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test183State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
