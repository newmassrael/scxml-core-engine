...
class Test210StateMachine(
    ...
    override fun isFinalState(state: Test210State): Boolean = when (state) {
        is Test210State.Fail, is Test210State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test210State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test210State, HistoryId>> =
            listOf(StateTarget(Test210State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test210State, HistoryId>(
            Test210State.S0,
            listOf(StateTarget(Test210State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test210State, HistoryId>(
            Test210State.S0,
            listOf(StateTarget(Test210State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun bindCurrentEvent(event: Test210Event) {
    ...
    override fun firstEnabledTransition(
        state: Test210State,
        event: Test210Event?
    ): EnabledTransition<Test210State, HistoryId>? = when (state) {
        is Test210State.S0 -> when {
            event is Test210Event.Event2 -> transitionS0At0
    ...
    // SCE-MAP: test210.scxml:6 :: _machine
    override fun onEntry(state: Test210State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test210.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test210State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
