...
class Test319StateMachine(
    ...
    override fun isFinalState(state: Test319State): Boolean = when (state) {
        is Test319State.Fail, is Test319State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test319State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test319State, HistoryId>> =
            listOf(StateTarget(Test319State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test319State, HistoryId>(
            Test319State.S0,
            listOf(StateTarget(Test319State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test319State, HistoryId>(
            Test319State.S0,
            listOf(StateTarget(Test319State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test319Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test319Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test319Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test319Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test319Event) {
    ...
    override fun firstEnabledTransition(
        state: Test319State,
        event: Test319Event?
    ): EnabledTransition<Test319State, HistoryId>? = when (state) {
        is Test319State.S0 -> when {
            event is Test319Event.Unbound -> transitionS0At0
            event is Test319Event.Bound -> transitionS0At1
            else -> null
        }
        else -> null
    }
    ...
    // SCE-MAP: test319.scxml:5 :: _machine
    override fun onEntry(state: Test319State, isDefaultEntry: Boolean) {
        when (state) {
            is Test319State.Fail -> {
                // SCE-MAP: test319.scxml:23 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test319State.Pass -> {
                // SCE-MAP: test319.scxml:22 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test319State.S0 -> {
                // SCE-MAP: test319.scxml:8 :: s0 :: _state_body
    ...
    // SCE-MAP: test319.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test319State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
