...
class Test215StateMachine(
    ...
    override fun isFinalState(state: Test215State): Boolean = when (state) {
        is Test215State.Fail, is Test215State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test215State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test215State, HistoryId>> =
            listOf(StateTarget(Test215State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test215State, HistoryId>(
            Test215State.S0,
            listOf(StateTarget(Test215State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test215State, HistoryId>(
            Test215State.S0,
            listOf(StateTarget(Test215State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test215Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test215Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test215Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test215Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test215Event) {
    ...
    override fun firstEnabledTransition(
        state: Test215State,
        event: Test215Event?
    ): EnabledTransition<Test215State, HistoryId>? = when (state) {
        is Test215State.S0 -> when {
            event is Test215Event.Done.Invoke -> transitionS0At0
    ...
    // SCE-MAP: test215.scxml:7 :: _machine
    override fun onEntry(state: Test215State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test215.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test215State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
