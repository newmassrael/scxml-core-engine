...
class Test226StateMachine(
    ...
    override fun isFinalState(state: Test226State): Boolean = when (state) {
        is Test226State.Fail, is Test226State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test226State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test226State, HistoryId>> =
            listOf(StateTarget(Test226State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test226State, HistoryId>(
            Test226State.S0,
            listOf(StateTarget(Test226State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test226State, HistoryId>(
            Test226State.S0,
            listOf(StateTarget(Test226State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun resolveState(stateId: String): Test226State? = when (stateId) {
        "fail" -> Test226State.Fail
        "pass" -> Test226State.Pass
        "s0" -> Test226State.S0
        else -> null
    }

    // W3C SCXML: Get state ID string from State object
    override fun stateIdOf(state: Test226State): String = when (state) {
    ...
    override fun documentOrderOf(state: Test226State): Int = when (state) {
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test226Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test226Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test226Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test226Event.Error.Execution, "<script> failed to execute")
        }
    }

    // W3C SCXML 5.10: Set _event before event processing
    private fun setCurrentEventInScriptEngine(event: Test226Event) {
        ensureScriptEngine()
        val engine = scriptEngine ?: error("scriptEngine is required (codegen invariant: needs_script_engine == true)")
        val sid = scriptSessionId ?: error("scriptSessionId must be initialized after ensureScriptEngine() (codegen invariant)")
    ...
    override fun bindCurrentEvent(event: Test226Event) {
    ...
    override fun firstEnabledTransition(
        state: Test226State,
        event: Test226Event?
    ): EnabledTransition<Test226State, HistoryId>? = when (state) {
        is Test226State.S0 -> when {
            event is Test226Event.VarBound -> transitionS0At0
    ...
    // SCE-MAP: test226.scxml:7 :: _machine
    override fun onEntry(state: Test226State, isDefaultEntry: Boolean) {
        when (state) {
            is Test226State.Fail -> {
                // SCE-MAP: test226.scxml:25 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test226State.Pass -> {
                // SCE-MAP: test226.scxml:24 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test226State.S0 -> {
            ...
            scheduleSend("__send_0", 3000L, Test226Event.Timeout)
                ...
                run {
                    ...
                    try {
                        ...
                        raisePlatformError(Test226Event.Error.Execution, "<invoke> <param name='Var1'> expr failed to evaluate")
                    }
                    deferInvoke(state, generatedInvokeId) {
                        val childSM = Test226sub1StateMachine(scriptEngine ?: error("scriptEngine is required for invoke (codegen invariant: parent needs_script_engine == true)"))
                        setInvokeParams(childSM, invokeParams)
                        // W3C SCXML 6.4: Static ID for done.invoke/cancel, generated ID for child events
                        startInvoke("_invoke_0", childSM, false, Test226Event.Done.Invoke, "", generatedInvokeId)
                    }
                }
            }
        }
    ...
    // SCE-MAP: test226.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test226State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
