...
class Test487StateMachine(
    ...
    override fun isFinalState(state: Test487State): Boolean = when (state) {
        is Test487State.Fail, is Test487State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test487State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test487State, HistoryId>> =
            listOf(StateTarget(Test487State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test487State, HistoryId>(
            Test487State.S0,
            listOf(StateTarget(Test487State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test487State, HistoryId>(
            Test487State.S0,
            listOf(StateTarget(Test487State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun bindCurrentEvent(event: Test487Event) {
    ...
    override fun firstEnabledTransition(
        state: Test487State,
        event: Test487Event?
    ): EnabledTransition<Test487State, HistoryId>? = when (state) {
        is Test487State.S0 -> when {
            event is Test487Event.Error.Execution -> transitionS0At0
    ...
    // SCE-MAP: test487.scxml:5 :: _machine
    override fun onEntry(state: Test487State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test487.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test487State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
