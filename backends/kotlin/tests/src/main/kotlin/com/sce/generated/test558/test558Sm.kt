...
// SCE-MAP: test558.scxml:5 :: _machine
...
class Test558StateMachine(
    ...
    override fun isFinalState(state: Test558State): Boolean = when (state) {
        is Test558State.Fail, is Test558State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test558State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test558State, HistoryId>> =
            listOf(StateTarget(Test558State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test558State, HistoryId>(
            Test558State.S0,
            listOf(StateTarget(Test558State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test558State, HistoryId>(
            Test558State.S0,
            listOf(StateTarget(Test558State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test558State, HistoryId>(
            Test558State.S1,
            listOf(StateTarget(Test558State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test558State, HistoryId>(
            Test558State.S1,
            listOf(StateTarget(Test558State.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test558Event) {
    ...
    override fun firstEnabledTransition(
        state: Test558State,
        event: Test558Event?
    ): EnabledTransition<Test558State, HistoryId>? = when (state) {
        is Test558State.S0 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(var1, \"this is a string\")", "var1 == 'this is a string'")) -> transitionS0At0
            event == null -> transitionS0At1
            else -> null
        }
        is Test558State.S1 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(var2, \"this is a string\")", "var2 == 'this is a string'")) -> transitionS1At0
    ...
    // SCE-MAP: test558.scxml:5 :: _machine
    override fun onEntry(state: Test558State, isDefaultEntry: Boolean) {
        when (state) {
            is Test558State.Fail -> {
                // SCE-MAP: test558.scxml:25 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test558State.Pass -> {
                // SCE-MAP: test558.scxml:24 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test558State.S0 -> {
                // SCE-MAP: test558.scxml:14 :: s0 :: _state_body
            }
            is Test558State.S1 -> {
                // SCE-MAP: test558.scxml:19 :: s1 :: _state_body
            }
        }
    }
    ...
    // SCE-MAP: test558.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test558State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
