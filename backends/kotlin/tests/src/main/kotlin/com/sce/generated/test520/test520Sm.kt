...
class Test520StateMachine(
    ...
    override fun isFinalState(state: Test520State): Boolean = when (state) {
        is Test520State.Fail, is Test520State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test520State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test520State, HistoryId>> =
            listOf(StateTarget(Test520State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test520State, HistoryId>(
            Test520State.S0,
            listOf(StateTarget(Test520State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test520State, HistoryId>(
            Test520State.S0,
            listOf(StateTarget(Test520State.Pass)),
            1,
            hasActions = false,
        ...
        val transitionS0At2 = EnabledTransition<Test520State, HistoryId>(
            Test520State.S0,
            listOf(StateTarget(Test520State.Fail)),
            2,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun resolveEventByName(name: String): Test520Event? = when (name) {
        "" -> Test520Event.Empty
        "error.communication" -> Test520Event.Error.Communication
    ...
    override fun eventNameOf(event: Test520Event): String? = when (event) {
        ...
        is Test520Event.Error.Communication -> "error.communication"
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test520Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test520Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun setCurrentEventInScriptEngine(event: Test520Event) {
        ensureScriptEngine()
        val engine = scriptEngine ?: error("scriptEngine is required (codegen invariant: needs_script_engine == true)")
        val sid = scriptSessionId ?: error("scriptSessionId must be initialized after ensureScriptEngine() (codegen invariant)")
    ...
    override fun bindCurrentEvent(event: Test520Event) {
    ...
    override fun firstEnabledTransition(
        state: Test520State,
        event: Test520Event?
    ): EnabledTransition<Test520State, HistoryId>? = when (state) {
        is Test520State.S0 -> when {
            event is Test520Event.HTTP.POST -> transitionS0At0
            event is Test520Event.HTTP.POST -> transitionS0At1
    ...
    // SCE-MAP: test520.scxml:3 :: _machine
    override fun onEntry(state: Test520State, isDefaultEntry: Boolean) {
        when (state) {
            is Test520State.Fail -> {
                // SCE-MAP: test520.scxml:22 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test520State.Pass -> {
                // SCE-MAP: test520.scxml:21 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test520State.S0 -> {
    ...
    // SCE-MAP: test520.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test520State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
