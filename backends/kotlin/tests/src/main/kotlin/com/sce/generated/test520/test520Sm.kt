...
class Test520StateMachine(
    ...
    private fun safeEvaluateGuard(guardExpr: String): Boolean {
        ...
        return try {
            ...
            raiseInternal(Test520Event.Error.Execution)
            false
        }
    }
            ...
            run resolveTarget@{
                ...
                try {
                    ...
                    if (target.startsWith("!")) {
                        raiseInternal(Test520Event.Error.Execution, EventMetadata(type = "platform", sendId = "__send_1"))
                        return@resolveTarget
                    }
                    // W3C SCXML C.1 (test496): Unreachable target (C++ SendHelper::isUnreachableTarget)
                    if (target.isEmpty() || target == "undefined") {
                        raiseInternal(Test520Event.Error.Communication, EventMetadata.platform())
                        return@resolveTarget
                    }
                    _resolvedTarget = target
                } catch (_: Exception) {
                    raiseInternal(Test520Event.Error.Execution, EventMetadata.platform())
                }
            }
