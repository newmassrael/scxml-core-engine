...
class Test241SceSynthInvokeInvoke0StateMachine(
    ...
    override fun isFinalState(state: Test241SceSynthInvokeInvoke0State): Boolean = when (state) {
        is Test241SceSynthInvokeInvoke0State.SubFinal1 -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test241SceSynthInvokeInvoke0State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test241SceSynthInvokeInvoke0State, HistoryId>> =
            listOf(StateTarget(Test241SceSynthInvokeInvoke0State.Sub01))

        // W3C SCXML 3.13: sub01's transition 0, as the microstep reads it.
        val transitionSub01At0 = EnabledTransition<Test241SceSynthInvokeInvoke0State, HistoryId>(
            Test241SceSynthInvokeInvoke0State.Sub01,
            listOf(StateTarget(Test241SceSynthInvokeInvoke0State.SubFinal1)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionSub01At1 = EnabledTransition<Test241SceSynthInvokeInvoke0State, HistoryId>(
            Test241SceSynthInvokeInvoke0State.Sub01,
            listOf(StateTarget(Test241SceSynthInvokeInvoke0State.SubFinal1)),
            1,
            hasActions = true,
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            ...
            raisePlatformError(Test241SceSynthInvokeInvoke0Event.Error.Execution, "<data id='Var1'> expr failed to evaluate")
        }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test241SceSynthInvokeInvoke0Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test241SceSynthInvokeInvoke0Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    override fun bindCurrentEvent(event: Test241SceSynthInvokeInvoke0Event) {
    ...
    ): EnabledTransition<Test241SceSynthInvokeInvoke0State, HistoryId>? = when (state) {
    ...
    // SCE-MAP: test241__sce_synth_invoke__invoke_0.scxml:3 :: _machine
    override fun onEntry(state: Test241SceSynthInvokeInvoke0State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test241__sce_synth_invoke__invoke_0.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test241SceSynthInvokeInvoke0State, transitionIndex: Int) {
        when (source) {
        is Test241SceSynthInvokeInvoke0State.Sub01 -> when (transitionIndex) {
            0 -> {
