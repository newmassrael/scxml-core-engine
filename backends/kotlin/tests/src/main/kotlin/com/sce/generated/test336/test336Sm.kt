...
class Test336StateMachine(
    ...
    override fun isFinalState(state: Test336State): Boolean = when (state) {
        is Test336State.Fail, is Test336State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test336State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test336State, HistoryId>> =
            listOf(StateTarget(Test336State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test336State, HistoryId>(
            Test336State.S0,
            listOf(StateTarget(Test336State.S1)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test336State, HistoryId>(
            Test336State.S0,
            listOf(StateTarget(Test336State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test336State, HistoryId>(
            Test336State.S1,
            listOf(StateTarget(Test336State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test336State, HistoryId>(
            Test336State.S1,
            listOf(StateTarget(Test336State.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test336Event) {
    ...
    override fun firstEnabledTransition(
        state: Test336State,
        event: Test336Event?
    ): EnabledTransition<Test336State, HistoryId>? = when (state) {
        is Test336State.S0 -> when {
            event is Test336Event.Foo -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test336State.S1 -> when {
            event is Test336Event.Bar -> transitionS1At0
    ...
    // SCE-MAP: test336.scxml:6 :: _machine
    override fun onEntry(state: Test336State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test336.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test336State, transitionIndex: Int) {
        when (source) {
        is Test336State.S0 -> when (transitionIndex) {
            0 -> {
