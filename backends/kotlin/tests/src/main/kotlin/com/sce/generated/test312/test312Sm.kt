...
class Test312StateMachine(
    ...
    override fun isFinalState(state: Test312State): Boolean = when (state) {
        is Test312State.Fail, is Test312State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test312State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test312State, HistoryId>> =
            listOf(StateTarget(Test312State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test312State, HistoryId>(
            Test312State.S0,
            listOf(StateTarget(Test312State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test312State, HistoryId>(
            Test312State.S0,
            listOf(StateTarget(Test312State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            ...
            raisePlatformError(Test312Event.Error.Execution, "<data id='Var1'> expr failed to evaluate")
        }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test312Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test312Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test312Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test312Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test312Event) {
    ...
    override fun firstEnabledTransition(
        state: Test312State,
        event: Test312Event?
    ): EnabledTransition<Test312State, HistoryId>? = when (state) {
        is Test312State.S0 -> when {
            event is Test312Event.Error.Execution -> transitionS0At0
    ...
    // SCE-MAP: test312.scxml:5 :: _machine
    override fun onEntry(state: Test312State, isDefaultEntry: Boolean) {
        when (state) {
            is Test312State.Fail -> {
                // SCE-MAP: test312.scxml:21 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test312State.Pass -> {
                // SCE-MAP: test312.scxml:20 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test312State.S0 -> {
                // SCE-MAP: test312.scxml:11 :: s0 :: _state_body
    ...
    // SCE-MAP: test312.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test312State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
