...
// SCE-MAP: test233.scxml:7 :: _machine
...
class Test233StateMachine(
    ...
    override fun isFinalState(state: Test233State): Boolean = when (state) {
        is Test233State.Fail, is Test233State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test233State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test233State, HistoryId>> =
            listOf(StateTarget(Test233State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test233State, HistoryId>(
            Test233State.S0,
            listOf(StateTarget(Test233State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test233State, HistoryId>(
            Test233State.S0,
            listOf(StateTarget(Test233State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test233Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test233Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test233Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test233Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test233Event) {
    ...
    override fun firstEnabledTransition(
        state: Test233State,
        event: Test233Event?
    ): EnabledTransition<Test233State, HistoryId>? = when (state) {
        is Test233State.S0 -> when {
            event is Test233Event.ChildToParent && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var1, 2)", "Var1 == 2")) -> transitionS0At0
    ...
    // SCE-MAP: test233.scxml:7 :: _machine
    override fun onEntry(state: Test233State, isDefaultEntry: Boolean) {
        when (state) {
            is Test233State.Fail -> {
                // SCE-MAP: test233.scxml:38 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test233State.Pass -> {
                // SCE-MAP: test233.scxml:37 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test233State.S0 -> {
                // SCE-MAP: test233.scxml:12 :: s0 :: _state_body
    ...
    // SCE-MAP: test233.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test233State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
