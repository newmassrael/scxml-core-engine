...
class Test324StateMachine(
    ...
    override fun isFinalState(state: Test324State): Boolean = when (state) {
        is Test324State.Fail, is Test324State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test324State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test324State, HistoryId>> =
            listOf(StateTarget(Test324State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test324State, HistoryId>(
            Test324State.S0,
            listOf(StateTarget(Test324State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test324State, HistoryId>(
            Test324State.S0,
            listOf(StateTarget(Test324State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test324State, HistoryId>(
            Test324State.S1,
            listOf(StateTarget(Test324State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test324State, HistoryId>(
            Test324State.S1,
            listOf(StateTarget(Test324State.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test324Event) {
    ...
    override fun firstEnabledTransition(
        state: Test324State,
        event: Test324Event?
    ): EnabledTransition<Test324State, HistoryId>? = when (state) {
        is Test324State.S0 -> when {
            event == null -> transitionS0At0
            else -> null
        }
        is Test324State.S1 -> when {
    ...
    // SCE-MAP: test324.scxml:5 :: _machine
    override fun onEntry(state: Test324State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test324.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test324State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
