...
// SCE-MAP: test554.scxml:7 :: _machine
...
class Test554StateMachine(
    ...
    override fun isFinalState(state: Test554State): Boolean = when (state) {
        is Test554State.Fail, is Test554State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test554State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test554State, HistoryId>> =
            listOf(StateTarget(Test554State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test554State, HistoryId>(
            Test554State.S0,
            listOf(StateTarget(Test554State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test554State, HistoryId>(
            Test554State.S0,
            listOf(StateTarget(Test554State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test554Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test554Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test554Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test554Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test554Event) {
    ...
    override fun firstEnabledTransition(
        state: Test554State,
        event: Test554Event?
    ): EnabledTransition<Test554State, HistoryId>? = when (state) {
        is Test554State.S0 -> when {
            event is Test554Event.Timer -> transitionS0At0
            event is Test554Event.Done.Invoke -> transitionS0At1
            else -> null
        }
        else -> null
    }
    ...
    // SCE-MAP: test554.scxml:7 :: _machine
    override fun onEntry(state: Test554State, isDefaultEntry: Boolean) {
        when (state) {
            is Test554State.Fail -> {
                // SCE-MAP: test554.scxml:28 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test554State.Pass -> {
                // SCE-MAP: test554.scxml:27 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test554State.S0 -> {
                // SCE-MAP: test554.scxml:10 :: s0 :: _state_body
            ...
            scheduleSend("__send_0", 1000L, Test554Event.Timer)
                ...
                run {
                    ...
                    if (!engineInv.hasVariable(sidInv, "__undefined_variable_for_error__")) {
                        raisePlatformError(Test554Event.Error.Execution, "<invoke> namelist names '__undefined_variable_for_error__', which the parent does not declare")
    ...
    // SCE-MAP: test554.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test554State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
