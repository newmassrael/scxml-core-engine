...
class Test562StateMachine(
    ...
    override fun isFinalState(state: Test562State): Boolean = when (state) {
        is Test562State.Fail, is Test562State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test562State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test562State, HistoryId>> =
            listOf(StateTarget(Test562State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test562State, HistoryId>(
            Test562State.S0,
            listOf(StateTarget(Test562State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test562State, HistoryId>(
            Test562State.S0,
            listOf(StateTarget(Test562State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun resolveState(stateId: String): Test562State? = when (stateId) {
        "fail" -> Test562State.Fail
        "pass" -> Test562State.Pass
        "s0" -> Test562State.S0
        else -> null
    }

    // W3C SCXML: Get state ID string from State object
    override fun stateIdOf(state: Test562State): String = when (state) {
        is Test562State.Fail -> "fail"
        is Test562State.Pass -> "pass"
        is Test562State.S0 -> "s0"
    }

    // W3C SCXML 3.13: Document order — entry order, and in reverse exit order
    override fun documentOrderOf(state: Test562State): Int = when (state) {
        is Test562State.Fail -> 2
        is Test562State.Pass -> 1
        is Test562State.S0 -> 0
    }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test562Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test562Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test562Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test562Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test562Event) {
    ...
    override fun firstEnabledTransition(
        state: Test562State,
        event: Test562Event?
    ): EnabledTransition<Test562State, HistoryId>? = when (state) {
        is Test562State.S0 -> when {
            event is Test562Event.Foo && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(_event.data, \"this is a string\")", "_event.data == 'this is a string'")) -> transitionS0At0
    ...
    // SCE-MAP: test562.scxml:5 :: _machine
    override fun onEntry(state: Test562State, isDefaultEntry: Boolean) {
        when (state) {
            is Test562State.Fail -> {
                // SCE-MAP: test562.scxml:23 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test562State.Pass -> {
                // SCE-MAP: test562.scxml:22 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test562State.S0 -> {
                // SCE-MAP: test562.scxml:8 :: s0 :: _state_body
    ...
    // SCE-MAP: test562.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test562State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
