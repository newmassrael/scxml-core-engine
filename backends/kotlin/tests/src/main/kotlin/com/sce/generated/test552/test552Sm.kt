...
class Test552StateMachine(
    ...
    override fun isFinalState(state: Test552State): Boolean = when (state) {
        is Test552State.Fail, is Test552State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test552State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test552State, HistoryId>> =
            listOf(StateTarget(Test552State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test552State, HistoryId>(
            Test552State.S0,
            listOf(StateTarget(Test552State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test552State, HistoryId>(
            Test552State.S0,
            listOf(StateTarget(Test552State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun bindCurrentEvent(event: Test552Event) {
    ...
    override fun firstEnabledTransition(
        state: Test552State,
        event: Test552Event?
    ): EnabledTransition<Test552State, HistoryId>? = when (state) {
        is Test552State.S0 -> when {
    ...
    // SCE-MAP: test552.scxml:6 :: _machine
    override fun onEntry(state: Test552State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test552.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test552State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
