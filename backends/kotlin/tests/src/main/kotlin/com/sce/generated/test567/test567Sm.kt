...
// SCE-MAP: test567.scxml:4 :: _machine
...
class Test567StateMachine(
    ...
    override fun isFinalState(state: Test567State): Boolean = when (state) {
        is Test567State.Fail, is Test567State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test567State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test567State, HistoryId>> =
            listOf(StateTarget(Test567State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test567State, HistoryId>(
            Test567State.S0,
            listOf(StateTarget(Test567State.S1)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test567State, HistoryId>(
            Test567State.S0,
            listOf(StateTarget(Test567State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test567State, HistoryId>(
            Test567State.S1,
            listOf(StateTarget(Test567State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test567State, HistoryId>(
            Test567State.S1,
            listOf(StateTarget(Test567State.Fail)),
            1,
            hasActions = false,
    ...
    override fun resolveState(stateId: String): Test567State? = when (stateId) {
        "fail" -> Test567State.Fail
        "pass" -> Test567State.Pass
        "s0" -> Test567State.S0
        "s1" -> Test567State.S1
        else -> null
    }
    ...
    override fun stateIdOf(state: Test567State): String = when (state) {
        is Test567State.Fail -> "fail"
        is Test567State.Pass -> "pass"
        is Test567State.S0 -> "s0"
        is Test567State.S1 -> "s1"
    }

    // W3C SCXML 3.13: Document order — entry order, and in reverse exit order
    override fun documentOrderOf(state: Test567State): Int = when (state) {
        is Test567State.Fail -> 3
        is Test567State.Pass -> 2
        is Test567State.S0 -> 0
        is Test567State.S1 -> 1
    }
    ...
    override fun bindCurrentEvent(event: Test567Event) {
    ...
    override fun firstEnabledTransition(
        state: Test567State,
        event: Test567Event?
    ): EnabledTransition<Test567State, HistoryId>? = when (state) {
        is Test567State.S0 -> when {
            event is Test567Event.Test -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test567State.S1 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var1, 2)", "Var1 == 2")) -> transitionS1At0
    ...
    // SCE-MAP: test567.scxml:4 :: _machine
    override fun onEntry(state: Test567State, isDefaultEntry: Boolean) {
        when (state) {
            is Test567State.Fail -> {
                // SCE-MAP: test567.scxml:31 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test567State.Pass -> {
                // SCE-MAP: test567.scxml:30 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test567State.S0 -> {
    ...
    // SCE-MAP: test567.scxml:4 :: _machine
    override fun executeTransitionContent(source: Test567State, transitionIndex: Int) {
        when (source) {
        is Test567State.S0 -> when (transitionIndex) {
            0 -> {
