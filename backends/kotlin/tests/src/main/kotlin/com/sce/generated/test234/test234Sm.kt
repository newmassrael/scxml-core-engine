...
class Test234StateMachine(
    ...
    override fun isFinalState(state: Test234State): Boolean = when (state) {
        is Test234State.Fail, is Test234State.Pass -> true
        else -> false
    }

    // §scxml-D-getChildStates: a state's <state>, <parallel> and <final>
    // children, in document order — for a <parallel>, its regions.
    override fun childStatesOf(state: Test234State): List<Test234State> =
        childStates[state] ?: emptyList()

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test234State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val childStates: Map<Test234State, List<Test234State>> = mapOf(
            Test234State.P0 to listOf(Test234State.P01, Test234State.P02),
        )

        val documentInitialTargetList: List<EntryTarget<Test234State, HistoryId>> =
            listOf(StateTarget(Test234State.P0))

        // W3C SCXML 3.13: p0's transition 0, as the microstep reads it.
        val transitionP0At0 = EnabledTransition<Test234State, HistoryId>(
            Test234State.P0,
            listOf(StateTarget(Test234State.Fail)),
            0,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: p01's transition 0, as the microstep reads it.
        val transitionP01At0 = EnabledTransition<Test234State, HistoryId>(
            Test234State.P01,
            listOf(StateTarget(Test234State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: p01's transition 1, as the microstep reads it.
        val transitionP01At1 = EnabledTransition<Test234State, HistoryId>(
            Test234State.P01,
            listOf(StateTarget(Test234State.Fail)),
            1,
            hasActions = false,
        ...
        val transitionS1At0 = EnabledTransition<Test234State, HistoryId>(
            Test234State.S1,
            listOf(StateTarget(Test234State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test234State, HistoryId>(
            Test234State.S1,
            listOf(StateTarget(Test234State.Fail)),
            1,
            hasActions = false,
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test234Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test234Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test234Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test234Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test234Event) {
    ...
    override fun firstEnabledTransition(
        state: Test234State,
        event: Test234Event?
    ): EnabledTransition<Test234State, HistoryId>? = when (state) {
        is Test234State.P0 -> when {
            event is Test234Event.Timeout -> transitionP0At0
            else -> null
        }
        is Test234State.P01 -> when {
            event is Test234Event.ChildToParent && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var1, 2)", "Var1 == 2")) -> transitionP01At0
            event is Test234Event.ChildToParent -> transitionP01At1
            else -> null
        }
        is Test234State.S1 -> when {
    ...
    // SCE-MAP: test234.scxml:8 :: _machine
    override fun onEntry(state: Test234State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test234.scxml:8 :: _machine
    override fun executeTransitionContent(source: Test234State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
