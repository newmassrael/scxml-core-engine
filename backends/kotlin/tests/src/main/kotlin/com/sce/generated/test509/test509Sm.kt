...
class Test509StateMachine(
    ...
    override fun isFinalState(state: Test509State): Boolean = when (state) {
        is Test509State.Fail, is Test509State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test509State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test509State, HistoryId>> =
            listOf(StateTarget(Test509State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test509State, HistoryId>(
            Test509State.S0,
            listOf(StateTarget(Test509State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test509State, HistoryId>(
            Test509State.S0,
            listOf(StateTarget(Test509State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun resolveEventByName(name: String): Test509Event? = when (name) {
        "error.communication" -> Test509Event.Error.Communication
    ...
    override fun eventNameOf(event: Test509Event): String? = when (event) {
        is Test509Event.Error.Communication -> "error.communication"
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test509Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test509Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test509Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test509Event.Error.Execution, "<script> failed to execute")
        }
    }

    // W3C SCXML 5.10: Set _event before event processing
    private fun setCurrentEventInScriptEngine(event: Test509Event) {
        ensureScriptEngine()
        val engine = scriptEngine ?: error("scriptEngine is required (codegen invariant: needs_script_engine == true)")
        val sid = scriptSessionId ?: error("scriptSessionId must be initialized after ensureScriptEngine() (codegen invariant)")
    ...
    override fun bindCurrentEvent(event: Test509Event) {
    ...
    override fun firstEnabledTransition(
        state: Test509State,
        event: Test509Event?
    ): EnabledTransition<Test509State, HistoryId>? = when (state) {
        is Test509State.S0 -> when {
            event is Test509Event.Test -> transitionS0At0
    ...
    // SCE-MAP: test509.scxml:6 :: _machine
    override fun onEntry(state: Test509State, isDefaultEntry: Boolean) {
        when (state) {
            ...
            is Test509State.Pass -> {
                // SCE-MAP: test509.scxml:18 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test509State.S0 -> {
                // SCE-MAP: test509.scxml:8 :: s0 :: _state_body
            ...
            run resolveTarget@{
                ...
                try {
                    ...
                    if (target.startsWith("!")) {
                        raisePlatformError(Test509Event.Error.Execution, "<send> targetexpr produced a target this processor cannot address", "__send_1")
                        return@resolveTarget
                    }
                    // W3C SCXML C.1 (test496): Unreachable target (C++ SendHelper::isUnreachableTarget)
                    if (target.isEmpty() || target == "undefined") {
                        raisePlatformError(Test509Event.Error.Communication, "<send> targetexpr evaluated to nothing, so there is no target to reach")
                        return@resolveTarget
                    }
                    _resolvedTarget = target
                } catch (_: Exception) {
                    raisePlatformError(Test509Event.Error.Execution, "<send> targetexpr failed to evaluate")
                }
            }
            _resolvedTarget?.let { _rt ->
            // W3C SCXML C.2: Validate dynamic target is HTTP URL
            if (!_rt.startsWith("http://") && !_rt.startsWith("https://")) {
                raisePlatformError(Test509Event.Error.Communication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL")
            } else {
    ...
    // SCE-MAP: test509.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test509State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
