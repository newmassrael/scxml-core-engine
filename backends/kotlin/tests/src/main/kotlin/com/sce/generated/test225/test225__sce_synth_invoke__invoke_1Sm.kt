...
class Test225SceSynthInvokeInvoke1StateMachine(
    ...
    override fun isFinalState(state: Test225SceSynthInvokeInvoke1State): Boolean = when (state) {
        is Test225SceSynthInvokeInvoke1State.SubFinal2 -> true
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test225SceSynthInvokeInvoke1State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test225SceSynthInvokeInvoke1State, HistoryId>> =
            listOf(StateTarget(Test225SceSynthInvokeInvoke1State.SubFinal2))
    }
    ...
    ): EnabledTransition<Test225SceSynthInvokeInvoke1State, HistoryId>? = when (state) {
        else -> null
    }
    ...
    // SCE-MAP: test225__sce_synth_invoke__invoke_1.scxml:3 :: _machine
    override fun onEntry(state: Test225SceSynthInvokeInvoke1State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test225__sce_synth_invoke__invoke_1.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test225SceSynthInvokeInvoke1State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
