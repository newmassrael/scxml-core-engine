...
class Test245StateMachine(
    ...
    override fun isFinalState(state: Test245State): Boolean = when (state) {
        is Test245State.Fail, is Test245State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test245State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test245State, HistoryId>> =
            listOf(StateTarget(Test245State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test245State, HistoryId>(
            Test245State.S0,
            listOf(StateTarget(Test245State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test245State, HistoryId>(
            Test245State.S0,
            listOf(StateTarget(Test245State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            val initResult_Var2 = engine.evaluateExpr(sid, com.sce.runtime.ScriptSource.lua("3", "3"))
    ...
    override fun bindCurrentEvent(event: Test245Event) {
    ...
    override fun firstEnabledTransition(
        state: Test245State,
        event: Test245Event?
    ): EnabledTransition<Test245State, HistoryId>? = when (state) {
        is Test245State.S0 -> when {
            event is Test245Event.Success -> transitionS0At0
    ...
    // SCE-MAP: test245.scxml:6 :: _machine
    override fun onEntry(state: Test245State, isDefaultEntry: Boolean) {
        when (state) {
            is Test245State.Fail -> {
                // SCE-MAP: test245.scxml:37 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test245State.Pass -> {
                // SCE-MAP: test245.scxml:36 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test245State.S0 -> {
    ...
    // SCE-MAP: test245.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test245State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
