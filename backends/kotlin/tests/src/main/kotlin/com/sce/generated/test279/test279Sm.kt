...
// SCE-MAP: test279.scxml:5 :: _machine
...
class Test279StateMachine(
    ...
    override fun isFinalState(state: Test279State): Boolean = when (state) {
        is Test279State.Fail, is Test279State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test279State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test279State, HistoryId>> =
            listOf(StateTarget(Test279State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test279State, HistoryId>(
            Test279State.S0,
            listOf(StateTarget(Test279State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test279State, HistoryId>(
            Test279State.S0,
            listOf(StateTarget(Test279State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test279Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test279Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test279Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test279Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test279Event) {
    ...
    override fun firstEnabledTransition(
        state: Test279State,
        event: Test279Event?
    ): EnabledTransition<Test279State, HistoryId>? = when (state) {
        is Test279State.S0 -> when {
    ...
    // SCE-MAP: test279.scxml:5 :: _machine
    override fun onEntry(state: Test279State, isDefaultEntry: Boolean) {
        when (state) {
            is Test279State.Fail -> {
                // SCE-MAP: test279.scxml:22 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test279State.Pass -> {
                // SCE-MAP: test279.scxml:21 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test279State.S0 -> {
                // SCE-MAP: test279.scxml:8 :: s0 :: _state_body
            }
            is Test279State.S1 -> {
                // SCE-MAP: test279.scxml:15 :: s1 :: _state_body
            }
        }
    }
    ...
    // SCE-MAP: test279.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test279State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
