...
class Test496StateMachine(
    ...
    override fun isAtomicState(state: Test496State): Boolean = when (state) {
        else -> true
    }
    ...
    override fun processEvent(
        state: Test496State,
        event: Test496Event
    ): TransitionResult<Test496State> {
        // W3C SCXML 5.10: Set _event before guard evaluation
        setCurrentEventInScriptEngine(event)
        return when (state) {
        is Test496State.S0 -> processS0(event)
    ...
    private fun processS0(
        event: Test496Event
    ): TransitionResult<Test496State> = when {
        event is Test496Event.Error.Communication -> TransitionResult.External(Test496State.Pass, Test496State.S0, 0)

        // W3C SCXML 3.12.1: Wildcard transition
        else -> TransitionResult.External(Test496State.Fail, Test496State.S0, 1)
    }
    ...
    // SCE-MAP: test496.scxml:3 :: _machine
    override fun onEntry(state: Test496State, pathChild: Test496State?) {
        when (state) {
