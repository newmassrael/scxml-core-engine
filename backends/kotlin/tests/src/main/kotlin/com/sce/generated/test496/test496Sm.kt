...
// SCE-MAP: test496.scxml:3 :: _machine

package com.sce.generated.test496
...
sealed interface Test496Event : Event {
    ...
    data object Event : Test496Event
    data object Foo : Test496Event
}
// --- State Machine (W3C SCXML) ---

class Test496StateMachine(
    scriptEngine: ScxmlScriptEngine,
) : StateMachineEngine<Test496State, Test496Event>(scriptEngine) {

    override val initialState: Test496State = Test496State.S0
    ...
    override fun isFinalState(state: Test496State): Boolean = when (state) {
        is Test496State.Fail, is Test496State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test496State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test496State, HistoryId>> =
            listOf(StateTarget(Test496State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test496State, HistoryId>(
            Test496State.S0,
            listOf(StateTarget(Test496State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test496State, HistoryId>(
            Test496State.S0,
            listOf(StateTarget(Test496State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun resolveState(stateId: String): Test496State? = when (stateId) {
        "fail" -> Test496State.Fail
        "pass" -> Test496State.Pass
        "s0" -> Test496State.S0
        else -> null
    }

    // W3C SCXML: Get state ID string from State object
    override fun stateIdOf(state: Test496State): String = when (state) {
        is Test496State.Fail -> "fail"
        is Test496State.Pass -> "pass"
        is Test496State.S0 -> "s0"
    }

    // W3C SCXML 3.13: Document order — entry order, and in reverse exit order
    override fun documentOrderOf(state: Test496State): Int = when (state) {
        is Test496State.Fail -> 2
        is Test496State.Pass -> 1
        is Test496State.S0 -> 0
    }

    // W3C SCXML 6.4: Resolve event name to Event object (cross-SM routing)
    override fun resolveEventByName(name: String): Test496Event? = when (name) {
        "error.communication" -> Test496Event.Error.Communication
    ...
    override fun eventNameOf(event: Test496Event): String? = when (event) {
        is Test496Event.Error.Communication -> "error.communication"
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test496Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test496Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun setCurrentEventInScriptEngine(event: Test496Event) {
        ensureScriptEngine()
        val engine = scriptEngine ?: error("scriptEngine is required (codegen invariant: needs_script_engine == true)")
        val sid = scriptSessionId ?: error("scriptSessionId must be initialized after ensureScriptEngine() (codegen invariant)")
    ...
    override fun bindCurrentEvent(event: Test496Event) {
    ...
    override fun firstEnabledTransition(
        state: Test496State,
        event: Test496Event?
    ): EnabledTransition<Test496State, HistoryId>? = when (state) {
        is Test496State.S0 -> when {
            event is Test496Event.Error.Communication -> transitionS0At0
    ...
    // SCE-MAP: test496.scxml:3 :: _machine
    override fun onEntry(state: Test496State, isDefaultEntry: Boolean) {
        when (state) {
            is Test496State.Fail -> {
                // SCE-MAP: test496.scxml:18 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test496State.Pass -> {
                // SCE-MAP: test496.scxml:17 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test496State.S0 -> {
    ...
    // SCE-MAP: test496.scxml:3 :: _machine
    override fun onExit(state: Test496State) {
        when (state) {
            is Test496State.Fail -> {
                // SCE-MAP: test496.scxml:18 :: fail :: _state_body
            }
    ...
    // SCE-MAP: test496.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test496State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
