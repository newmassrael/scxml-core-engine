...
class Test452StateMachine(
     ...
     * `foo` was assigned a value of another type, or the engine refused.
     */
    fun foo(): Long? =
        com.sce.runtime.DatamodelRead.readInt(scriptEngine, scriptSessionId, "foo")
    ...
    override fun isFinalState(state: Test452State): Boolean = when (state) {
        is Test452State.Fail, is Test452State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test452State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test452State, HistoryId>> =
            listOf(StateTarget(Test452State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test452State, HistoryId>(
            Test452State.S0,
            listOf(StateTarget(Test452State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test452State, HistoryId>(
            Test452State.S0,
            listOf(StateTarget(Test452State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun bindCurrentEvent(event: Test452Event) {
    ...
    override fun firstEnabledTransition(
        state: Test452State,
        event: Test452Event?
    ): EnabledTransition<Test452State, HistoryId>? = when (state) {
        is Test452State.S0 -> when {
            event is Test452Event.Event1 && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(foo.bar, 1)", "foo.bar == 1")) -> transitionS0At0
    ...
    // SCE-MAP: test452.scxml:5 :: _machine
    override fun onEntry(state: Test452State, isDefaultEntry: Boolean) {
        when (state) {
            is Test452State.Fail -> {
                // SCE-MAP: test452.scxml:26 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test452State.Pass -> {
                // SCE-MAP: test452.scxml:25 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test452State.S0 -> {
    ...
    // SCE-MAP: test452.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test452State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
