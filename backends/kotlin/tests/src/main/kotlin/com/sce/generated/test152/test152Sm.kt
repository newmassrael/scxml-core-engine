...
// SCE-MAP: test152.scxml:6 :: _machine
...
sealed interface Test152State : State {
    data object Fail : Test152State
    data object Pass : Test152State
    data object S0 : Test152State
...
sealed interface Test152Event : Event {
    ...
    data object Foo : Test152Event
}
// --- State Machine (W3C SCXML) ---

class Test152StateMachine(
    scriptEngine: ScxmlScriptEngine,
) : StateMachineEngine<Test152State, Test152Event>(scriptEngine) {
    ...
    override val initialState: Test152State = Test152State.S0
    ...
    override fun isFinalState(state: Test152State): Boolean = when (state) {
        is Test152State.Fail, is Test152State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test152State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test152State, HistoryId>> =
            listOf(StateTarget(Test152State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test152State, HistoryId>(
            Test152State.S0,
            listOf(StateTarget(Test152State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test152State, HistoryId>(
            Test152State.S0,
            listOf(StateTarget(Test152State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test152State, HistoryId>(
            Test152State.S1,
            listOf(StateTarget(Test152State.S2)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test152State, HistoryId>(
            Test152State.S1,
            listOf(StateTarget(Test152State.Fail)),
            1,
            hasActions = false,
        ...
        val transitionS2At0 = EnabledTransition<Test152State, HistoryId>(
            Test152State.S2,
            listOf(StateTarget(Test152State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS2At1 = EnabledTransition<Test152State, HistoryId>(
            Test152State.S2,
            listOf(StateTarget(Test152State.Fail)),
            1,
            hasActions = false,
    ...
    override fun resolveState(stateId: String): Test152State? = when (stateId) {
        "fail" -> Test152State.Fail
        "pass" -> Test152State.Pass
        "s0" -> Test152State.S0
        "s1" -> Test152State.S1
        "s2" -> Test152State.S2
        else -> null
    ...
    override fun stateIdOf(state: Test152State): String = when (state) {
        is Test152State.Fail -> "fail"
        is Test152State.Pass -> "pass"
        is Test152State.S0 -> "s0"
        is Test152State.S1 -> "s1"
        is Test152State.S2 -> "s2"
    }

    // W3C SCXML 3.13: Document order — entry order, and in reverse exit order
    override fun documentOrderOf(state: Test152State): Int = when (state) {
        is Test152State.Fail -> 4
        is Test152State.Pass -> 3
        is Test152State.S0 -> 0
        is Test152State.S1 -> 1
        is Test152State.S2 -> 2
    }

    // W3C SCXML 6.4: Resolve event name to Event object (cross-SM routing)
    override fun resolveEventByName(name: String): Test152Event? = when (name) {
    ...
    override fun eventNameOf(event: Test152Event): String? = when (event) {
    ...
    private fun setCurrentEventInScriptEngine(event: Test152Event) {
        ensureScriptEngine()
        val engine = scriptEngine ?: error("scriptEngine is required (codegen invariant: needs_script_engine == true)")
        val sid = scriptSessionId ?: error("scriptSessionId must be initialized after ensureScriptEngine() (codegen invariant)")
    ...
    override fun bindCurrentEvent(event: Test152Event) {
    ...
    override fun firstEnabledTransition(
        state: Test152State,
        event: Test152Event?
    ): EnabledTransition<Test152State, HistoryId>? = when (state) {
        is Test152State.S0 -> when {
            event is Test152Event.Error.Execution -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test152State.S1 -> when {
            event is Test152Event.Error.Execution -> transitionS1At0
            event is Test152Event.Bar -> transitionS1At1
            else -> null
        }
        is Test152State.S2 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var1, 0)", "Var1 == 0")) -> transitionS2At0
    ...
    // SCE-MAP: test152.scxml:6 :: _machine
    override fun onEntry(state: Test152State, isDefaultEntry: Boolean) {
        when (state) {
            is Test152State.Fail -> {
                // SCE-MAP: test152.scxml:48 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test152State.Pass -> {
                // SCE-MAP: test152.scxml:47 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test152State.S0 -> {
                // SCE-MAP: test152.scxml:17 :: s0 :: _state_body
            ...
            is Test152State.S1 -> {
                // SCE-MAP: test152.scxml:29 :: s1 :: _state_body
            ...
            is Test152State.S2 -> {
                // SCE-MAP: test152.scxml:41 :: s2 :: _state_body
            }
        }
    }
    ...
    // SCE-MAP: test152.scxml:6 :: _machine
    override fun onExit(state: Test152State) {
        when (state) {
            is Test152State.Fail -> {
                // SCE-MAP: test152.scxml:48 :: fail :: _state_body
            }
    ...
    // SCE-MAP: test152.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test152State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
