...
class Test152StateMachine(
    ...
    override fun isAtomicState(state: Test152State): Boolean = when (state) {
        else -> true
    }
    ...
    override fun processEvent(
        state: Test152State,
        event: Test152Event
    ): TransitionResult<Test152State> {
        // W3C SCXML 5.10: Set _event before guard evaluation
        setCurrentEventInScriptEngine(event)
        return when (state) {
        is Test152State.S0 -> processS0(event)
        is Test152State.S1 -> processS1(event)
    ...
    override fun processNullEvent(
        state: Test152State
    ): TransitionResult<Test152State> = when (state) {
        is Test152State.S2 -> processNullS2()
        else -> TransitionResult.Ignored
    }
    ...
    ): TransitionResult<Test152State> = when {
        safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var1, 0)", "Var1 == 0")) -> TransitionResult.External(Test152State.Pass, Test152State.S2, 4)
        // W3C SCXML 3.13: First unconditional transition wins (document order)
        else -> TransitionResult.External(Test152State.Fail, Test152State.S2, 5)
    }
    ...
    private fun processS0(
        ...
        event is Test152Event.Error.Execution -> TransitionResult.External(Test152State.S1, Test152State.S0, 0)

        // W3C SCXML 3.12.1: Wildcard transition
        else -> TransitionResult.External(Test152State.Fail, Test152State.S0, 1)
    }

    private fun processS1(
        event: Test152Event
    ): TransitionResult<Test152State> = when {
        event is Test152Event.Error.Execution -> TransitionResult.External(Test152State.S2, Test152State.S1, 2)

        event is Test152Event.Bar -> TransitionResult.External(Test152State.Fail, Test152State.S1, 3)
    ...
    // SCE-MAP: test152.scxml:6 :: _machine
    override fun onEntry(state: Test152State, pathChild: Test152State?) {
        when (state) {
