...
class Test513StateMachine(
    ...
    override fun isFinalState(state: Test513State): Boolean = when (state) {
        is Test513State.Fail, is Test513State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test513State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test513State, HistoryId>> =
            listOf(StateTarget(Test513State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test513State, HistoryId>(
            Test513State.S0,
            listOf(StateTarget(Test513State.Pass)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test513State, HistoryId>(
            Test513State.S0,
            listOf(StateTarget(Test513State.Fail)),
            1,
            hasActions = true,
            isInternal = false,
        )
    ...
    override fun resolveEventByName(name: String): Test513Event? = when (name) {
        "error.communication" -> Test513Event.Error.Communication
    ...
    override fun eventNameOf(event: Test513Event): String? = when (event) {
        is Test513Event.Error.Communication -> "error.communication"
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test513Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test513Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test513Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test513Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test513Event) {
    ...
    override fun firstEnabledTransition(
        state: Test513State,
        event: Test513Event?
    ): EnabledTransition<Test513State, HistoryId>? = when (state) {
        is Test513State.S0 -> when {
            event is Test513Event.Test -> transitionS0At0
            event is Test513Event.Timeout -> transitionS0At1
            else -> null
        }
        else -> null
    }
    ...
    // SCE-MAP: test513.scxml:20 :: _machine
    override fun onEntry(state: Test513State, isDefaultEntry: Boolean) {
        when (state) {
            ...
            run resolveTarget@{
                ...
                try {
                    ...
                    if (target.startsWith("!")) {
                        raisePlatformError(Test513Event.Error.Execution, "<send> targetexpr produced a target this processor cannot address", "__send_1")
                        return@resolveTarget
                    }
                    // W3C SCXML C.1 (test496): Unreachable target (C++ SendHelper::isUnreachableTarget)
                    if (target.isEmpty() || target == "undefined") {
                        raisePlatformError(Test513Event.Error.Communication, "<send> targetexpr evaluated to nothing, so there is no target to reach")
                        return@resolveTarget
                    }
                    _resolvedTarget = target
                } catch (_: Exception) {
                    raisePlatformError(Test513Event.Error.Execution, "<send> targetexpr failed to evaluate")
                }
            }
            _resolvedTarget?.let { _rt ->
            // W3C SCXML C.2: Validate dynamic target is HTTP URL
            if (!_rt.startsWith("http://") && !_rt.startsWith("https://")) {
                raisePlatformError(Test513Event.Error.Communication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL")
            } else {
    ...
    // SCE-MAP: test513.scxml:20 :: _machine
    override fun executeTransitionContent(source: Test513State, transitionIndex: Int) {
        when (source) {
