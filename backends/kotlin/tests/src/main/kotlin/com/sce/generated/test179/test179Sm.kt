...
class Test179StateMachine(
    ...
    override fun isFinalState(state: Test179State): Boolean = when (state) {
        is Test179State.Fail, is Test179State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test179State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test179State, HistoryId>> =
            listOf(StateTarget(Test179State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test179State, HistoryId>(
            Test179State.S0,
            listOf(StateTarget(Test179State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test179State, HistoryId>(
            Test179State.S0,
            listOf(StateTarget(Test179State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun bindCurrentEvent(event: Test179Event) {
    ...
    override fun firstEnabledTransition(
        state: Test179State,
        event: Test179Event?
    ): EnabledTransition<Test179State, HistoryId>? = when (state) {
        is Test179State.S0 -> when {
            event is Test179Event.Event1 && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(_event.data, 123)", "_event.data == 123")) -> transitionS0At0
    ...
    // SCE-MAP: test179.scxml:4 :: _machine
    override fun onEntry(state: Test179State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test179.scxml:4 :: _machine
    override fun executeTransitionContent(source: Test179State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
