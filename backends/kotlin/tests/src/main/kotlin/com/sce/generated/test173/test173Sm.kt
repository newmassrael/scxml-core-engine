...
class Test173StateMachine(
    ...
    override fun isFinalState(state: Test173State): Boolean = when (state) {
        is Test173State.Fail, is Test173State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test173State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test173State, HistoryId>> =
            listOf(StateTarget(Test173State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test173State, HistoryId>(
            Test173State.S0,
            listOf(StateTarget(Test173State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test173State, HistoryId>(
            Test173State.S0,
            listOf(StateTarget(Test173State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test173Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test173Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test173Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test173Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test173Event) {
    ...
    override fun firstEnabledTransition(
        state: Test173State,
        event: Test173Event?
    ): EnabledTransition<Test173State, HistoryId>? = when (state) {
        is Test173State.S0 -> when {
            event is Test173Event.Event1 -> transitionS0At0
    ...
    // SCE-MAP: test173.scxml:5 :: _machine
    override fun onEntry(state: Test173State, isDefaultEntry: Boolean) {
        when (state) {
            is Test173State.Fail -> {
                // SCE-MAP: test173.scxml:22 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test173State.Pass -> {
                // SCE-MAP: test173.scxml:21 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test173State.S0 -> {
            ...
            run resolveTarget@{
                ...
                try {
                    ...
                    if (target.startsWith("!")) {
                        raisePlatformError(Test173Event.Error.Execution, "<send> targetexpr produced a target this processor cannot address", "__send_0")
                        return@resolveTarget
                    }
                    // W3C SCXML C.1 (test496): Unreachable target (C++ SendHelper::isUnreachableTarget)
                    if (target.isEmpty() || target == "undefined") {
                        raisePlatformError(Test173Event.Error.Communication, "<send> targetexpr evaluated to nothing, so there is no target to reach")
                        return@resolveTarget
                    }
                    _resolvedTarget = target
                } catch (_: Exception) {
                    raisePlatformError(Test173Event.Error.Execution, "<send> targetexpr failed to evaluate")
                }
            }
    ...
    // SCE-MAP: test173.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test173State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
