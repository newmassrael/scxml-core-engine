...
// SCE-MAP: test518.scxml:5 :: _machine
...
class Test518StateMachine(
    ...
    override fun isFinalState(state: Test518State): Boolean = when (state) {
        is Test518State.Fail, is Test518State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test518State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test518State, HistoryId>> =
            listOf(StateTarget(Test518State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test518State, HistoryId>(
            Test518State.S0,
            listOf(StateTarget(Test518State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test518State, HistoryId>(
            Test518State.S0,
            listOf(StateTarget(Test518State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            ...
            raisePlatformError(Test518Event.Error.Execution, "<data id='Var1'> expr failed to evaluate")
        }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test518Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test518Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test518Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test518Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test518Event) {
    ...
    override fun firstEnabledTransition(
        state: Test518State,
        event: Test518Event?
    ): EnabledTransition<Test518State, HistoryId>? = when (state) {
        is Test518State.S0 -> when {
            event is Test518Event.Test -> transitionS0At0
    ...
    // SCE-MAP: test518.scxml:5 :: _machine
    override fun onEntry(state: Test518State, isDefaultEntry: Boolean) {
        when (state) {
            is Test518State.Fail -> {
                // SCE-MAP: test518.scxml:23 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test518State.Pass -> {
                // SCE-MAP: test518.scxml:22 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test518State.S0 -> {
            ...
            run resolveTarget@{
                ...
                try {
                    ...
                    if (target.startsWith("!")) {
                        raisePlatformError(Test518Event.Error.Execution, "<send> targetexpr produced a target this processor cannot address", "__send_1")
                        return@resolveTarget
                    }
                    // W3C SCXML C.1 (test496): Unreachable target (C++ SendHelper::isUnreachableTarget)
                    if (target.isEmpty() || target == "undefined") {
                        raisePlatformError(Test518Event.Error.Communication, "<send> targetexpr evaluated to nothing, so there is no target to reach")
                        return@resolveTarget
                    }
                    _resolvedTarget = target
                } catch (_: Exception) {
                    raisePlatformError(Test518Event.Error.Execution, "<send> targetexpr failed to evaluate")
                }
            }
            _resolvedTarget?.let { _rt ->
            // W3C SCXML C.2: Validate dynamic target is HTTP URL
            if (!_rt.startsWith("http://") && !_rt.startsWith("https://")) {
                raisePlatformError(Test518Event.Error.Communication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL")
            } else {
            ...
            run {
                ...
                if (!engineH.hasVariable(sidH, "Var1")) {
                    raisePlatformError(Test518Event.Error.Execution, "<send> namelist names 'Var1', which is not declared")
                    return@run
                }
                try {
    ...
    // SCE-MAP: test518.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test518State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
