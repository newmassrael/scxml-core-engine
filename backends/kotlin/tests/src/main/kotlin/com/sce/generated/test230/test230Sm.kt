...
class Test230StateMachine(
    ...
    override fun isCompoundState(state: Test230State): Boolean = when (state) {
        is Test230State.S0 -> true
        else -> false
    }

    // W3C SCXML 3.7: Check if state is a <final> element
    override fun isFinalState(state: Test230State): Boolean = when (state) {
        is Test230State.Fail, is Test230State.Final -> true
        else -> false
    }

    // §scxml-D-getChildStates: a state's <state>, <parallel> and <final>
    // children, in document order — for a <parallel>, its regions.
    override fun childStatesOf(state: Test230State): List<Test230State> =
        childStates[state] ?: emptyList()

    // W3C SCXML 3.3: a compound state's initial transition target, as written.
    override fun initialTargetsOf(state: Test230State): List<EntryTarget<Test230State, HistoryId>> =
        initialTargets[state] ?: emptyList()

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test230State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val childStates: Map<Test230State, List<Test230State>> = mapOf(
            Test230State.S0 to listOf(Test230State.S01, Test230State.S02),
        )

        val initialTargets: Map<Test230State, List<EntryTarget<Test230State, HistoryId>>> = mapOf(
            Test230State.S0 to listOf(StateTarget(Test230State.S01)),
        )

        val documentInitialTargetList: List<EntryTarget<Test230State, HistoryId>> =
            listOf(StateTarget(Test230State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test230State, HistoryId>(
            Test230State.S0,
            listOf(StateTarget(Test230State.Final)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS01At0 = EnabledTransition<Test230State, HistoryId>(
            Test230State.S01,
            listOf(StateTarget(Test230State.S02)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS01At1 = EnabledTransition<Test230State, HistoryId>(
            Test230State.S01,
            listOf(StateTarget(Test230State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS02At0 = EnabledTransition<Test230State, HistoryId>(
            Test230State.S02,
            listOf(StateTarget(Test230State.Final)),
            0,
            hasActions = false,
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test230Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test230Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    override fun bindCurrentEvent(event: Test230Event) {
    ...
    override fun firstEnabledTransition(
        state: Test230State,
        event: Test230Event?
    ): EnabledTransition<Test230State, HistoryId>? = when (state) {
        is Test230State.S0 -> when {
            event is Test230Event.Timeout -> transitionS0At0
            else -> null
        }
        is Test230State.S01 -> when {
            event is Test230Event.ChildToParent -> transitionS01At0
            event != null -> transitionS01At1
            else -> null
        }
        is Test230State.S02 -> when {
            event is Test230Event.Done.Invoke -> transitionS02At0
            else -> null
        }
        else -> null
    }
    ...
    // SCE-MAP: test230.scxml:8 :: _machine
    override fun onEntry(state: Test230State, isDefaultEntry: Boolean) {
        when (state) {
            is Test230State.Fail -> {
                // SCE-MAP: test230.scxml:64 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test230State.Final -> {
                // SCE-MAP: test230.scxml:63 :: final :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test230State.S0 -> {
            ...
            is Test230State.S01 -> {
                // SCE-MAP: test230.scxml:43 :: s01 :: _state_body
            }
            is Test230State.S02 -> {
                // SCE-MAP: test230.scxml:56 :: s02 :: _state_body
            }
        }
    }
    ...
    // SCE-MAP: test230.scxml:8 :: _machine
    override fun executeTransitionContent(source: Test230State, transitionIndex: Int) {
        when (source) {
