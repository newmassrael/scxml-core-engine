...
class Test323StateMachine(
    ...
    override fun isFinalState(state: Test323State): Boolean = when (state) {
        is Test323State.Fail, is Test323State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test323State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test323State, HistoryId>> =
            listOf(StateTarget(Test323State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test323State, HistoryId>(
            Test323State.S0,
            listOf(StateTarget(Test323State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test323State, HistoryId>(
            Test323State.S0,
            listOf(StateTarget(Test323State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test323Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test323Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test323Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test323Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test323Event) {
    ...
    override fun firstEnabledTransition(
        state: Test323State,
        event: Test323Event?
    ): EnabledTransition<Test323State, HistoryId>? = when (state) {
        is Test323State.S0 -> when {
    ...
    // SCE-MAP: test323.scxml:5 :: _machine
    override fun onEntry(state: Test323State, isDefaultEntry: Boolean) {
        when (state) {
            is Test323State.Fail -> {
                // SCE-MAP: test323.scxml:17 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test323State.Pass -> {
                // SCE-MAP: test323.scxml:16 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test323State.S0 -> {
                // SCE-MAP: test323.scxml:10 :: s0 :: _state_body
            }
        }
    }
    ...
    // SCE-MAP: test323.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test323State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
