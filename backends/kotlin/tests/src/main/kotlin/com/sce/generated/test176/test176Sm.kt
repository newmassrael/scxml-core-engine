...
// SCE-MAP: test176.scxml:5 :: _machine
...
sealed interface Test176State : State {
    data object Fail : Test176State
    data object Pass : Test176State
...
sealed interface Test176Event : Event {
    ...
    data object Event1 : Test176Event
}
// --- State Machine (W3C SCXML) ---

class Test176StateMachine(
    scriptEngine: ScxmlScriptEngine,
) : StateMachineEngine<Test176State, Test176Event>(scriptEngine) {
    ...
    override val initialState: Test176State = Test176State.S0
    ...
    override fun isFinalState(state: Test176State): Boolean = when (state) {
        is Test176State.Fail, is Test176State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test176State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test176State, HistoryId>> =
            listOf(StateTarget(Test176State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test176State, HistoryId>(
            Test176State.S0,
            listOf(StateTarget(Test176State.S1)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test176State, HistoryId>(
            Test176State.S0,
            listOf(StateTarget(Test176State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test176State, HistoryId>(
            Test176State.S1,
            listOf(StateTarget(Test176State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test176State, HistoryId>(
            Test176State.S1,
            listOf(StateTarget(Test176State.Fail)),
            1,
            hasActions = false,
    ...
    override fun resolveState(stateId: String): Test176State? = when (stateId) {
        "fail" -> Test176State.Fail
        "pass" -> Test176State.Pass
        "s0" -> Test176State.S0
        "s1" -> Test176State.S1
        else -> null
    }
    ...
    override fun stateIdOf(state: Test176State): String = when (state) {
        is Test176State.Fail -> "fail"
        is Test176State.Pass -> "pass"
        is Test176State.S0 -> "s0"
        is Test176State.S1 -> "s1"
    }

    // W3C SCXML 3.13: Document order — entry order, and in reverse exit order
    override fun documentOrderOf(state: Test176State): Int = when (state) {
        is Test176State.Fail -> 3
        is Test176State.Pass -> 2
        is Test176State.S0 -> 0
        is Test176State.S1 -> 1
    }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test176Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test176Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test176Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test176Event.Error.Execution, "<script> failed to execute")
        }
    }

    // W3C SCXML 5.10: Set _event before event processing
    private fun setCurrentEventInScriptEngine(event: Test176Event) {
        ensureScriptEngine()
        val engine = scriptEngine ?: error("scriptEngine is required (codegen invariant: needs_script_engine == true)")
        val sid = scriptSessionId ?: error("scriptSessionId must be initialized after ensureScriptEngine() (codegen invariant)")
    ...
    override fun bindCurrentEvent(event: Test176Event) {
    ...
    override fun firstEnabledTransition(
        state: Test176State,
        event: Test176Event?
    ): EnabledTransition<Test176State, HistoryId>? = when (state) {
        is Test176State.S0 -> when {
            event is Test176Event.Event1 -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test176State.S1 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var2, 2)", "Var2 == 2")) -> transitionS1At0
    ...
    // SCE-MAP: test176.scxml:5 :: _machine
    override fun onEntry(state: Test176State, isDefaultEntry: Boolean) {
        when (state) {
            is Test176State.Fail -> {
                // SCE-MAP: test176.scxml:31 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test176State.Pass -> {
                // SCE-MAP: test176.scxml:30 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test176State.S0 -> {
                // SCE-MAP: test176.scxml:11 :: s0 :: _state_body
    ...
    // SCE-MAP: test176.scxml:5 :: _machine
    override fun onExit(state: Test176State) {
        when (state) {
            is Test176State.Fail -> {
                // SCE-MAP: test176.scxml:31 :: fail :: _state_body
            }
    ...
    // SCE-MAP: test176.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test176State, transitionIndex: Int) {
        when (source) {
        is Test176State.S0 -> when (transitionIndex) {
            0 -> {
