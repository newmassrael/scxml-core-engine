...
class Test276StateMachine(
    ...
    override fun isFinalState(state: Test276State): Boolean = when (state) {
        is Test276State.Fail, is Test276State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test276State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test276State, HistoryId>> =
            listOf(StateTarget(Test276State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test276State, HistoryId>(
            Test276State.S0,
            listOf(StateTarget(Test276State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test276State, HistoryId>(
            Test276State.S0,
            listOf(StateTarget(Test276State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test276Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test276Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test276Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test276Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test276Event) {
    ...
    override fun firstEnabledTransition(
        state: Test276State,
        event: Test276Event?
    ): EnabledTransition<Test276State, HistoryId>? = when (state) {
        is Test276State.S0 -> when {
            event is Test276Event.Event1 -> transitionS0At0
            event is Test276Event.Event0 -> transitionS0At1
            else -> null
        }
        else -> null
    }
    ...
    // SCE-MAP: test276.scxml:6 :: _machine
    override fun onEntry(state: Test276State, isDefaultEntry: Boolean) {
        when (state) {
            is Test276State.Fail -> {
                // SCE-MAP: test276.scxml:17 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test276State.Pass -> {
                // SCE-MAP: test276.scxml:16 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test276State.S0 -> {
    ...
    // SCE-MAP: test276.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test276State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
