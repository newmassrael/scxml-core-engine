...
class Test239SceSynthInvokeInvoke1StateMachine(
    ...
    override fun isFinalState(state: Test239SceSynthInvokeInvoke1State): Boolean = when (state) {
        is Test239SceSynthInvokeInvoke1State.Final -> true
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test239SceSynthInvokeInvoke1State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test239SceSynthInvokeInvoke1State, HistoryId>> =
            listOf(StateTarget(Test239SceSynthInvokeInvoke1State.Final))
    }
    ...
    ): EnabledTransition<Test239SceSynthInvokeInvoke1State, HistoryId>? = when (state) {
        else -> null
    }
    ...
    // SCE-MAP: test239__sce_synth_invoke__invoke_1.scxml:3 :: _machine
    override fun onEntry(state: Test239SceSynthInvokeInvoke1State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test239__sce_synth_invoke__invoke_1.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test239SceSynthInvokeInvoke1State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
