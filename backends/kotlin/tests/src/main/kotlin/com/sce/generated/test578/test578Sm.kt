...
// SCE-MAP: test578.scxml:5 :: _machine
...
class Test578StateMachine(
    ...
    override fun isFinalState(state: Test578State): Boolean = when (state) {
        is Test578State.Fail, is Test578State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test578State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test578State, HistoryId>> =
            listOf(StateTarget(Test578State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test578State, HistoryId>(
            Test578State.S0,
            listOf(StateTarget(Test578State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test578State, HistoryId>(
            Test578State.S0,
            listOf(StateTarget(Test578State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test578Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test578Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test578Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test578Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test578Event) {
    ...
    override fun firstEnabledTransition(
        state: Test578State,
        event: Test578Event?
    ): EnabledTransition<Test578State, HistoryId>? = when (state) {
        is Test578State.S0 -> when {
            event is Test578Event.Foo && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(_event.data.productName, \"bar\")", "_event.data.productName == 'bar'")) -> transitionS0At0
    ...
    // SCE-MAP: test578.scxml:5 :: _machine
    override fun onEntry(state: Test578State, isDefaultEntry: Boolean) {
        when (state) {
            is Test578State.Fail -> {
                // SCE-MAP: test578.scxml:21 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test578State.Pass -> {
                // SCE-MAP: test578.scxml:20 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test578State.S0 -> {
    ...
    // SCE-MAP: test578.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test578State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
