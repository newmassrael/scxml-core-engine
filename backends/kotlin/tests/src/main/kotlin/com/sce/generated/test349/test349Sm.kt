...
class Test349StateMachine(
    ...
    override fun isFinalState(state: Test349State): Boolean = when (state) {
        is Test349State.Fail, is Test349State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test349State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test349State, HistoryId>> =
            listOf(StateTarget(Test349State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test349State, HistoryId>(
            Test349State.S0,
            listOf(StateTarget(Test349State.S2)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test349State, HistoryId>(
            Test349State.S0,
            listOf(StateTarget(Test349State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS2At0 = EnabledTransition<Test349State, HistoryId>(
            Test349State.S2,
            listOf(StateTarget(Test349State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS2At1 = EnabledTransition<Test349State, HistoryId>(
            Test349State.S2,
            listOf(StateTarget(Test349State.Fail)),
            1,
            hasActions = false,
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test349Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test349Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    override fun bindCurrentEvent(event: Test349Event) {
    ...
    override fun firstEnabledTransition(
        state: Test349State,
        event: Test349Event?
    ): EnabledTransition<Test349State, HistoryId>? = when (state) {
        is Test349State.S0 -> when {
            event is Test349Event.S0Event -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test349State.S2 -> when {
            event is Test349Event.S0Event2 -> transitionS2At0
    ...
    // SCE-MAP: test349.scxml:5 :: _machine
    override fun onEntry(state: Test349State, isDefaultEntry: Boolean) {
        when (state) {
            is Test349State.Fail -> {
                // SCE-MAP: test349.scxml:31 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test349State.Pass -> {
                // SCE-MAP: test349.scxml:30 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test349State.S0 -> {
    ...
    // SCE-MAP: test349.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test349State, transitionIndex: Int) {
        when (source) {
        is Test349State.S0 -> when (transitionIndex) {
            0 -> {
