...
class Test350StateMachine(
    ...
    override fun isFinalState(state: Test350State): Boolean = when (state) {
        is Test350State.Fail, is Test350State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test350State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test350State, HistoryId>> =
            listOf(StateTarget(Test350State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test350State, HistoryId>(
            Test350State.S0,
            listOf(StateTarget(Test350State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test350State, HistoryId>(
            Test350State.S0,
            listOf(StateTarget(Test350State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test350Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test350Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test350Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test350Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test350Event) {
    ...
    override fun firstEnabledTransition(
        state: Test350State,
        event: Test350Event?
    ): EnabledTransition<Test350State, HistoryId>? = when (state) {
        is Test350State.S0 -> when {
            event is Test350Event.S0Event -> transitionS0At0
    ...
    // SCE-MAP: test350.scxml:6 :: _machine
    override fun onEntry(state: Test350State, isDefaultEntry: Boolean) {
        when (state) {
            is Test350State.Fail -> {
                // SCE-MAP: test350.scxml:26 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test350State.Pass -> {
                // SCE-MAP: test350.scxml:25 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test350State.S0 -> {
    ...
    // SCE-MAP: test350.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test350State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
