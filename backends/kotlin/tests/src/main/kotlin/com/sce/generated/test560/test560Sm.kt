...
// SCE-MAP: test560.scxml:5 :: _machine
...
class Test560StateMachine(
    ...
    override fun isFinalState(state: Test560State): Boolean = when (state) {
        is Test560State.Fail, is Test560State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test560State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test560State, HistoryId>> =
            listOf(StateTarget(Test560State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test560State, HistoryId>(
            Test560State.S0,
            listOf(StateTarget(Test560State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test560State, HistoryId>(
            Test560State.S0,
            listOf(StateTarget(Test560State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test560Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test560Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test560Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test560Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test560Event) {
    ...
    override fun firstEnabledTransition(
        state: Test560State,
        event: Test560Event?
    ): EnabledTransition<Test560State, HistoryId>? = when (state) {
        is Test560State.S0 -> when {
            event is Test560Event.Foo && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(_event.data.aParam, 1)", "_event.data.aParam == 1")) -> transitionS0At0
    ...
    // SCE-MAP: test560.scxml:5 :: _machine
    override fun onEntry(state: Test560State, isDefaultEntry: Boolean) {
        when (state) {
            is Test560State.Fail -> {
                // SCE-MAP: test560.scxml:20 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test560State.Pass -> {
                // SCE-MAP: test560.scxml:19 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test560State.S0 -> {
    ...
    // SCE-MAP: test560.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test560State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
