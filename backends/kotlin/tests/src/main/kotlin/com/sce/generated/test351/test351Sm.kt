...
class Test351StateMachine(
    ...
    override fun isFinalState(state: Test351State): Boolean = when (state) {
        is Test351State.Fail, is Test351State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test351State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test351State, HistoryId>> =
            listOf(StateTarget(Test351State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test351State, HistoryId>(
            Test351State.S0,
            listOf(StateTarget(Test351State.S1)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test351State, HistoryId>(
            Test351State.S0,
            listOf(StateTarget(Test351State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test351State, HistoryId>(
            Test351State.S1,
            listOf(StateTarget(Test351State.S2)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test351State, HistoryId>(
            Test351State.S1,
            listOf(StateTarget(Test351State.Fail)),
            1,
            hasActions = false,
        ...
        val transitionS2At0 = EnabledTransition<Test351State, HistoryId>(
            Test351State.S2,
            listOf(StateTarget(Test351State.S3)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS2At1 = EnabledTransition<Test351State, HistoryId>(
            Test351State.S2,
            listOf(StateTarget(Test351State.Fail)),
            1,
            hasActions = false,
        ...
        val transitionS3At0 = EnabledTransition<Test351State, HistoryId>(
            Test351State.S3,
            listOf(StateTarget(Test351State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS3At1 = EnabledTransition<Test351State, HistoryId>(
            Test351State.S3,
            listOf(StateTarget(Test351State.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test351Event) {
    ...
    override fun firstEnabledTransition(
        state: Test351State,
        event: Test351Event?
    ): EnabledTransition<Test351State, HistoryId>? = when (state) {
        is Test351State.S0 -> when {
            event is Test351Event.S0Event -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test351State.S1 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var1, \"send1\")", "Var1 == 'send1'")) -> transitionS1At0
            event == null -> transitionS1At1
            else -> null
        }
        is Test351State.S2 -> when {
            event is Test351Event.S0Event2 -> transitionS2At0
            event != null -> transitionS2At1
            else -> null
        }
        is Test351State.S3 -> when {
    ...
    // SCE-MAP: test351.scxml:5 :: _machine
    override fun onEntry(state: Test351State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test351.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test351State, transitionIndex: Int) {
        when (source) {
        is Test351State.S0 -> when (transitionIndex) {
            0 -> {
                // SCE-MAP: test351.scxml:18 :: s0 :: _transition_0


            executeAssign(com.sce.runtime.ScriptSource.lua("Var1", "Var1"), com.sce.runtime.ScriptSource.lua("_event.sendid", "_event.sendid"))
            }
            else -> {}
        }
        is Test351State.S2 -> when (transitionIndex) {
            0 -> {
