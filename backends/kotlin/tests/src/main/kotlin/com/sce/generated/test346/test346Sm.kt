...
class Test346StateMachine(
    ...
    override fun isFinalState(state: Test346State): Boolean = when (state) {
        is Test346State.Fail, is Test346State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test346State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test346State, HistoryId>> =
            listOf(StateTarget(Test346State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S0,
            listOf(StateTarget(Test346State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S0,
            listOf(StateTarget(Test346State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S1,
            emptyList(),
            0,
            hasActions = false,
        ...
        val transitionS1At1 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S1,
            listOf(StateTarget(Test346State.S2)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At2 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S1,
            listOf(StateTarget(Test346State.Fail)),
            2,
            hasActions = false,
        ...
        val transitionS2At0 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S2,
            emptyList(),
            0,
            hasActions = false,
        ...
        val transitionS2At1 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S2,
            listOf(StateTarget(Test346State.S3)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS2At2 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S2,
            listOf(StateTarget(Test346State.Fail)),
            2,
            hasActions = false,
        ...
        val transitionS3At0 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S3,
            emptyList(),
            0,
            hasActions = false,
        ...
        val transitionS3At1 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S3,
            listOf(StateTarget(Test346State.Pass)),
            1,
            hasActions = false,
            isInternal = false,
        )

        // W3C SCXML 3.13: s3's transition 2, as the microstep reads it.
        val transitionS3At2 = EnabledTransition<Test346State, HistoryId>(
            Test346State.S3,
            listOf(StateTarget(Test346State.Fail)),
            2,
            hasActions = false,
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test346Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test346Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    override fun bindCurrentEvent(event: Test346Event) {
    ...
    override fun firstEnabledTransition(
        state: Test346State,
        event: Test346Event?
    ): EnabledTransition<Test346State, HistoryId>? = when (state) {
        is Test346State.S0 -> when {
            event is Test346Event.Error.Execution -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test346State.S1 -> when {
            event is Test346Event.Event1 -> transitionS1At0
            event is Test346Event.Error.Execution -> transitionS1At1
            event != null -> transitionS1At2
            else -> null
        }
        is Test346State.S2 -> when {
            event is Test346Event.Event2 -> transitionS2At0
            event is Test346Event.Error.Execution -> transitionS2At1
            event != null -> transitionS2At2
            else -> null
        }
        is Test346State.S3 -> when {
            event is Test346Event.Event3 -> transitionS3At0
            event is Test346Event.Error.Execution -> transitionS3At1
            event != null -> transitionS3At2
            else -> null
        }
        else -> null
    }
    ...
    // SCE-MAP: test346.scxml:7 :: _machine
    override fun onEntry(state: Test346State, isDefaultEntry: Boolean) {
        when (state) {
            is Test346State.Fail -> {
                // SCE-MAP: test346.scxml:55 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test346State.Pass -> {
                // SCE-MAP: test346.scxml:54 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test346State.S0 -> {
    ...
    // SCE-MAP: test346.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test346State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
