...
class Test342StateMachine(
    ...
    override fun isFinalState(state: Test342State): Boolean = when (state) {
        is Test342State.Fail, is Test342State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test342State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test342State, HistoryId>> =
            listOf(StateTarget(Test342State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test342State, HistoryId>(
            Test342State.S0,
            listOf(StateTarget(Test342State.S1)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test342State, HistoryId>(
            Test342State.S0,
            listOf(StateTarget(Test342State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test342State, HistoryId>(
            Test342State.S1,
            listOf(StateTarget(Test342State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test342State, HistoryId>(
            Test342State.S1,
            listOf(StateTarget(Test342State.Fail)),
            1,
            hasActions = false,
    ...
    override fun resolveState(stateId: String): Test342State? = when (stateId) {
        "fail" -> Test342State.Fail
        "pass" -> Test342State.Pass
        "s0" -> Test342State.S0
        "s1" -> Test342State.S1
        else -> null
    }
    ...
    override fun stateIdOf(state: Test342State): String = when (state) {
        is Test342State.Fail -> "fail"
        is Test342State.Pass -> "pass"
        is Test342State.S0 -> "s0"
        is Test342State.S1 -> "s1"
    }

    // W3C SCXML 3.13: Document order — entry order, and in reverse exit order
    override fun documentOrderOf(state: Test342State): Int = when (state) {
        is Test342State.Fail -> 3
        is Test342State.Pass -> 2
        is Test342State.S0 -> 0
        is Test342State.S1 -> 1
    }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test342Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test342Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    override fun bindCurrentEvent(event: Test342Event) {
    ...
    override fun firstEnabledTransition(
        state: Test342State,
        event: Test342Event?
    ): EnabledTransition<Test342State, HistoryId>? = when (state) {
        is Test342State.S0 -> when {
            event is Test342Event.Foo -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test342State.S1 -> when {
    ...
    // SCE-MAP: test342.scxml:4 :: _machine
    override fun onEntry(state: Test342State, isDefaultEntry: Boolean) {
        when (state) {
            is Test342State.Fail -> {
                // SCE-MAP: test342.scxml:26 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test342State.Pass -> {
                // SCE-MAP: test342.scxml:25 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test342State.S0 -> {
    ...
    // SCE-MAP: test342.scxml:4 :: _machine
    override fun executeTransitionContent(source: Test342State, transitionIndex: Int) {
        when (source) {
