...
class Test190StateMachine(
    ...
    override fun isFinalState(state: Test190State): Boolean = when (state) {
        is Test190State.Fail, is Test190State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test190State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test190State, HistoryId>> =
            listOf(StateTarget(Test190State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test190State, HistoryId>(
            Test190State.S0,
            listOf(StateTarget(Test190State.S1)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test190State, HistoryId>(
            Test190State.S0,
            listOf(StateTarget(Test190State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test190State, HistoryId>(
            Test190State.S1,
            listOf(StateTarget(Test190State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test190State, HistoryId>(
            Test190State.S1,
            listOf(StateTarget(Test190State.Fail)),
            1,
            hasActions = false,
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            ...
            raisePlatformError(Test190Event.Error.Execution, "<data id='Var1'> expr failed to evaluate")
        }
        ...
        try {
            ...
            raisePlatformError(Test190Event.Error.Execution, "<data id='Var2'> expr failed to evaluate")
        }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test190Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test190Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test190Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test190Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test190Event) {
    ...
    override fun firstEnabledTransition(
        state: Test190State,
        event: Test190Event?
    ): EnabledTransition<Test190State, HistoryId>? = when (state) {
        is Test190State.S0 -> when {
            event is Test190Event.Event1 -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test190State.S1 -> when {
            event is Test190Event.Event2 -> transitionS1At0
    ...
    // SCE-MAP: test190.scxml:7 :: _machine
    override fun onEntry(state: Test190State, isDefaultEntry: Boolean) {
        when (state) {
            is Test190State.Fail -> {
                // SCE-MAP: test190.scxml:36 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test190State.Pass -> {
                // SCE-MAP: test190.scxml:35 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test190State.S0 -> {
            ...
            run resolveTarget@{
                ...
                try {
                    ...
                    if (target.startsWith("!")) {
                        raisePlatformError(Test190Event.Error.Execution, "<send> targetexpr produced a target this processor cannot address", "__send_0")
                        return@resolveTarget
                    }
                    // W3C SCXML C.1 (test496): Unreachable target (C++ SendHelper::isUnreachableTarget)
                    if (target.isEmpty() || target == "undefined") {
                        raisePlatformError(Test190Event.Error.Communication, "<send> targetexpr evaluated to nothing, so there is no target to reach")
                        return@resolveTarget
                    }
                    _resolvedTarget = target
                } catch (_: Exception) {
                    raisePlatformError(Test190Event.Error.Execution, "<send> targetexpr failed to evaluate")
                }
            }
    ...
    // SCE-MAP: test190.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test190State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
