...
// SCE-MAP: test354.scxml:6 :: _machine
...
class Test354StateMachine(
    ...
    override fun isFinalState(state: Test354State): Boolean = when (state) {
        is Test354State.Fail, is Test354State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test354State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test354State, HistoryId>> =
            listOf(StateTarget(Test354State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test354State, HistoryId>(
            Test354State.S0,
            listOf(StateTarget(Test354State.S1)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test354State, HistoryId>(
            Test354State.S0,
            listOf(StateTarget(Test354State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test354State, HistoryId>(
            Test354State.S1,
            listOf(StateTarget(Test354State.S2)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test354State, HistoryId>(
            Test354State.S1,
            listOf(StateTarget(Test354State.Fail)),
            1,
            hasActions = false,
        ...
        val transitionS2At0 = EnabledTransition<Test354State, HistoryId>(
            Test354State.S2,
            listOf(StateTarget(Test354State.S3)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS2At1 = EnabledTransition<Test354State, HistoryId>(
            Test354State.S2,
            listOf(StateTarget(Test354State.Fail)),
            1,
            hasActions = false,
        ...
        val transitionS3At0 = EnabledTransition<Test354State, HistoryId>(
            Test354State.S3,
            listOf(StateTarget(Test354State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS3At1 = EnabledTransition<Test354State, HistoryId>(
            Test354State.S3,
            listOf(StateTarget(Test354State.Fail)),
            1,
            hasActions = false,
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test354Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test354Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    override fun bindCurrentEvent(event: Test354Event) {
    ...
    override fun firstEnabledTransition(
        state: Test354State,
        event: Test354Event?
    ): EnabledTransition<Test354State, HistoryId>? = when (state) {
        is Test354State.S0 -> when {
            event is Test354Event.Event1 -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test354State.S1 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var2, 1)", "Var2 == 1")) -> transitionS1At0
            event == null -> transitionS1At1
            else -> null
        }
        is Test354State.S2 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(Var3, 2)", "Var3 == 2")) -> transitionS2At0
            event == null -> transitionS2At1
            else -> null
        }
        is Test354State.S3 -> when {
            event is Test354Event.Event2 -> transitionS3At0
    ...
    // SCE-MAP: test354.scxml:6 :: _machine
    override fun onEntry(state: Test354State, isDefaultEntry: Boolean) {
        when (state) {
            is Test354State.Fail -> {
                // SCE-MAP: test354.scxml:54 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test354State.Pass -> {
                // SCE-MAP: test354.scxml:53 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test354State.S0 -> {
            ...
            is Test354State.S2 -> {
                // SCE-MAP: test354.scxml:35 :: s2 :: _state_body
            }
            is Test354State.S3 -> {
    ...
    // SCE-MAP: test354.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test354State, transitionIndex: Int) {
        when (source) {
        is Test354State.S0 -> when (transitionIndex) {
            0 -> {
