...
class Test159StateMachine(
    ...
    override fun isFinalState(state: Test159State): Boolean = when (state) {
        is Test159State.Fail, is Test159State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test159State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test159State, HistoryId>> =
            listOf(StateTarget(Test159State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test159State, HistoryId>(
            Test159State.S0,
            listOf(StateTarget(Test159State.Fail)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test159State, HistoryId>(
            Test159State.S0,
            listOf(StateTarget(Test159State.Pass)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun ensureScriptEngine() {
        ...
        try {
            ...
            raisePlatformError(Test159Event.Error.Execution, "<data id='Var1'> expr failed to evaluate")
        }
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test159Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test159Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test159Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test159Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test159Event) {
    ...
    override fun firstEnabledTransition(
        state: Test159State,
        event: Test159Event?
    ): EnabledTransition<Test159State, HistoryId>? = when (state) {
        is Test159State.S0 -> when {
    ...
    // SCE-MAP: test159.scxml:6 :: _machine
    override fun onEntry(state: Test159State, isDefaultEntry: Boolean) {
        when (state) {
            is Test159State.Fail -> {
                // SCE-MAP: test159.scxml:22 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test159State.Pass -> {
                // SCE-MAP: test159.scxml:21 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test159State.S0 -> {
    ...
    // SCE-MAP: test159.scxml:6 :: _machine
    override fun executeTransitionContent(source: Test159State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
