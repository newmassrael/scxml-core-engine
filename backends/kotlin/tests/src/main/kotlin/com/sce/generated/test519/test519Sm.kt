...
class Test519StateMachine(
    ...
    override fun isFinalState(state: Test519State): Boolean = when (state) {
        is Test519State.Fail, is Test519State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test519State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test519State, HistoryId>> =
            listOf(StateTarget(Test519State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test519State, HistoryId>(
            Test519State.S0,
            listOf(StateTarget(Test519State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test519State, HistoryId>(
            Test519State.S0,
            listOf(StateTarget(Test519State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test519Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test519Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test519Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test519Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test519Event) {
    ...
    override fun firstEnabledTransition(
        state: Test519State,
        event: Test519Event?
    ): EnabledTransition<Test519State, HistoryId>? = when (state) {
        is Test519State.S0 -> when {
            event is Test519Event.Test && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("_scxml_eq(_event.data.param1, 1)", "_event.data[\"param1\"] == 1")) -> transitionS0At0
    ...
    // SCE-MAP: test519.scxml:3 :: _machine
    override fun onEntry(state: Test519State, isDefaultEntry: Boolean) {
        when (state) {
            is Test519State.Fail -> {
                // SCE-MAP: test519.scxml:20 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test519State.Pass -> {
                // SCE-MAP: test519.scxml:19 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test519State.S0 -> {
            ...
            run resolveTarget@{
                ...
                try {
                    ...
                    if (target.startsWith("!")) {
                        raisePlatformError(Test519Event.Error.Execution, "<send> targetexpr produced a target this processor cannot address", "__send_1")
                        return@resolveTarget
                    }
                    // W3C SCXML C.1 (test496): Unreachable target (C++ SendHelper::isUnreachableTarget)
                    if (target.isEmpty() || target == "undefined") {
                        raisePlatformError(Test519Event.Error.Communication, "<send> targetexpr evaluated to nothing, so there is no target to reach")
                        return@resolveTarget
                    }
                    _resolvedTarget = target
                } catch (_: Exception) {
                    raisePlatformError(Test519Event.Error.Execution, "<send> targetexpr failed to evaluate")
                }
            }
            _resolvedTarget?.let { _rt ->
            // W3C SCXML C.2: Validate dynamic target is HTTP URL
            if (!_rt.startsWith("http://") && !_rt.startsWith("https://")) {
                raisePlatformError(Test519Event.Error.Communication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL")
            } else {
    ...
    // SCE-MAP: test519.scxml:3 :: _machine
    override fun executeTransitionContent(source: Test519State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
