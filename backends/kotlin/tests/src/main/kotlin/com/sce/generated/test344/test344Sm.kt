...
class Test344StateMachine(
    ...
    override fun isFinalState(state: Test344State): Boolean = when (state) {
        is Test344State.Fail, is Test344State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test344State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test344State, HistoryId>> =
            listOf(StateTarget(Test344State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test344State, HistoryId>(
            Test344State.S0,
            listOf(StateTarget(Test344State.Fail)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test344State, HistoryId>(
            Test344State.S0,
            listOf(StateTarget(Test344State.S1)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test344State, HistoryId>(
            Test344State.S1,
            listOf(StateTarget(Test344State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test344State, HistoryId>(
            Test344State.S1,
            listOf(StateTarget(Test344State.Fail)),
            1,
            hasActions = false,
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test344Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test344Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test344Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test344Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test344Event) {
    ...
    override fun firstEnabledTransition(
        state: Test344State,
        event: Test344Event?
    ): EnabledTransition<Test344State, HistoryId>? = when (state) {
        is Test344State.S0 -> when {
            event == null && safeEvaluateGuard(com.sce.runtime.ScriptSource.lua("error(\"SCXML cond is not valid ECMAScript: return: unsupported ECMAScript construct: reserved word 'return' used as a value. Extended SCXML expressions must use the stateless subset.\")", "return")) -> transitionS0At0
            event == null -> transitionS0At1
            else -> null
        }
        is Test344State.S1 -> when {
            event is Test344Event.Error.Execution -> transitionS1At0
    ...
    // SCE-MAP: test344.scxml:7 :: _machine
    override fun onEntry(state: Test344State, isDefaultEntry: Boolean) {
        when (state) {
            is Test344State.Fail -> {
                // SCE-MAP: test344.scxml:24 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test344State.Pass -> {
                // SCE-MAP: test344.scxml:23 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test344State.S0 -> {
                // SCE-MAP: test344.scxml:10 :: s0 :: _state_body
            }
            is Test344State.S1 -> {
                // SCE-MAP: test344.scxml:15 :: s1 :: _state_body
    ...
    // SCE-MAP: test344.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test344State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
