...
// SCE-MAP: test302.scxml:7 :: _machine
...
class Test302StateMachine(
    ...
    override fun isFinalState(state: Test302State): Boolean = when (state) {
        is Test302State.Fail, is Test302State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test302State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test302State, HistoryId>> =
            listOf(StateTarget(Test302State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test302State, HistoryId>(
            Test302State.S0,
            listOf(StateTarget(Test302State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test302State, HistoryId>(
            Test302State.S0,
            listOf(StateTarget(Test302State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    private fun safeEvaluateGuard(guardExpr: com.sce.runtime.ScriptSource): Boolean {
        ...
        return try {
            ...
            raisePlatformError(Test302Event.Error.Execution, "a <transition> cond failed to evaluate")
            false
        }
    }
    ...
    private fun evaluateSendContent(source: com.sce.runtime.ScriptSource): String {
        ...
        return try {
            ...
            raisePlatformError(Test302Event.Error.Execution, "an expression could not be serialised to JSON")
            ""
        }
    }
    ...
    private fun executeAssign(location: com.sce.runtime.ScriptSource, expr: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test302Event.Error.Execution, "<assign> failed")
        }
    }
    ...
    private fun executeScriptBlock(script: com.sce.runtime.ScriptSource) {
        ...
        try {
            ...
            raisePlatformError(Test302Event.Error.Execution, "<script> failed to execute")
        }
    }
    ...
    override fun bindCurrentEvent(event: Test302Event) {
    ...
    override fun firstEnabledTransition(
        state: Test302State,
        event: Test302Event?
    ): EnabledTransition<Test302State, HistoryId>? = when (state) {
        is Test302State.S0 -> when {
    ...
    // SCE-MAP: test302.scxml:7 :: _machine
    override fun onEntry(state: Test302State, isDefaultEntry: Boolean) {
        when (state) {
            is Test302State.Fail -> {
                // SCE-MAP: test302.scxml:16 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test302State.Pass -> {
                // SCE-MAP: test302.scxml:15 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test302State.S0 -> {
                // SCE-MAP: test302.scxml:10 :: s0 :: _state_body
            }
        }
    }
    ...
    // SCE-MAP: test302.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test302State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
