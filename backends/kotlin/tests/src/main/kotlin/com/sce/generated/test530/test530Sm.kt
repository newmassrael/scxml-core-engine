...
class Test530StateMachine(
    ...
    override fun isFinalState(state: Test530State): Boolean = when (state) {
        is Test530State.Fail, is Test530State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test530State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test530State, HistoryId>> =
            listOf(StateTarget(Test530State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test530State, HistoryId>(
            Test530State.S0,
            listOf(StateTarget(Test530State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test530State, HistoryId>(
            Test530State.S0,
            listOf(StateTarget(Test530State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
    ...
    override fun bindCurrentEvent(event: Test530Event) {
    ...
    override fun firstEnabledTransition(
        state: Test530State,
        event: Test530Event?
    ): EnabledTransition<Test530State, HistoryId>? = when (state) {
        is Test530State.S0 -> when {
            event is Test530Event.Done.Invoke -> transitionS0At0
    ...
    // SCE-MAP: test530.scxml:7 :: _machine
    override fun onEntry(state: Test530State, isDefaultEntry: Boolean) {
        when (state) {
    ...
    // SCE-MAP: test530.scxml:7 :: _machine
    override fun executeTransitionContent(source: Test530State, transitionIndex: Int) {
        when (source) {
        else -> {}
        }
    }
