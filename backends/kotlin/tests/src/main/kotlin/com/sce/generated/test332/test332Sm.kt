...
// SCE-MAP: test332.scxml:5 :: _machine
...
class Test332StateMachine(
    ...
    override fun isFinalState(state: Test332State): Boolean = when (state) {
        is Test332State.Fail, is Test332State.Pass -> true
        else -> false
    }

    // W3C SCXML 3.2: the target of the document's own initial transition, as
    // written.
    override val documentInitialTargets: List<EntryTarget<Test332State, HistoryId>>
        get() = documentInitialTargetList

    private companion object {
        val documentInitialTargetList: List<EntryTarget<Test332State, HistoryId>> =
            listOf(StateTarget(Test332State.S0))

        // W3C SCXML 3.13: s0's transition 0, as the microstep reads it.
        val transitionS0At0 = EnabledTransition<Test332State, HistoryId>(
            Test332State.S0,
            listOf(StateTarget(Test332State.S1)),
            0,
            hasActions = true,
            isInternal = false,
        )
        ...
        val transitionS0At1 = EnabledTransition<Test332State, HistoryId>(
            Test332State.S0,
            listOf(StateTarget(Test332State.Fail)),
            1,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At0 = EnabledTransition<Test332State, HistoryId>(
            Test332State.S1,
            listOf(StateTarget(Test332State.Pass)),
            0,
            hasActions = false,
            isInternal = false,
        )
        ...
        val transitionS1At1 = EnabledTransition<Test332State, HistoryId>(
            Test332State.S1,
            listOf(StateTarget(Test332State.Fail)),
            1,
            hasActions = false,
    ...
    override fun bindCurrentEvent(event: Test332Event) {
    ...
    override fun firstEnabledTransition(
        state: Test332State,
        event: Test332Event?
    ): EnabledTransition<Test332State, HistoryId>? = when (state) {
        is Test332State.S0 -> when {
            (event is Test332Event.Error || event is Test332Event.Error.Execution) -> transitionS0At0
            event != null -> transitionS0At1
            else -> null
        }
        is Test332State.S1 -> when {
    ...
    // SCE-MAP: test332.scxml:5 :: _machine
    override fun onEntry(state: Test332State, isDefaultEntry: Boolean) {
        when (state) {
            is Test332State.Fail -> {
                // SCE-MAP: test332.scxml:31 :: fail :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test332State.Pass -> {
                // SCE-MAP: test332.scxml:30 :: pass :: _state_body
                // W3C SCXML 3.7: Top-level final state reached
                markFinalStateReached()
            }
            is Test332State.S0 -> {
    ...
    // SCE-MAP: test332.scxml:5 :: _machine
    override fun executeTransitionContent(source: Test332State, transitionIndex: Int) {
        when (source) {
        is Test332State.S0 -> when (transitionIndex) {
            0 -> {
