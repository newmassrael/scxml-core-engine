...
// From: integration_resources/late_data_binds_on_first_entry/late_data_binds_on_first_entry.scxml
//
...
//! Generated Rust state machine for `late_data_binds_on_first_entry`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: late_data_binds_on_first_entry.scxml:26 :: _machine"]
...
pub enum LateDataBindsOnFirstEntryState {
...
pub enum LateDataBindsOnFirstEntryEvent {
    Back,
    Bump,
    ErrorExecution,
    Go,
...
impl LateDataBindsOnFirstEntryEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [LateDataBindsOnFirstEntryEvent] = &[
...
pub struct LateDataBindsOnFirstEntryPolicy {
...
impl LateDataBindsOnFirstEntryPolicy {
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `contentSeen` was
    /// assigned a value of another type, or the engine refused.
    pub fn content_seen(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "contentSeen",
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `v` was
    /// assigned a value of another type, or the engine refused.
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) =
            se.setup_system_variables(&sid, "late_data_binds_on_first_entry", &io_processors)
        {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'contentSeen' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se,
            &sid,
            "contentSeen",
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                LateDataBindsOnFirstEntryEvent::ErrorExecution,
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                LateDataBindsOnFirstEntryEvent::ErrorExecution,
                "<data id='contentSeen'> expr failed to evaluate",
            ));
        }
...
impl StatePolicy for LateDataBindsOnFirstEntryPolicy {
    type State = LateDataBindsOnFirstEntryState;
    type Event = LateDataBindsOnFirstEntryEvent;
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            LateDataBindsOnFirstEntryState::Idle => 0,
            LateDataBindsOnFirstEntryState::S => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            LateDataBindsOnFirstEntryEvent::Back => "back",
            LateDataBindsOnFirstEntryEvent::Bump => "bump",
            LateDataBindsOnFirstEntryEvent::ErrorExecution => "error.execution",
            LateDataBindsOnFirstEntryEvent::Go => "go",
            LateDataBindsOnFirstEntryEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "back" => Some(LateDataBindsOnFirstEntryEvent::Back),
            "bump" => Some(LateDataBindsOnFirstEntryEvent::Bump),
            "error.execution" => Some(LateDataBindsOnFirstEntryEvent::ErrorExecution),
            "go" => Some(LateDataBindsOnFirstEntryEvent::Go),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            LateDataBindsOnFirstEntryState::Idle => "idle",
            LateDataBindsOnFirstEntryState::S => "s",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "idle" => Some(LateDataBindsOnFirstEntryState::Idle),
            "s" => Some(LateDataBindsOnFirstEntryState::S),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            LateDataBindsOnFirstEntryState::S => {
                ...
                if engine.claim_late_binding_first_entry(LateDataBindsOnFirstEntryState::S) {
                    self.ensure_script_engine();
                    let sid = self.session_id.as_ref().unwrap().clone();
                    let se = self.script_engine.clone();
                    let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                    // W3C SCXML 5.2/5.3: Initialize 'v' from expr (s)
                    if let Err(e) =
                        sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
                            se, &sid, "v", "1",
                        )
                    {
                        ::sce_rust_runtime::sce_log_error!("s: {}", e);
                        engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                            LateDataBindsOnFirstEntryEvent::ErrorExecution,
                    ...
                    // W3C SCXML B.2: Inline content for 'c' (s, eval with string fallback)
                    if let Err(e) = sce_rust_runtime::helpers::datamodel_init::eval_or_set_string(
                        se, &sid, "c", "7", "7",
                    ) {
                        ::sce_rust_runtime::sce_log_error!("Failed to init 'c' in s: {}", e);
                        engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                            LateDataBindsOnFirstEntryEvent::ErrorExecution,
                            "<data id='c'> content failed to initialise",
                        ));
                    }
                }
                ...
                'action_block: {
                    ...
                    {
                        // W3C SCXML 5.3: <assign location="seen">
                        self.ensure_script_engine();
                        let sid = self.session_id.as_ref().unwrap().clone();
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        let expr = "_scxml_add((seen * 10), v)";
                        // W3C SCXML 5.3: Assign via execute_script preserves Lua reference identity for
                        // table values (e.g. `Var2 = _event` — test 329 requires `Var2 == _event`). Going
                        // through evaluate_expression + set_variable would round-trip through ScriptValue
                        // and create a fresh table, breaking reference equality.
                        let assign_script = format!("{} = {}", "seen", expr);
                        if let Err(e) = se.execute_script(&sid, &assign_script) {
                            ::sce_rust_runtime::sce_log_error!("Assign failed for 'seen': {}", e);
                            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                LateDataBindsOnFirstEntryEvent::ErrorExecution,
                                "<assign> to 'seen' failed",
                            ));
                            // W3C SCXML 3.8/3.9: Error stops subsequent actions in this onentry/onexit block
                            break 'action_block;
                        }
                    ...
                    {
                        // W3C SCXML 5.3: <assign location="contentSeen">
                        self.ensure_script_engine();
                        let sid = self.session_id.as_ref().unwrap().clone();
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        ...
                        // through evaluate_expression + set_variable would round-trip through ScriptValue
                        // and create a fresh table, breaking reference equality.
                        let assign_script = format!("{} = {}", "contentSeen", expr);
                        if let Err(e) = se.execute_script(&sid, &assign_script) {
                            ::sce_rust_runtime::sce_log_error!(
                                "Assign failed for 'contentSeen': {}",
                                e
                            );
                            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                LateDataBindsOnFirstEntryEvent::ErrorExecution,
                                "<assign> to 'contentSeen' failed",
                            ));
                            // W3C SCXML 3.8/3.9: Error stops subsequent actions in this onentry/onexit block
                            break 'action_block;
                        }
    ...
    fn first_enabled_transition(
        ...
        match state {
            LateDataBindsOnFirstEntryState::Idle => {
                if event == LateDataBindsOnFirstEntryEvent::Go {
                    {
            ...
            LateDataBindsOnFirstEntryState::S => {
                if event == LateDataBindsOnFirstEntryEvent::Bump {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[],
                ...
                if event == LateDataBindsOnFirstEntryEvent::Back {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(
            ...
            LateDataBindsOnFirstEntryState::S => {
                match transition_index {
                    0 => {
                        ...
                        {
                            // W3C SCXML 5.3: <assign location="v">
                            self.ensure_script_engine();
                            let sid = self.session_id.as_ref().unwrap().clone();
                            let se = self.script_engine.clone();
                            let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                            ...
                            // through evaluate_expression + set_variable would round-trip through ScriptValue
                            // and create a fresh table, breaking reference equality.
                            let assign_script = format!("{} = {}", "v", expr);
                            if let Err(e) = se.execute_script(&sid, &assign_script) {
                                ::sce_rust_runtime::sce_log_error!("Assign failed for 'v': {}", e);
                                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                    LateDataBindsOnFirstEntryEvent::ErrorExecution,
                                    "<assign> to 'v' failed",
                                ));
