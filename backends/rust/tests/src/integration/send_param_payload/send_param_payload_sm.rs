...
//! Generated Rust state machine for `send_param_payload`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
pub enum SendParamPayloadState {
...
pub struct SendParamPayloadPolicy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<SendParamPayloadState>>,
    ...
    // W3C SCXML 6.4: Static invoke child 'inv_emitter' (send_param_payload__sce_synth_invoke__inv_emitter)
    child_inv_emitter: Option<Box<sce_rust_runtime::Engine<super::send_param_payload__sce_synth_invoke__inv_emitter_sm::SendParamPayloadSceSynthInvokeInvEmitterPolicy>>>,
    pending_done_invoke_inv_emitter: bool,
...
impl SendParamPayloadPolicy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            child_inv_emitter: None,
            pending_done_invoke_inv_emitter: false,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "send_param_payload", &io_processors) {
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains(".inv_emitter") {
                ...
                let mut child_policy = super::send_param_payload__sce_synth_invoke__inv_emitter_sm::SendParamPayloadSceSynthInvokeInvEmitterPolicy::new();
                ...
                self.child_inv_emitter = Some(Box::new(child_engine));
                ...
                {
                    self.pending_done_invoke_inv_emitter = true;
    ...
    fn do_tick_children(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        if !self.pending_done_invoke_inv_emitter {
            ...
            if let Some(mut child) = self.child_inv_emitter.take() {
                ...
                if child.is_in_final_state() && !self.pending_done_invoke_inv_emitter {
                    self.pending_done_invoke_inv_emitter = true;
                    // W3C SCXML 5.5 + 6.3.1: Lift the child's stashed donedata onto
...
impl StatePolicy for SendParamPayloadPolicy {
    type State = SendParamPayloadState;
    type Event = SendParamPayloadEvent;
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            ...
            "failSiblingParamLost" => Some(SendParamPayloadState::FailSiblingParamLost),
            "failStringType" => Some(SendParamPayloadState::FailStringType),
            "internalPhase" => Some(SendParamPayloadState::InternalPhase),
            "paramErrorPhase" => Some(SendParamPayloadState::ParamErrorPhase),
            "pass" => Some(SendParamPayloadState::Pass),
            "typedPhase" => Some(SendParamPayloadState::TypedPhase),
            _ => None,
        }
    }
        ...
        match state {
            SendParamPayloadState::AwaitChild => {
                ...
                if self.child_inv_emitter.is_some() {
                    ...
                    self.child_inv_emitter = None;
                }
                self.active_invokes.remove("inv_emitter");
                self.pending_done_invoke_inv_emitter = false;
            }
            _ => {}
        }
        match state {
...
impl SendParamPayloadPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: SendParamPayloadState,
        event: SendParamPayloadEvent,
        current_state: &mut SendParamPayloadState,
        ...
        match check_state {
            SendParamPayloadState::AwaitChild => {
                ...
                if event == SendParamPayloadEvent::FromChild {
                    ...
                    if self.safe_evaluate_guard(
                        ...
                        *current_state = SendParamPayloadState::InternalPhase;
                ...
                if event == SendParamPayloadEvent::FromChild {
                    ...
                    *current_state = SendParamPayloadState::FailChildPayload;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            SendParamPayloadState::InternalPhase => {
                ...
                if event == SendParamPayloadEvent::Loopback {
                    ...
                    if self.safe_evaluate_guard(
                        ...
                        *current_state = SendParamPayloadState::TypedPhase;
                        *transition_taken = true;
                        return true;
                    }
                }
            ...
            SendParamPayloadState::Pass => false,
            SendParamPayloadState::TypedPhase => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                ...
                if event == SendParamPayloadEvent::Typed {
                    ...
                    *current_state = SendParamPayloadState::FailDuplicateParams;
                    *transition_taken = true;
                    return true;
                }
                false
