...
//! Generated Rust state machine for `other`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: other.scxml:6 :: _machine"]
...
impl OtherEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [OtherEvent] = &[];
}
...
pub struct OtherPolicy {
    ...
    last_transition_source_state: OtherState,
...
impl OtherPolicy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: OtherState::Speak,
            session_id: None,
...
impl StatePolicy for OtherPolicy {
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            OtherEvent::ErrorExecution => "error.execution",
            OtherEvent::FromOther => "from.other",
            OtherEvent::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(OtherEvent::ErrorExecution),
            "from.other" => Some(OtherEvent::FromOther),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            OtherState::Done => "done",
            OtherState::Speak => "speak",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "done" => Some(OtherState::Done),
            "speak" => Some(OtherState::Speak),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        if let Some(ref parent_queue) = self.parent_external_queue {
                            if let Ok(mut q) = parent_queue.lock() {
                                q.push(("from.other".to_string(), event_data.to_string()));
                            }
                        } else {
                        }
...
impl OtherPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: OtherState,
        event: OtherEvent,
        current_state: &mut OtherState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            OtherState::Done => false,
            OtherState::Speak => {
