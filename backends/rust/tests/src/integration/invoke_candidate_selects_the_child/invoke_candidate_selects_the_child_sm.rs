...
// From: integration_resources/invoke_candidate_selects_the_child/invoke_candidate_selects_the_child.scxml
//
...
//! Generated Rust state machine for `invoke_candidate_selects_the_child`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: invoke_candidate_selects_the_child.scxml:34 :: _machine"]
...
pub enum InvokeCandidateSelectsTheChildState {
    NoChild,
    Pass,
...
pub enum InvokeCandidateSelectsTheChildEvent {
    ErrorExecution,
    FromChosen,
    FromOther,
...
impl InvokeCandidateSelectsTheChildEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [InvokeCandidateSelectsTheChildEvent] = &[
        InvokeCandidateSelectsTheChildEvent::FromChosen,
        InvokeCandidateSelectsTheChildEvent::FromOther,
    ];
}
...
pub struct InvokeCandidateSelectsTheChildPolicy {
    // W3C SCXML 3.13: Last transition metadata
    last_transition_is_internal: bool,
    last_transition_is_targetless: bool,
    last_transition_source_state: InvokeCandidateSelectsTheChildState,
    ...
    pending_invokes:
        Vec<sce_rust_runtime::invoke::PendingInvoke<InvokeCandidateSelectsTheChildState>>,
    ...
    // candidate for '_invoke_0', at most one ever filled.
    child_invoke_0_chosen: Option<Box<sce_rust_runtime::Engine<super::chosen_sm::ChosenPolicy>>>,
    child_invoke_0_other: Option<Box<sce_rust_runtime::Engine<super::other_sm::OtherPolicy>>>,
...
impl InvokeCandidateSelectsTheChildPolicy {
    ...
    /// answer — the session is not initialized yet, `pick` was
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: InvokeCandidateSelectsTheChildState::Probe,
            ...
            child_invoke_0_chosen: None,
            child_invoke_0_other: None,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) =
            se.setup_system_variables(&sid, "invoke_candidate_selects_the_child", &io_processors)
        {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'pick' from expr (global)
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                ...
                "<data id='pick'> expr failed to evaluate",
            ));
        }
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut __sce_selected = String::new();
                // W3C SCXML 6.4.3: Evaluate srcexpr at runtime
                self.ensure_script_engine();
                let sid = self.session_id.as_ref().unwrap().clone();
                let se = self.script_engine.clone();
                let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                match se.evaluate_expression(&sid, "pick") {
                    Ok(sce_rust_runtime::ScriptValue::Null) | Err(_) => {
                        ::sce_rust_runtime::sce_log_error!(
                            "Hybrid invoke: srcexpr '{}' evaluation failed",
                            "pick"
                        );
                        engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                            InvokeCandidateSelectsTheChildEvent::ErrorExecution,
                            "<invoke srcexpr='pick'> could not be evaluated",
                        ));
                        continue;
                    }
                    Ok(_path) => {
                        ::sce_rust_runtime::sce_log_debug!(
                            "Hybrid invoke: srcexpr evaluated to {:?}",
                            _path
                        );
                        // §scxml-6.4 + SCE_ACCEPTED_SUBSET.md §2.13: the value
                        // chooses among the declared candidates, matched on
                        // the document STEM so `file:x.scxml`, `./x.scxml` and
                        // an absolute path all name one child.
                        let raw = match &_path {
                ...
                // §scxml-6.4 + SCE_ACCEPTED_SUBSET.md §2.13: the value chooses
                // among the declared candidates, each with a slot of its own.
                if __sce_selected == "chosen" {
                    ...
                    let mut child_policy = super::chosen_sm::ChosenPolicy::new();
                    ...
                    if child_policy.session_id.is_none() {
                        static HYBRID_SESSION_COUNTER: core::sync::atomic::AtomicU64 =
                            core::sync::atomic::AtomicU64::new(0);
                        let cid = HYBRID_SESSION_COUNTER
                    ...
                    self.child_invoke_0_chosen = Some(Box::new(child_engine));
                ...
                if __sce_selected == "other" {
                    // W3C SCXML 6.4: Create pre-generated child (Rust AOT: child resolved at codegen time)
                    // Engine DI Parity RFC: pass parent's script engine when child needs it.
                    let mut child_policy = super::other_sm::OtherPolicy::new();
                    child_policy.parent_external_queue = Some(engine.get_external_queue_handle());
                    child_policy.invoke_id = pending.invoke_id.clone();
                    child_policy.child_session_id = child_session_id.clone();
                    ...
                    self.child_invoke_0_other = Some(Box::new(child_engine));

                    // W3C SCXML 6.4: Drain child-to-parent events raised during initialize
                    sce_rust_runtime::helpers::invoke_processing::drain_and_raise_child_events(
                        &self
                            .child_invoke_0_other
                            .as_ref()
                            .unwrap()
                            .policy()
                            .parent_external_queue,
    ...
    fn do_tick_children(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        if !self.pending_done_invoke_invoke_0 {
            if let Some(mut child) = self.child_invoke_0_chosen.take() {
                sce_rust_runtime::helpers::invoke_processing::drain_and_raise_child_events(
                ...
                self.child_invoke_0_chosen = Some(child);
            }
        }

        if !self.pending_done_invoke_invoke_0 {
            if let Some(mut child) = self.child_invoke_0_other.take() {
                sce_rust_runtime::helpers::invoke_processing::drain_and_raise_child_events(
                    &child.policy().parent_external_queue,
                    &self.active_invokes,
                    "_invoke_0",
                ...
                self.child_invoke_0_other = Some(child);
            }
        }
    }
}
...
impl StatePolicy for InvokeCandidateSelectsTheChildPolicy {
    type State = InvokeCandidateSelectsTheChildState;
    type Event = InvokeCandidateSelectsTheChildEvent;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            InvokeCandidateSelectsTheChildState::NoChild => true,
            InvokeCandidateSelectsTheChildState::Pass => true,
            InvokeCandidateSelectsTheChildState::WrongChild => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            InvokeCandidateSelectsTheChildState::NoChild => 3,
            InvokeCandidateSelectsTheChildState::Pass => 1,
            InvokeCandidateSelectsTheChildState::Probe => 0,
            InvokeCandidateSelectsTheChildState::WrongChild => 2,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            InvokeCandidateSelectsTheChildEvent::ErrorExecution => "error.execution",
            InvokeCandidateSelectsTheChildEvent::FromChosen => "from.chosen",
            InvokeCandidateSelectsTheChildEvent::FromOther => "from.other",
            InvokeCandidateSelectsTheChildEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(InvokeCandidateSelectsTheChildEvent::ErrorExecution),
            "from.chosen" => Some(InvokeCandidateSelectsTheChildEvent::FromChosen),
            "from.other" => Some(InvokeCandidateSelectsTheChildEvent::FromOther),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            InvokeCandidateSelectsTheChildState::NoChild => "noChild",
            InvokeCandidateSelectsTheChildState::Pass => "pass",
            InvokeCandidateSelectsTheChildState::Probe => "probe",
            InvokeCandidateSelectsTheChildState::WrongChild => "wrongChild",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "noChild" => Some(InvokeCandidateSelectsTheChildState::NoChild),
            "pass" => Some(InvokeCandidateSelectsTheChildState::Pass),
            "probe" => Some(InvokeCandidateSelectsTheChildState::Probe),
            "wrongChild" => Some(InvokeCandidateSelectsTheChildState::WrongChild),
            _ => None,
        }
    }
    ...
    fn execute_exit_actions(
        ...
        match state {
            InvokeCandidateSelectsTheChildState::Probe => {
                ...
                if self.child_invoke_0_chosen.is_some() {
                    self.child_invoke_0_chosen = None;
                }
                if self.child_invoke_0_other.is_some() {
                    self.child_invoke_0_other = None;
                }
...
impl InvokeCandidateSelectsTheChildPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: InvokeCandidateSelectsTheChildState,
        event: InvokeCandidateSelectsTheChildEvent,
        current_state: &mut InvokeCandidateSelectsTheChildState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            InvokeCandidateSelectsTheChildState::NoChild => false,
            InvokeCandidateSelectsTheChildState::Pass => false,
            InvokeCandidateSelectsTheChildState::Probe => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InvokeCandidateSelectsTheChildEvent::FromChosen {
                    ...
                    *current_state = InvokeCandidateSelectsTheChildState::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InvokeCandidateSelectsTheChildEvent::FromOther {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = InvokeCandidateSelectsTheChildState::WrongChild;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InvokeCandidateSelectsTheChildEvent::ErrorExecution {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = InvokeCandidateSelectsTheChildState::NoChild;
                    *transition_taken = true;
                    return true;
                }
                false
            }
            InvokeCandidateSelectsTheChildState::WrongChild => false,
            _ => false,
        }
    }
}
