...
//! Generated Rust state machine for `chosen`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: chosen.scxml:5 :: _machine"]
...
impl ChosenEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [ChosenEvent] = &[];
}
...
pub struct ChosenPolicy {
    ...
    last_transition_source_state: ChosenState,
...
impl ChosenPolicy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: ChosenState::Speak,
            session_id: None,
...
impl StatePolicy for ChosenPolicy {
    type State = ChosenState;
    type Event = ChosenEvent;
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            ChosenState::Done => "done",
            ChosenState::Speak => "speak",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "done" => Some(ChosenState::Done),
            "speak" => Some(ChosenState::Speak),
            _ => None,
        }
    }
...
impl ChosenPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: ChosenState,
        event: ChosenEvent,
        current_state: &mut ChosenState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            ChosenState::Done => false,
            ChosenState::Speak => {
                // W3C SCXML 3.13: Eventless transitions
                if event == ChosenEvent::Null {
                    ...
                    *current_state = ChosenState::Done;
