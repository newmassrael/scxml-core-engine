...
//! Generated Rust state machine for `internal_chain_is_bounded`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: internal_chain_is_bounded.scxml:90 :: _machine"]
...
pub enum InternalChainIsBoundedState {
    Alt,
    Bounded,
...
impl InternalChainIsBoundedEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [InternalChainIsBoundedEvent] = &[
        InternalChainIsBoundedEvent::Alternate,
        InternalChainIsBoundedEvent::Bounded,
...
impl InternalChainIsBoundedPolicy {
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `links` was
    /// assigned a value of another type, or the engine refused.
    pub fn links(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "links",
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `beats` was
    /// assigned a value of another type, or the engine refused.
    pub fn beats(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "beats",
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `alts` was
    /// assigned a value of another type, or the engine refused.
    pub fn alts(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "alts",
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `pending` was
    /// assigned a value of another type, or the engine refused.
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: InternalChainIsBoundedState::Idle,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "internal_chain_is_bounded", &io_processors)
        {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'links' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "links", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
        ...
        // W3C SCXML 5.2/5.3: Initialize 'beats' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "beats", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
        ...
        // W3C SCXML 5.2/5.3: Initialize 'alts' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "alts", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                InternalChainIsBoundedEvent::ErrorExecution,
                "<data id='links'> expr failed to evaluate",
            ));
        }
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                InternalChainIsBoundedEvent::ErrorExecution,
                "<data id='beats'> expr failed to evaluate",
            ));
        }
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                InternalChainIsBoundedEvent::ErrorExecution,
                "<data id='pending'> expr failed to evaluate",
            ));
        }
...
impl StatePolicy for InternalChainIsBoundedPolicy {
    type State = InternalChainIsBoundedState;
    type Event = InternalChainIsBoundedEvent;
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            ...
            InternalChainIsBoundedEvent::Poke => "poke",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            ...
            "poke" => Some(InternalChainIsBoundedEvent::Poke),
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "alt" => Some(InternalChainIsBoundedState::Alt),
            "bounded" => Some(InternalChainIsBoundedState::Bounded),
            "idle" => Some(InternalChainIsBoundedState::Idle),
            "ignoring" => Some(InternalChainIsBoundedState::Ignoring),
            "resuming" => Some(InternalChainIsBoundedState::Resuming),
            "spin" => Some(InternalChainIsBoundedState::Spin),
            _ => None,
        }
    }
...
impl InternalChainIsBoundedPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: InternalChainIsBoundedState,
        event: InternalChainIsBoundedEvent,
        current_state: &mut InternalChainIsBoundedState,
        ...
        match check_state {
            InternalChainIsBoundedState::Alt => {
                ...
                if event == InternalChainIsBoundedEvent::Tick {
                ...
                if event == InternalChainIsBoundedEvent::Null {
                    if self.safe_evaluate_guard("_scxml_eq(pending, 1)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
            ...
            InternalChainIsBoundedState::Bounded => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InternalChainIsBoundedEvent::Link {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("(laps < 999)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        self.has_transition_actions = true;
            ...
            InternalChainIsBoundedState::Idle => {
                ...
                if event == InternalChainIsBoundedEvent::Poke {
                    ...
                    *current_state = InternalChainIsBoundedState::Idle;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InternalChainIsBoundedEvent::Bounded {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 1;
                    self.has_transition_actions = true;
                    ...
                    *current_state = InternalChainIsBoundedState::Bounded;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InternalChainIsBoundedEvent::Spin {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 2;
                    self.has_transition_actions = true;
                    ...
                    *current_state = InternalChainIsBoundedState::Spin;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InternalChainIsBoundedEvent::Resume {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    ...
                    *current_state = InternalChainIsBoundedState::Resuming;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InternalChainIsBoundedEvent::Alternate {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
            ...
            InternalChainIsBoundedState::Resuming => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == InternalChainIsBoundedEvent::Beat {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("(beats < 1499)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        self.has_transition_actions = true;
