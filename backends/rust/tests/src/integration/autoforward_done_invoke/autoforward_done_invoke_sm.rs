...
//! Generated Rust state machine for `autoforward_done_invoke`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: autoforward_done_invoke.scxml:55 :: _machine"]
...
pub enum AutoforwardDoneInvokeState {
    Fail,
    Pass,
...
impl AutoforwardDoneInvokeEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [AutoforwardDoneInvokeEvent] = &[
        AutoforwardDoneInvokeEvent::SawPlatform,
        AutoforwardDoneInvokeEvent::SawProbeOnly,
    ];
}
...
pub struct AutoforwardDoneInvokePolicy {
    ...
    // W3C SCXML 6.4: Static invoke child 'inv_watch' (autoforward_done_invoke__sce_synth_invoke__inv_watch)
    child_inv_watch: Option<Box<sce_rust_runtime::Engine<super::autoforward_done_invoke__sce_synth_invoke__inv_watch_sm::AutoforwardDoneInvokeSceSynthInvokeInvWatchPolicy>>>,
    pending_done_invoke_inv_watch: bool,
    // W3C SCXML 6.4: Static invoke child 'inv_short' (autoforward_done_invoke__sce_synth_invoke__inv_short)
    child_inv_short: Option<Box<sce_rust_runtime::Engine<super::autoforward_done_invoke__sce_synth_invoke__inv_short_sm::AutoforwardDoneInvokeSceSynthInvokeInvShortPolicy>>>,
    pending_done_invoke_inv_short: bool,
...
impl AutoforwardDoneInvokePolicy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: AutoforwardDoneInvokeState::Phase,
            ...
            child_inv_short: None,
            pending_done_invoke_inv_short: false,
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains(".inv_watch") {
                ...
                let mut child_policy = super::autoforward_done_invoke__sce_synth_invoke__inv_watch_sm::AutoforwardDoneInvokeSceSynthInvokeInvWatchPolicy::new();
            ...
            if pending.invoke_id.contains(".inv_short") {
                ...
                // Engine DI Parity RFC: forward parent's script engine to child when the
                // child Policy was generated with `model.needs_script_engine = true`.
                let mut child_policy = super::autoforward_done_invoke__sce_synth_invoke__inv_short_sm::AutoforwardDoneInvokeSceSynthInvokeInvShortPolicy::new();
                // W3C SCXML 6.4: Pass parent session info to child for #_parent routing
                child_policy.parent_external_queue = Some(engine.get_external_queue_handle());
                child_policy.invoke_id = pending.invoke_id.clone();
                child_policy.child_session_id = child_session_id.clone();
                ...
                // W3C SCXML 6.4: Store child BEFORE draining events so autoforward can find it
                self.child_inv_short = Some(Box::new(child_engine));
                ...
                if self
                    ...
                    .map_or(false, |c| c.is_in_final_state())
                {
                    self.pending_done_invoke_inv_short = true;
                    // W3C SCXML 5.5 + 6.3.1: Lift the child's stashed donedata onto
                    // done.invoke.<id>._event.data. Mirrors the C++ AOT contract in
                    // tools/codegen/templates/invoke_methods.jinja2 (child->donedataAtFinal()
                    // + EventMetadataHelper::createDoneInvokeEvent).
    ...
    fn do_tick_children(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        if !self.pending_done_invoke_inv_short {
            ...
            if let Some(mut child) = self.child_inv_short.take() {
                ...
                // W3C SCXML 6.4: Check if child reached final state after tick
                if child.is_in_final_state() && !self.pending_done_invoke_inv_short {
                    self.pending_done_invoke_inv_short = true;
                    // W3C SCXML 5.5 + 6.3.1: Lift the child's stashed donedata onto
                    // done.invoke.<id>._event.data.
                    let donedata = child.donedata_at_final().to_string();
                ...
                // Put child back
                self.child_inv_short = Some(child);
            }
        }
    }
    ...
    fn deliver_to_child_session(
        ...
        }
        {
            let addressed = self
                ...
                .map_or(false, |cs| cs.session_id == child_session_id);
            if addressed {
                if let Some(ref mut child) = self.child_inv_short {
                    child.raise_external_by_name(event_name, event_data);
                    return true;
                }
            }
...
impl Default for AutoforwardDoneInvokePolicy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for AutoforwardDoneInvokePolicy {
    type State = AutoforwardDoneInvokeState;
    type Event = AutoforwardDoneInvokeEvent;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            AutoforwardDoneInvokeState::Fail => true,
            AutoforwardDoneInvokeState::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            AutoforwardDoneInvokeState::Fail => 2,
            AutoforwardDoneInvokeState::Pass => 1,
            AutoforwardDoneInvokeState::Phase => 0,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            ...
            AutoforwardDoneInvokeEvent::Probe => "probe",
            AutoforwardDoneInvokeEvent::SawPlatform => "sawPlatform",
            AutoforwardDoneInvokeEvent::SawProbeOnly => "sawProbeOnly",
            AutoforwardDoneInvokeEvent::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            ...
            "probe" => Some(AutoforwardDoneInvokeEvent::Probe),
            "sawPlatform" => Some(AutoforwardDoneInvokeEvent::SawPlatform),
            "sawProbeOnly" => Some(AutoforwardDoneInvokeEvent::SawProbeOnly),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            AutoforwardDoneInvokeState::Fail => "fail",
            AutoforwardDoneInvokeState::Pass => "pass",
            AutoforwardDoneInvokeState::Phase => "phase",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(AutoforwardDoneInvokeState::Fail),
            "pass" => Some(AutoforwardDoneInvokeState::Pass),
            "phase" => Some(AutoforwardDoneInvokeState::Phase),
            _ => None,
        }
    }
...
impl AutoforwardDoneInvokePolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: AutoforwardDoneInvokeState,
        event: AutoforwardDoneInvokeEvent,
        current_state: &mut AutoforwardDoneInvokeState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            AutoforwardDoneInvokeState::Fail => false,
            AutoforwardDoneInvokeState::Pass => false,
            AutoforwardDoneInvokeState::Phase => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == AutoforwardDoneInvokeEvent::DoneInvokeInvShort {
                ...
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == AutoforwardDoneInvokeEvent::SawPlatform {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    ...
                    *current_state = AutoforwardDoneInvokeState::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == AutoforwardDoneInvokeEvent::SawProbeOnly {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    ...
                    *current_state = AutoforwardDoneInvokeState::Fail;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == AutoforwardDoneInvokeEvent::ErrorExecution {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
