...
//! Generated Rust state machine for `parallel_regions_take_own_transitions`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: parallel_regions_take_own_transitions.scxml:24 :: _machine"]
...
pub enum ParallelRegionsTakeOwnTransitionsState {
    Budget,
    Drive,
    Judging,
...
pub enum ParallelRegionsTakeOwnTransitionsEvent {
    Check,
    E,
    ErrorExecution,
...
impl ParallelRegionsTakeOwnTransitionsEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [ParallelRegionsTakeOwnTransitionsEvent] = &[
        ParallelRegionsTakeOwnTransitionsEvent::Check,
        ParallelRegionsTakeOwnTransitionsEvent::E,
    ];
}
...
pub struct ParallelRegionsTakeOwnTransitionsPolicy {
    // W3C SCXML 3.13: Last transition metadata
    last_transition_is_internal: bool,
    last_transition_is_targetless: bool,
    last_transition_source_state: ParallelRegionsTakeOwnTransitionsState,
    ...
    active_states:
        ::sce_rust_runtime::helpers::hierarchy::StateChain<ParallelRegionsTakeOwnTransitionsState>,
...
impl ParallelRegionsTakeOwnTransitionsPolicy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: ParallelRegionsTakeOwnTransitionsState::Working,
    ...
    fn state_name_matches(state: ParallelRegionsTakeOwnTransitionsState, name: &str) -> bool {
...
impl StatePolicy for ParallelRegionsTakeOwnTransitionsPolicy {
    type State = ParallelRegionsTakeOwnTransitionsState;
    type Event = ParallelRegionsTakeOwnTransitionsEvent;
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            ParallelRegionsTakeOwnTransitionsState::Budget => {
                Some(ParallelRegionsTakeOwnTransitionsState::Run)
            }
    ...
    fn is_compound_state(state: Self::State) -> bool {
        match state {
            ParallelRegionsTakeOwnTransitionsState::Budget => true,
            ParallelRegionsTakeOwnTransitionsState::Drive => true,
            ParallelRegionsTakeOwnTransitionsState::Running => true,
            _ => false,
        }
    }
    ...
    fn is_parallel_state(state: Self::State) -> bool {
        match state {
            ParallelRegionsTakeOwnTransitionsState::Run => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            ParallelRegionsTakeOwnTransitionsEvent::Check => "check",
            ParallelRegionsTakeOwnTransitionsEvent::E => "e",
            ParallelRegionsTakeOwnTransitionsEvent::ErrorExecution => "error.execution",
            ParallelRegionsTakeOwnTransitionsEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "check" => Some(ParallelRegionsTakeOwnTransitionsEvent::Check),
            "e" => Some(ParallelRegionsTakeOwnTransitionsEvent::E),
            "error.execution" => Some(ParallelRegionsTakeOwnTransitionsEvent::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            ParallelRegionsTakeOwnTransitionsState::Budget => "budget",
            ParallelRegionsTakeOwnTransitionsState::Drive => "drive",
            ParallelRegionsTakeOwnTransitionsState::Judging => "judging",
            ParallelRegionsTakeOwnTransitionsState::Run => "run",
            ParallelRegionsTakeOwnTransitionsState::Running => "running",
            ParallelRegionsTakeOwnTransitionsState::Settled => "settled",
            ParallelRegionsTakeOwnTransitionsState::Within => "within",
            ParallelRegionsTakeOwnTransitionsState::Working => "working",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "budget" => Some(ParallelRegionsTakeOwnTransitionsState::Budget),
            "drive" => Some(ParallelRegionsTakeOwnTransitionsState::Drive),
            "judging" => Some(ParallelRegionsTakeOwnTransitionsState::Judging),
            "run" => Some(ParallelRegionsTakeOwnTransitionsState::Run),
            "running" => Some(ParallelRegionsTakeOwnTransitionsState::Running),
            "settled" => Some(ParallelRegionsTakeOwnTransitionsState::Settled),
            "within" => Some(ParallelRegionsTakeOwnTransitionsState::Within),
            "working" => Some(ParallelRegionsTakeOwnTransitionsState::Working),
            _ => None,
        }
    }
    ...
    fn process_transition(
        ...
        if event == Self::null_event() {
            ...
            let mut seen: ::sce_rust_runtime::SceDedupSet<(
                ParallelRegionsTakeOwnTransitionsState,
                usize,
...
impl ParallelRegionsTakeOwnTransitionsPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: ParallelRegionsTakeOwnTransitionsState,
        event: ParallelRegionsTakeOwnTransitionsEvent,
        current_state: &mut ParallelRegionsTakeOwnTransitionsState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
        mut collect_mode: Option<&mut TransitionList>,
    ) -> bool {
        match check_state {
            ParallelRegionsTakeOwnTransitionsState::Budget => false,
            ParallelRegionsTakeOwnTransitionsState::Drive => false,
            ParallelRegionsTakeOwnTransitionsState::Judging => {
                    ...
                    if self.safe_evaluate_guard("(_scxml_eq(n, 1) and _scxml_eq(m, 1))", engine) {
                        ...
                        if let Some(ref mut collect) = collect_mode {
                            collect.push_bounded(TransitionInfo {
                                source: check_state,
                                target: ParallelRegionsTakeOwnTransitionsState::Settled,
                                transition_index: 0,
                                has_actions: false,
                                is_internal: false,
                                is_targetless: false,
                            ...
                            *current_state = ParallelRegionsTakeOwnTransitionsState::Settled;
            ...
            ParallelRegionsTakeOwnTransitionsState::Run => false,
            ParallelRegionsTakeOwnTransitionsState::Running => false,
            ParallelRegionsTakeOwnTransitionsState::Settled => false,
            ParallelRegionsTakeOwnTransitionsState::Within => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == ParallelRegionsTakeOwnTransitionsEvent::E {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 0;
                    ...
                    if let Some(ref mut collect) = collect_mode {
                        collect.push_bounded(TransitionInfo {
                            source: check_state,
                            target: ParallelRegionsTakeOwnTransitionsState::Within,
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                            is_targetless: false,
                        ...
                        *current_state = ParallelRegionsTakeOwnTransitionsState::Within;
                        *transition_taken = true;
                    }
                    return true;
                }
            ...
            ParallelRegionsTakeOwnTransitionsState::Working => {
                ...
                if event == ParallelRegionsTakeOwnTransitionsEvent::E {
                    ...
                    if let Some(ref mut collect) = collect_mode {
                        collect.push_bounded(TransitionInfo {
                            source: check_state,
                            target: ParallelRegionsTakeOwnTransitionsState::Judging,
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                            is_targetless: false,
                        ...
                        *current_state = ParallelRegionsTakeOwnTransitionsState::Judging;
                        *transition_taken = true;
                    }
                    return true;
                }
    ...
    fn transition_domain(
        ...
        target: ParallelRegionsTakeOwnTransitionsState,
        is_internal: bool,
    ) -> Option<ParallelRegionsTakeOwnTransitionsState> {
