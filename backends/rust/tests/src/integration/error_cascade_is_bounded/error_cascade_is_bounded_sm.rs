...
//! Generated Rust state machine for `error_cascade_is_bounded`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: error_cascade_is_bounded.scxml:45 :: _machine"]
...
pub enum ErrorCascadeIsBoundedState {
...
pub enum ErrorCascadeIsBoundedEvent {
    Boom,
    ErrorExecution,
    Poke,
    Reset,
...
impl ErrorCascadeIsBoundedEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [ErrorCascadeIsBoundedEvent] = &[
        ErrorCascadeIsBoundedEvent::Boom,
        ErrorCascadeIsBoundedEvent::Poke,
        ErrorCascadeIsBoundedEvent::Reset,
        ErrorCascadeIsBoundedEvent::Settle,
        ErrorCascadeIsBoundedEvent::Spin,
    ];
}
...
impl ErrorCascadeIsBoundedPolicy {
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `repairs` was
    /// assigned a value of another type, or the engine refused.
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `runs` was
    /// assigned a value of another type, or the engine refused.
    pub fn runs(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "runs",
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: ErrorCascadeIsBoundedState::Idle,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "error_cascade_is_bounded", &io_processors)
        {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'repairs' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "repairs", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
        ...
        // W3C SCXML 5.2/5.3: Initialize 'runs' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "runs", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                ErrorCascadeIsBoundedEvent::ErrorExecution,
                "<data id='repairs'> expr failed to evaluate",
            ));
        }
...
impl StatePolicy for ErrorCascadeIsBoundedPolicy {
    type State = ErrorCascadeIsBoundedState;
    type Event = ErrorCascadeIsBoundedEvent;
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            ErrorCascadeIsBoundedState::Idle => 0,
            ErrorCascadeIsBoundedState::Runaway => 2,
            ErrorCascadeIsBoundedState::Settling => 1,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            ErrorCascadeIsBoundedEvent::Boom => "boom",
            ErrorCascadeIsBoundedEvent::ErrorExecution => "error.execution",
            ErrorCascadeIsBoundedEvent::Poke => "poke",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "boom" => Some(ErrorCascadeIsBoundedEvent::Boom),
            "error.execution" => Some(ErrorCascadeIsBoundedEvent::ErrorExecution),
            "poke" => Some(ErrorCascadeIsBoundedEvent::Poke),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            ErrorCascadeIsBoundedState::Idle => "idle",
            ErrorCascadeIsBoundedState::Runaway => "runaway",
            ErrorCascadeIsBoundedState::Settling => "settling",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "idle" => Some(ErrorCascadeIsBoundedState::Idle),
            "runaway" => Some(ErrorCascadeIsBoundedState::Runaway),
            "settling" => Some(ErrorCascadeIsBoundedState::Settling),
            _ => None,
        }
    }
...
impl ErrorCascadeIsBoundedPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: ErrorCascadeIsBoundedState,
        event: ErrorCascadeIsBoundedEvent,
        current_state: &mut ErrorCascadeIsBoundedState,
        ...
        match check_state {
            ErrorCascadeIsBoundedState::Idle => {
                ...
                if event == ErrorCascadeIsBoundedEvent::Poke {
                    ...
                    *current_state = ErrorCascadeIsBoundedState::Idle;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == ErrorCascadeIsBoundedEvent::Boom {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                ...
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == ErrorCascadeIsBoundedEvent::Settle {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    ...
                    *current_state = ErrorCascadeIsBoundedState::Settling;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == ErrorCascadeIsBoundedEvent::Spin {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    ...
                    *current_state = ErrorCascadeIsBoundedState::Runaway;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            ErrorCascadeIsBoundedState::Runaway => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == ErrorCascadeIsBoundedEvent::ErrorExecution {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 0;
                    self.has_transition_actions = true;
            ...
            ErrorCascadeIsBoundedState::Settling => {
                ...
                if event == ErrorCascadeIsBoundedEvent::ErrorExecution {
                    ...
                    if self.safe_evaluate_guard("(repairs < 3)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        self.has_transition_actions = true;
