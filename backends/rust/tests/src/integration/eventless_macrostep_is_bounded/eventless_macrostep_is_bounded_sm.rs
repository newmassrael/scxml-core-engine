...
//! Generated Rust state machine for `eventless_macrostep_is_bounded`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
pub enum EventlessMacrostepIsBoundedState {
    BoundedA,
    BoundedB,
...
pub enum EventlessMacrostepIsBoundedEvent {
    Bounded,
    ErrorExecution,
    Poke,
    Reset,
...
impl EventlessMacrostepIsBoundedEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [EventlessMacrostepIsBoundedEvent] = &[
        EventlessMacrostepIsBoundedEvent::Bounded,
        EventlessMacrostepIsBoundedEvent::Poke,
        EventlessMacrostepIsBoundedEvent::Reset,
        EventlessMacrostepIsBoundedEvent::Spin,
    ];
}
...
pub struct EventlessMacrostepIsBoundedPolicy {
    // W3C SCXML 3.13: Last transition metadata
    last_transition_is_internal: bool,
    last_transition_is_targetless: bool,
    last_transition_source_state: EventlessMacrostepIsBoundedState,
...
impl EventlessMacrostepIsBoundedPolicy {
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `laps` was
    /// assigned a value of another type, or the engine refused.
    pub fn laps(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "laps",
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `spins` was
    /// assigned a value of another type, or the engine refused.
    pub fn spins(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "spins",
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: EventlessMacrostepIsBoundedState::Idle,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) =
            se.setup_system_variables(&sid, "eventless_macrostep_is_bounded", &io_processors)
        {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'laps' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "laps", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
        ...
        // W3C SCXML 5.2/5.3: Initialize 'spins' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "spins", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                EventlessMacrostepIsBoundedEvent::ErrorExecution,
                "<data id='laps'> expr failed to evaluate",
            ));
        }
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                EventlessMacrostepIsBoundedEvent::ErrorExecution,
                "<data id='spins'> expr failed to evaluate",
            ));
        }
...
impl StatePolicy for EventlessMacrostepIsBoundedPolicy {
    type State = EventlessMacrostepIsBoundedState;
    type Event = EventlessMacrostepIsBoundedEvent;
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            EventlessMacrostepIsBoundedState::BoundedA => 1,
            EventlessMacrostepIsBoundedState::BoundedB => 2,
            EventlessMacrostepIsBoundedState::Idle => 0,
            EventlessMacrostepIsBoundedState::SpinA => 3,
            EventlessMacrostepIsBoundedState::SpinB => 4,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            EventlessMacrostepIsBoundedEvent::Bounded => "bounded",
            EventlessMacrostepIsBoundedEvent::ErrorExecution => "error.execution",
            EventlessMacrostepIsBoundedEvent::Poke => "poke",
            EventlessMacrostepIsBoundedEvent::Reset => "reset",
            EventlessMacrostepIsBoundedEvent::Spin => "spin",
            EventlessMacrostepIsBoundedEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "bounded" => Some(EventlessMacrostepIsBoundedEvent::Bounded),
            "error.execution" => Some(EventlessMacrostepIsBoundedEvent::ErrorExecution),
            "poke" => Some(EventlessMacrostepIsBoundedEvent::Poke),
            "reset" => Some(EventlessMacrostepIsBoundedEvent::Reset),
            "spin" => Some(EventlessMacrostepIsBoundedEvent::Spin),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            EventlessMacrostepIsBoundedState::BoundedA => "bounded_a",
            EventlessMacrostepIsBoundedState::BoundedB => "bounded_b",
            EventlessMacrostepIsBoundedState::Idle => "idle",
            EventlessMacrostepIsBoundedState::SpinA => "spin_a",
            EventlessMacrostepIsBoundedState::SpinB => "spin_b",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "bounded_a" => Some(EventlessMacrostepIsBoundedState::BoundedA),
            "bounded_b" => Some(EventlessMacrostepIsBoundedState::BoundedB),
            "idle" => Some(EventlessMacrostepIsBoundedState::Idle),
            "spin_a" => Some(EventlessMacrostepIsBoundedState::SpinA),
            "spin_b" => Some(EventlessMacrostepIsBoundedState::SpinB),
            _ => None,
        }
    }
...
impl EventlessMacrostepIsBoundedPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: EventlessMacrostepIsBoundedState,
        event: EventlessMacrostepIsBoundedEvent,
        current_state: &mut EventlessMacrostepIsBoundedState,
        ...
        match check_state {
            EventlessMacrostepIsBoundedState::BoundedA => {
                ...
                if event == EventlessMacrostepIsBoundedEvent::Poke {
                ...
                if event == EventlessMacrostepIsBoundedEvent::Null {
                    if self.safe_evaluate_guard("(laps < 500)", engine) {
                        ...
                        *current_state = EventlessMacrostepIsBoundedState::BoundedB;
                        *transition_taken = true;
                        return true;
                    }
                }
            ...
            EventlessMacrostepIsBoundedState::BoundedB => {
                ...
                if event == EventlessMacrostepIsBoundedEvent::Null {
                    ...
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                    *current_state = EventlessMacrostepIsBoundedState::BoundedA;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            EventlessMacrostepIsBoundedState::Idle => {
                ...
                if event == EventlessMacrostepIsBoundedEvent::Poke {
                    ...
                    *current_state = EventlessMacrostepIsBoundedState::Idle;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == EventlessMacrostepIsBoundedEvent::Bounded {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 1;
                    self.has_transition_actions = false;
                ...
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == EventlessMacrostepIsBoundedEvent::Spin {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    ...
                    *current_state = EventlessMacrostepIsBoundedState::SpinA;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            EventlessMacrostepIsBoundedState::SpinA => {
                ...
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == EventlessMacrostepIsBoundedEvent::Reset {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 2;
                    self.has_transition_actions = false;
                ...
                if event == EventlessMacrostepIsBoundedEvent::Null {
                    ...
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                    *current_state = EventlessMacrostepIsBoundedState::SpinB;
                    *transition_taken = true;
                    return true;
                }
                false
