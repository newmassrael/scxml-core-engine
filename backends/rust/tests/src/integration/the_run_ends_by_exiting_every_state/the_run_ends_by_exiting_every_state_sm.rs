...
//! Generated Rust state machine for `the_run_ends_by_exiting_every_state`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: the_run_ends_by_exiting_every_state.scxml:37 :: _machine"]
...
pub enum TheRunEndsByExitingEveryStateState {
    Done,
...
pub enum TheRunEndsByExitingEveryStateEvent {
    ErrorExecution,
    Finish,
...
impl TheRunEndsByExitingEveryStateEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [TheRunEndsByExitingEveryStateEvent] =
        &[TheRunEndsByExitingEveryStateEvent::Finish];
}
...
pub struct TheRunEndsByExitingEveryStatePolicy {
...
impl TheRunEndsByExitingEveryStatePolicy {
    ...
    /// answer — the session is not initialized yet, `order` was
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `finalExits` was
    /// assigned a value of another type, or the engine refused.
    pub fn final_exits(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "finalExits",
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `selfInFinal` was
    /// assigned a value of another type, or the engine refused.
    pub fn self_in_final(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "selfInFinal",
    ...
    fn activate(&mut self, state: TheRunEndsByExitingEveryStateState) -> bool {
    ...
    fn deactivate(&mut self, state: TheRunEndsByExitingEveryStateState) {
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) =
            se.setup_system_variables(&sid, "the_run_ends_by_exiting_every_state", &io_processors)
        {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'order' from expr (global)
        ...
        // W3C SCXML 5.2/5.3: Initialize 'finalExits' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se,
            &sid,
            "finalExits",
        ...
        // W3C SCXML 5.2/5.3: Initialize 'selfInFinal' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se,
            &sid,
            "selfInFinal",
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                TheRunEndsByExitingEveryStateEvent::ErrorExecution,
                "<data id='order'> expr failed to evaluate",
            ));
        }
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                TheRunEndsByExitingEveryStateEvent::ErrorExecution,
                "<data id='finalExits'> expr failed to evaluate",
            ));
        }
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                TheRunEndsByExitingEveryStateEvent::ErrorExecution,
                "<data id='selfInFinal'> expr failed to evaluate",
            ));
        }
    ...
    fn state_name_matches(state: TheRunEndsByExitingEveryStateState, name: &str) -> bool {
...
impl StatePolicy for TheRunEndsByExitingEveryStatePolicy {
    type State = TheRunEndsByExitingEveryStateState;
    type Event = TheRunEndsByExitingEveryStateEvent;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            TheRunEndsByExitingEveryStateState::Done => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            TheRunEndsByExitingEveryStateState::Inner => {
                Some(TheRunEndsByExitingEveryStateState::Outer)
            }
            _ => None,
        }
    }
    ...
    fn is_compound_state(state: Self::State) -> bool {
        match state {
            TheRunEndsByExitingEveryStateState::Outer => true,
            _ => false,
        }
    }
    ...
    fn get_child_states(state: Self::State) -> &'static [Self::State] {
        match state {
            TheRunEndsByExitingEveryStateState::Outer => {
                &[TheRunEndsByExitingEveryStateState::Inner]
            }
            _ => &[],
        }
    ...
    fn get_initial_targets(
        ...
        match state {
            TheRunEndsByExitingEveryStateState::Outer => &[::sce_rust_runtime::EntryTarget::State(
                TheRunEndsByExitingEveryStateState::Inner,
            )],
            _ => &[],
        }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            TheRunEndsByExitingEveryStateState::Done => 2,
            TheRunEndsByExitingEveryStateState::Inner => 1,
            TheRunEndsByExitingEveryStateState::Outer => 0,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            TheRunEndsByExitingEveryStateEvent::ErrorExecution => "error.execution",
            TheRunEndsByExitingEveryStateEvent::Finish => "finish",
            TheRunEndsByExitingEveryStateEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(TheRunEndsByExitingEveryStateEvent::ErrorExecution),
            "finish" => Some(TheRunEndsByExitingEveryStateEvent::Finish),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            TheRunEndsByExitingEveryStateState::Done => "done",
            TheRunEndsByExitingEveryStateState::Inner => "inner",
            TheRunEndsByExitingEveryStateState::Outer => "outer",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "done" => Some(TheRunEndsByExitingEveryStateState::Done),
            "inner" => Some(TheRunEndsByExitingEveryStateState::Inner),
            "outer" => Some(TheRunEndsByExitingEveryStateState::Outer),
            _ => None,
        }
    }
    ...
    fn execute_exit_actions(
        ...
        match state {
            TheRunEndsByExitingEveryStateState::Done => {
                ...
                'action_block: {
                    {
                        // W3C SCXML 5.3: <assign location="finalExits">
                        self.ensure_script_engine();
                        let sid = self.session_id.as_ref().unwrap().clone();
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        let expr = "_scxml_add(finalExits, 1)";
                        ...
                        let assign_script = format!("{} = {}", "finalExits", expr);
                        if let Err(e) = se.execute_script(&sid, &assign_script) {
                            ::sce_rust_runtime::sce_log_error!(
                                "Assign failed for 'finalExits': {}",
                                e
                            );
                            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                TheRunEndsByExitingEveryStateEvent::ErrorExecution,
                                "<assign> to 'finalExits' failed",
                            ));
                    ...
                    if self.is_state_active("done") {
                        {
                            // W3C SCXML 5.3: <assign location="selfInFinal">
                            self.ensure_script_engine();
                            let sid = self.session_id.as_ref().unwrap().clone();
                            let se = self.script_engine.clone();
                            let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                            ...
                            // through evaluate_expression + set_variable would round-trip through ScriptValue
                            // and create a fresh table, breaking reference equality.
                            let assign_script = format!("{} = {}", "selfInFinal", expr);
                            if let Err(e) = se.execute_script(&sid, &assign_script) {
                                ::sce_rust_runtime::sce_log_error!(
                                    "Assign failed for 'selfInFinal': {}",
                                    e
                                );
                                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                    TheRunEndsByExitingEveryStateEvent::ErrorExecution,
                                    "<assign> to 'selfInFinal' failed",
                                ));
                                // W3C SCXML 3.8/3.9: Error stops subsequent actions in this onentry/onexit block
                                break 'action_block;
                            }
            ...
            TheRunEndsByExitingEveryStateState::Inner => {
                ...
                'action_block: {
                    {
                        // W3C SCXML 5.3: <assign location="order">
                        self.ensure_script_engine();
                        let sid = self.session_id.as_ref().unwrap().clone();
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        let expr = "_scxml_add((order * 10), 1)";
                        // W3C SCXML 5.3: Assign via execute_script preserves Lua reference identity for
                        // table values (e.g. `Var2 = _event` — test 329 requires `Var2 == _event`). Going
                        // through evaluate_expression + set_variable would round-trip through ScriptValue
                        // and create a fresh table, breaking reference equality.
                        let assign_script = format!("{} = {}", "order", expr);
                        if let Err(e) = se.execute_script(&sid, &assign_script) {
                            ::sce_rust_runtime::sce_log_error!("Assign failed for 'order': {}", e);
                            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                TheRunEndsByExitingEveryStateEvent::ErrorExecution,
                                "<assign> to 'order' failed",
                            ));
                            // W3C SCXML 3.8/3.9: Error stops subsequent actions in this onentry/onexit block
                            break 'action_block;
                        }
                ...
                'action_block: {
                    {
                        ...
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        let expr = "_scxml_add((order * 10), 2)";
                        // W3C SCXML 5.3: Assign via execute_script preserves Lua reference identity for
                        // table values (e.g. `Var2 = _event` — test 329 requires `Var2 == _event`). Going
                        // through evaluate_expression + set_variable would round-trip through ScriptValue
                        // and create a fresh table, breaking reference equality.
    ...
    fn first_enabled_transition(
        ...
        match state {
            TheRunEndsByExitingEveryStateState::Inner => {
                if event == TheRunEndsByExitingEveryStateEvent::Finish {
                    {
