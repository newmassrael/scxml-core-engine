...
//! Generated Rust state machine for `xml_data_is_a_dom_tree`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: xml_data_is_a_dom_tree.scxml:44 :: _machine"]
...
pub enum XmlDataIsADomTreeState {
    NotADocument,
    NoText,
...
pub enum XmlDataIsADomTreeEvent {
    ErrorExecution,
...
impl XmlDataIsADomTreeEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [XmlDataIsADomTreeEvent] = &[];
}
...
impl XmlDataIsADomTreePolicy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: XmlDataIsADomTreeState::Reading,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "xml_data_is_a_dom_tree", &io_processors) {
...
impl StatePolicy for XmlDataIsADomTreePolicy {
    type State = XmlDataIsADomTreeState;
    type Event = XmlDataIsADomTreeEvent;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            XmlDataIsADomTreeState::NotADocument => true,
            XmlDataIsADomTreeState::NoText => true,
            XmlDataIsADomTreeState::Settled => true,
            XmlDataIsADomTreeState::WrongTree => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            XmlDataIsADomTreeState::NotADocument => 4,
            XmlDataIsADomTreeState::NoText => 6,
            XmlDataIsADomTreeState::Reading => 0,
            XmlDataIsADomTreeState::ReadingText => 2,
            XmlDataIsADomTreeState::Settled => 3,
            XmlDataIsADomTreeState::Traversing => 1,
            XmlDataIsADomTreeState::WrongTree => 5,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            XmlDataIsADomTreeEvent::ErrorExecution => "error.execution",
            XmlDataIsADomTreeEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(XmlDataIsADomTreeEvent::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            XmlDataIsADomTreeState::NotADocument => "notADocument",
            XmlDataIsADomTreeState::NoText => "noText",
            XmlDataIsADomTreeState::Reading => "reading",
            XmlDataIsADomTreeState::ReadingText => "readingText",
            XmlDataIsADomTreeState::Settled => "settled",
            XmlDataIsADomTreeState::Traversing => "traversing",
            XmlDataIsADomTreeState::WrongTree => "wrongTree",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "notADocument" => Some(XmlDataIsADomTreeState::NotADocument),
            "noText" => Some(XmlDataIsADomTreeState::NoText),
            "reading" => Some(XmlDataIsADomTreeState::Reading),
            "readingText" => Some(XmlDataIsADomTreeState::ReadingText),
            "settled" => Some(XmlDataIsADomTreeState::Settled),
            "traversing" => Some(XmlDataIsADomTreeState::Traversing),
            "wrongTree" => Some(XmlDataIsADomTreeState::WrongTree),
            _ => None,
        }
    }
...
impl XmlDataIsADomTreePolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: XmlDataIsADomTreeState,
        event: XmlDataIsADomTreeEvent,
        current_state: &mut XmlDataIsADomTreeState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            XmlDataIsADomTreeState::NotADocument => false,
            XmlDataIsADomTreeState::NoText => false,
            XmlDataIsADomTreeState::Reading => {
                // W3C SCXML 3.13: Eventless transitions
                if event == XmlDataIsADomTreeEvent::Null {
                    if self.safe_evaluate_guard("((((doc.nodeType == 9) and (doc.nodeName == \"#document\")) and (doc.documentElement.tagName == \"books\")) and _scxml_truthy(doc:hasAttribute(\"count\")))", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = XmlDataIsADomTreeState::Traversing;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = XmlDataIsADomTreeState::NotADocument;
                            *transition_taken = true;
                        return true;
                    }
                }
            ...
            XmlDataIsADomTreeState::ReadingText => {
                // W3C SCXML 3.13: Eventless transitions
                if event == XmlDataIsADomTreeEvent::Null {
                    if self.safe_evaluate_guard("((((doc.documentElement.firstChild.firstChild.nodeType == 3) and (doc.documentElement.firstChild.firstChild.nodeValue == \"first\")) and (doc.documentElement.textContent == \"first\")) and (doc.documentElement.lastChild:hasChildNodes() == false))", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = XmlDataIsADomTreeState::Settled;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = XmlDataIsADomTreeState::NoText;
                            *transition_taken = true;
                        return true;
                    }
                }
            ...
            XmlDataIsADomTreeState::Settled => false,
            XmlDataIsADomTreeState::Traversing => {
                // W3C SCXML 3.13: Eventless transitions
                if event == XmlDataIsADomTreeEvent::Null {
                    if self.safe_evaluate_guard("(((((#doc.documentElement.childNodes == 2) and (doc.documentElement.firstChild:getAttribute(\"title\") == \"t1\")) and (doc.documentElement.lastChild:getAttribute(\"title\") == \"t2\")) and (doc.documentElement.lastChild.previousSibling:getAttribute(\"title\") == \"t1\")) and (doc.documentElement.firstChild.parentNode.tagName == \"books\"))", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = XmlDataIsADomTreeState::ReadingText;
                            *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = XmlDataIsADomTreeState::WrongTree;
                            *transition_taken = true;
                        return true;
                    }
                }
            ...
            XmlDataIsADomTreeState::WrongTree => false,
            _ => false,
        }
    }
}
