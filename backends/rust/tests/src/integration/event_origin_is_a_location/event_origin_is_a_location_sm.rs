...
//! Generated Rust state machine for `event_origin_is_a_location`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
pub enum EventOriginIsALocationState {
    AwaitReply,
    Fail,
    Pass,
    Phase,
...
impl EventOriginIsALocationEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [EventOriginIsALocationEvent] = &[
        EventOriginIsALocationEvent::FromChild,
        EventOriginIsALocationEvent::ReplyArrived,
    ];
}
...
pub struct EventOriginIsALocationPolicy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<EventOriginIsALocationState>>,
    ...
    // W3C SCXML 6.4: Static invoke child 'inv_peer' (event_origin_is_a_location__sce_synth_invoke__inv_peer)
    child_inv_peer: Option<Box<sce_rust_runtime::Engine<super::event_origin_is_a_location__sce_synth_invoke__inv_peer_sm::EventOriginIsALocationSceSynthInvokeInvPeerPolicy>>>,
    pending_done_invoke_inv_peer: bool,
...
impl EventOriginIsALocationPolicy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            child_inv_peer: None,
            pending_done_invoke_inv_peer: false,
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains(".inv_peer") {
                ...
                let mut child_policy = super::event_origin_is_a_location__sce_synth_invoke__inv_peer_sm::EventOriginIsALocationSceSynthInvokeInvPeerPolicy::new(self.script_engine.clone());
                ...
                self.child_inv_peer = Some(Box::new(child_engine));
                ...
                {
                    self.pending_done_invoke_inv_peer = true;
    ...
    fn do_tick_children(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        if !self.pending_done_invoke_inv_peer {
            ...
            if let Some(mut child) = self.child_inv_peer.take() {
                ...
                if child.is_in_final_state() && !self.pending_done_invoke_inv_peer {
                    self.pending_done_invoke_inv_peer = true;
                    // W3C SCXML 5.5 + 6.3.1: Lift the child's stashed donedata onto
                ...
                // Put child back
                self.child_inv_peer = Some(child);
            }
        }
    }
...
impl StatePolicy for EventOriginIsALocationPolicy {
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            EventOriginIsALocationState::Fail => true,
            EventOriginIsALocationState::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            EventOriginIsALocationState::AwaitReply => Some(EventOriginIsALocationState::Phase),
            EventOriginIsALocationState::Waiting => Some(EventOriginIsALocationState::Phase),
            _ => None,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            EventOriginIsALocationState::AwaitReply => 2,
            EventOriginIsALocationState::Fail => 4,
            EventOriginIsALocationState::Pass => 3,
            EventOriginIsALocationState::Phase => 0,
            EventOriginIsALocationState::Waiting => 1,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            ...
            EventOriginIsALocationEvent::FromChild => "fromChild",
            EventOriginIsALocationEvent::Reply => "reply",
            EventOriginIsALocationEvent::ReplyArrived => "replyArrived",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            ...
            "fromChild" => Some(EventOriginIsALocationEvent::FromChild),
            "reply" => Some(EventOriginIsALocationEvent::Reply),
            "replyArrived" => Some(EventOriginIsALocationEvent::ReplyArrived),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            EventOriginIsALocationState::AwaitReply => "await_reply",
            EventOriginIsALocationState::Fail => "fail",
            EventOriginIsALocationState::Pass => "pass",
            EventOriginIsALocationState::Phase => "phase",
            EventOriginIsALocationState::Waiting => "waiting",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "await_reply" => Some(EventOriginIsALocationState::AwaitReply),
            "fail" => Some(EventOriginIsALocationState::Fail),
            "pass" => Some(EventOriginIsALocationState::Pass),
            "phase" => Some(EventOriginIsALocationState::Phase),
            "waiting" => Some(EventOriginIsALocationState::Waiting),
            _ => None,
        }
    }
        ...
        match state {
            EventOriginIsALocationState::Phase => {
                ...
                if self.child_inv_peer.is_some() {
                    ...
                    self.child_inv_peer = None;
                }
                self.active_invokes.remove("inv_peer");
                self.pending_done_invoke_inv_peer = false;
            }
            _ => {}
        }
        match state {
