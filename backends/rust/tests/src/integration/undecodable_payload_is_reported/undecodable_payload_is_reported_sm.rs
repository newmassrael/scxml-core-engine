...
//! Generated Rust state machine for `undecodable_payload_is_reported`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: undecodable_payload_is_reported.scxml:56 :: _machine"]
...
pub enum UndecodablePayloadIsReportedState {
    Accepted,
...
pub enum UndecodablePayloadIsReportedEvent {
    Answer,
    ErrorExecution,
    Note,
...
impl UndecodablePayloadIsReportedEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [UndecodablePayloadIsReportedEvent] = &[
...
pub struct UndecodablePayloadIsReportedPolicy {
    // W3C SCXML 3.13: Last transition metadata
    last_transition_is_internal: bool,
    last_transition_is_targetless: bool,
    last_transition_source_state: UndecodablePayloadIsReportedState,
...
impl UndecodablePayloadIsReportedPolicy {
    ...
    /// answer — the session is not initialized yet, `answers` was
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `notes` was
    /// assigned a value of another type, or the engine refused.
    pub fn notes(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "notes",
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: UndecodablePayloadIsReportedState::Waiting,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) =
            se.setup_system_variables(&sid, "undecodable_payload_is_reported", &io_processors)
        {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'answers' from expr (global)
        ...
        // W3C SCXML 5.2/5.3: Initialize 'notes' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "notes", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                UndecodablePayloadIsReportedEvent::ErrorExecution,
                "<data id='answers'> expr failed to evaluate",
            ));
        }
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                UndecodablePayloadIsReportedEvent::ErrorExecution,
                "<data id='notes'> expr failed to evaluate",
            ));
        }
...
impl StatePolicy for UndecodablePayloadIsReportedPolicy {
    type State = UndecodablePayloadIsReportedState;
    type Event = UndecodablePayloadIsReportedEvent;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            UndecodablePayloadIsReportedState::Accepted => true,
            _ => false,
        }
    }
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            UndecodablePayloadIsReportedState::Accepted => 1,
            UndecodablePayloadIsReportedState::Waiting => 0,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            UndecodablePayloadIsReportedEvent::Answer => "answer",
            UndecodablePayloadIsReportedEvent::ErrorExecution => "error.execution",
            UndecodablePayloadIsReportedEvent::Note => "note",
            UndecodablePayloadIsReportedEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "answer" => Some(UndecodablePayloadIsReportedEvent::Answer),
            "error.execution" => Some(UndecodablePayloadIsReportedEvent::ErrorExecution),
            "note" => Some(UndecodablePayloadIsReportedEvent::Note),
            _ => None,
        }
    }
            ...
            UndecodablePayloadIsReportedState::Waiting => {
                match self.last_transition_index {
                    1 => {
                        ...
                        {
                            // W3C SCXML 5.3: <assign location="answers">
                            self.ensure_script_engine();
                            let sid = self.session_id.as_ref().unwrap().clone();
                            let se = self.script_engine.clone();
                            let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                            let expr = "_scxml_add(answers, 1)";
                            ...
                            let assign_script = format!("{} = {}", "answers", expr);
                            if let Err(e) = se.execute_script(&sid, &assign_script) {
                                ::sce_rust_runtime::sce_log_error!(
                                    "Assign failed for 'answers': {}",
                                    e
                                );
                                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                    UndecodablePayloadIsReportedEvent::ErrorExecution,
                                    "<assign> to 'answers' failed",
                                ));
                            }
                        }
                    }
                    2 => {
                        ...
                        {
                            // W3C SCXML 5.3: <assign location="notes">
                            self.ensure_script_engine();
                            let sid = self.session_id.as_ref().unwrap().clone();
                            let se = self.script_engine.clone();
                            let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                            let expr = "_scxml_add(notes, 1)";
                            // W3C SCXML 5.3: Assign via execute_script preserves Lua reference identity for
                            // table values (e.g. `Var2 = _event` — test 329 requires `Var2 == _event`). Going
                            // through evaluate_expression + set_variable would round-trip through ScriptValue
                            // and create a fresh table, breaking reference equality.
                            let assign_script = format!("{} = {}", "notes", expr);
                            if let Err(e) = se.execute_script(&sid, &assign_script) {
                                ::sce_rust_runtime::sce_log_error!(
                                    "Assign failed for 'notes': {}",
                                    e
                                );
                                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                    UndecodablePayloadIsReportedEvent::ErrorExecution,
                                    "<assign> to 'notes' failed",
                                ));
                            }
                        }
                    }
...
impl UndecodablePayloadIsReportedPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: UndecodablePayloadIsReportedState,
        event: UndecodablePayloadIsReportedEvent,
        current_state: &mut UndecodablePayloadIsReportedState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            UndecodablePayloadIsReportedState::Accepted => false,
            UndecodablePayloadIsReportedState::Waiting => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == UndecodablePayloadIsReportedEvent::Answer {
                    ...
                    if self.safe_evaluate_guard("_scxml_truthy(_event.data.done)", engine) {
                        ...
                        *current_state = UndecodablePayloadIsReportedState::Accepted;
                ...
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == UndecodablePayloadIsReportedEvent::Note {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
