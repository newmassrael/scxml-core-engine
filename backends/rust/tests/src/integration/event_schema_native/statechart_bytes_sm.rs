...
//! Generated Rust state machine for `statechart_bytes`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: statechart_bytes.scxml:13 :: _machine"]
...
impl StatechartBytesEvent {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [StatechartBytesEvent] =
        &[StatechartBytesEvent::SignalReceived];
}

// ── NL→IR Item C1 Path A: EventSchema typed `_event.data` payload ──
#[derive(Clone, Debug, Default, PartialEq)]
pub struct StatechartBytesSignalReceivedPayload {
    pub raw: ::sce_rust_runtime::SceBytes<8>,
}
...
#[derive(Clone, Debug, Default, PartialEq)]
pub enum StatechartBytesPayload {
    /// Schemaless / not-yet-populated default (W3C §5.10 empty `_event.data`).
    #[default]
    None,
    SignalReceived(StatechartBytesSignalReceivedPayload),
}

/// NL→IR Item C1 Path A: per-event typed `_event.data` inject seam.
/// Bring into scope with `use …::StatechartBytesInject;` to call the
/// `raise_<event>` methods on the engine.
pub trait StatechartBytesInject {
    /// Typed `_event.data` inject for `signal.received`.
    /// Binds the event name and payload variant in one call (name↔type
    /// pairing cannot be constructed inconsistently).
    fn raise_signal_received(&mut self, payload: StatechartBytesSignalReceivedPayload);
}

impl StatechartBytesInject for ::sce_rust_runtime::Engine<StatechartBytesPolicy> {
    fn raise_signal_received(&mut self, payload: StatechartBytesSignalReceivedPayload) {
        let data = ::sce_rust_runtime::payload_wire(format_args!(
            "{{\"raw\":{}}}",
            ::sce_rust_runtime::event_payload::quote(
                &::sce_rust_runtime::event_payload::bytes_as_payload_text(&payload.raw)
            )
        ));
        self.raise_external_typed_with_data(
            StatechartBytesEvent::SignalReceived,
            StatechartBytesPayload::SignalReceived(payload),
            &data,
        );
    }
}
...
pub struct StatechartBytesPolicy {
    ...
    pending_payload: StatechartBytesPayload,
...
impl StatechartBytesPolicy {
    pub fn new() -> Self {
        Self {
            last_transition_is_internal: false,
            last_transition_is_targetless: false,
            last_transition_source_state: StatechartBytesState::Waiting,
            pending_payload: StatechartBytesPayload::default(),
            session_id: None,
...
impl Default for StatechartBytesPolicy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for StatechartBytesPolicy {
    type State = StatechartBytesState;
    type Event = StatechartBytesEvent;
    ...
    fn get_document_order(state: Self::State) -> u32 {
        match state {
            StatechartBytesState::Done => 1,
            StatechartBytesState::Waiting => 0,
        }
    }

    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            StatechartBytesEvent::SignalReceived => "signal.received",
            StatechartBytesEvent::Null => "",
        }
    }

    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "signal.received" => Some(StatechartBytesEvent::SignalReceived),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            StatechartBytesState::Done => "done",
            StatechartBytesState::Waiting => "waiting",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "done" => Some(StatechartBytesState::Done),
            "waiting" => Some(StatechartBytesState::Waiting),
            _ => None,
        }
    }
    ...
    fn lift_event_payload(
        ...
        if self.pending_payload != StatechartBytesPayload::None {
            return Ok(());
        }
        match event {
            StatechartBytesEvent::SignalReceived => {
                ...
                self.pending_payload =
                    StatechartBytesPayload::SignalReceived(StatechartBytesSignalReceivedPayload {
                        raw: fields.bytes("raw")?,
                    });
            }
            _ => {}
        }
...
impl StatechartBytesPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: StatechartBytesState,
        event: StatechartBytesEvent,
        current_state: &mut StatechartBytesState,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            StatechartBytesState::Done => false,
            StatechartBytesState::Waiting => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == StatechartBytesEvent::SignalReceived {
                    // NL→IR Item C1 Path A: native typed `_event.data` guard
                    if matches!(&self.pending_payload, StatechartBytesPayload::SignalReceived(ev) if ev.raw == b"ack")
                    {
                        ...
                        *current_state = StatechartBytesState::Done;
