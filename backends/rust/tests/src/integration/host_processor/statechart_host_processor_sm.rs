...
//! Generated Rust state machine for `statechart_host_processor`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
pub enum StatechartHostProcessorState {
...
impl StatechartHostProcessorEvent {
...
impl StatechartHostProcessorPolicy {
    ...
    /// answer — the session is not initialized yet, `served` was
    /// assigned a value of another type, or the engine refused.
    pub fn served(&self) -> Option<i64> {
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `plain` was
    /// assigned a value of another type, or the engine refused.
    pub fn plain(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "plain",
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: StatechartHostProcessorState::Dispatching,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "statechart_host_processor", &io_processors)
        {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'served' from expr (global)
        ...
        // W3C SCXML 5.2/5.3: Initialize 'plain' from expr (global)
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            se, &sid, "plain", "0",
        ) {
            ::sce_rust_runtime::sce_log_error!("global: {}", e);
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                ...
                "<data id='served'> expr failed to evaluate",
            ));
        }
...
impl StatePolicy for StatechartHostProcessorPolicy {
    type State = StatechartHostProcessorState;
    type Event = StatechartHostProcessorEvent;
            ...
            StatechartHostProcessorState::Dispatching => {
                ...
                'action_block: {
...

                    {
                        ...
                        let event_data: &str = "{\"within\":\"2500\"}";
            ...
            StatechartHostProcessorState::Dispatching => {
                match self.last_transition_index {
                    0 => {
                        ...
                        {
                            // W3C SCXML 5.3: <assign location="plain">
                            self.ensure_script_engine();
                            let sid = self.session_id.as_ref().unwrap().clone();
                            let se = self.script_engine.clone();
                            let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                            let expr = "_scxml_add(plain, 1)";
                            ...
                            let assign_script = format!("{} = {}", "plain", expr);
                            if let Err(e) = se.execute_script(&sid, &assign_script) {
                                ::sce_rust_runtime::sce_log_error!(
                                    "Assign failed for 'plain': {}",
                                    e
                                );
                                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                    StatechartHostProcessorEvent::ErrorExecution,
                                    "<assign> to 'plain' failed",
                                ));
                            }
                        }
                    }
                    1 => {
                        ...
                        {
                            // W3C SCXML 5.3: <assign location="served">
                            self.ensure_script_engine();
                            let sid = self.session_id.as_ref().unwrap().clone();
                            let se = self.script_engine.clone();
                            let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                            let expr = "_scxml_add(served, 1)";
                            // W3C SCXML 5.3: Assign via execute_script preserves Lua reference identity for
                            // table values (e.g. `Var2 = _event` — test 329 requires `Var2 == _event`). Going
                            // through evaluate_expression + set_variable would round-trip through ScriptValue
                            // and create a fresh table, breaking reference equality.
                            let assign_script = format!("{} = {}", "served", expr);
                            if let Err(e) = se.execute_script(&sid, &assign_script) {
                                ::sce_rust_runtime::sce_log_error!(
                                    "Assign failed for 'served': {}",
                                    e
                                );
                                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                    StatechartHostProcessorEvent::ErrorExecution,
                                    "<assign> to 'served' failed",
                                ));
                            }
                        }
                    }
...
impl StatechartHostProcessorPolicy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: StatechartHostProcessorState,
        event: StatechartHostProcessorEvent,
        current_state: &mut StatechartHostProcessorState,
        ...
        match check_state {
            StatechartHostProcessorState::Dispatching => {
                ...
                if event == StatechartHostProcessorEvent::PlainArrived {
                ...
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == StatechartHostProcessorEvent::TurnDone {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
