...
// From: resources/351/test351.scxml
//
...
//! Generated Rust state machine for `test351`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test351.scxml:5 :: _machine"]
...
impl Test351Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test351Event] =
        &[Test351Event::S0Event, Test351Event::S0Event2];
}
...
impl Test351Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test351State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test351", &io_processors) {
...
impl StatePolicy for Test351Policy {
    type State = Test351State;
    type Event = Test351Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test351State::Fail => true,
            Test351State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test351Event::ErrorExecution => "error.execution",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test351Event::ErrorExecution),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test351State::Fail => "fail",
            Test351State::Pass => "pass",
            Test351State::S0 => "s0",
            Test351State::S1 => "s1",
            Test351State::S2 => "s2",
            Test351State::S3 => "s3",
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test351State::Fail),
            "pass" => Some(Test351State::Pass),
            "s0" => Some(Test351State::S0),
            "s1" => Some(Test351State::S1),
            "s2" => Some(Test351State::S2),
            "s3" => Some(Test351State::S3),
            _ => None,
        }
    }
        ...
        match state {
            Test351State::S0 => {
                ...
                'action_block: {
                    ...
                    {
                        let send_id = ::sce_rust_runtime::sce_string_from_str("send1");
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test351Event::S0Event);
            ...
            Test351State::S2 => {
                ...
                'action_block: {
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test351Event::S0Event2);
                            // W3C SCXML 5.10.1: External send — preserve sendid and SCXML event processor origintype
                            meta.metadata = sce_rust_runtime::EventMetadata::external(
                                send_id.clone(),
                                ::sce_rust_runtime::SceString::new(),
...
impl Test351Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test351State,
        event: Test351Event,
        current_state: &mut Test351State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test351State::Fail => false,
            Test351State::Pass => false,
            Test351State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test351Event::S0Event {
                    ...
                    *current_state = Test351State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test351Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test351State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test351State::S1 => {
                ...
                if event == Test351Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, \"send1\")", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        self.has_transition_actions = false;
            ...
            Test351State::S2 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test351Event::S0Event2 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 0;
                    self.has_transition_actions = true;
            ...
            Test351State::S3 => {
                ...
                if event == Test351Event::Null {
                    ...
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                    *current_state = Test351State::Pass;
                    *transition_taken = true;
                    return true;
                }
                false
