...
// From: resources/276/test276.scxml
//
...
//! Generated Rust state machine for `test276`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test276.scxml:6 :: _machine"]
// SCE-MAP: test276.scxml:6 :: _machine
...
impl Test276Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test276Event] =
        &[Test276Event::Event0, Test276Event::Event1];
}
...
pub struct Test276Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test276State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test276sub1)
    child_invoke_0: Option<Box<sce_rust_runtime::Engine<super::test276sub1_sm::Test276sub1Policy>>>,
...
impl Test276Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test276State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test276", &io_processors) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy =
                    super::test276sub1_sm::Test276sub1Policy::new(self.script_engine.clone());
...
impl StatePolicy for Test276Policy {
    type State = Test276State;
    type Event = Test276Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test276State::Fail => true,
            Test276State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test276Event::CancelInvoke => "cancel.invoke",
            Test276Event::DoneInvoke => "done.invoke",
            Test276Event::ErrorExecution => "error.execution",
            Test276Event::Event0 => "event0",
            Test276Event::Event1 => "event1",
            Test276Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test276Event::CancelInvoke),
            "done.invoke" => Some(Test276Event::DoneInvoke),
            "error.execution" => Some(Test276Event::ErrorExecution),
            "event0" => Some(Test276Event::Event0),
            "event1" => Some(Test276Event::Event1),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test276State::Fail => "fail",
            Test276State::Pass => "pass",
            Test276State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test276State::Fail),
            "pass" => Some(Test276State::Pass),
            "s0" => Some(Test276State::S0),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test276State::S0 => {
                // SCE-MAP: test276.scxml:8 :: s0 :: _state_body
...
impl Test276Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test276State,
        event: Test276Event,
        current_state: &mut Test276State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test276State::Fail => false,
            Test276State::Pass => false,
            Test276State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test276Event::Event1 {
                    ...
                    *current_state = Test276State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test276Event::Event0 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test276State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
