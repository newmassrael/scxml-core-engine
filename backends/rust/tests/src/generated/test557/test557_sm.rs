...
// From: resources/557/test557.scxml
//
...
//! Generated Rust state machine for `test557`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test557.scxml:5 :: _machine"]
...
impl Test557Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test557Event] = &[];
}
...
impl Test557Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test557State::S0,
    ...
    fn ensure_script_engine(&mut self) {
     ...
     <book title="title2"></book>
        ...
        {
            let base_path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../../resources/557/");
     ...
     <book title="title2"></book>
        ...
        // W3C SCXML 5.2.2: Load variable 'var2' from src (global)
        {
            ...
            if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_src(
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                    Test557Event::ErrorExecution,
                    "<data id='var2' src='file:test557.txt'> could not be loaded",
                ));
            }
        }
...
impl StatePolicy for Test557Policy {
    type State = Test557State;
    type Event = Test557Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test557State::Fail => true,
            Test557State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test557Event::ErrorExecution => "error.execution",
            Test557Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test557Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test557State::Fail => "fail",
            Test557State::Pass => "pass",
            Test557State::S0 => "s0",
            Test557State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test557State::Fail),
            "pass" => Some(Test557State::Pass),
            "s0" => Some(Test557State::S0),
            "s1" => Some(Test557State::S1),
            _ => None,
        }
    }
...
impl Test557Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test557State,
        event: Test557Event,
        current_state: &mut Test557State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test557State::Fail => false,
            Test557State::Pass => false,
            Test557State::S0 => {
