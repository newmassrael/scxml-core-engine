...
// From: resources/510/test510.scxml
//
...
//! Generated Rust state machine for `test510`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test510.scxml:5 :: _machine"]
...
impl Test510Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test510Event] = &[Test510Event::Test];
}
...
impl StatePolicy for Test510Policy {
    type State = Test510State;
    type Event = Test510Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test510State::Fail => true,
            Test510State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test510State::Fail => "fail",
            Test510State::Pass => "pass",
            Test510State::S0 => "s0",
            Test510State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test510State::Fail),
            "pass" => Some(Test510State::Pass),
            "s0" => Some(Test510State::S0),
            "s1" => Some(Test510State::S1),
            _ => None,
        }
    }
        ...
        match state {
            Test510State::S0 => {
                ...
                'action_block: {
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "_ioprocessors.basichttp.location") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test510Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test510Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
                        ...
                        if let Some(ref _rt) = _resolved_target {
                            // W3C SCXML C.2: BasicHTTP send to HTTP target
                            {
                                ...
                                if !_rt.starts_with("http://") && !_rt.starts_with("https://") {
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test510Event::ErrorCommunication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL"));
                                } else {
                                    let http_params =
                    ...
                    // W3C SCXML 3.8.1: <raise event="internal">
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(
                        Test510Event::Internal,
                    ));
                }
            }
            _ => {}
...
impl Test510Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test510State,
        event: Test510Event,
        current_state: &mut Test510State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test510State::Fail => false,
            Test510State::Pass => false,
            Test510State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test510Event::Internal {
                    ...
                    *current_state = Test510State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test510Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test510State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test510State::S1 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test510Event::Test {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test510State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
