...
// From: resources/488/test488.scxml
//
...
//! Generated Rust state machine for `test488`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test488Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test488Event] = &[];
}
...
impl Test488Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test488State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test488", &io_processors) {
...
impl StatePolicy for Test488Policy {
    type State = Test488State;
    type Event = Test488Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test488State::Fail => true,
            Test488State::Pass => true,
            Test488State::S02 => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test488State::S01 => Some(Test488State::S0),
            Test488State::S02 => Some(Test488State::S0),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test488State::Fail => "fail",
            Test488State::Pass => "pass",
            Test488State::S0 => "s0",
            Test488State::S01 => "s01",
            Test488State::S02 => "s02",
            Test488State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test488State::Fail),
            "pass" => Some(Test488State::Pass),
            "s0" => Some(Test488State::S0),
            "s01" => Some(Test488State::S01),
            "s02" => Some(Test488State::S02),
            "s1" => Some(Test488State::S1),
            _ => None,
        }
    }
    ...
    fn get_initial_children(
        ...
        match state {
            Test488State::S0 => {
                ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([Test488State::S01])
            }
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test488State::S0 => Test488State::S01,
            _ => state,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test488State::S02 => {
                ...
                let mut done_data_ok = true;
                {
                    ...
                    match se.evaluate_expression(&sid, "nil.invalidProperty") {
                        ...
                        Err(e) => {
                            ...
                            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                Test488Event::ErrorExecution,
                                "<donedata> <param name='someParam'> failed to evaluate",
                            ));
                        }
                    }
                    if done_data_ok {
                ...
                if done_data_ok {
                    let mut done_meta =
                        sce_rust_runtime::EventWithMetadata::new(Test488Event::DoneStateS0);
...
impl Test488Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test488State,
        event: Test488Event,
        current_state: &mut Test488State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test488State::Fail => false,
            Test488State::Pass => false,
            Test488State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test488Event::ErrorExecution {
                    ...
                    *current_state = Test488State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test488Event::DoneStateS0 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test488State::Fail;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
            ...
            Test488State::S01 => {
                ...
                if event == Test488Event::Null {
                    // Unconditional eventless transition
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                    *current_state = Test488State::S02;
                    *transition_taken = true;
                    return true;
                }
                false
            }
            Test488State::S02 => false,
            Test488State::S1 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test488Event::DoneStateS0 {
                    ...
                    *current_state = Test488State::Pass;
                    *transition_taken = true;
                    return true;
                }
