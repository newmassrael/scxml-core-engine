...
impl Test318Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test318Event] = &[];
}
...
impl StatePolicy for Test318Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test318State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test318State::Fail),
            "pass" => Some(Test318State::Pass),
            "s0" => Some(Test318State::S0),
            "s1" => Some(Test318State::S1),
            _ => None,
        }
    }
                ...
                if event == Test318Event::Foo {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test318State::S1)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test318State::S1 => {
                if event == Test318Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, \"foo\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test318State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test318Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test318State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
