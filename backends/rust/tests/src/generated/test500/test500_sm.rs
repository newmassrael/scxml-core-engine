...
// From: resources/500/test500.scxml
//
...
//! Generated Rust state machine for `test500`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test500.scxml:5 :: _machine"]
...
impl Test500Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test500Event] = &[];
}
...
impl Test500Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test500State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test500", &io_processors) {
...
impl StatePolicy for Test500Policy {
    type State = Test500State;
    type Event = Test500Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test500State::Fail => true,
            Test500State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test500Event::ErrorExecution => "error.execution",
            Test500Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test500Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test500State::Fail => "fail",
            Test500State::Pass => "pass",
            Test500State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test500State::Fail),
            "pass" => Some(Test500State::Pass),
            "s0" => Some(Test500State::S0),
            _ => None,
        }
    }
...
impl Test500Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test500State,
        event: Test500Event,
        current_state: &mut Test500State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test500State::Fail => false,
            Test500State::Pass => false,
            Test500State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test500Event::Null {
                    if self.safe_evaluate_guard("(_typeof(Var1) ~= \"undefined\")", engine) {
                        ...
                        *current_state = Test500State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test500State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
