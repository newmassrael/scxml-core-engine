...
// From: resources/423/test423.scxml
//
...
//! Generated Rust state machine for `test423`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test423.scxml:4 :: _machine"]
...
impl Test423Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test423Event] = &[Test423Event::ExternalEvent2];
}
...
impl Test423Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test423State::S0,
            session_id: None,
...
impl Default for Test423Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test423Policy {
    type State = Test423State;
    type Event = Test423Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test423State::Fail => true,
            Test423State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test423Event::ErrorExecution => "error.execution",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test423Event::ErrorExecution),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test423State::Fail => "fail",
            Test423State::Pass => "pass",
            Test423State::S0 => "s0",
            Test423State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test423State::Fail),
            "pass" => Some(Test423State::Pass),
            "s0" => Some(Test423State::S0),
            "s1" => Some(Test423State::S1),
            _ => None,
        }
    }
...
impl Test423Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test423State,
        event: Test423Event,
        current_state: &mut Test423State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test423State::Fail => false,
            Test423State::Pass => false,
            Test423State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test423Event::InternalEvent {
                    ...
                    *current_state = Test423State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test423Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test423State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test423State::S1 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test423Event::ExternalEvent2 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test423State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
