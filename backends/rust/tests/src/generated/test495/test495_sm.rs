...
// From: resources/495/test495.scxml
//
...
//! Generated Rust state machine for `test495`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test495.scxml:4 :: _machine"]
...
impl Test495Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test495State::S0,
...
impl Default for Test495Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test495Policy {
    type State = Test495State;
    type Event = Test495Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test495State::Fail => true,
            Test495State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test495State::Fail => "fail",
            Test495State::Pass => "pass",
            Test495State::S0 => "s0",
            Test495State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test495State::Fail),
            "pass" => Some(Test495State::Pass),
            "s0" => Some(Test495State::S0),
            "s1" => Some(Test495State::S1),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test495Event::Event1);
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test495Event::Event2);
                            meta.set_event_data(event_data);
                            engine.raise(meta);
                        }
...
impl Test495Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test495State,
        event: Test495Event,
        current_state: &mut Test495State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test495State::Fail => false,
            Test495State::Pass => false,
            Test495State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test495Event::Event1 {
                    ...
                    *current_state = Test495State::Fail;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test495Event::Event2 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test495State::S1;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test495State::S1 => {
                ...
                if event == Test495Event::Event1 {
                    ...
                    *current_state = Test495State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test495Event::Null
