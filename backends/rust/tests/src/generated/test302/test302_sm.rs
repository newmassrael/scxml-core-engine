...
// From: resources/302/test302.scxml
//
...
//! Generated Rust state machine for `test302`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test302Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test302Event] = &[];
}
...
impl Test302Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test302State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test302", &io_processors) {
...
impl StatePolicy for Test302Policy {
    type State = Test302State;
    type Event = Test302Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test302State::Fail => true,
            Test302State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test302Event::ErrorExecution => "error.execution",
            Test302Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test302Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test302State::Fail => "fail",
            Test302State::Pass => "pass",
            Test302State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test302State::Fail),
            "pass" => Some(Test302State::Pass),
            "s0" => Some(Test302State::S0),
            _ => None,
        }
    }
...
impl Test302Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test302State,
        event: Test302Event,
        current_state: &mut Test302State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test302State::Fail => false,
            Test302State::Pass => false,
            Test302State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test302Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        ...
                        *current_state = Test302State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test302State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
