...
// From: resources/503/test503.scxml
//
...
//! Generated Rust state machine for `test503`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test503Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test503Event] = &[];
}
...
impl Test503Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test503State::S1,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test503", &io_processors) {
...
impl StatePolicy for Test503Policy {
    type State = Test503State;
    type Event = Test503Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test503State::Fail => true,
            Test503State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test503Event::Bar => "bar",
            Test503Event::ErrorExecution => "error.execution",
            Test503Event::Foo => "foo",
            Test503Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "bar" => Some(Test503Event::Bar),
            "error.execution" => Some(Test503Event::ErrorExecution),
            "foo" => Some(Test503Event::Foo),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test503State::Fail => "fail",
            Test503State::Pass => "pass",
            Test503State::S1 => "s1",
            Test503State::S2 => "s2",
            Test503State::S3 => "s3",
        }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test503State::Fail),
            "pass" => Some(Test503State::Pass),
            "s1" => Some(Test503State::S1),
            "s2" => Some(Test503State::S2),
            "s3" => Some(Test503State::S3),
            _ => None,
        }
    }
        ...
        match state {
            Test503State::S1 => {
                ...
                'action_block: {
                    ...
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test503Event::Foo));

                    // W3C SCXML 3.8.1: <raise event="bar">
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test503Event::Bar));
                }
            }
            _ => {}
        }
...
impl Test503Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test503State,
        event: Test503Event,
        current_state: &mut Test503State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test503State::Fail => false,
            Test503State::Pass => false,
            Test503State::S1 => {
            ...
            Test503State::S2 => {
                ...
                if event == Test503Event::Bar {
                    ...
                    *current_state = Test503State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test503State::S3 => {
                ...
                if event == Test503Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test503State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
