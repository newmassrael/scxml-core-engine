...
// From: resources/407/test407.scxml
//
...
//! Generated Rust state machine for `test407`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test407.scxml:5 :: _machine"]
...
impl Test407Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test407Event] = &[];
}
...
impl Test407Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test407State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test407", &io_processors) {
...
impl StatePolicy for Test407Policy {
    type State = Test407State;
    type Event = Test407Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test407State::Fail => true,
            Test407State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test407Event::ErrorExecution => "error.execution",
            Test407Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test407Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test407State::Fail => "fail",
            Test407State::Pass => "pass",
            Test407State::S0 => "s0",
            Test407State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test407State::Fail),
            "pass" => Some(Test407State::Pass),
            "s0" => Some(Test407State::S0),
            "s1" => Some(Test407State::S1),
            _ => None,
        }
    }
...
impl Test407Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test407State,
        event: Test407Event,
        current_state: &mut Test407State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test407State::Fail => false,
            Test407State::Pass => false,
            Test407State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test407Event::Null {
            ...
            Test407State::S1 => {
                ...
                if event == Test407Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test407State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test407State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
