...
// From: resources/550/test550.scxml
//
...
//! Generated Rust state machine for `test550`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test550Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test550Event] = &[];
}
...
impl Test550Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test550State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test550", &io_processors) {
...
impl StatePolicy for Test550Policy {
    type State = Test550State;
    type Event = Test550Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test550State::Fail => true,
            Test550State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test550Event::ErrorExecution => "error.execution",
            Test550Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test550Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test550State::Fail => "fail",
            Test550State::Pass => "pass",
            Test550State::S0 => "s0",
            Test550State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test550State::Fail),
            "pass" => Some(Test550State::Pass),
            "s0" => Some(Test550State::S0),
            "s1" => Some(Test550State::S1),
            _ => None,
        }
    }
...
impl Test550Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test550State,
        event: Test550Event,
        current_state: &mut Test550State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test550State::Fail => false,
            Test550State::Pass => false,
            Test550State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test550Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        ...
                        *current_state = Test550State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test550State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
