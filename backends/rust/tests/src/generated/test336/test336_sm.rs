...
// From: resources/336/test336.scxml
//
...
//! Generated Rust state machine for `test336`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test336.scxml:6 :: _machine"]
...
impl Test336Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test336Event] =
        &[Test336Event::Bar, Test336Event::Foo];
}
...
impl Test336Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test336State::S0,
...
impl StatePolicy for Test336Policy {
    type State = Test336State;
    type Event = Test336Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test336State::Fail => true,
            Test336State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test336Event::Bar => "bar",
            Test336Event::Baz => "baz",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "bar" => Some(Test336Event::Bar),
            "baz" => Some(Test336Event::Baz),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test336State::Fail => "fail",
            Test336State::Pass => "pass",
            Test336State::S0 => "s0",
            Test336State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test336State::Fail),
            "pass" => Some(Test336State::Pass),
            "s0" => Some(Test336State::S0),
            "s1" => Some(Test336State::S1),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test336Event::Foo);
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test336Event::Baz);
                            // W3C SCXML 5.10.1: External send — preserve sendid and SCXML event processor origintype
                            meta.metadata = sce_rust_runtime::EventMetadata::external(
                                send_id.clone(),
                                ::sce_rust_runtime::SceString::new(),
                            ...
                            let _resolved_target: Option<String> = {
                                ...
                                match se.evaluate_expression(&sid, "_event.origin") {
                                        ...
                                        ) =>
                                    {
                                        ...
                                        engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test336Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                        None
                                    }
                                    Ok(val) => {
                                        ...
                                        if trimmed.starts_with("!") {
                                            ...
                                            {
                                                let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test336Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
...
impl Test336Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test336State,
        event: Test336Event,
        current_state: &mut Test336State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test336State::Fail => false,
            Test336State::Pass => false,
            Test336State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test336Event::Foo {
                    ...
                    *current_state = Test336State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test336Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test336State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test336State::S1 => {
                ...
                if event == Test336Event::Bar {
                    ...
                    *current_state = Test336State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
