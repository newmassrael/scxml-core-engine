...
// From: resources/553/test553.scxml
//
...
//! Generated Rust state machine for `test553`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test553Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test553Event] =
        &[Test553Event::Event1, Test553Event::Timeout];
}
...
impl Test553Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test553State::S0,
...
impl StatePolicy for Test553Policy {
    type State = Test553State;
    type Event = Test553Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test553State::Fail => true,
            Test553State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test553Event::ErrorExecution => "error.execution",
            Test553Event::Event1 => "event1",
            Test553Event::Timeout => "timeout",
            Test553Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test553Event::ErrorExecution),
            "event1" => Some(Test553Event::Event1),
            "timeout" => Some(Test553Event::Timeout),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test553State::Fail => "fail",
            Test553State::Pass => "pass",
            Test553State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test553State::Fail),
            "pass" => Some(Test553State::Pass),
            "s0" => Some(Test553State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
...

                    {
                        ...
                        if !_send_aborted {
                            ...
                            {
                                let mut meta =
                                    sce_rust_runtime::EventWithMetadata::new(Test553Event::Event1);
...
impl Test553Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test553State,
        event: Test553Event,
        current_state: &mut Test553State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test553State::Fail => false,
            Test553State::Pass => false,
            Test553State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test553Event::Timeout {
                    ...
                    *current_state = Test553State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test553Event::Event1 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test553State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
