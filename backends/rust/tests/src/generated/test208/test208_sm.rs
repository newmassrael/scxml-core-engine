...
// From: resources/208/test208.scxml
//
...
//! Generated Rust state machine for `test208`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test208.scxml:6 :: _machine"]
...
impl Test208Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test208Event] = &[Test208Event::Event2];
}
...
impl Test208Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test208State::S0,
            session_id: None,
...
impl Default for Test208Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test208Policy {
    type State = Test208State;
    type Event = Test208Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test208State::Fail => true,
            Test208State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test208State::Fail => "fail",
            Test208State::Pass => "pass",
            Test208State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test208State::Fail),
            "pass" => Some(Test208State::Pass),
            "s0" => Some(Test208State::S0),
            _ => None,
        }
    }
...
impl Test208Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test208State,
        event: Test208Event,
        current_state: &mut Test208State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test208State::Fail => false,
            Test208State::Pass => false,
            Test208State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test208Event::Event2 {
                    ...
                    *current_state = Test208State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test208Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test208State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
