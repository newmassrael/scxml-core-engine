...
impl Test322Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test322Event] = &[];
}
...
impl StatePolicy for Test322Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test322State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test322State::Fail),
            "pass" => Some(Test322State::Pass),
            "s0" => Some(Test322State::S0),
            "s1" => Some(Test322State::S1),
            "s2" => Some(Test322State::S2),
            _ => None,
        }
    }
                ...
                if event == Test322Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test322State::S1)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test322Event::ErrorExecution {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test322State::S2)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test322State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test322State::S2 => {
                if event == Test322Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, _sessionid)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test322State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
