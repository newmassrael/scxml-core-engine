...
impl Test531Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test531Event] = &[Test531Event::Test];
}
...
impl Test531Policy {
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test531", &io_processors) {
...
impl StatePolicy for Test531Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test531State::S0)]
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test531Event::ErrorCommunication => "error.communication",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.communication" => Some(Test531Event::ErrorCommunication),
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test531State::Fail),
            "pass" => Some(Test531State::Pass),
            "s0" => Some(Test531State::S0),
            _ => None,
        }
    }
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "_ioprocessors.basichttp.location") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test531Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test531Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
                        ...
                        if let Some(ref _rt) = _resolved_target {
                            // W3C SCXML C.2: BasicHTTP send to HTTP target
                            {
                                ...
                                if !_rt.starts_with("http://") && !_rt.starts_with("https://") {
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test531Event::ErrorCommunication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL"));
                                } else {
                                    let mut http_params =
                ...
                if event == Test531Event::Test {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test531State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test531State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
