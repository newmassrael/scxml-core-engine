...
impl Test446Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test446Event] = &[];
}
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        // W3C SCXML 5.2.2: Load variable 'var2' from src (global)
        {
            ...
            if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_src(
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                    Test446Event::ErrorExecution,
                    "<data id='var2' src='file:test446.txt'> could not be loaded",
                ));
            }
        }
...
impl StatePolicy for Test446Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test446State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test446State::Fail),
            "pass" => Some(Test446State::Pass),
            "s0" => Some(Test446State::S0),
            "s1" => Some(Test446State::S1),
            _ => None,
        }
    }
                ...
                if event == Test446Event::Null {
                    if self.safe_evaluate_guard("_isArray(var1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test446State::S1)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test446Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test446State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test446State::S1 => {
                if event == Test446Event::Null {
                    if self.safe_evaluate_guard("_isArray(var2)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test446State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
