...
impl Test277Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test277Event] = &[];
}
...
impl StatePolicy for Test277Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test277State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test277State::Fail),
            "pass" => Some(Test277State::Pass),
            "s0" => Some(Test277State::S0),
            "s1" => Some(Test277State::S1),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test277State::S0 => {
                // SCE-MAP: test277.scxml:11 :: s0 :: _state_body
            ...
            Test277State::S1 => {
                // SCE-MAP: test277.scxml:20 :: s1 :: _state_body
                // W3C SCXML 3.8: onentry block 1/1
                // Labeled block allows actions to break out on error (W3C 3.8: error stops block)
                'action_block: {
                    {
                ...
                if event == Test277Event::ErrorExecution {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test277State::S1)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test277State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test277State::S1 => {
                if event == Test277Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test277State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
