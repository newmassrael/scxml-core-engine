...
impl Test310Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test310Event] = &[];
}
...
impl Test310Policy {
    ...
    fn activate(&mut self, state: Test310State) -> bool {
    ...
    fn deactivate(&mut self, state: Test310State) {
...
impl StatePolicy for Test310Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test310State::P)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test310State::Fail),
            "p" => Some(Test310State::P),
            "pass" => Some(Test310State::Pass),
            "s0" => Some(Test310State::S0),
            "s1" => Some(Test310State::S1),
            _ => None,
        }
    }
                ...
                if event == Test310Event::Null {
                    if self.is_state_active("s1") {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test310State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test310Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test310State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
