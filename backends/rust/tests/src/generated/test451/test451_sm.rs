...
// From: resources/451/test451.scxml
//
...
//! Generated Rust state machine for `test451`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test451.scxml:5 :: _machine"]
...
impl Test451Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test451Event] = &[];
}
...
pub struct Test451Policy {
    ...
    active_states: ::sce_rust_runtime::helpers::hierarchy::StateChain<Test451State>,
...
impl Default for Test451Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test451Policy {
    type State = Test451State;
    type Event = Test451Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test451State::Fail => true,
            Test451State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test451State::S0 => Some(Test451State::P),
            Test451State::S1 => Some(Test451State::P),
            _ => None,
        }
    }
    ...
    fn get_parallel_regions(state: Self::State) -> &'static [Self::State] {
        match state {
            Test451State::P => &[Test451State::S0, Test451State::S1],
            _ => &[],
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test451State::Fail => "fail",
            Test451State::P => "p",
            Test451State::Pass => "pass",
            Test451State::S0 => "s0",
            Test451State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test451State::Fail),
            "p" => Some(Test451State::P),
            "pass" => Some(Test451State::Pass),
            "s0" => Some(Test451State::S0),
            "s1" => Some(Test451State::S1),
            _ => None,
        }
    }
    ...
    fn process_transition(
        ...
        if event == Self::null_event() {
            ...
            let mut seen: ::sce_rust_runtime::SceDedupSet<(Test451State, usize)> =
...
impl Test451Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test451State,
        event: Test451Event,
        current_state: &mut Test451State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
        mut collect_mode: Option<&mut TransitionList>,
    ) -> bool {
        match check_state {
            Test451State::Fail => false,
            Test451State::P => false,
            Test451State::Pass => false,
            Test451State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test451Event::Null {
                    if self.is_state_active("s1") {
                        ...
                        if let Some(ref mut collect) = collect_mode {
                            ...
                            *current_state = Test451State::Pass;
                        ...
                        if let Some(ref mut collect) = collect_mode {
                            ...
                            *current_state = Test451State::Fail;
                            *transition_taken = true;
                        }
                        return true;
                    }
    ...
    fn compute_exit_set(
        ...
        is_internal: bool,
        is_targetless: bool,
    ) -> ::sce_rust_runtime::helpers::hierarchy::StateChain<Test451State> {
    ...
    fn execute_microstep(
        ...
        let mut states_to_exit: ::sce_rust_runtime::helpers::hierarchy::StateChain<Test451State> =
