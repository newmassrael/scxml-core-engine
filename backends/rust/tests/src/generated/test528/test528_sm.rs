...
// From: resources/528/test528.scxml
//
...
//! Generated Rust state machine for `test528`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test528Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test528Event] = &[];
}
...
impl Test528Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test528State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test528", &io_processors) {
...
impl StatePolicy for Test528Policy {
    type State = Test528State;
    type Event = Test528Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test528State::Fail => true,
            Test528State::Pass => true,
            Test528State::S02 => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test528State::S01 => Some(Test528State::S0),
            Test528State::S02 => Some(Test528State::S0),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test528State::Fail => "fail",
            Test528State::Pass => "pass",
            Test528State::S0 => "s0",
            Test528State::S01 => "s01",
            Test528State::S02 => "s02",
            Test528State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test528State::Fail),
            "pass" => Some(Test528State::Pass),
            "s0" => Some(Test528State::S0),
            "s01" => Some(Test528State::S01),
            "s02" => Some(Test528State::S02),
            "s1" => Some(Test528State::S1),
            _ => None,
        }
    }
    ...
    fn get_initial_children(
        ...
        match state {
            Test528State::S0 => {
                ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([Test528State::S01])
            }
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test528State::S0 => Test528State::S01,
            _ => state,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test528State::S02 => {
                ...
                if done_data_ok {
                    let mut done_meta =
                        sce_rust_runtime::EventWithMetadata::new(Test528Event::DoneStateS0);
...
impl Test528Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test528State,
        event: Test528Event,
        current_state: &mut Test528State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test528State::Fail => false,
            Test528State::Pass => false,
            Test528State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test528Event::ErrorExecution {
                    ...
                    *current_state = Test528State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test528Event::DoneStateS0 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test528State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test528State::S01 => {
                ...
                if event == Test528Event::Null {
                    // Unconditional eventless transition
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                    *current_state = Test528State::S02;
                    *transition_taken = true;
                    return true;
                }
                false
            }
            Test528State::S02 => false,
            Test528State::S1 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test528Event::DoneStateS0 {
                    ...
                    *current_state = Test528State::Pass;
                    *transition_taken = true;
                    return true;
                }
