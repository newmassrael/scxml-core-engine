...
// From: resources/453/test453.scxml
//
...
//! Generated Rust state machine for `test453`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test453Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test453Event] = &[];
}
...
impl Test453Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test453State::S0,
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = se.execute_script(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                ...
                "<data id='var1'> function expression failed",
            ));
        }
...
impl StatePolicy for Test453Policy {
    type State = Test453State;
    type Event = Test453Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test453State::Fail => true,
            Test453State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test453State::Fail => "fail",
            Test453State::Pass => "pass",
            Test453State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test453State::Fail),
            "pass" => Some(Test453State::Pass),
            "s0" => Some(Test453State::S0),
            _ => None,
        }
    }
...
impl Test453Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test453State,
        event: Test453Event,
        current_state: &mut Test453State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test453State::Fail => false,
            Test453State::Pass => false,
            Test453State::S0 => {
                ...
                if event == Test453Event::Event1 {
                    ...
                    if self.safe_evaluate_guard("_scxml_eq(var1(2), 3)", engine) {
                        ...
                        *current_state = Test453State::Pass;
                        *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test453Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test453State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
