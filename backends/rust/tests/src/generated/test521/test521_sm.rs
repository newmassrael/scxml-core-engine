...
// From: resources/521/test521.scxml
//
...
//! Generated Rust state machine for `test521`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test521Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test521Event] = &[];
}
...
impl Test521Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test521State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test521", &io_processors) {
...
impl StatePolicy for Test521Policy {
    type State = Test521State;
    type Event = Test521Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test521State::Fail => true,
            Test521State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test521Event::ErrorCommunication => "error.communication",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.communication" => Some(Test521Event::ErrorCommunication),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test521State::Fail => "fail",
            Test521State::Pass => "pass",
            Test521State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test521State::Fail),
            "pass" => Some(Test521State::Pass),
            "s0" => Some(Test521State::S0),
            _ => None,
        }
    }
            ...
            Test521State::S0 => {
                ...
                'action_block: {
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "nil") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test521Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test521Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test521Event::Timeout);
...
impl Test521Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test521State,
        event: Test521Event,
        current_state: &mut Test521State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test521State::Fail => false,
            Test521State::Pass => false,
            Test521State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test521Event::ErrorCommunication {
                    ...
                    *current_state = Test521State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test521Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test521State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
