...
impl Test156Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test156Event] = &[];
}
...
impl StatePolicy for Test156Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test156State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test156State::Fail),
            "pass" => Some(Test156State::Pass),
            "s0" => Some(Test156State::S0),
            _ => None,
        }
    }
                                        ...
                                        'foreach_body: {
                                            {
                                                    ...
                                                    se.execute_script(&sid, &assign_script)
                                                {
                                                    ...
                                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test156Event::ErrorExecution, "<assign> to 'Var1' failed"));
...

                                            {
                                                    ...
                                                    se.execute_script(&sid, &assign_script)
                                                {
                                                    ...
                                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test156Event::ErrorExecution, "<assign> to 'Var5' failed"));
                                                    // W3C SCXML 4.6: Stop current foreach iteration and loop on error
                                                    iteration_success = false;
                                                    break 'foreach_body;
                                                }
                ...
                if event == Test156Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test156State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test156Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test156State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
