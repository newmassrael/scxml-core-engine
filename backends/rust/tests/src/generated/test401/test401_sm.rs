...
// From: resources/401/test401.scxml
//
...
//! Generated Rust state machine for `test401`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test401.scxml:7 :: _machine"]
...
impl Test401Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test401Event] =
        &[Test401Event::Error, Test401Event::Foo];
}
...
impl Test401Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test401State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test401", &io_processors) {
...
impl StatePolicy for Test401Policy {
    type State = Test401State;
    type Event = Test401Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test401State::Fail => true,
            Test401State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test401Event::Error => "error",
            Test401Event::ErrorExecution => "error.execution",
            Test401Event::Foo => "foo",
            Test401Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error" => Some(Test401Event::Error),
            "error.execution" => Some(Test401Event::ErrorExecution),
            "foo" => Some(Test401Event::Foo),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test401State::Fail => "fail",
            Test401State::Pass => "pass",
            Test401State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test401State::Fail),
            "pass" => Some(Test401State::Pass),
            "s0" => Some(Test401State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test401Event::Foo);
...
impl Test401Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test401State,
        event: Test401Event,
        current_state: &mut Test401State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test401State::Fail => false,
            Test401State::Pass => false,
            Test401State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test401Event::Foo {
                    ...
                    *current_state = Test401State::Fail;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Precomputed prefix match for "error"
                if event == Test401Event::Error || event == Test401Event::ErrorExecution {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test401State::Pass;
                    *transition_taken = true;
                    return true;
                }
                false
