...
// From: resources/561/test561.scxml
//
...
//! Generated Rust state machine for `test561`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test561Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test561Event] = &[Test561Event::Foo];
}
...
impl Test561Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test561State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test561", &io_processors) {
...
impl StatePolicy for Test561Policy {
    type State = Test561State;
    type Event = Test561Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test561State::Fail => true,
            Test561State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test561State::Fail => "fail",
            Test561State::Pass => "pass",
            Test561State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test561State::Fail),
            "pass" => Some(Test561State::Pass),
            "s0" => Some(Test561State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test561Event::Foo);
...
impl Test561Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test561State,
        event: Test561Event,
        current_state: &mut Test561State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test561State::Fail => false,
            Test561State::Pass => false,
            Test561State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test561Event::Foo {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_scxml_eq(_event.data:getElementsByTagName(\"book\")[2]:getAttribute(\"title\"), \"title2\")", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            ...
                            *current_state = Test561State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test561Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test561State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
