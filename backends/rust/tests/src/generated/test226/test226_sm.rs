...
// From: resources/226/test226.scxml
//
...
//! Generated Rust state machine for `test226`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test226Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test226Event] = &[Test226Event::VarBound];
}
...
pub struct Test226Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test226State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test226sub1)
    child_invoke_0: Option<Box<sce_rust_runtime::Engine<super::test226sub1_sm::Test226sub1Policy>>>,
...
impl Test226Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test226State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test226", &io_processors) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy =
                    super::test226sub1_sm::Test226sub1Policy::new(self.script_engine.clone());
...
impl StatePolicy for Test226Policy {
    type State = Test226State;
    type Event = Test226Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test226State::Fail => true,
            Test226State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test226Event::CancelInvoke => "cancel.invoke",
            Test226Event::DoneInvoke => "done.invoke",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test226Event::CancelInvoke),
            "done.invoke" => Some(Test226Event::DoneInvoke),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test226State::Fail => "fail",
            Test226State::Pass => "pass",
            Test226State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test226State::Fail),
            "pass" => Some(Test226State::Pass),
            "s0" => Some(Test226State::S0),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test226State::S0 => {
                // SCE-MAP: test226.scxml:10 :: s0 :: _state_body
...
impl Test226Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test226State,
        event: Test226Event,
        current_state: &mut Test226State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test226State::Fail => false,
            Test226State::Pass => false,
            Test226State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test226Event::VarBound {
                    ...
                    *current_state = Test226State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test226Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test226State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
