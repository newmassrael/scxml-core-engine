...
impl Test228Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test228Event] = &[];
}
...
impl StatePolicy for Test228Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test228State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test228State::Fail),
            "pass" => Some(Test228State::Pass),
            "s0" => Some(Test228State::S0),
            "s1" => Some(Test228State::S1),
            _ => None,
        }
    }
    ...
    fn first_enabled_transition(
        ...
        match state {
                ...
                if event == Test228Event::DoneInvoke {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test228State::S1)],
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test228State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test228State::S1 => {
                if event == Test228Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, \"foo\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test228State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
