...
// From: resources/513/test513.scxml
//
...
//! Generated Rust state machine for `test513`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test513.scxml:20 :: _machine"]
...
impl Test513Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test513Event] =
        &[Test513Event::Test, Test513Event::Timeout];
}
...
impl Test513Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test513State::S0,
...
impl StatePolicy for Test513Policy {
    type State = Test513State;
    type Event = Test513Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test513State::Fail => true,
            Test513State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test513Event::ErrorCommunication => "error.communication",
            Test513Event::ErrorExecution => "error.execution",
            Test513Event::Test => "test",
            Test513Event::Timeout => "timeout",
            Test513Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.communication" => Some(Test513Event::ErrorCommunication),
            "error.execution" => Some(Test513Event::ErrorExecution),
            "test" => Some(Test513Event::Test),
            "timeout" => Some(Test513Event::Timeout),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test513State::Fail => "fail",
            Test513State::Pass => "pass",
            Test513State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test513State::Fail),
            "pass" => Some(Test513State::Pass),
            "s0" => Some(Test513State::S0),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test513State::Fail => {
                // SCE-MAP: test513.scxml:52 :: fail :: _state_body
        ...
        Err(e) => ::sce_rust_runtime::sce_log_error!("Log expression eval failed: {}", e),
            ...
            Test513State::Pass => {
                // SCE-MAP: test513.scxml:46 :: pass :: _state_body
                // W3C SCXML 3.8: onentry block 1/1
                // Labeled block allows actions to break out on error (W3C 3.8: error stops block)
                'action_block: {
                    // W3C SCXML 4.7: <log> with script engine expression
        ...
        Err(e) => ::sce_rust_runtime::sce_log_error!("Log expression eval failed: {}", e),
            ...
            Test513State::S0 => {
                // SCE-MAP: test513.scxml:22 :: s0 :: _state_body
                // W3C SCXML 3.8: onentry block 1/1
                // Labeled block allows actions to break out on error (W3C 3.8: error stops block)
                'action_block: {
                    {
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "_ioprocessors.basichttp.location") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test513Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test513Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
                        ...
                        if let Some(ref _rt) = _resolved_target {
                            // W3C SCXML C.2: BasicHTTP send to HTTP target
                            {
                                ...
                                if !_rt.starts_with("http://") && !_rt.starts_with("https://") {
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test513Event::ErrorCommunication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL"));
                                } else {
                                    let http_params =
...
impl Test513Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test513State,
        event: Test513Event,
        current_state: &mut Test513State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test513State::Fail => false,
            Test513State::Pass => false,
            Test513State::S0 => {
