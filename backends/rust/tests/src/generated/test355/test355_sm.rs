...
impl Test355Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test355Event] = &[];
}
...
impl StatePolicy for Test355Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test355State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test355State::Fail),
            "pass" => Some(Test355State::Pass),
            "s0" => Some(Test355State::S0),
            "s1" => Some(Test355State::S1),
            _ => None,
        }
    }
                ...
                if event == Test355Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test355State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test355Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test355State::Fail)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
