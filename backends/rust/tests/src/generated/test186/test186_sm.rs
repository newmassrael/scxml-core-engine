...
#![doc = "SCE-MAP: test186.scxml:6 :: _machine"]
...
impl Test186Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test186Event] = &[Test186Event::Event1];
}
...
impl StatePolicy for Test186Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test186State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test186State::Fail),
            "pass" => Some(Test186State::Pass),
            "s0" => Some(Test186State::S0),
            "s1" => Some(Test186State::S1),
            _ => None,
        }
    }
                ...
                if event == Test186Event::Event1 {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test186State::S1)],
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test186State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test186State::S1 => {
                if event == Test186Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var2, 1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test186State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
