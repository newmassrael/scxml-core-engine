...
// From: resources/338/test338.scxml
//
...
//! Generated Rust state machine for `test338`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test338Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test338Event] =
        &[Test338Event::Event0, Test338Event::Event1];
}
...
pub struct Test338Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test338State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test338__sce_synth_invoke__invoke_0)
    child_invoke_0: Option<
        Box<
            sce_rust_runtime::Engine<
                super::test338__sce_synth_invoke__invoke_0_sm::Test338SceSynthInvokeInvoke0Policy,
            >,
        >,
    >,
...
impl Test338Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test338State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test338", &io_processors) {
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy = super::test338__sce_synth_invoke__invoke_0_sm::Test338SceSynthInvokeInvoke0Policy::new();
...
impl StatePolicy for Test338Policy {
    type State = Test338State;
    type Event = Test338Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test338State::Fail => true,
            Test338State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test338Event::CancelInvoke => "cancel.invoke",
            Test338Event::DoneInvoke => "done.invoke",
            Test338Event::ErrorExecution => "error.execution",
            Test338Event::Event0 => "event0",
            Test338Event::Event1 => "event1",
            Test338Event::Timeout => "timeout",
            Test338Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test338Event::CancelInvoke),
            "done.invoke" => Some(Test338Event::DoneInvoke),
            "error.execution" => Some(Test338Event::ErrorExecution),
            "event0" => Some(Test338Event::Event0),
            "event1" => Some(Test338Event::Event1),
            "timeout" => Some(Test338Event::Timeout),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test338State::Fail => "fail",
            Test338State::Pass => "pass",
            Test338State::S0 => "s0",
            Test338State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test338State::Fail),
            "pass" => Some(Test338State::Pass),
            "s0" => Some(Test338State::S0),
            "s1" => Some(Test338State::S1),
            _ => None,
        }
    }
...
impl Test338Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test338State,
        event: Test338Event,
        current_state: &mut Test338State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test338State::Fail => false,
            Test338State::Pass => false,
            Test338State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test338Event::Event1 {
                    ...
                    *current_state = Test338State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test338Event::Event0 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    ...
                    *current_state = Test338State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test338State::S1 => {
                ...
                if event == Test338Event::Null {
                    if self.safe_evaluate_guard("(Var1 == Var2)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test338State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
