...
// From: resources/339/test339.scxml
//
...
//! Generated Rust state machine for `test339`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test339Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test339Event] = &[];
}
...
impl Test339Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test339State::S0,
            session_id: None,
...
impl Default for Test339Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test339Policy {
    type State = Test339State;
    type Event = Test339Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test339State::Fail => true,
            Test339State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test339State::Fail => "fail",
            Test339State::Pass => "pass",
            Test339State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test339State::Fail),
            "pass" => Some(Test339State::Pass),
            "s0" => Some(Test339State::S0),
            _ => None,
        }
    }
        ...
        match state {
            Test339State::S0 => {
                ...
                'action_block: {
                    ...
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test339Event::Foo));
                }
            }
            _ => {}
        }
...
impl Test339Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test339State,
        event: Test339Event,
        current_state: &mut Test339State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test339State::Fail => false,
            Test339State::Pass => false,
            Test339State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test339Event::Foo {
                    ...
                    *current_state = Test339State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test339Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test339State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
