...
impl Test562Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test562Event] = &[Test562Event::Foo];
}
...
impl StatePolicy for Test562Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test562State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test562State::Fail),
            "pass" => Some(Test562State::Pass),
            "s0" => Some(Test562State::S0),
            _ => None,
        }
    }
                        ...
                        let event_data_string: String =
                            {
                                ...
                                let se = self.script_engine.clone();
                                let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                                match se.evaluate_expression(&sid, "\"this is a string\"") {
        ...
        match state {
            Test562State::S0 => {
                if event == Test562Event::Foo {
                    if self
                        .safe_evaluate_guard("_scxml_eq(_event.data, \"this is a string\")", engine)
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test562State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test562State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
