...
// From: resources/519/test519.scxml
//
...
//! Generated Rust state machine for `test519`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test519.scxml:3 :: _machine"]
...
impl Test519Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test519Event] = &[Test519Event::Test];
}
...
impl Test519Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test519State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test519", &io_processors) {
...
impl StatePolicy for Test519Policy {
    type State = Test519State;
    type Event = Test519Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test519State::Fail => true,
            Test519State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test519Event::ErrorCommunication => "error.communication",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.communication" => Some(Test519Event::ErrorCommunication),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test519State::Fail => "fail",
            Test519State::Pass => "pass",
            Test519State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test519State::Fail),
            "pass" => Some(Test519State::Pass),
            "s0" => Some(Test519State::S0),
            _ => None,
        }
    }
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "_ioprocessors.basichttp.location") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test519Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test519Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
...
impl Test519Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test519State,
        event: Test519Event,
        current_state: &mut Test519State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test519State::Fail => false,
            Test519State::Pass => false,
            Test519State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test519Event::Test {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_scxml_eq(_event.data.param1, 1)", engine) {
                        ...
                        *current_state = Test519State::Pass;
                        *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test519Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test519State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
