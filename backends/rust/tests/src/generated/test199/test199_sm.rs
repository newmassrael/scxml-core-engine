...
// From: resources/199/test199.scxml
//
...
//! Generated Rust state machine for `test199`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test199Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test199Event] = &[];
}
...
impl Test199Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test199State::S0,
            session_id: None,
...
impl Default for Test199Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test199Policy {
    type State = Test199State;
    type Event = Test199Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test199State::Fail => true,
            Test199State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test199State::Fail => "fail",
            Test199State::Pass => "pass",
            Test199State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test199State::Fail),
            "pass" => Some(Test199State::Pass),
            "s0" => Some(Test199State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        // W3C SCXML 6.2: Unsupported send type "unsupported_type" raises error.execution
                        {
                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test199Event::ErrorExecution, "<send type='unsupported_type'> names a processor this platform does not support");
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test199Event::Timeout);
...
impl Test199Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test199State,
        event: Test199Event,
        current_state: &mut Test199State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test199State::Fail => false,
            Test199State::Pass => false,
            Test199State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test199Event::ErrorExecution {
                    ...
                    *current_state = Test199State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test199Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test199State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
