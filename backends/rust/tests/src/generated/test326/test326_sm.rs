...
impl Test326Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test326Event] = &[];
}
...
impl StatePolicy for Test326Policy {
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test326State::Fail),
            "pass" => Some(Test326State::Pass),
            "s0" => Some(Test326State::S0),
            "s1" => Some(Test326State::S1),
            "s2" => Some(Test326State::S2),
            _ => None,
        }
    }
