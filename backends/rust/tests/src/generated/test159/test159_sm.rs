...
impl Test159Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test159Event] = &[];
}
...
impl StatePolicy for Test159Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test159State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test159State::Fail),
            "pass" => Some(Test159State::Pass),
            "s0" => Some(Test159State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        // W3C SCXML 6.2: Invalid target "!invalid" raises error.execution
                        {
                            ...
                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test159Event::ErrorExecution, "<send target='!invalid'> is not a target this processor can address");
                ...
                if event == Test159Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test159State::Fail)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test159Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test159State::Pass)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
