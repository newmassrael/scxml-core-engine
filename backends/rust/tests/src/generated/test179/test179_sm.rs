...
impl Test179Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test179Event] = &[Test179Event::Event1];
}
...
impl StatePolicy for Test179Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test179State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test179State::Fail),
            "pass" => Some(Test179State::Pass),
            "s0" => Some(Test179State::S0),
            _ => None,
        }
    }
                        ...
                        let event_data_string: String =
                            {
                                ...
                                let se = self.script_engine.clone();
                                let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                                match se.evaluate_expression(&sid, "123") {
        ...
        match state {
            Test179State::S0 => {
                if event == Test179Event::Event1 {
                    if self.safe_evaluate_guard("_scxml_eq(_event.data, 123)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test179State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test179State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
