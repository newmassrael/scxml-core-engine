...
// From: resources/216/test216.scxml
//
...
//! Generated Rust state machine for `test216`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test216Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test216Event] = &[];
}
...
pub struct Test216Policy {
    ...
    last_transition_source_state: Test216State,
    ...
    child_invoke_0:
        Option<Box<sce_rust_runtime::Engine<super::test216_hybrid0_sm::Test216Hybrid0Policy>>>,
...
impl Test216Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test216State::S0,
                ...
                match se.evaluate_expression(&sid, "Var1") {
                    Ok(sce_rust_runtime::ScriptValue::Null) | Err(_) => {
                        ::sce_rust_runtime::sce_log_error!(
                            "Hybrid invoke: srcexpr 'Var1' evaluation failed"
                        );
                        engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                            Test216Event::ErrorExecution,
                            "<invoke> srcexpr 'Var1' failed to evaluate",
                        ));
                        continue;
                    }
                    Ok(_path) => {
...
impl StatePolicy for Test216Policy {
    type State = Test216State;
    type Event = Test216Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test216State::Fail => true,
            Test216State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test216State::Fail => "fail",
            Test216State::Pass => "pass",
            Test216State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test216State::Fail),
            "pass" => Some(Test216State::Pass),
            "s0" => Some(Test216State::S0),
            _ => None,
        }
    }
            ...
            Test216State::S0 => {
                ...
                'action_block: {
...

                    {
                        ...
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        let expr = "\"file:test216sub1.scxml\"";
...
impl Test216Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test216State,
        event: Test216Event,
        current_state: &mut Test216State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test216State::Fail => false,
            Test216State::Pass => false,
            Test216State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test216Event::DoneInvoke {
                    ...
                    *current_state = Test216State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test216Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test216State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
