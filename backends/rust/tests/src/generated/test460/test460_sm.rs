...
// From: resources/460/test460.scxml
//
...
//! Generated Rust state machine for `test460`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test460Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test460Event] = &[];
}
...
impl Test460Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test460State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test460", &io_processors) {
...
impl StatePolicy for Test460Policy {
    type State = Test460State;
    type Event = Test460Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test460State::Fail => true,
            Test460State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test460Event::ErrorExecution => "error.execution",
            Test460Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test460Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test460State::Fail => "fail",
            Test460State::Pass => "pass",
            Test460State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test460State::Fail),
            "pass" => Some(Test460State::Pass),
            "s0" => Some(Test460State::S0),
            _ => None,
        }
    }
                                        ...
                                        'foreach_body: {
                                            {
                                                ...
                                                let se = self.script_engine.clone();
                                                let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                                                let expr = "_concat(_concat({}, Var1), {4})";
                                                    ...
                                                    se.execute_script(&sid, &assign_script)
                                                {
                                                    ...
                                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test460Event::ErrorExecution, "<assign> to 'Var1' failed"));
...

                                            {
                                                    ...
                                                    se.execute_script(&sid, &assign_script)
                                                {
                                                    ...
                                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test460Event::ErrorExecution, "<assign> to 'Var2' failed"));
                                                    // W3C SCXML 4.6: Stop current foreach iteration and loop on error
                                                    iteration_success = false;
                                                    break 'foreach_body;
                                                }
...
impl Test460Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test460State,
        event: Test460Event,
        current_state: &mut Test460State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test460State::Fail => false,
            Test460State::Pass => false,
            Test460State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test460Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var2, 3)", engine) {
                        ...
                        *current_state = Test460State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test460State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
