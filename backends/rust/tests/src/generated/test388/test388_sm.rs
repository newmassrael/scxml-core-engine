...
/// never in a configuration — so it is named apart from `Test388State`,
...
impl Test388Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test388Event] =
        &[Test388Event::Entering, Test388Event::Timeout];
}
...
impl StatePolicy for Test388Policy {
    ...
    fn get_child_states(state: Self::State) -> &'static [Self::State] {
        match state {
            Test388State::S0 => &[Test388State::S01, Test388State::S02],
            Test388State::S01 => &[Test388State::S011, Test388State::S012],
            Test388State::S02 => &[Test388State::S021, Test388State::S022],
            _ => &[],
        }
    }
    ...
    fn get_initial_targets(
        ...
        match state {
            Test388State::S0 => &[::sce_rust_runtime::EntryTarget::State(Test388State::S01)],
            Test388State::S01 => &[::sce_rust_runtime::EntryTarget::State(Test388State::S011)],
            Test388State::S02 => &[::sce_rust_runtime::EntryTarget::State(Test388State::S021)],
            _ => &[],
        }
    }
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test388State::S012)]
    }

    // §scxml-3.10: the state a <history> is declared in.
    fn get_history_parent(history: Self::History) -> Self::State {
        match history {
            Test388History::S0HistDeep => Test388State::S0,
            Test388History::S0HistShallow => Test388State::S0,
        }
    }
    ...
    fn get_history_default_targets(
        ...
        match history {
            Test388History::S0HistDeep => {
                &[::sce_rust_runtime::EntryTarget::State(Test388State::S022)]
            }
            Test388History::S0HistShallow => {
                &[::sce_rust_runtime::EntryTarget::State(Test388State::S02)]
            }
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test388State::Fail),
            "pass" => Some(Test388State::Pass),
            "s0" => Some(Test388State::S0),
            "s01" => Some(Test388State::S01),
            "s011" => Some(Test388State::S011),
            "s012" => Some(Test388State::S012),
            "s02" => Some(Test388State::S02),
            "s021" => Some(Test388State::S021),
            "s022" => Some(Test388State::S022),
            "s1" => Some(Test388State::S1),
            "s2" => Some(Test388State::S2),
            _ => None,
        }
    }
    ...
    fn history_value(&self, history: Self::History) -> Option<&[Self::State]> {
        match history {
            Test388History::S0HistDeep => self.history_s0HistDeep.as_deref(),
            Test388History::S0HistShallow => self.history_s0HistShallow.as_deref(),
        }
    }
            ...
            Test388State::S0 => {
                if event == Test388Event::EnteringS012 {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test388State::S1)],
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                        });
                ...
                if event == Test388Event::EnteringS012 {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test388State::S2)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
                    ...
                    || event == Test388Event::EnteringS022
                {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test388State::Fail)],
                            transition_index: 2,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test388Event::EnteringS011 {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 3)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test388State::Pass)],
                            transition_index: 3,
                            has_actions: false,
                            is_internal: false,
                        });
