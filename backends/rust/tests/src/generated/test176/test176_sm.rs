...
// From: resources/176/test176.scxml
//
...
//! Generated Rust state machine for `test176`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test176.scxml:5 :: _machine"]
...
impl Test176Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test176Event] = &[Test176Event::Event1];
}
...
impl Test176Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test176State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test176", &io_processors) {
...
impl StatePolicy for Test176Policy {
    type State = Test176State;
    type Event = Test176Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test176State::Fail => true,
            Test176State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test176State::Fail => "fail",
            Test176State::Pass => "pass",
            Test176State::S0 => "s0",
            Test176State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test176State::Fail),
            "pass" => Some(Test176State::Pass),
            "s0" => Some(Test176State::S0),
            "s1" => Some(Test176State::S1),
            _ => None,
        }
    }
            ...
            Test176State::S0 => {
                ...
                'action_block: {
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test176Event::Event1);
...
impl Test176Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test176State,
        event: Test176Event,
        current_state: &mut Test176State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test176State::Fail => false,
            Test176State::Pass => false,
            Test176State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test176Event::Event1 {
                    ...
                    *current_state = Test176State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test176Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test176State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test176State::S1 => {
                ...
                if event == Test176Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var2, 2)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test176State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
