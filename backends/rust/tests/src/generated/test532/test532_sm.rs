...
impl Test532Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test532Event] = &[Test532Event::HTTPPOST];
}
...
impl Test532Policy {
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test532", &io_processors) {
...
impl StatePolicy for Test532Policy {
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test532Event::Empty => "",
            Test532Event::ErrorCommunication => "error.communication",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            ...
            "error.communication" => Some(Test532Event::ErrorCommunication),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test532State::Fail => "fail",
            Test532State::Pass => "pass",
            Test532State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test532State::Fail),
            "pass" => Some(Test532State::Pass),
            "s0" => Some(Test532State::S0),
            _ => None,
        }
    }
                        ...
                        let event_data_string: String =
                            {
                                ...
                                let se = self.script_engine.clone();
                                let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                                match se.evaluate_expression(&sid, "\"some content\"") {
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "_ioprocessors.basichttp.location") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test532Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test532Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
                        ...
                        if let Some(ref _rt) = _resolved_target {
                            // W3C SCXML C.2: BasicHTTP send to HTTP target
                            {
                                ...
                                if !_rt.starts_with("http://") && !_rt.starts_with("https://") {
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test532Event::ErrorCommunication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL"));
                                } else {
                                    let http_params =
