...
// From: resources/525/test525.scxml
//
...
//! Generated Rust state machine for `test525`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test525.scxml:6 :: _machine"]
// SCE-MAP: test525.scxml:6 :: _machine
...
impl Test525Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test525Event] = &[];
}
...
impl Test525Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test525State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test525", &io_processors) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::eval_or_set_string(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                ...
                "<data id='Var1'> content failed to initialise",
            ));
        }
...
impl StatePolicy for Test525Policy {
    type State = Test525State;
    type Event = Test525Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test525State::Fail => true,
            Test525State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test525Event::ErrorExecution => "error.execution",
            Test525Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test525Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test525State::Fail => "fail",
            Test525State::Pass => "pass",
            Test525State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test525State::Fail),
            "pass" => Some(Test525State::Pass),
            "s0" => Some(Test525State::S0),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test525State::S0 => {
                ...
                'action_block: {
                    {
                        // W3C SCXML 5.6: <foreach array="Var1" item="Var3">
                        self.ensure_script_engine();
                        let sid = self.session_id.as_ref().unwrap().clone();
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        ...
                        if !sce_rust_runtime::helpers::foreach::is_legal_variable_name(item_name) {
                            ...
                            match se.evaluate_expression(&sid, "Var1") {
                                ...
                                Err(e) => {
                                    ::sce_rust_runtime::sce_log_error!(
                                        "Foreach: failed to evaluate array 'Var1': {}",
                                        e
                                    );
                                    engine.raise(
                                        sce_rust_runtime::EventWithMetadata::platform_error(
...
impl Test525Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test525State,
        event: Test525Event,
        current_state: &mut Test525State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test525State::Fail => false,
            Test525State::Pass => false,
            Test525State::S0 => {
                ...
                if event == Test525Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var2, 3)", engine) {
                        ...
                        *current_state = Test525State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test525State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
