...
// From: resources/567/test567.scxml
//
...
//! Generated Rust state machine for `test567`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test567Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test567Event] = &[Test567Event::Test];
}
...
impl Test567Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test567State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test567", &io_processors) {
...
impl StatePolicy for Test567Policy {
    type State = Test567State;
    type Event = Test567Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test567State::Fail => true,
            Test567State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test567Event::ErrorCommunication => "error.communication",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.communication" => Some(Test567Event::ErrorCommunication),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test567State::Fail => "fail",
            Test567State::Pass => "pass",
            Test567State::S0 => "s0",
            Test567State::S1 => "s1",
        }
    }
                                        ...
                                        std::collections::HashMap::<String, Vec<String>>::new();
                                    {
                                        ...
                                        match se.evaluate_expression(&sid, "2") {
                                            ...
                                            Err(e) => {
                                                ...
                                                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test567Event::ErrorExecution, "<send> <param name='param1'> expr failed to evaluate"));
                                            }
                                        }
                                    }
...
impl Test567Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test567State,
        event: Test567Event,
        current_state: &mut Test567State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test567State::Fail => false,
            Test567State::Pass => false,
            Test567State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test567Event::Test {
                    ...
                    *current_state = Test567State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test567Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test567State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test567State::S1 => {
                ...
                if event == Test567Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test567State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
