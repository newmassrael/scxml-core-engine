...
impl Test312Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test312Event] = &[];
}
...
impl StatePolicy for Test312Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test312State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test312State::Fail),
            "pass" => Some(Test312State::Pass),
            "s0" => Some(Test312State::S0),
            _ => None,
        }
    }
                ...
                if event == Test312Event::ErrorExecution {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test312State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test312State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
