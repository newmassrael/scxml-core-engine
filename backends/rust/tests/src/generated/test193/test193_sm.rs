...
// From: resources/193/test193.scxml
//
...
//! Generated Rust state machine for `test193`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test193.scxml:5 :: _machine"]
...
impl Test193Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test193Event] = &[
        Test193Event::Event1,
        Test193Event::Internal,
        Test193Event::Timeout,
    ];
}
...
pub struct Test193Policy {
    ...
    last_transition_source_state: Test193State,
...
impl Test193Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test193State::S0,
...
impl Default for Test193Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test193Policy {
    type State = Test193State;
    type Event = Test193Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test193State::Fail => true,
            Test193State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test193Event::ErrorExecution => "error.execution",
            Test193Event::Event1 => "event1",
            Test193Event::Internal => "internal",
            Test193Event::Timeout => "timeout",
            Test193Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test193Event::ErrorExecution),
            "event1" => Some(Test193Event::Event1),
            "internal" => Some(Test193Event::Internal),
            "timeout" => Some(Test193Event::Timeout),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test193State::Fail => "fail",
            Test193State::Pass => "pass",
            Test193State::S0 => "s0",
            Test193State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test193State::Fail),
            "pass" => Some(Test193State::Pass),
            "s0" => Some(Test193State::S0),
            "s1" => Some(Test193State::S1),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test193Event::Internal);
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test193Event::Event1);
                            // W3C SCXML 5.10.1: External send — preserve sendid and SCXML event processor origintype
                            meta.metadata = sce_rust_runtime::EventMetadata::external(
                                send_id.clone(),
                                ::sce_rust_runtime::SceString::new(),
...
impl Test193Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test193State,
        event: Test193Event,
        current_state: &mut Test193State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test193State::Fail => false,
            Test193State::Pass => false,
            Test193State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test193Event::Event1 {
                    ...
                    *current_state = Test193State::Fail;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test193Event::Internal {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test193State::S1;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test193State::S1 => {
                ...
                if event == Test193Event::Event1 {
                    ...
                    *current_state = Test193State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test193Event::Timeout {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
