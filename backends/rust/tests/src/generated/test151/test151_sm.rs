...
impl Test151Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test151Event] = &[Test151Event::Error];
}
...
impl StatePolicy for Test151Policy {
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test151State::Fail),
            "pass" => Some(Test151State::Pass),
            "s0" => Some(Test151State::S0),
            "s1" => Some(Test151State::S1),
            "s2" => Some(Test151State::S2),
            _ => None,
        }
    }
    ...
    fn try_transition_in_state(
        ...
        match check_state {
            ...
            Test151State::S2 => {
                ...
                if event == Test151Event::Null {
                    if self.safe_evaluate_guard("(_typeof(Var5) ~= \"undefined\")", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
