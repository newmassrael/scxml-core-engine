...
// From: resources/313/test313.scxml
//
...
//! Generated Rust state machine for `test313`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test313Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test313Event] = &[];
}
...
impl Test313Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test313State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test313", &io_processors) {
...
impl StatePolicy for Test313Policy {
    type State = Test313State;
    type Event = Test313Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test313State::Fail => true,
            Test313State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test313State::Fail => "fail",
            Test313State::Pass => "pass",
            Test313State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test313State::Fail),
            "pass" => Some(Test313State::Pass),
            "s0" => Some(Test313State::S0),
            _ => None,
        }
    }
        ...
        match state {
            Test313State::S0 => {
                ...
                'action_block: {
                    ...
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test313Event::Foo));
                }
            }
            _ => {}
        }
...
impl Test313Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test313State,
        event: Test313Event,
        current_state: &mut Test313State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test313State::Fail => false,
            Test313State::Pass => false,
            Test313State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test313Event::ErrorExecution {
                    ...
                    *current_state = Test313State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test313Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test313State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
