...
// From: resources/303/test303.scxml
//
...
//! Generated Rust state machine for `test303`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test303Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test303Event] = &[];
}
...
impl Test303Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test303State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test303", &io_processors) {
...
impl StatePolicy for Test303Policy {
    type State = Test303State;
    type Event = Test303Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test303State::Fail => true,
            Test303State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test303Event::ErrorExecution => "error.execution",
            Test303Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test303Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test303State::Fail => "fail",
            Test303State::Pass => "pass",
            Test303State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test303State::Fail),
            "pass" => Some(Test303State::Pass),
            "s0" => Some(Test303State::S0),
            _ => None,
        }
    }
            ...
            Test303State::S0 => {
                ...
                'action_block: {
...

                    {
                        ...
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        if let Err(e) = se.execute_script(&sid, "Var1 = 1") {
                            ::sce_rust_runtime::sce_log_error!("Script execution failed: {}", e);
                            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                                Test303Event::ErrorExecution,
...
impl Test303Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test303State,
        event: Test303Event,
        current_state: &mut Test303State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test303State::Fail => false,
            Test303State::Pass => false,
            Test303State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test303Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        ...
                        *current_state = Test303State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test303State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
