...
impl Test233Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test233Event] = &[Test233Event::ChildToParent];
}
...
impl StatePolicy for Test233Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test233State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test233State::Fail),
            "pass" => Some(Test233State::Pass),
            "s0" => Some(Test233State::S0),
            _ => None,
        }
    }
                ...
                if event == Test233Event::ChildToParent {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test233State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test233State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
