...
impl Test344Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test344Event] = &[];
}
...
impl StatePolicy for Test344Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test344State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test344State::Fail),
            "pass" => Some(Test344State::Pass),
            "s0" => Some(Test344State::S0),
            "s1" => Some(Test344State::S1),
            _ => None,
        }
    }
                ...
                if event == Test344Event::Null {
                    if self.safe_evaluate_guard("error(\"SCXML cond is not valid ECMAScript: return: unsupported ECMAScript construct: reserved word 'return' used as a value. Extended SCXML expressions must use the stateless subset.\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test344State::Fail)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test344Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test344State::S1)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test344Event::ErrorExecution {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test344State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
