...
// From: resources/172/test172.scxml
//
...
//! Generated Rust state machine for `test172`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test172Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test172Event] = &[Test172Event::Event2];
}
...
impl Test172Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test172State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test172", &io_processors) {
...
impl StatePolicy for Test172Policy {
    type State = Test172State;
    type Event = Test172Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test172State::Fail => true,
            Test172State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test172State::Fail => "fail",
            Test172State::Pass => "pass",
            Test172State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test172State::Fail),
            "pass" => Some(Test172State::Pass),
            "s0" => Some(Test172State::S0),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test172State::S0 => {
                // SCE-MAP: test172.scxml:9 :: s0 :: _state_body
...
impl Test172Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test172State,
        event: Test172Event,
        current_state: &mut Test172State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test172State::Fail => false,
            Test172State::Pass => false,
            Test172State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test172Event::Event2 {
                    ...
                    *current_state = Test172State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test172Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test172State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
