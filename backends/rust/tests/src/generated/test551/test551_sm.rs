...
// From: resources/551/test551.scxml
//
...
//! Generated Rust state machine for `test551`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test551.scxml:5 :: _machine"]
// SCE-MAP: test551.scxml:5 :: _machine
...
impl Test551Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test551Event] = &[];
}
...
impl Test551Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test551State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test551", &io_processors) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::eval_or_set_string(
            ...
            ::sce_rust_runtime::sce_log_error!("Failed to init 'Var1' in s1: {}", e);
        }
...
impl StatePolicy for Test551Policy {
    type State = Test551State;
    type Event = Test551Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test551State::Fail => true,
            Test551State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test551Event::ErrorExecution => "error.execution",
            Test551Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test551Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test551State::Fail => "fail",
            Test551State::Pass => "pass",
            Test551State::S0 => "s0",
            Test551State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test551State::Fail),
            "pass" => Some(Test551State::Pass),
            "s0" => Some(Test551State::S0),
            "s1" => Some(Test551State::S1),
            _ => None,
        }
    }
...
impl Test551Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test551State,
        event: Test551Event,
        current_state: &mut Test551State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test551State::Fail => false,
            Test551State::Pass => false,
            Test551State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test551Event::Null {
                    if self.safe_evaluate_guard("(_typeof(Var1) ~= \"undefined\")", engine) {
                        ...
                        *current_state = Test551State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test551State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
