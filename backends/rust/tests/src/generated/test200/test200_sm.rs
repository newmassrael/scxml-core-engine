...
// From: resources/200/test200.scxml
//
...
//! Generated Rust state machine for `test200`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test200Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test200Event] = &[Test200Event::Event1];
}
...
impl Test200Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test200State::S0,
...
impl Default for Test200Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test200Policy {
    type State = Test200State;
    type Event = Test200Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test200State::Fail => true,
            Test200State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test200State::Fail => "fail",
            Test200State::Pass => "pass",
            Test200State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test200State::Fail),
            "pass" => Some(Test200State::Pass),
            "s0" => Some(Test200State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test200Event::Event1);
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test200Event::Timeout);
                            // W3C SCXML 5.10.1: External send — preserve sendid and SCXML event processor origintype
                            meta.metadata = sce_rust_runtime::EventMetadata::external(
                                send_id.clone(),
                                ::sce_rust_runtime::SceString::new(),
...
impl Test200Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test200State,
        event: Test200Event,
        current_state: &mut Test200State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test200State::Fail => false,
            Test200State::Pass => false,
            Test200State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test200Event::Event1 {
                    ...
                    *current_state = Test200State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test200Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test200State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
