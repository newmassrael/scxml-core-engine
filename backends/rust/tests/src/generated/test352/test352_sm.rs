...
// From: resources/352/test352.scxml
//
...
//! Generated Rust state machine for `test352`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test352.scxml:4 :: _machine"]
...
impl Test352Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test352Event] = &[Test352Event::S0Event];
}
...
impl Test352Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test352State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test352", &io_processors) {
...
impl StatePolicy for Test352Policy {
    type State = Test352State;
    type Event = Test352Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test352State::Fail => true,
            Test352State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test352State::Fail => "fail",
            Test352State::Pass => "pass",
            Test352State::S0 => "s0",
            Test352State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test352State::Fail),
            "pass" => Some(Test352State::Pass),
            "s0" => Some(Test352State::S0),
            "s1" => Some(Test352State::S1),
            _ => None,
        }
    }
            ...
            Test352State::S0 => {
                ...
                'action_block: {
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test352Event::S0Event);
...
impl Test352Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test352State,
        event: Test352Event,
        current_state: &mut Test352State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test352State::Fail => false,
            Test352State::Pass => false,
            Test352State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test352Event::S0Event {
                    ...
                    *current_state = Test352State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test352Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test352State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test352State::S1 => {
                ...
                if event == Test352Event::Null {
                    if self.safe_evaluate_guard(
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test352State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
