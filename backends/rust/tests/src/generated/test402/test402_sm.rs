...
// From: resources/402/test402.scxml
//
...
//! Generated Rust state machine for `test402`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test402.scxml:6 :: _machine"]
...
impl Test402Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test402Event] =
        &[Test402Event::Error, Test402Event::Timeout];
}
...
impl Test402Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test402State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test402", &io_processors) {
...
impl StatePolicy for Test402Policy {
    type State = Test402State;
    type Event = Test402Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test402State::Fail => true,
            Test402State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test402State::S01 => Some(Test402State::S0),
            Test402State::S02 => Some(Test402State::S0),
            Test402State::S03 => Some(Test402State::S0),
            _ => None,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test402Event::Error => "error",
            Test402Event::ErrorExecution => "error.execution",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error" => Some(Test402Event::Error),
            "error.execution" => Some(Test402Event::ErrorExecution),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test402State::Fail => "fail",
            Test402State::Pass => "pass",
            Test402State::S0 => "s0",
            Test402State::S01 => "s01",
            Test402State::S02 => "s02",
            Test402State::S03 => "s03",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test402State::Fail),
            "pass" => Some(Test402State::Pass),
            "s0" => Some(Test402State::S0),
            "s01" => Some(Test402State::S01),
            "s02" => Some(Test402State::S02),
            "s03" => Some(Test402State::S03),
            _ => None,
        }
    }
    ...
    fn get_initial_children(
        ...
        match state {
            Test402State::S0 => {
                ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([Test402State::S01])
            }
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test402State::S0 => Test402State::S01,
            _ => state,
        }
    }
...
impl Test402Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test402State,
        event: Test402Event,
        current_state: &mut Test402State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test402State::Fail => false,
            Test402State::Pass => false,
            Test402State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test402Event::Timeout {
                    ...
                    *current_state = Test402State::Fail;
            ...
            Test402State::S01 => {
                ...
                if event == Test402Event::Event1 {
                    ...
                    *current_state = Test402State::S02;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test402Event::Null
            ...
            Test402State::S02 => {
                ...
                if event == Test402Event::Error || event == Test402Event::ErrorExecution {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 0;
                    self.has_transition_actions = false;
                    ...
                    *current_state = Test402State::S03;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
            ...
            Test402State::S03 => {
                ...
                if event == Test402Event::Event2 {
                    ...
                    *current_state = Test402State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
