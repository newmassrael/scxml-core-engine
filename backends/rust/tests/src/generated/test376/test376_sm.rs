...
// From: resources/376/test376.scxml
//
...
//! Generated Rust state machine for `test376`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test376Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test376Event] = &[];
}
...
impl Test376Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test376State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test376", &io_processors) {
...
impl StatePolicy for Test376Policy {
    type State = Test376State;
    type Event = Test376Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test376State::Fail => true,
            Test376State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test376Event::ErrorExecution => "error.execution",
            Test376Event::Event1 => "event1",
            Test376Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test376Event::ErrorExecution),
            "event1" => Some(Test376Event::Event1),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test376State::Fail => "fail",
            Test376State::Pass => "pass",
            Test376State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test376State::Fail),
            "pass" => Some(Test376State::Pass),
            "s0" => Some(Test376State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        // W3C SCXML 6.2: Invalid target "!invalid" raises error.execution
                        {
                            ...
                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test376Event::ErrorExecution, "<send target='!invalid'> is not a target this processor can address");
...
impl Test376Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test376State,
        event: Test376Event,
        current_state: &mut Test376State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test376State::Fail => false,
            Test376State::Pass => false,
            Test376State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test376Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        ...
                        *current_state = Test376State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test376State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
