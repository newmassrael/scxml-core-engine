...
impl Test560Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test560Event] = &[Test560Event::Foo];
}
...
impl StatePolicy for Test560Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test560State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test560State::Fail),
            "pass" => Some(Test560State::Pass),
            "s0" => Some(Test560State::S0),
            _ => None,
        }
    }
        ...
        match state {
            Test560State::S0 => {
                if event == Test560Event::Foo {
                    if self.safe_evaluate_guard("_scxml_eq(_event.data.aParam, 1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test560State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test560State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
