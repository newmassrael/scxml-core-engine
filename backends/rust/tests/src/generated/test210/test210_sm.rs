...
// From: resources/210/test210.scxml
//
...
//! Generated Rust state machine for `test210`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test210.scxml:6 :: _machine"]
...
impl Test210Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test210Event] = &[Test210Event::Event2];
}
...
impl Test210Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test210State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test210", &io_processors) {
...
impl StatePolicy for Test210Policy {
    type State = Test210State;
    type Event = Test210Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test210State::Fail => true,
            Test210State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test210State::Fail => "fail",
            Test210State::Pass => "pass",
            Test210State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test210State::Fail),
            "pass" => Some(Test210State::Pass),
            "s0" => Some(Test210State::S0),
            _ => None,
        }
    }
        ...
        match state {
            Test210State::S0 => {
                ...
                'action_block: {
                    ...
                    // W3C SCXML 6.3: <cancel sendidexpr="Var1">
                    {
                        self.ensure_script_engine();
                        let sid = self.session_id.as_ref().unwrap().clone();
                        let se = self.script_engine.clone();
                        ...
                        match se.evaluate_expression(&sid, "Var1") {
                            ...
                            _ => {
                                ::sce_rust_runtime::sce_log_error!(
                                    "Cancel: failed to evaluate sendidexpr 'Var1'"
                                );
                            }
                        }
                    }
...
impl Test210Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test210State,
        event: Test210Event,
        current_state: &mut Test210State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test210State::Fail => false,
            Test210State::Pass => false,
            Test210State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test210Event::Event2 {
                    ...
                    *current_state = Test210State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test210Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test210State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
