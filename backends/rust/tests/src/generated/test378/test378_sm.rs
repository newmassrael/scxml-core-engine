...
// From: resources/378/test378.scxml
//
...
//! Generated Rust state machine for `test378`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test378.scxml:6 :: _machine"]
// SCE-MAP: test378.scxml:6 :: _machine
...
impl Test378Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test378Event] = &[];
}
...
impl Test378Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test378State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test378", &io_processors) {
...
impl StatePolicy for Test378Policy {
    type State = Test378State;
    type Event = Test378Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test378State::Fail => true,
            Test378State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test378Event::ErrorExecution => "error.execution",
            Test378Event::Event1 => "event1",
            Test378Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test378Event::ErrorExecution),
            "event1" => Some(Test378Event::Event1),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test378State::Fail => "fail",
            Test378State::Pass => "pass",
            Test378State::S0 => "s0",
            Test378State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test378State::Fail),
            "pass" => Some(Test378State::Pass),
            "s0" => Some(Test378State::S0),
            "s1" => Some(Test378State::S1),
            _ => None,
        }
    }
    ...
    fn execute_exit_actions(
        ...
        match state {
            Test378State::S0 => {
                // SCE-MAP: test378.scxml:11 :: s0 :: _state_body
...
impl Test378Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test378State,
        event: Test378Event,
        current_state: &mut Test378State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test378State::Fail => false,
            Test378State::Pass => false,
            Test378State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test378Event::Null {
            ...
            Test378State::S1 => {
                ...
                if event == Test378Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test378State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test378State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
