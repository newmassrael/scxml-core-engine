...
// From: resources/325/test325.scxml
//
...
//! Generated Rust state machine for `test325`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test325.scxml:7 :: _machine"]
...
impl Test325Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test325Event] = &[];
}
...
impl Test325Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test325State::S0,
...
impl StatePolicy for Test325Policy {
    type State = Test325State;
    type Event = Test325Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test325State::Fail => true,
            Test325State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test325Event::ErrorExecution => "error.execution",
            Test325Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test325Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test325State::Fail => "fail",
            Test325State::Pass => "pass",
            Test325State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test325State::Fail),
            "pass" => Some(Test325State::Pass),
            "s0" => Some(Test325State::S0),
            _ => None,
        }
    }
...
impl Test325Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test325State,
        event: Test325Event,
        current_state: &mut Test325State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test325State::Fail => false,
            Test325State::Pass => false,
            Test325State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test325Event::Null {
                    if self.safe_evaluate_guard("(_typeof(Var1) ~= \"undefined\")", engine) {
                        ...
                        *current_state = Test325State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test325State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
