...
// From: resources/396/test396.scxml
//
...
//! Generated Rust state machine for `test396`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test396Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test396Event] = &[];
}
...
impl Test396Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test396State::S0,
            session_id: None,
...
impl StatePolicy for Test396Policy {
    type State = Test396State;
    type Event = Test396Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test396State::Fail => true,
            Test396State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test396State::Fail => "fail",
            Test396State::Pass => "pass",
            Test396State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test396State::Fail),
            "pass" => Some(Test396State::Pass),
            "s0" => Some(Test396State::S0),
            _ => None,
        }
    }
...
impl Test396Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test396State,
        event: Test396Event,
        current_state: &mut Test396State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test396State::Fail => false,
            Test396State::Pass => false,
            Test396State::S0 => {
