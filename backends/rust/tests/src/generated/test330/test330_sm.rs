...
// From: resources/330/test330.scxml
//
...
//! Generated Rust state machine for `test330`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test330.scxml:5 :: _machine"]
...
impl Test330Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test330Event] = &[];
}
...
impl Test330Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test330State::S0,
            session_id: None,
...
impl Default for Test330Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test330Policy {
    type State = Test330State;
    type Event = Test330Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test330State::Fail => true,
            Test330State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test330State::Fail => "fail",
            Test330State::Pass => "pass",
            Test330State::S0 => "s0",
            Test330State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test330State::Fail),
            "pass" => Some(Test330State::Pass),
            "s0" => Some(Test330State::S0),
            "s1" => Some(Test330State::S1),
            _ => None,
        }
    }
        ...
        match state {
            Test330State::S0 => {
                ...
                'action_block: {
                    ...
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test330Event::Foo));
                }
            }
            Test330State::S1 => {
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test330Event::Foo);
...
impl Test330Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test330State,
        event: Test330Event,
        current_state: &mut Test330State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test330State::Fail => false,
            Test330State::Pass => false,
            Test330State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test330Event::Foo {
                    ...
                    *current_state = Test330State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test330Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test330State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test330State::S1 => {
                ...
                if event == Test330Event::Foo {
                    ...
                    *current_state = Test330State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
