...
// From: resources/372/test372.scxml
//
...
//! Generated Rust state machine for `test372`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test372.scxml:6 :: _machine"]
// SCE-MAP: test372.scxml:6 :: _machine
...
impl Test372Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test372Event] = &[];
}
...
impl Test372Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test372State::S0final,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test372", &io_processors) {
...
impl StatePolicy for Test372Policy {
    type State = Test372State;
    type Event = Test372Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test372State::Fail => true,
            Test372State::Pass => true,
            Test372State::S0final => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test372State::S0final => Some(Test372State::S0),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test372State::Fail => "fail",
            Test372State::Pass => "pass",
            Test372State::S0 => "s0",
            Test372State::S0final => "s0final",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test372State::Fail),
            "pass" => Some(Test372State::Pass),
            "s0" => Some(Test372State::S0),
            "s0final" => Some(Test372State::S0final),
            _ => None,
        }
    }
    ...
    fn get_initial_children(
        ...
        match state {
            Test372State::S0 => ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([
                Test372State::S0final,
            ]),
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test372State::S0 => Test372State::S0final,
            _ => state,
        }
    }
...
impl Test372Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test372State,
        event: Test372Event,
        current_state: &mut Test372State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test372State::Fail => false,
            Test372State::Pass => false,
            Test372State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test372Event::DoneStateS0 {
                    ...
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        ...
                        *current_state = Test372State::Pass;
                        *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test372Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test372State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            }
            Test372State::S0final => false,
            _ => false,
        }
    }
}
