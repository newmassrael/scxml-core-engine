...
// From: resources/529/test529.scxml
//
...
//! Generated Rust state machine for `test529`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test529Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test529Event] = &[];
}
...
impl Test529Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test529State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test529", &io_processors) {
...
impl StatePolicy for Test529Policy {
    type State = Test529State;
    type Event = Test529Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test529State::Fail => true,
            Test529State::Pass => true,
            Test529State::S02 => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test529State::S01 => Some(Test529State::S0),
            Test529State::S02 => Some(Test529State::S0),
            _ => None,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test529Event::DoneStateS0 => "done.state.s0",
            Test529Event::ErrorExecution => "error.execution",
            Test529Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "done.state.s0" => Some(Test529Event::DoneStateS0),
            "error.execution" => Some(Test529Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test529State::Fail => "fail",
            Test529State::Pass => "pass",
            Test529State::S0 => "s0",
            Test529State::S01 => "s01",
            Test529State::S02 => "s02",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test529State::Fail),
            "pass" => Some(Test529State::Pass),
            "s0" => Some(Test529State::S0),
            "s01" => Some(Test529State::S01),
            "s02" => Some(Test529State::S02),
            _ => None,
        }
    }
    ...
    fn get_initial_children(
        ...
        match state {
            Test529State::S0 => {
                ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([Test529State::S01])
            }
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test529State::S0 => Test529State::S01,
            _ => state,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test529State::S02 => {
                // SCE-MAP: test529.scxml:18 :: s02 :: _state_body
                ...
                if done_data_ok {
                    let mut done_meta =
                        sce_rust_runtime::EventWithMetadata::new(Test529Event::DoneStateS0);
...
impl Test529Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test529State,
        event: Test529Event,
        current_state: &mut Test529State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test529State::Fail => false,
            Test529State::Pass => false,
            Test529State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test529Event::DoneStateS0 {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_scxml_eq(_event.data, 21)", engine) {
                        ...
                        *current_state = Test529State::Pass;
                ...
                if event == Test529Event::DoneStateS0 {
                    ...
                    *current_state = Test529State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test529State::S01 => {
                ...
                if event == Test529Event::Null {
                    // Unconditional eventless transition
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                    *current_state = Test529State::S02;
                    *transition_taken = true;
                    return true;
                }
                false
            }
            Test529State::S02 => false,
            _ => false,
        }
    }
}
