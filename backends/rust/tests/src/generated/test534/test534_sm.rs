...
// From: resources/534/test534.scxml
//
...
//! Generated Rust state machine for `test534`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test534.scxml:3 :: _machine"]
...
impl Test534Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test534Event] = &[Test534Event::Test];
}
...
impl Test534Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test534State::S0,
...
impl StatePolicy for Test534Policy {
    type State = Test534State;
    type Event = Test534Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test534State::Fail => true,
            Test534State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test534State::Fail => "fail",
            Test534State::Pass => "pass",
            Test534State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test534State::Fail),
            "pass" => Some(Test534State::Pass),
            "s0" => Some(Test534State::S0),
            _ => None,
        }
    }
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "_ioprocessors.basichttp.location") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test534Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test534Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
                        ...
                        if let Some(ref _rt) = _resolved_target {
                            // W3C SCXML C.2: BasicHTTP send to HTTP target
                            {
                                ...
                                if !_rt.starts_with("http://") && !_rt.starts_with("https://") {
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test534Event::ErrorCommunication, "<send> over BasicHTTPEventProcessor resolved a target that is not an http(s) URL"));
                                } else {
                                    let http_params =
...
impl Test534Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test534State,
        event: Test534Event,
        current_state: &mut Test534State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test534State::Fail => false,
            Test534State::Pass => false,
            Test534State::S0 => {
                ...
                if event == Test534Event::Test {
                    ...
                    if self.safe_evaluate_guard(
                        ...
                        *current_state = Test534State::Pass;
                        *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test534Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test534State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
