...
// From: resources/234/test234.scxml
//
...
//! Generated Rust state machine for `test234`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test234.scxml:8 :: _machine"]
...
impl Test234Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test234Event] =
        &[Test234Event::ChildToParent, Test234Event::Timeout];
}
...
pub struct Test234Policy {
    ...
    active_states: ::sce_rust_runtime::helpers::hierarchy::StateChain<Test234State>,
    ...
    child_invoke_0: Option<
        Box<
            sce_rust_runtime::Engine<
                super::test234__sce_synth_invoke__invoke_0_sm::Test234SceSynthInvokeInvoke0Policy,
            >,
        >,
    >,
    ...
    child_invoke_1: Option<
        Box<
            sce_rust_runtime::Engine<
                super::test234__sce_synth_invoke__invoke_1_sm::Test234SceSynthInvokeInvoke1Policy,
            >,
        >,
    >,
...
impl Test234Policy {
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `Var2` was
    /// assigned a value of another type, or the engine refused.
    pub fn var2(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "Var2",
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test234", &io_processors) {
    ...
    fn state_name_matches(state: Test234State, name: &str) -> bool {
            ...
            if pending.invoke_id.contains("._invoke_0") {
                ...
                self.active_invokes.insert(
                    ...
                    sce_rust_runtime::invoke::ChildSession {
                        ...
                        autoforward: false,
                        finalize_script: "Var1 = _event.data.aParam".to_string(),
                    },
                );
            ...
            if pending.invoke_id.contains("._invoke_1") {
                ...
                self.active_invokes.insert(
                    ...
                    sce_rust_runtime::invoke::ChildSession {
                        ...
                        parent_session_id: self.session_id.clone().unwrap_or_default(),
                        autoforward: false,
                        finalize_script: "Var2 = _event.data.aParam".to_string(),
                    },
                );
    ...
    fn do_execute_finalize_for_child_event(
        &mut self,
        event_with_meta: &sce_rust_runtime::EventWithMetadata<Test234Event>,
...
impl StatePolicy for Test234Policy {
    type State = Test234State;
    type Event = Test234Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test234State::Fail => true,
            Test234State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test234State::P01 => Some(Test234State::P0),
            Test234State::P02 => Some(Test234State::P0),
            _ => None,
        }
    }
    ...
    fn get_parallel_regions(state: Self::State) -> &'static [Self::State] {
        match state {
            Test234State::P0 => &[Test234State::P01, Test234State::P02],
            _ => &[],
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test234Event::CancelInvoke => "cancel.invoke",
            Test234Event::ChildToParent => "childToParent",
            Test234Event::DoneInvoke => "done.invoke",
            Test234Event::ErrorExecution => "error.execution",
            Test234Event::Timeout => "timeout",
            Test234Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test234Event::CancelInvoke),
            "childToParent" => Some(Test234Event::ChildToParent),
            "done.invoke" => Some(Test234Event::DoneInvoke),
            "error.execution" => Some(Test234Event::ErrorExecution),
            "timeout" => Some(Test234Event::Timeout),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test234State::Fail => "fail",
            Test234State::P0 => "p0",
            Test234State::P01 => "p01",
            Test234State::P02 => "p02",
            Test234State::Pass => "pass",
            Test234State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test234State::Fail),
            "p0" => Some(Test234State::P0),
            "p01" => Some(Test234State::P01),
            "p02" => Some(Test234State::P02),
            "pass" => Some(Test234State::Pass),
            "s1" => Some(Test234State::S1),
            _ => None,
        }
    }
...
impl Test234Policy {
    ...
    fn try_transition_in_state(
        ...
        match check_state {
            ...
            Test234State::P0 => {
                ...
                if event == Test234Event::Timeout {
                    ...
                    if let Some(ref mut collect) = collect_mode {
                        ...
                        *current_state = Test234State::Fail;
            ...
            Test234State::P01 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test234Event::ChildToParent {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 2)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
            ...
            Test234State::P02 => false,
            Test234State::Pass => false,
            Test234State::S1 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test234Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var2, 1)", engine) {
                        ...
                        if let Some(ref mut collect) = collect_mode {
                            ...
                            *current_state = Test234State::Pass;
                            *transition_taken = true;
                        }
                        return true;
                    } else {
    ...
    fn compute_exit_set(
        ...
        is_internal: bool,
        is_targetless: bool,
    ) -> ::sce_rust_runtime::helpers::hierarchy::StateChain<Test234State> {
    ...
    fn execute_microstep(
        ...
        let mut states_to_exit: ::sce_rust_runtime::helpers::hierarchy::StateChain<Test234State> =
        ...
        for trans in transitions {
            ...
            let domain: Option<Test234State> = if is_internal_to_descendant {
                Some(trans.source)
            } else {
