...
// From: resources/173/test173.scxml
//
...
//! Generated Rust state machine for `test173`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test173.scxml:5 :: _machine"]
// SCE-MAP: test173.scxml:5 :: _machine
...
impl Test173Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test173Event] = &[Test173Event::Event1];
}
...
impl Test173Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test173State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test173", &io_processors) {
...
impl StatePolicy for Test173Policy {
    type State = Test173State;
    type Event = Test173Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test173State::Fail => true,
            Test173State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test173State::Fail => "fail",
            Test173State::Pass => "pass",
            Test173State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test173State::Fail),
            "pass" => Some(Test173State::Pass),
            "s0" => Some(Test173State::S0),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test173State::S0 => {
                // SCE-MAP: test173.scxml:10 :: s0 :: _state_body
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "Var1") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test173Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test173Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
...
impl Test173Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test173State,
        event: Test173Event,
        current_state: &mut Test173State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test173State::Fail => false,
            Test173State::Pass => false,
            Test173State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test173Event::Event1 {
                    ...
                    *current_state = Test173State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test173Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test173State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
