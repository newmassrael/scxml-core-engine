...
// From: resources/349/test349.scxml
//
...
//! Generated Rust state machine for `test349`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test349Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test349Event] =
        &[Test349Event::S0Event, Test349Event::S0Event2];
}
...
impl Test349Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test349State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test349", &io_processors) {
...
impl StatePolicy for Test349Policy {
    type State = Test349State;
    type Event = Test349Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test349State::Fail => true,
            Test349State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test349State::Fail => "fail",
            Test349State::Pass => "pass",
            Test349State::S0 => "s0",
            Test349State::S2 => "s2",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test349State::Fail),
            "pass" => Some(Test349State::Pass),
            "s0" => Some(Test349State::S0),
            "s2" => Some(Test349State::S2),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test349Event::S0Event);
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test349Event::S0Event2);
                            // W3C SCXML 5.10.1: External send — preserve sendid and SCXML event processor origintype
                            meta.metadata = sce_rust_runtime::EventMetadata::external(
                                send_id.clone(),
                                ::sce_rust_runtime::SceString::new(),
...
impl Test349Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test349State,
        event: Test349Event,
        current_state: &mut Test349State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test349State::Fail => false,
            Test349State::Pass => false,
            Test349State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test349Event::S0Event {
                    ...
                    *current_state = Test349State::S2;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test349Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test349State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test349State::S2 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test349Event::S0Event2 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_index = 0;
                    self.has_transition_actions = false;
                    ...
                    *current_state = Test349State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
