...
#![doc = "SCE-MAP: test183.scxml:5 :: _machine"]
...
impl Test183Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test183Event] = &[];
}
...
impl StatePolicy for Test183Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test183State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test183State::Fail),
            "pass" => Some(Test183State::Pass),
            "s0" => Some(Test183State::S0),
            _ => None,
        }
    }
                ...
                if event == Test183Event::Null {
                    if self.safe_evaluate_guard("(_typeof(Var1) ~= \"undefined\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test183State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test183Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test183State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
