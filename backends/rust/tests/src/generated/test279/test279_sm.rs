...
impl Test279Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test279Event] = &[];
}
...
impl StatePolicy for Test279Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test279State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test279State::Fail),
            "pass" => Some(Test279State::Pass),
            "s0" => Some(Test279State::S0),
            "s1" => Some(Test279State::S1),
            _ => None,
        }
    }
                ...
                if event == Test279Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test279State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test279Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test279State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
