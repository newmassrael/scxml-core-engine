...
#![doc = "SCE-MAP: test530.scxml:7 :: _machine"]
...
impl Test530Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test530Event] = &[];
}
...
pub struct Test530Policy {
    ...
    child_invoke_0:
        Option<Box<sce_rust_runtime::Engine<super::test530_hybrid0_sm::Test530Hybrid0Policy>>>,
        ...
        for pending in &invokes_to_execute {
            if pending.document_id == "_invoke_0" {
                ...
                match se.evaluate_expression(&sid, "Var1") {
                    Ok(sce_rust_runtime::ScriptValue::Null)
                    ...
                    | Err(_) => {
                        ...
                        engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                            Test530Event::ErrorExecution,
                            "<invoke contentexpr='Var1'> could not be evaluated",
                        ));
                        continue;
                    }
                    Ok(_content) => {
                        ::sce_rust_runtime::sce_log_debug!(
                            "Hybrid invoke: contentexpr evaluated to {:?}",
                            _content
                        );
                    }
                }
                ...
                if child_policy.session_id.is_none() {
                    static HYBRID_SESSION_COUNTER: core::sync::atomic::AtomicU64 =
                        core::sync::atomic::AtomicU64::new(0);
                    let cid =
                        HYBRID_SESSION_COUNTER.fetch_add(1, core::sync::atomic::Ordering::Relaxed);
...
impl StatePolicy for Test530Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test530State::S0)]
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test530State::Fail => "fail",
            Test530State::Pass => "pass",
            Test530State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test530State::Fail),
            "pass" => Some(Test530State::Pass),
            "s0" => Some(Test530State::S0),
            _ => None,
        }
    }
                ...
                if event == Test530Event::DoneInvoke {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test530State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test530State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
