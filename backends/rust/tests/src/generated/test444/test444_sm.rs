...
// From: resources/444/test444.scxml
//
...
//! Generated Rust state machine for `test444`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test444.scxml:5 :: _machine"]
// SCE-MAP: test444.scxml:5 :: _machine
...
impl Test444Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test444Event] = &[];
}
...
impl Test444Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test444State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test444", &io_processors) {
...
impl StatePolicy for Test444Policy {
    type State = Test444State;
    type Event = Test444Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test444State::Fail => true,
            Test444State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test444Event::ErrorExecution => "error.execution",
            Test444Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test444Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test444State::Fail => "fail",
            Test444State::Pass => "pass",
            Test444State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test444State::Fail),
            "pass" => Some(Test444State::Pass),
            "s0" => Some(Test444State::S0),
            _ => None,
        }
    }
...
impl Test444Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test444State,
        event: Test444Event,
        current_state: &mut Test444State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test444State::Fail => false,
            Test444State::Pass => false,
            Test444State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test444Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq((function() var1 = _scxml_tonumber(var1) + 1 return var1 end)(), 2)", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test444State::Pass;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test444State::Fail;
                            *transition_taken = true;
                        return true;
                    }
                }
