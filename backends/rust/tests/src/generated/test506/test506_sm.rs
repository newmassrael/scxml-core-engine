...
// From: resources/506/test506.scxml
//
...
//! Generated Rust state machine for `test506`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test506Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test506Event] = &[];
}
...
impl Test506Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test506State::S1,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test506", &io_processors) {
...
impl StatePolicy for Test506Policy {
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test506State::Fail => true,
            Test506State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test506State::S21 => Some(Test506State::S2),
            _ => None,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test506Event::Bar => "bar",
            Test506Event::ErrorExecution => "error.execution",
            Test506Event::Foo => "foo",
            Test506Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "bar" => Some(Test506Event::Bar),
            "error.execution" => Some(Test506Event::ErrorExecution),
            "foo" => Some(Test506Event::Foo),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test506State::Fail => "fail",
            Test506State::Pass => "pass",
            Test506State::S1 => "s1",
            Test506State::S2 => "s2",
            Test506State::S21 => "s21",
            Test506State::S3 => "s3",
            Test506State::S4 => "s4",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test506State::Fail),
            "pass" => Some(Test506State::Pass),
            "s1" => Some(Test506State::S1),
            "s2" => Some(Test506State::S2),
            "s21" => Some(Test506State::S21),
            "s3" => Some(Test506State::S3),
            "s4" => Some(Test506State::S4),
            _ => None,
        }
    }
    ...
    fn get_initial_children(
        ...
        match state {
            Test506State::S2 => {
                ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([Test506State::S21])
            }
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test506State::S2 => Test506State::S21,
            _ => state,
        }
    }
        ...
        match state {
            Test506State::S1 => {
                ...
                'action_block: {
                    ...
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test506Event::Foo));

                    // W3C SCXML 3.8.1: <raise event="bar">
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test506Event::Bar));
                }
            }
            _ => {}
        }
...
impl Test506Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test506State,
        event: Test506Event,
        current_state: &mut Test506State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test506State::Fail => false,
            Test506State::Pass => false,
            Test506State::S1 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test506Event::Null {
            ...
            Test506State::S2 => {
                ...
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test506Event::Bar {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_scxml_eq(Var3, 1)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                ...
                if event == Test506Event::Bar {
                    ...
                    *current_state = Test506State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            }
            Test506State::S21 => false,
            Test506State::S3 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test506Event::Null {
            ...
            Test506State::S4 => {
                ...
                if event == Test506Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var2, 2)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test506State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
