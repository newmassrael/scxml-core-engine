...
impl Test225Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test225Event] = &[];
}
...
impl StatePolicy for Test225Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test225State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test225State::Fail),
            "pass" => Some(Test225State::Pass),
            "s0" => Some(Test225State::S0),
            "s1" => Some(Test225State::S1),
            _ => None,
        }
    }
            ...
            Test225State::S0 => {
                ...
                }
                {
                    ...
                    if !id_stored {
                        engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                            Test225Event::ErrorExecution,
                            "<invoke idlocation='Var2'> could not take the invoke id",
                        ));
                    }
                    if id_stored {
                        sce_rust_runtime::invoke::defer_invoke(
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test225State::S1)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test225State::S1 => {
                if event == Test225Event::Null {
                    if self.safe_evaluate_guard("(Var1 == Var2)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test225State::Fail)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test225Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test225State::Pass)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
