...
impl Test331Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test331Event] = &[Test331Event::Error];
}
...
impl StatePolicy for Test331Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test331State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test331State::Fail),
            "pass" => Some(Test331State::Pass),
            "s0" => Some(Test331State::S0),
            "s1" => Some(Test331State::S1),
            "s2" => Some(Test331State::S2),
            "s3" => Some(Test331State::S3),
            "s4" => Some(Test331State::S4),
            "s5" => Some(Test331State::S5),
            _ => None,
        }
    }
    ...
    fn first_enabled_transition(
        ...
        match state {
                ...
                if event == Test331Event::Foo {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test331State::S1)],
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test331State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test331State::S1 => {
                if event == Test331Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, \"internal\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test331State::S2)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test331Event::Error || event == Test331Event::ErrorExecution {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test331State::S3)],
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                        });
            ...
            Test331State::S3 => {
                if event == Test331Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, \"platform\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test331State::S4)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test331Event::Foo {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test331State::S5)],
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                        });
            ...
            Test331State::S5 => {
                if event == Test331Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, \"external\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test331State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
