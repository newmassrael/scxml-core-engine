...
// From: resources/287/test287.scxml
//
...
//! Generated Rust state machine for `test287`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test287Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test287Event] = &[];
}
...
impl Test287Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test287State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test287", &io_processors) {
...
impl StatePolicy for Test287Policy {
    type State = Test287State;
    type Event = Test287Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test287State::Fail => true,
            Test287State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test287Event::ErrorExecution => "error.execution",
            Test287Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test287Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test287State::Fail => "fail",
            Test287State::Pass => "pass",
            Test287State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test287State::Fail),
            "pass" => Some(Test287State::Pass),
            "s0" => Some(Test287State::S0),
            _ => None,
        }
    }
...
impl Test287Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test287State,
        event: Test287Event,
        current_state: &mut Test287State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test287State::Fail => false,
            Test287State::Pass => false,
            Test287State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test287Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        ...
                        *current_state = Test287State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test287State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
