...
// From: resources/314/test314.scxml
//
...
//! Generated Rust state machine for `test314`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test314Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test314Event] = &[];
}
...
impl Test314Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test314State::S01,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test314", &io_processors) {
...
impl StatePolicy for Test314Policy {
    type State = Test314State;
    type Event = Test314Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test314State::Fail => true,
            Test314State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_parent(state: Self::State) -> Option<Self::State> {
        match state {
            Test314State::S01 => Some(Test314State::S0),
            Test314State::S02 => Some(Test314State::S0),
            Test314State::S03 => Some(Test314State::S0),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test314State::Fail => "fail",
            Test314State::Pass => "pass",
            Test314State::S0 => "s0",
            Test314State::S01 => "s01",
            Test314State::S02 => "s02",
            Test314State::S03 => "s03",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test314State::Fail),
            "pass" => Some(Test314State::Pass),
            "s0" => Some(Test314State::S0),
            "s01" => Some(Test314State::S01),
            "s02" => Some(Test314State::S02),
            "s03" => Some(Test314State::S03),
            _ => None,
        }
    }
    ...
    fn get_initial_children(
        ...
        match state {
            Test314State::S0 => {
                ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([Test314State::S01])
            }
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test314State::S0 => Test314State::S01,
            _ => state,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test314State::S03 => {
                // SCE-MAP: test314.scxml:24 :: s03 :: _state_body
                ...
                'action_block: {
                    ...
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test314Event::Foo));
                }
            }
            _ => {}
        }
...
impl Test314Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test314State,
        event: Test314Event,
        current_state: &mut Test314State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test314State::Fail => false,
            Test314State::Pass => false,
            Test314State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test314Event::ErrorExecution {
                    ...
                    *current_state = Test314State::Fail;
            ...
            Test314State::S01 => {
                ...
                if event == Test314Event::Null {
                    // Unconditional eventless transition
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                    *current_state = Test314State::S02;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test314State::S02 => {
                ...
                if event == Test314Event::Null {
                    ...
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;
                    *current_state = Test314State::S03;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test314State::S03 => {
                ...
                if event == Test314Event::ErrorExecution {
                    ...
                    *current_state = Test314State::Pass;
                    *transition_taken = true;
                    return true;
                }
