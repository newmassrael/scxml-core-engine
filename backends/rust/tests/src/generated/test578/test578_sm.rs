...
// From: resources/578/test578.scxml
//
...
//! Generated Rust state machine for `test578`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test578.scxml:5 :: _machine"]
...
impl Test578Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test578Event] = &[Test578Event::Foo];
}
...
impl Test578Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test578State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test578", &io_processors) {
...
impl StatePolicy for Test578Policy {
    type State = Test578State;
    type Event = Test578Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test578State::Fail => true,
            Test578State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test578State::Fail => "fail",
            Test578State::Pass => "pass",
            Test578State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test578State::Fail),
            "pass" => Some(Test578State::Pass),
            "s0" => Some(Test578State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test578Event::Foo);
...
impl Test578Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test578State,
        event: Test578Event,
        current_state: &mut Test578State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test578State::Fail => false,
            Test578State::Pass => false,
            Test578State::S0 => {
                ...
                if event == Test578Event::Foo {
                        ...
                        .safe_evaluate_guard("_scxml_eq(_event.data.productName, \"bar\")", engine)
                    {
                        ...
                        *current_state = Test578State::Pass;
                        *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test578Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test578State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
