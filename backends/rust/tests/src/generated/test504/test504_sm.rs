...
impl Test504Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test504Event] = &[];
}
...
pub struct Test504Policy {
    ...
    active_states: ::sce_rust_runtime::helpers::hierarchy::StateChain<Test504State>,
...
impl Test504Policy {
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `Var3` was
    /// assigned a value of another type, or the engine refused.
    pub fn var3(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "Var3",
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `Var4` was
    /// assigned a value of another type, or the engine refused.
    pub fn var4(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "Var4",
    ...
    /// session, so a reader frozen at generation time would answer the
    /// document's literal for the whole run. `None` means the machine cannot
    /// answer — the session is not initialized yet, `Var5` was
    /// assigned a value of another type, or the engine refused.
    pub fn var5(&self) -> Option<i64> {
        ::sce_rust_runtime::helpers::datamodel_read::read_int(
            self.script_engine.as_ref(),
            self.session_id.as_deref(),
            "Var5",
...
impl StatePolicy for Test504Policy {
    ...
    fn get_parallel_regions(state: Self::State) -> &'static [Self::State] {
        match state {
            Test504State::P => &[Test504State::Ps1, Test504State::Ps2],
            _ => &[],
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test504State::Fail),
            "p" => Some(Test504State::P),
            "pass" => Some(Test504State::Pass),
            "ps1" => Some(Test504State::Ps1),
            "ps2" => Some(Test504State::Ps2),
            "s1" => Some(Test504State::S1),
            "s2" => Some(Test504State::S2),
            "s3" => Some(Test504State::S3),
            "s4" => Some(Test504State::S4),
            "s5" => Some(Test504State::S5),
            "s6" => Some(Test504State::S6),
            _ => None,
        }
    }
    ...
    fn get_initial_children(
        ...
        match state {
            Test504State::S2 => {
                ::sce_rust_runtime::helpers::hierarchy::state_chain_from_slice([Test504State::P])
            }
    ...
    fn get_initial_or_history_child(&self, state: Self::State) -> Self::State {
        match state {
            Test504State::S2 => Test504State::P,
            _ => state,
        }
    }
    ...
    fn process_transition(
        ...
        if event == Self::null_event() {
            ...
            let mut seen: ::sce_rust_runtime::SceDedupSet<(Test504State, usize)> =
        ...
        match check_state {
            ...
            Test504State::P => {
                ...
                if event == Test504Event::Bar {
                    ...
                    if self.safe_evaluate_guard("_scxml_eq(Var4, 1)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
            ...
            Test504State::S4 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test504Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var2, 2)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        self.has_transition_actions = false;
            ...
            Test504State::S5 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test504Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var3, 2)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        self.has_transition_actions = false;
            ...
            Test504State::S6 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test504Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var5, 1)", engine) {
                        // W3C SCXML 3.4: Track transition metadata
                        self.last_transition_source_state = check_state;
                        self.last_transition_index = 0;
                        self.has_transition_actions = false;
