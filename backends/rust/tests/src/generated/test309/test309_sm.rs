...
// From: resources/309/test309.scxml
//
...
//! Generated Rust state machine for `test309`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test309.scxml:5 :: _machine"]
// SCE-MAP: test309.scxml:5 :: _machine
...
impl Test309Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test309Event] = &[];
}
...
impl Test309Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test309State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test309", &io_processors) {
...
impl StatePolicy for Test309Policy {
    type State = Test309State;
    type Event = Test309Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test309State::Fail => true,
            Test309State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test309Event::ErrorExecution => "error.execution",
            Test309Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test309Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test309State::Fail => "fail",
            Test309State::Pass => "pass",
            Test309State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test309State::Fail),
            "pass" => Some(Test309State::Pass),
            "s0" => Some(Test309State::S0),
            _ => None,
        }
    }
...
impl Test309Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test309State,
        event: Test309Event,
        current_state: &mut Test309State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test309State::Fail => false,
            Test309State::Pass => false,
            Test309State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test309Event::Null {
                    if self.safe_evaluate_guard("error(\"SCXML cond is not valid ECMAScript: return: unsupported ECMAScript construct: reserved word 'return' used as a value. Extended SCXML expressions must use the stateless subset.\")", engine) {
                        ...
                        self.last_transition_is_targetless = false;
                            *current_state = Test309State::Fail;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                            *current_state = Test309State::Pass;
                            *transition_taken = true;
                        return true;
                    }
                }
