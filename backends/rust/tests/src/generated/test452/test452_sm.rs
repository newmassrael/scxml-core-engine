...
// From: resources/452/test452.scxml
//
...
//! Generated Rust state machine for `test452`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test452Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test452Event] = &[];
}
...
impl Test452Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test452State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        // W3C SCXML 5.2/5.3: Initialize 'foo' from expr (global)
        ...
        if let Err(e) = se.execute_script(
            &sid,
            "testobject = function() local self = {} self.bar = 0 return self end",
        ) {
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_expr(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                ...
                "<data id='foo'> expr failed to evaluate",
            ));
        }
        ...
        if let Err(e) = se.execute_script(
            ...
            engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                Test452Event::ErrorExecution,
                "a top-level <script> failed to execute",
            ));
        }
...
impl StatePolicy for Test452Policy {
    type State = Test452State;
    type Event = Test452Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test452State::Fail => true,
            Test452State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test452State::Fail => "fail",
            Test452State::Pass => "pass",
            Test452State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test452State::Fail),
            "pass" => Some(Test452State::Pass),
            "s0" => Some(Test452State::S0),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test452State::S0 => {
                // SCE-MAP: test452.scxml:13 :: s0 :: _state_body
                // W3C SCXML 3.8: onentry block 1/1
                // Labeled block allows actions to break out on error (W3C 3.8: error stops block)
                'action_block: {
                    {
                        // W3C SCXML 5.3: <assign location="foo">
                        self.ensure_script_engine();
                        let sid = self.session_id.as_ref().unwrap().clone();
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                    ...
                    {
                        // W3C SCXML 5.3: <assign location="foo.bar">
                        self.ensure_script_engine();
                        let sid = self.session_id.as_ref().unwrap().clone();
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
...
impl Test452Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test452State,
        event: Test452Event,
        current_state: &mut Test452State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test452State::Fail => false,
            Test452State::Pass => false,
            Test452State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test452Event::Event1 {
                    // W3C SCXML 5.9: Script engine guard
                    if self.safe_evaluate_guard("_scxml_eq(foo.bar, 1)", engine) {
                        ...
                        *current_state = Test452State::Pass;
                        *transition_taken = true;
                        return true;
                    }
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test452Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test452State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
