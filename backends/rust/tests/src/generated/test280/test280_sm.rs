...
// From: resources/280/test280.scxml
//
...
//! Generated Rust state machine for `test280`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test280Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test280Event] = &[];
}
...
impl Test280Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test280State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test280", &io_processors) {
...
impl StatePolicy for Test280Policy {
    type State = Test280State;
    type Event = Test280Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test280State::Fail => true,
            Test280State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test280Event::ErrorExecution => "error.execution",
            Test280Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test280Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test280State::Fail => "fail",
            Test280State::Pass => "pass",
            Test280State::S0 => "s0",
            Test280State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test280State::Fail),
            "pass" => Some(Test280State::Pass),
            "s0" => Some(Test280State::S0),
            "s1" => Some(Test280State::S1),
            _ => None,
        }
    }
...
impl Test280Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test280State,
        event: Test280Event,
        current_state: &mut Test280State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test280State::Fail => false,
            Test280State::Pass => false,
            Test280State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test280Event::Null {
            ...
            Test280State::S1 => {
                ...
                if event == Test280Event::Null {
                    if self.safe_evaluate_guard("(Var1 == Var2)", engine) {
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test280State::Pass;
                        *transition_taken = true;
                        return true;
                    } else {
                        // W3C SCXML 3.4: Track transition metadata
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test280State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
