...
// From: resources/496/test496.scxml
//
...
//! Generated Rust state machine for `test496`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test496.scxml:3 :: _machine"]
...
impl Test496Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test496Event] = &[];
}
...
impl Test496Policy {
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test496", &io_processors) {
...
impl StatePolicy for Test496Policy {
    type State = Test496State;
    type Event = Test496Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test496State::Fail => true,
            Test496State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test496Event::ErrorCommunication => "error.communication",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.communication" => Some(Test496Event::ErrorCommunication),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test496State::Fail => "fail",
            Test496State::Pass => "pass",
            Test496State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test496State::Fail),
            "pass" => Some(Test496State::Pass),
            "s0" => Some(Test496State::S0),
            _ => None,
        }
    }
        ...
        match state {
            Test496State::S0 => {
                ...
                'action_block: {
                        ...
                        let _resolved_target: Option<String> = {
                            ...
                            match se.evaluate_expression(&sid, "nil") {
                                    ...
                                    ) =>
                                {
                                    ...
                                    engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(Test496Event::ErrorCommunication, "<send> targetexpr evaluated to nothing, so there is no target to reach"));
                                    None
                                }
                                Ok(val) => {
                                    ...
                                    if trimmed.starts_with("!") {
                                        ...
                                        {
                                            let mut err_meta = sce_rust_runtime::EventWithMetadata::platform_error(Test496Event::ErrorExecution, "<send> targetexpr produced a target this processor cannot address");
                    ...
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test496Event::Foo));
                }
            }
            _ => {}
        }
...
impl Test496Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test496State,
        event: Test496Event,
        current_state: &mut Test496State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test496State::Fail => false,
            Test496State::Pass => false,
            Test496State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test496Event::ErrorCommunication {
                    ...
                    *current_state = Test496State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test496Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test496State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
