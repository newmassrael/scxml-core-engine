...
impl Test436Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test436Event] = &[];
}
...
impl Test436Policy {
    ...
    fn activate(&mut self, state: Test436State) -> bool {
    ...
    fn deactivate(&mut self, state: Test436State) {
...
impl StatePolicy for Test436Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test436State::P)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test436State::Fail),
            "p" => Some(Test436State::P),
            "pass" => Some(Test436State::Pass),
            "ps0" => Some(Test436State::Ps0),
            "ps1" => Some(Test436State::Ps1),
            "s1" => Some(Test436State::S1),
            _ => None,
        }
    }
    ...
    fn first_enabled_transition(
        ...
        match state {
            Test436State::Ps0 => {
                if event == Test436Event::Null {
                    if self.is_state_active("s1") {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test436State::Fail)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test436Event::Null {
                    if self.is_state_active("ps1") {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test436State::Pass)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
