...
impl Test319Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test319Event] = &[];
}
...
impl StatePolicy for Test319Policy {
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test319State::Fail),
            "pass" => Some(Test319State::Pass),
            "s0" => Some(Test319State::S0),
            _ => None,
        }
    }
        ...
        match state {
            Test319State::S0 => {
                ...
                'action_block: {
                    ...
                    if self.safe_evaluate_guard("(_typeof(_event) ~= \"undefined\")", engine) {
