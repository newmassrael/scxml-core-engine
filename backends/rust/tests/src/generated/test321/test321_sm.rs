...
impl Test321Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test321Event] = &[];
}
...
impl StatePolicy for Test321Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test321State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test321State::Fail),
            "pass" => Some(Test321State::Pass),
            "s0" => Some(Test321State::S0),
            _ => None,
        }
    }
            ...
            Test321State::S0 => {
                if event == Test321Event::Null {
                    if self.safe_evaluate_guard("(_typeof(Var1) ~= \"undefined\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test321State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test321Event::Null {
                    if self.safe_evaluate_guard("true", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test321State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
