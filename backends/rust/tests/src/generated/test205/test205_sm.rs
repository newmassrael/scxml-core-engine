...
impl Test205Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test205Event] = &[Test205Event::Event1];
}
...
impl StatePolicy for Test205Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test205State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test205State::Fail),
            "pass" => Some(Test205State::Pass),
            "s0" => Some(Test205State::S0),
            "s1" => Some(Test205State::S1),
            _ => None,
        }
    }
                ...
                if event == Test205Event::Event1 {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test205State::S1)],
                            transition_index: 0,
                            has_actions: true,
                            is_internal: false,
                        });
                ...
                {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test205State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test205State::S1 => {
                if event == Test205Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test205State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
