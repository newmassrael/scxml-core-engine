...
// From: resources/337/test337.scxml
//
...
//! Generated Rust state machine for `test337`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test337Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test337Event] = &[];
}
...
impl Test337Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test337State::S0,
            session_id: None,
...
impl Default for Test337Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test337Policy {
    type State = Test337State;
    type Event = Test337Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test337State::Fail => true,
            Test337State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test337State::Fail => "fail",
            Test337State::Pass => "pass",
            Test337State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test337State::Fail),
            "pass" => Some(Test337State::Pass),
            "s0" => Some(Test337State::S0),
            _ => None,
        }
    }
        ...
        match state {
            Test337State::S0 => {
                ...
                'action_block: {
                    ...
                    engine.raise(sce_rust_runtime::EventWithMetadata::new(Test337Event::Foo));
                }
            }
            _ => {}
        }
...
impl Test337Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test337State,
        event: Test337Event,
        current_state: &mut Test337State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test337State::Fail => false,
            Test337State::Pass => false,
            Test337State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test337Event::Foo {
                    ...
                    *current_state = Test337State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test337Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test337State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
