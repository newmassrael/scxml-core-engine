...
impl Test558Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test558Event] = &[];
}
    ...
    fn do_initialize_data_model(&mut self, engine: &mut Engine<Self>) {
        ...
        // W3C SCXML 5.2.2: Load variable 'var2' from src (global)
        {
            ...
            if let Err(e) = sce_rust_runtime::helpers::datamodel_init::initialize_variable_from_src(
                ...
                engine.raise(sce_rust_runtime::EventWithMetadata::platform_error(
                    Test558Event::ErrorExecution,
                    "<data id='var2' src='file:test558.txt'> could not be loaded",
                ));
            }
        }
...
impl StatePolicy for Test558Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test558State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test558State::Fail),
            "pass" => Some(Test558State::Pass),
            "s0" => Some(Test558State::S0),
            "s1" => Some(Test558State::S1),
            _ => None,
        }
    }
        ...
        match state {
            Test558State::S0 => {
                if event == Test558Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(var1, \"this is a string\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test558State::S1)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test558Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test558State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
            ...
            Test558State::S1 => {
                if event == Test558Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(var2, \"this is a string\")", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test558State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
