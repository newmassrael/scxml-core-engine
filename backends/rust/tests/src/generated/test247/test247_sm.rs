...
// From: resources/247/test247.scxml
//
...
//! Generated Rust state machine for `test247`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test247Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test247Event] = &[Test247Event::Timeout];
}
...
pub struct Test247Policy {
    ...
    pending_invokes: Vec<sce_rust_runtime::invoke::PendingInvoke<Test247State>>,
    ...
    // W3C SCXML 6.4: Static invoke child '_invoke_0' (test247__sce_synth_invoke__invoke_0)
    child_invoke_0: Option<
        Box<
            sce_rust_runtime::Engine<
                super::test247__sce_synth_invoke__invoke_0_sm::Test247SceSynthInvokeInvoke0Policy,
            >,
        >,
    >,
...
impl Test247Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test247State::S0,
            session_id: None,
    ...
    fn do_execute_pending_invokes(&mut self, engine: &mut sce_rust_runtime::Engine<Self>) {
        ...
        for pending in &invokes_to_execute {
            if pending.invoke_id.contains("._invoke_0") {
                ...
                let mut child_policy = super::test247__sce_synth_invoke__invoke_0_sm::Test247SceSynthInvokeInvoke0Policy::new();
...
impl Default for Test247Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test247Policy {
    type State = Test247State;
    type Event = Test247Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test247State::Fail => true,
            Test247State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test247Event::CancelInvoke => "cancel.invoke",
            Test247Event::DoneInvoke => "done.invoke",
            Test247Event::ErrorExecution => "error.execution",
            Test247Event::Timeout => "timeout",
            Test247Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "cancel.invoke" => Some(Test247Event::CancelInvoke),
            "done.invoke" => Some(Test247Event::DoneInvoke),
            "error.execution" => Some(Test247Event::ErrorExecution),
            "timeout" => Some(Test247Event::Timeout),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test247State::Fail => "fail",
            Test247State::Pass => "pass",
            Test247State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test247State::Fail),
            "pass" => Some(Test247State::Pass),
            "s0" => Some(Test247State::S0),
            _ => None,
        }
    }
...
impl Test247Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test247State,
        event: Test247Event,
        current_state: &mut Test247State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test247State::Fail => false,
            Test247State::Pass => false,
            Test247State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test247Event::DoneInvoke {
                    ...
                    *current_state = Test247State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test247Event::Timeout {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test247State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
