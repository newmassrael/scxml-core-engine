...
impl Test554Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test554Event] = &[Test554Event::Timer];
}
...
impl StatePolicy for Test554Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test554State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test554State::Fail),
            "pass" => Some(Test554State::Pass),
            "s0" => Some(Test554State::S0),
            _ => None,
        }
    }
                ...
                if event == Test554Event::Timer {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test554State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test554Event::DoneInvoke {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test554State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
