...
impl Test445Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test445Event] = &[];
}
...
impl StatePolicy for Test445Policy {
    ...
    fn get_document_initial_targets(
        ...
        &[::sce_rust_runtime::EntryTarget::State(Test445State::S0)]
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test445State::Fail),
            "pass" => Some(Test445State::Pass),
            "s0" => Some(Test445State::S0),
            _ => None,
        }
    }
        ...
        match state {
            Test445State::S0 => {
                if event == Test445Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(var1, nil)", engine) {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test445State::Pass)],
                            transition_index: 0,
                            has_actions: false,
                            is_internal: false,
                        });
                ...
                if event == Test445Event::Null {
                    {
                        return Some(::sce_rust_runtime::EnabledTransition {
                            source: state,
                            targets: &[::sce_rust_runtime::EntryTarget::State(Test445State::Fail)],
                            transition_index: 1,
                            has_actions: false,
                            is_internal: false,
                        });
