...
// From: resources/178/test178.scxml
//
...
//! Generated Rust state machine for `test178`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test178.scxml:10 :: _machine"]
...
impl Test178Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test178Event] = &[Test178Event::Event1];
}
...
pub struct Test178Policy {
    ...
    last_transition_source_state: Test178State,
...
impl Test178Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test178State::S0,
...
impl StatePolicy for Test178Policy {
    type State = Test178State;
    type Event = Test178Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test178State::Fail => true,
            Test178State::Final => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test178State::Fail => "fail",
            Test178State::Final => "final",
            Test178State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test178State::Fail),
            "final" => Some(Test178State::Final),
            "s0" => Some(Test178State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        let event_data_string: String = {
                            ...
                            match se.evaluate_expression(&sid, "2") {
                                Ok(val) => {
                                    wire_params.entry("Var1".to_string()).or_default().push(val);
                                }
                                Err(e) => {
                                    ::sce_rust_runtime::sce_log_error!(
                                        "send param 'Var1' eval failed: {}",
                                        e
                                    );
                                    engine.raise(
                                        sce_rust_runtime::EventWithMetadata::platform_error(
                                            Test178Event::ErrorExecution,
                                            "<send> <param name='Var1'> expr failed to evaluate",
                                        ),
                                    );
                                }
                            }
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test178Event::Event1);
...
impl Test178Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test178State,
        event: Test178Event,
        current_state: &mut Test178State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test178State::Fail => false,
            Test178State::Final => false,
            Test178State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test178Event::Event1 {
                    ...
                    *current_state = Test178State::Final;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test178Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test178State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
