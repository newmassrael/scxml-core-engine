...
// From: resources/522/test522.scxml
//
...
//! Generated Rust state machine for `test522`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
#![doc = "SCE-MAP: test522.scxml:6 :: _machine"]
// SCE-MAP: test522.scxml:6 :: _machine
...
impl Test522Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test522Event] =
        &[Test522Event::Error, Test522Event::Timeout];
}
...
impl Test522Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test522State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test522", &io_processors) {
...
impl StatePolicy for Test522Policy {
    type State = Test522State;
    type Event = Test522Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test522State::Fail => true,
            Test522State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test522Event::Error => "error",
            Test522Event::ErrorCommunication => "error.communication",
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error" => Some(Test522Event::Error),
            "error.communication" => Some(Test522Event::ErrorCommunication),
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test522State::Fail => "fail",
            Test522State::Pass => "pass",
            Test522State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test522State::Fail),
            "pass" => Some(Test522State::Pass),
            "s0" => Some(Test522State::S0),
            _ => None,
        }
    }
    ...
    fn execute_entry_actions(
        ...
        match state {
            Test522State::S0 => {
                // SCE-MAP: test522.scxml:8 :: s0 :: _state_body
...
impl Test522Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test522State,
        event: Test522Event,
        current_state: &mut Test522State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test522State::Fail => false,
            Test522State::Pass => false,
            Test522State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test522Event::Timeout {
                    ...
                    *current_state = Test522State::Fail;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Precomputed prefix match for "error"
                if event == Test522Event::Error
                    || event == Test522Event::ErrorCommunication
                    || event == Test522Event::ErrorExecution
                {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    ...
                    )
                {
                    ...
                    *current_state = Test522State::Pass;
                    *transition_taken = true;
                    return true;
                }
                false
