...
// From: resources/185/test185.scxml
//
...
//! Generated Rust state machine for `test185`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test185Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test185Event] =
        &[Test185Event::Event1, Test185Event::Event2];
}
...
impl Test185Policy {
    pub fn new() -> Self {
        Self {
            ...
            last_transition_source_state: Test185State::S0,
            session_id: None,
...
impl Default for Test185Policy {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StatePolicy for Test185Policy {
    type State = Test185State;
    type Event = Test185Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test185State::Fail => true,
            Test185State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test185State::Fail => "fail",
            Test185State::Pass => "pass",
            Test185State::S0 => "s0",
            Test185State::S1 => "s1",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test185State::Fail),
            "pass" => Some(Test185State::Pass),
            "s0" => Some(Test185State::S0),
            "s1" => Some(Test185State::S1),
            _ => None,
        }
    }
            ...
            Test185State::S0 => {
                ...
                'action_block: {
...

                    {
                        ...
                        {
                            let mut meta =
                                sce_rust_runtime::EventWithMetadata::new(Test185Event::Event1);
...
impl Test185Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test185State,
        event: Test185Event,
        current_state: &mut Test185State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test185State::Fail => false,
            Test185State::Pass => false,
            Test185State::S0 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test185Event::Event1 {
                    ...
                    *current_state = Test185State::S1;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
                if event != Test185Event::Null
                    ...
                    )
                {
                    ...
                    *current_state = Test185State::Fail;
                    *transition_taken = true;
                    return true;
                }
                false
            ...
            Test185State::S1 => {
                // W3C SCXML 3.12: Event-triggered transitions (document order)
                // W3C SCXML 5.9.3: Direct enum comparison
                if event == Test185Event::Event2 {
                    // W3C SCXML 3.4: Track transition metadata
                    self.last_transition_source_state = check_state;
                    self.last_transition_is_internal = false;
                    self.last_transition_is_targetless = false;

                    *current_state = Test185State::Pass;
                    *transition_taken = true;
                    return true;
                }
                // W3C SCXML 5.9.3: Runtime event descriptor matching
