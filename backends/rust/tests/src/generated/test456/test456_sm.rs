...
// From: resources/456/test456.scxml
//
...
//! Generated Rust state machine for `test456`.
//!
//! Generated by SCE Code Generator (Rust backend).
...
impl Test456Event {
    ...
    /// on the name.
    pub const EXTERNALLY_DRIVABLE_EVENTS: &'static [Test456Event] = &[];
}
...
impl Test456Policy {
    ...
    pub fn new(script_engine: std::sync::Arc<dyn sce_rust_runtime::IScriptEngine>) -> Self {
        Self {
            ...
            last_transition_source_state: Test456State::S0,
    ...
    fn ensure_script_engine(&mut self) {
        ...
        if let Err(e) = se.setup_system_variables(&sid, "test456", &io_processors) {
...
impl StatePolicy for Test456Policy {
    type State = Test456State;
    type Event = Test456Event;
    ...
    fn is_final_state(state: Self::State) -> bool {
        match state {
            Test456State::Fail => true,
            Test456State::Pass => true,
            _ => false,
        }
    }
    ...
    fn get_event_name(event: Self::Event) -> &'static str {
        match event {
            Test456Event::ErrorExecution => "error.execution",
            Test456Event::Null => "",
        }
    }
    ...
    fn get_event_from_name(name: &str) -> Option<Self::Event> {
        match name {
            "error.execution" => Some(Test456Event::ErrorExecution),
            _ => None,
        }
    }
    ...
    fn get_state_name(state: Self::State) -> &'static str {
        match state {
            Test456State::Fail => "fail",
            Test456State::Pass => "pass",
            Test456State::S0 => "s0",
        }
    }
    ...
    fn get_state_from_name(name: &str) -> Option<Self::State> {
        match name {
            "fail" => Some(Test456State::Fail),
            "pass" => Some(Test456State::Pass),
            "s0" => Some(Test456State::S0),
            _ => None,
        }
    }
                ...
                'action_block: {
                    {
                        ...
                        let se = self.script_engine.clone();
                        let se: &dyn sce_rust_runtime::IScriptEngine = &*se;
                        if let Err(e) = se.execute_script(&sid, "Var1 = _scxml_add(Var1, 1)") {
...
impl Test456Policy {
    ...
    fn try_transition_in_state(
        &mut self,
        check_state: Test456State,
        event: Test456Event,
        current_state: &mut Test456State,
        transition_taken: &mut bool,
        engine: &mut sce_rust_runtime::Engine<Self>,
    ) -> bool {
        match check_state {
            Test456State::Fail => false,
            Test456State::Pass => false,
            Test456State::S0 => {
                // W3C SCXML 3.13: Eventless transitions
                if event == Test456Event::Null {
                    if self.safe_evaluate_guard("_scxml_eq(Var1, 1)", engine) {
                        ...
                        *current_state = Test456State::Pass;
                        *transition_taken = true;
                        ...
                        self.last_transition_is_internal = false;
                        self.last_transition_is_targetless = false;
                        *current_state = Test456State::Fail;
                        *transition_taken = true;
                        return true;
                    }
                }
