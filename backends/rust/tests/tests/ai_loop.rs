...
/// defaults cannot tell the two apart, and that is why the 27 scenarios
/// written before this one did not.
#[test]
fn an_internal_region_root_transition_leaves_the_sibling_region() {
    let mut e = started();
    ...
    step(&mut e, AiLoopEvent::SessionLost);
    assert!(
        holds(&e, AiLoopState::Rebuilding),
        "precondition: the liveness watch has to be off its default, or nothing \
         below can tell a domain that spared it from one that reset it; active: {:?}",
        active(&e)
    );
    ...
    assert!(
        holds(&e, AiLoopState::Paused),
        "the transition's own target is entered whichever domain the engine \
         resolved, so this half failing means it did not fire at all; active: {:?}",
        active(&e)
    );
    assert!(
        holds(&e, AiLoopState::Rebuilding) && !holds(&e, AiLoopState::Alive),
        "an internal region-root transition has the region as its domain, so the \
         watch keeps what it saw; reading `alive` here means the domain reached \
         the document root and every region was restarted underneath the cycle; \
         active: {:?}",
        active(&e)
    );
}
