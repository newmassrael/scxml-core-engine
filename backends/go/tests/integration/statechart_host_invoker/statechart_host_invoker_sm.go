// SCE-GENERATED — DO NOT EDIT
// source-hash: fcc36ca5ace7ff619d1d0a3cef283e3a12d9eaf17570ba300dc27c1de6383a2f


// SPDX-License-Identifier: MIT
// SPDX-FileCopyrightText: Copyright (c) 2025-2026 [Author of input SCXML file]
//
// Generated by SCE Code Generator (Go backend)
// From: sce-build/tests/fixtures/host_processor/statechart_host_invoker.scxml
//
// This generated code is MIT licensed and may be freely used in any project.
// Runtime engine dependency (sce-go-runtime): LGPL-2.1-or-later WITH LicenseRef-SCE-Linking-Exception OR LicenseRef-SCE-Commercial
// Full license: https://github.com/newmassrael/scxml-core-engine/blob/main/LICENSE
// Generated Go state machine for `statechart_host_invoker`.
//
// Generated by SCE Code Generator (Go backend).
// Implements sce.StatePolicy with full transition logic,
// entry/exit actions, and event processing.


// SCE-MAP: statechart_host_invoker.scxml:106 :: _machine

package statechart_host_invoker

import (
	"fmt"
	"path/filepath"
	"strings"
	"time"

	sce "github.com/newmassrael/sce-go-runtime"
)

// Ensure imports are used
var (
	_ = fmt.Sprintf
	_ = filepath.Join
	_ = strings.HasPrefix
	_ = time.Duration(0)
)

// ======================================================================
// State type (W3C SCXML 3.3)
// ======================================================================

type StatechartHostInvokerState int

const (
	StatechartHostInvokerStateDone StatechartHostInvokerState = 0
	StatechartHostInvokerStateEvaluating StatechartHostInvokerState = 1
	StatechartHostInvokerStateInvoking StatechartHostInvokerState = 2
	StatechartHostInvokerStateLocating StatechartHostInvokerState = 3
	StatechartHostInvokerStateTimed StatechartHostInvokerState = 4
	StatechartHostInvokerStateTyped StatechartHostInvokerState = 5
)

func (s StatechartHostInvokerState) String() string {
	switch s {
	case StatechartHostInvokerStateDone:
		return "done"
	case StatechartHostInvokerStateEvaluating:
		return "evaluating"
	case StatechartHostInvokerStateInvoking:
		return "invoking"
	case StatechartHostInvokerStateLocating:
		return "locating"
	case StatechartHostInvokerStateTimed:
		return "timed"
	case StatechartHostInvokerStateTyped:
		return "typed"
	}
	return "unknown"
}

// StatechartHostInvokerStateFromName is the read half of the pair above — a state
// id back into the state it names (W3C SCXML 3.3).
//
// A host that persists where a machine was has to write it down as TEXT: the
// constants above are a build artefact of one binary, and the process that
// resumes is a different one. String publishes the name and this reads it back,
// which is what lets a journal survive its own record and reach
// sce.Engine.EnterAt.
//
// The second return is false for a name this document does not declare. A name
// guessed at rather than refused is how a restore reaches a configuration
// nobody recorded — and the refusal is what makes a typo in a journal a
// reported failure instead of a machine quietly somewhere else.
//
// Emitted from the same loop over the document's states as String, so the two
// age together.
func StatechartHostInvokerStateFromName(name string) (StatechartHostInvokerState, bool) {
	switch name {
	case "done":
		return StatechartHostInvokerStateDone, true
	case "evaluating":
		return StatechartHostInvokerStateEvaluating, true
	case "invoking":
		return StatechartHostInvokerStateInvoking, true
	case "locating":
		return StatechartHostInvokerStateLocating, true
	case "timed":
		return StatechartHostInvokerStateTimed, true
	case "typed":
		return StatechartHostInvokerStateTyped, true
	}
	var zero StatechartHostInvokerState
	return zero, false
}

// StatechartHostInvokerAllStates is every state this document declares, in the
// order the constants above are issued.
//
// Emitted from the same loop, so a walk over it is a walk over the document
// rather than over a list somebody maintained beside it — a test that spells
// its own list goes on passing when the document grows a state.
var StatechartHostInvokerAllStates = []StatechartHostInvokerState{
	StatechartHostInvokerStateDone,
	StatechartHostInvokerStateEvaluating,
	StatechartHostInvokerStateInvoking,
	StatechartHostInvokerStateLocating,
	StatechartHostInvokerStateTimed,
	StatechartHostInvokerStateTyped,
}

// StatechartHostInvokerTarget is one token of a target list, as the document wrote
// it (W3C SCXML 3.13): a state, or a <history> the engine dereferences.
type StatechartHostInvokerTarget = sce.EntryTarget[StatechartHostInvokerState, sce.HistoryID]

// ======================================================================
// Document structure (W3C SCXML 3.2-3.4, 3.10)
//
// Package-level tables, because the structure is a fact about the document
// and not about a run: the engine's Appendix D procedures read them through
// the policy methods below, and a transition's target list is handed out as
// a slice of them rather than rebuilt on every selection.
// ======================================================================

// childStatesOfStatechartHostInvoker is §scxml-D-getChildStates per state: its
// <state>, <parallel> and <final> children, in document order.
var childStatesOfStatechartHostInvoker = [6][]StatechartHostInvokerState{
}

// initialTargetsOfStatechartHostInvoker is each compound state's initial transition
// target, as written (§scxml-3.3).
var initialTargetsOfStatechartHostInvoker = [6][]StatechartHostInvokerTarget{
}

// documentInitialTargetsOfStatechartHostInvoker is the target of the document's own
// initial transition, as written (§scxml-3.2).
var documentInitialTargetsOfStatechartHostInvoker = []StatechartHostInvokerTarget{sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateInvoking)}

// transitionTargetsOfStatechartHostInvoker is each transition's target list, as
// written (§scxml-3.13), by source state and the transition's index among its
// source's own transitions. A targetless transition's entry is empty.
var transitionTargetsOfStatechartHostInvoker = [6][][]StatechartHostInvokerTarget{
	StatechartHostInvokerStateDone: {
		1: {sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateInvoking)},
	},
	StatechartHostInvokerStateInvoking: {
		3: {sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateDone)},
		4: {sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateEvaluating)},
		5: {sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateLocating)},
		6: {sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateTimed)},
		7: {sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateTyped)},
	},
	StatechartHostInvokerStateTimed: {
		2: {sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateDone)},
	},
	StatechartHostInvokerStateTyped: {
		3: {sce.StateTarget[StatechartHostInvokerState, sce.HistoryID](StatechartHostInvokerStateTyped)},
	},
}

// ======================================================================
// Event type (W3C SCXML 3.12)
// ======================================================================

type StatechartHostInvokerEvent int

const (
	StatechartHostInvokerEventAgain StatechartHostInvokerEvent = 0
	StatechartHostInvokerEventDoneInvoke StatechartHostInvokerEvent = 1
	StatechartHostInvokerEventDoneInvokePerm StatechartHostInvokerEvent = 2
	StatechartHostInvokerEventDoneInvokeProbe StatechartHostInvokerEvent = 3
	StatechartHostInvokerEventDoneInvokeProbe2 StatechartHostInvokerEvent = 4
	StatechartHostInvokerEventDoneInvokeSlow StatechartHostInvokerEvent = 5
	StatechartHostInvokerEventErrorExecution StatechartHostInvokerEvent = 6
	StatechartHostInvokerEventErrorInvoke StatechartHostInvokerEvent = 7
	StatechartHostInvokerEventErrorInvokeSlow StatechartHostInvokerEvent = 8
	StatechartHostInvokerEventEvaluate StatechartHostInvokerEvent = 9
	StatechartHostInvokerEventForget StatechartHostInvokerEvent = 10
	StatechartHostInvokerEventLeak StatechartHostInvokerEvent = 11
	StatechartHostInvokerEventLeave StatechartHostInvokerEvent = 12
	StatechartHostInvokerEventLocate StatechartHostInvokerEvent = 13
	StatechartHostInvokerEventPing StatechartHostInvokerEvent = 14
	StatechartHostInvokerEventRetype StatechartHostInvokerEvent = 15
	StatechartHostInvokerEventTime StatechartHostInvokerEvent = 16
	StatechartHostInvokerEventType StatechartHostInvokerEvent = 17
	// W3C SCXML 3.13: Sentinel for eventless transition dispatch
	StatechartHostInvokerEventNull StatechartHostInvokerEvent = 18
)

func (e StatechartHostInvokerEvent) String() string {
	switch e {
	case StatechartHostInvokerEventAgain:
		return "again"
	case StatechartHostInvokerEventDoneInvoke:
		return "done.invoke"
	case StatechartHostInvokerEventDoneInvokePerm:
		return "done.invoke.perm"
	case StatechartHostInvokerEventDoneInvokeProbe:
		return "done.invoke.probe"
	case StatechartHostInvokerEventDoneInvokeProbe2:
		return "done.invoke.probe2"
	case StatechartHostInvokerEventDoneInvokeSlow:
		return "done.invoke.slow"
	case StatechartHostInvokerEventErrorExecution:
		return "error.execution"
	case StatechartHostInvokerEventErrorInvoke:
		return "error.invoke"
	case StatechartHostInvokerEventErrorInvokeSlow:
		return "error.invoke.slow"
	case StatechartHostInvokerEventEvaluate:
		return "evaluate"
	case StatechartHostInvokerEventForget:
		return "forget"
	case StatechartHostInvokerEventLeak:
		return "leak"
	case StatechartHostInvokerEventLeave:
		return "leave"
	case StatechartHostInvokerEventLocate:
		return "locate"
	case StatechartHostInvokerEventPing:
		return "ping"
	case StatechartHostInvokerEventRetype:
		return "retype"
	case StatechartHostInvokerEventTime:
		return "time"
	case StatechartHostInvokerEventType:
		return "type"
	case StatechartHostInvokerEventNull:
		return ""
	}
	return "unknown"
}

// ── NL→IR Item C1 Path A: typed `_event.data` payload channel ─────────
// NL→IR Item C1 Path A (EventSchema MCU native lowering): typed
// `_event.data` payload channel for the EventSchema-imported events whose
// transition guards lowered to a native Go comparison (no script engine).
// The Go twin of the Rust `StatechartHostInvokerPayload` enum / C11 tagged union: a tag
// plus one `pending<Event>Payload` field per guarded event on the policy.
type StatechartHostInvokerPayloadTag int

const (
	StatechartHostInvokerPayloadTagNone StatechartHostInvokerPayloadTag = iota
	StatechartHostInvokerPayloadTagDoneInvokePerm
)

// StatechartHostInvokerDoneInvokePermPayload is the NL→IR Item C1 Path A typed `_event.data`
// payload for `done.invoke.perm`. Internal to the package; consumers inject it via
// the `RaiseDoneInvokePerm` seam below (fields are unexported by design).
type StatechartHostInvokerDoneInvokePermPayload struct {
	granted bool
}


// ======================================================================
// Policy struct
// ======================================================================

type StatechartHostInvokerPolicy struct {
	// NL→IR Item C1 Path A: the current event's typed `_event.data` payload,
	// lifted from the dequeued event by PopulateEventMetadata and read by the
	// native transition guards (`p.pending<Event>Payload.<field>`). Mirrors
	// the Rust policy's `pending_payload` / the C11 `sm->pending_payload`.
	pendingPayloadTag StatechartHostInvokerPayloadTag
	pendingDoneInvokePermPayload StatechartHostInvokerDoneInvokePermPayload

	// W3C SCXML 5.10.1: External event flag
	nextEventIsExternal bool
	pendingEventName string
	pendingEventData string
	pendingEventType string
	pendingEventSendid string
	pendingEventOrigin string
	pendingEventOrigintype string
	pendingEventInvokeid string
	// W3C SCXML 5.10: Session ID
	SessionID string
	// W3C SCXML B.1: Script engine handle (per-instance, Path B+ Q1=(d) Go=interface ref)
	ScriptEngine sce.IScriptEngine
	scriptEngineInitialized bool
	// W3C SCXML C.2.3: inbound BasicHTTP endpoint serving this machine,
	// declared by the deployment before Initialize(). The address belongs to
	// whoever runs the listener, so the machine takes it from here rather than
	// guessing one. Empty means no such endpoint is deployed, and no BasicHTTP
	// entry is published in _ioprocessors.
	BasicHTTPAccessURI string
	pendingInvokes []sce.PendingInvoke[StatechartHostInvokerState]
	activeInvokes  map[string]*sce.ChildSession
	// W3C SCXML 6.4: Parent communication
	ParentExternalQueue chan sce.ParentEvent
	InvokeID           string
	ChildSessionID     string
	// §scxml-6.4.3: invoke param VALUES staged by the parent, applied after
	// this machine's datamodel init so they override its `<data>` defaults.
	pendingParams      map[string]interface{}
}

// NewStatechartHostInvokerPolicy creates a new policy with default values.
func NewStatechartHostInvokerPolicy() StatechartHostInvokerPolicy {
	return StatechartHostInvokerPolicy{
		pendingInvokes: make([]sce.PendingInvoke[StatechartHostInvokerState], 0),
		activeInvokes:  make(map[string]*sce.ChildSession),
	}
}

// Started reports what the `started` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `started` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Started() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "started")
}

// Started2 reports what the `started2` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `started2` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Started2() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "started2")
}

// Refused reports what the `refused` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `refused` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Refused() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "refused")
}

// Ended reports what the `ended` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `ended` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Ended() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "ended")
}

// Entered reports what the `entered` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `entered` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Entered() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "entered")
}

// Destination reports what the `destination` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `destination` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Destination() (string, bool) {
	return sce.ReadDatamodelString(p.ScriptEngine, p.SessionID, "destination")
}

// N reports what the `n` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `n` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) N() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "n")
}

// Dropped reports what the `dropped` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `dropped` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Dropped() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "dropped")
}

// DoneId reports what the `doneId` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `doneId` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) DoneId() (string, bool) {
	return sce.ReadDatamodelString(p.ScriptEngine, p.SessionID, "doneId")
}

// Matched reports what the `matched` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `matched` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Matched() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "matched")
}

// Slot reports what the `slot` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `slot` was assigned a value of another type, or the engine refused.
//
// The value as JSON text, serialised by the engine's own JSON.stringify
// (§scxml-B-2) so the key order is the document's.
func (p *StatechartHostInvokerPolicy) Slot() (string, bool) {
	return sce.ReadDatamodelJSON(p.ScriptEngine, p.SessionID, "slot")
}

// Slotted reports what the `slotted` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `slotted` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Slotted() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "slotted")
}

// Pinged reports what the `pinged` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `pinged` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Pinged() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "pinged")
}

// Leaked reports what the `leaked` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `leaked` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Leaked() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "leaked")
}

// Lost reports what the `lost` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `lost` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Lost() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "lost")
}

// Expired reports what the `expired` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `expired` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Expired() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "expired")
}

// Finished reports what the `finished` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `finished` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Finished() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "finished")
}

// Misdated reports what the `misdated` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `misdated` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Misdated() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "misdated")
}

// Granted reports what the `granted` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `granted` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Granted() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "granted")
}

// Denied reports what the `denied` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `denied` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Denied() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "denied")
}

// Unreadable reports what the `unreadable` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `unreadable` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Unreadable() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "unreadable")
}

// Scope reports what the `scope` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `scope` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Scope() (string, bool) {
	return sce.ReadDatamodelString(p.ScriptEngine, p.SessionID, "scope")
}

// Level reports what the `level` datamodel variable is holding now
// (W3C SCXML 5.3).
//
// The live value, not the authored one: `<assign>` writes into the session, so
// a reader frozen at generation time would answer the document's literal for
// the whole run. The second return value is false when the machine cannot
// answer — no script engine is set, the session is not initialised yet,
// `level` was assigned a value of another type, or the engine refused.
func (p *StatechartHostInvokerPolicy) Level() (int64, bool) {
	return sce.ReadDatamodelInt(p.ScriptEngine, p.SessionID, "level")
}




// ensureScriptEngine lazily initializes the script engine session.
//
// Engine DI Parity RFC: panics if ScriptEngine was not set on the policy
// before Initialize() — mirrors Kotlin's mandatory constructor parameter
// contract. Silent no-op would otherwise turn missed injection into a
// hidden runtime regression (every <assign>/<log>/guard becomes a no-op
// and the SM walks to its default-state final without diagnostic).
func (p *StatechartHostInvokerPolicy) ensureScriptEngine() {
	if p.scriptEngineInitialized {
		return
	}
	engine := p.ScriptEngine
	if engine == nil {
		panic("StatechartHostInvokerPolicy: ScriptEngine field must be set before Initialize()")
	}
	sessionID := p.SessionID
	if sessionID == "" {
		sessionID = sce.GenerateSessionID()
		p.SessionID = sessionID
	}
	if !engine.HasSession(sessionID) {
		_ = engine.CreateSession(sessionID)
	}
	// W3C SCXML C.1.1 / C.2.3: the _ioprocessors entries come from the same
	// helper every other backend uses, so a machine reads the same entry names
	// and the same addresses whichever one runs it.
	_ = engine.SetupSystemVariables(sessionID, "statechart_host_invoker",
		sce.BuildIoProcessors(sessionID, p.BasicHTTPAccessURI))
	p.scriptEngineInitialized = true
}

// InitializeDataModel sets up datamodel via script engine (W3C SCXML 5.3).
func (p *StatechartHostInvokerPolicy) InitializeDataModel(eng *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) {
	p.ensureScriptEngine()
	engine := p.ScriptEngine
	sessionID := p.SessionID

	// W3C SCXML 5.3: Initialize _sessionid
	_ = engine.SetVariable(sessionID, "_sessionid", sessionID)

	// W3C SCXML 5.3: Initialize _name
	_ = engine.SetVariable(sessionID, "_name", "statechart_host_invoker")

	// W3C SCXML 5.2.2: Initialize global datamodel variables
	// W3C SCXML 5.2/5.3: Initialize started from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "started", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='started'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "started", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize started2 from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "started2", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='started2'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "started2", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize refused from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "refused", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='refused'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "refused", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize ended from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "ended", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='ended'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "ended", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize entered from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "entered", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='entered'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "entered", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize destination from expr="'pane://dyn'"
	{
		result, err := engine.EvaluateExpression(sessionID, `"pane://dyn"`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "destination", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='destination'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "destination", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize n from expr="7"
	{
		result, err := engine.EvaluateExpression(sessionID, `7`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "n", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='n'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "n", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize dropped from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "dropped", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='dropped'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "dropped", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize doneId from expr="''"
	{
		result, err := engine.EvaluateExpression(sessionID, `""`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "doneId", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='doneId'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "doneId", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize matched from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "matched", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='matched'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "matched", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize slot from expr="({ id: '', sid: '' })"
	{
		result, err := engine.EvaluateExpression(sessionID, `{["id"] = "", ["sid"] = ""}`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "slot", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='slot'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "slot", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize slotted from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "slotted", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='slotted'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "slotted", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize pinged from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "pinged", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='pinged'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "pinged", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize leaked from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "leaked", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='leaked'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "leaked", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize lost from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "lost", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='lost'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "lost", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize expired from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "expired", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='expired'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "expired", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize finished from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "finished", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='finished'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "finished", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize misdated from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "misdated", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='misdated'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "misdated", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize granted from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "granted", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='granted'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "granted", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize denied from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "denied", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='denied'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "denied", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize unreadable from expr="0"
	{
		result, err := engine.EvaluateExpression(sessionID, `0`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "unreadable", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='unreadable'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "unreadable", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize scope from expr="'calendar'"
	{
		result, err := engine.EvaluateExpression(sessionID, `"calendar"`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "scope", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='scope'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "scope", nil)
		}
	}
	// W3C SCXML 5.2/5.3: Initialize level from expr="2"
	{
		result, err := engine.EvaluateExpression(sessionID, `2`)
		if err == nil {
			_ = engine.SetVariable(sessionID, "level", result)
		} else {
			eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<data id='level'> expr failed to evaluate"))
			_ = engine.SetVariable(sessionID, "level", nil)
		}
	}



	// §scxml-6.4.3: apply the invoke params the parent staged — overriding the
	// `<data>` default this block just seeded, and only for a name the child
	// declares ("if the names do not match, the Processor MUST NOT add the
	// value ... to the invoked session's data model"). `HasVariable` is that
	// filter, and it is this channel's only copy of the rule.
	if len(p.pendingParams) > 0 {
		for paramName, paramValue := range p.pendingParams {
			if engine.HasVariable(sessionID, paramName) {
				_ = engine.SetVariable(sessionID, paramName, paramValue)
			}
		}
		p.pendingParams = nil
	}
}

// evaluateGuard evaluates a guard condition via script engine.
// W3C SCXML 4.3.2: If a condition expression cannot be evaluated, the processor
// MUST treat it as false and MUST raise error.execution.
func (p *StatechartHostInvokerPolicy) evaluateGuard(guard string, eng *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) bool {
	p.ensureScriptEngine()
	engine := p.ScriptEngine
	result, err := engine.EvaluateExpression(p.SessionID, guard)
	if err != nil {
		eng.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "a <transition> cond failed to evaluate"))
		return false
	}
	return sce.ScriptToBool(result)
}

// executeScript executes a script block via script engine.
func (p *StatechartHostInvokerPolicy) executeScript(script string) error {
	p.ensureScriptEngine()
	engine := p.ScriptEngine
	return engine.ExecuteScript(p.SessionID, script)
}

// setCurrentEvent binds _event system variable (W3C SCXML 5.10) and answers
// which rung of §scxml-B-2-8-1 the payload got, because the binding is the
// only place that knows.
//
// This used to be `_ = engine.SetCurrentEvent(...)`: the ladder decided
// between a DOM, a value and a space-normalized string, and the decision was
// dropped one line later. A payload that announced structure and would not
// parse therefore reached the document as raw characters, every
// _event.data.<field> read empty, and nothing anywhere could say so — see
// sce.PayloadReading.
func (p *StatechartHostInvokerPolicy) setCurrentEvent(name string) sce.PayloadReading {
	p.ensureScriptEngine()
	engine := p.ScriptEngine
	data := p.pendingEventData
	eventType := p.pendingEventType
	sendID := p.pendingEventSendid
	// W3C SCXML C.1: `_event.origin` is the sender's published
	// `_ioprocessors` location, not its bare session id — and this is the one
	// place that publishes `_event` to the document, so this is where the id
	// becomes a location. The engine keeps the bare id in
	// `EventMetadata.Origin` because its session-keyed lookups (<finalize>
	// dispatch, cancelled-invoke filtering) match on it; converting at the
	// raise would make one value serve two consumers that need different
	// spellings. The conversion itself lives in `sce.PublishedOrigin`, the
	// port of the `IOProcessorHelper::publishedOrigin` the C++ engines share:
	// a second spelling of the rule is how the backends would stop agreeing.
	origin := sce.PublishedOrigin(p.pendingEventOrigin)
	originType := p.pendingEventOrigintype
	invokeID := p.pendingEventInvokeid
	reading, err := engine.SetCurrentEvent(p.SessionID, sce.SetCurrentEventArgs{
		Name:       name,
		Data:       data,
		Type:       eventType,
		SendID:     sendID,
		Origin:     origin,
		OriginType: originType,
		InvokeID:   invokeID,
	})
	if err != nil {
		// An engine that could not bind _event at all has a larger problem
		// than an unreadable payload, and reporting PayloadUndecodable for it
		// would point a host at the wrong thing.
		return sce.PayloadAbsent
	}
	return reading
}

// assignVariable assigns a value to a datamodel variable via script engine.
func (p *StatechartHostInvokerPolicy) assignVariable(location, expr string) error {
	p.ensureScriptEngine()
	engine := p.ScriptEngine
	script := location + " = " + expr
	return engine.ExecuteScript(p.SessionID, script)
}

// storeIDInLocation writes a generated id to an `idlocation` (W3C SCXML
// 6.2.4, 6.4.1). The location is a location expression, so this is the
// assignment `<assign>` makes: luaLocation is already lowered, so a member
// path lands, and the error is the caller's to report as error.execution
// (W3C SCXML 5.9.2). The id is quoted here because an invoke's is a run-time
// value.
func (p *StatechartHostInvokerPolicy) storeIDInLocation(luaLocation, id string) error {
	return p.assignVariable(luaLocation, sce.QuoteLuaString(id))
}

// getVariable reads a datamodel variable from the script engine.
func (p *StatechartHostInvokerPolicy) getVariable(name string) interface{} {
	p.ensureScriptEngine()
	engine := p.ScriptEngine
	result, err := engine.GetVariable(p.SessionID, name)
	if err != nil {
		return nil
	}
	return result
}

// SetParamValueInScriptEngine stores an invoke param's VALUE for deferred
// application after datamodel init (§scxml-6.4.3).
//
// Params are applied at the end of InitializeDataModel, overriding the
// `<data>` default only for names the child declares. The value crosses as a
// value: this took the parent's value rendered as Lua source and
// re-evaluated it here until 2026-08-21, which survived every value a test
// had tried and lost the ones Lua cannot spell — `ToScriptLiteral` renders a
// non-finite number as `+Inf`, which is not a Lua expression, so an
// `<invoke>` `<param expr="1/0"/>` arrived as nothing at all.
func (p *StatechartHostInvokerPolicy) SetParamValueInScriptEngine(name string, value interface{}) {
	if p.pendingParams == nil {
		p.pendingParams = make(map[string]interface{})
	}
	p.pendingParams[name] = value
}


// ExecutePendingInvokes starts deferred child state machines (W3C SCXML 6.4).
// Port of Rust do_execute_pending_invokes().
func (p *StatechartHostInvokerPolicy) ExecutePendingInvokes(engine *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) {
	if len(p.pendingInvokes) == 0 {
		return
	}

	// W3C SCXML 6.4: Copy-and-clear to prevent iterator invalidation
	invokesToExecute := make([]sce.PendingInvoke[StatechartHostInvokerState], len(p.pendingInvokes))
	copy(invokesToExecute, p.pendingInvokes)
	p.pendingInvokes = p.pendingInvokes[:0]

	for _, pending := range invokesToExecute {
		if pending.DocumentID == "done._invoke_0" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			p.ensureScriptEngine()
			// W3C SCXML 6.4.1: the invocation's generated id goes to
			// `idlocation` when the element is evaluated. It is the id the host
			// is handed and `done.invoke.<id>` names, so the document can match
			// on what it stored. The write is the assignment `<assign>` makes, so
			// a location that cannot take the id starts nothing (W3C SCXML
			// 5.9.2), like any argument that cannot be evaluated.
			if err := p.storeIDInLocation(`doneId`, "done._invoke_0"); err != nil {
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke idlocation='doneId'> could not take the invoke id"))
				continue
			}
			hostInvokeParams := map[string][]string{}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "done._invoke_0",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "req" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			p.ensureScriptEngine()
			if val, err := p.ScriptEngine.EvaluateExpression(p.SessionID, `destination`); err == nil {
				hostInvokeSrc = sce.ToWireString(val)
			} else {
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke srcexpr='destination'> could not be evaluated"))
				continue
			}
			hostInvokeParams := map[string][]string{}
			// W3C SCXML 6.4.1: a namelist location that is not declared, or
			// does not evaluate, starts nothing — the rule the scxml invoke
			// reads its namelist by.
			namelistFailed := false
			if !namelistFailed {
				if !p.ScriptEngine.HasVariable(p.SessionID, "n") {
					namelistFailed = true
				} else if val, err := p.ScriptEngine.EvaluateExpression(p.SessionID, `n`); err == nil {
					hostInvokeParams["n"] = append(hostInvokeParams["n"], sce.ToWireString(val))
				} else {
					namelistFailed = true
				}
			}
			if namelistFailed {
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> namelist names a location that could not be read"))
				continue
			}
			hostInvokeParams["twice"] = append(hostInvokeParams["twice"], "a")
			if paramVal, paramErr := p.ScriptEngine.EvaluateExpression(p.SessionID, `_scxml_add(n, 1)`); paramErr == nil {
				// W3C SCXML C.2: the value crosses as text, rendered by the
				// neutral helper — an engine literal would put this machine's
				// language on the wire.
				hostInvokeParams["twice"] = append(hostInvokeParams["twice"], sce.ToWireString(paramVal))
			} else {
				// W3C SCXML 5.7.1: the pair is dropped AND the failure is
				// reported.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> <param name='twice'> expr failed to evaluate"))
			}
			if paramVal, paramErr := p.ScriptEngine.EvaluateExpression(p.SessionID, `n.nope.deeper`); paramErr == nil {
				// W3C SCXML C.2: the value crosses as text, rendered by the
				// neutral helper — an engine literal would put this machine's
				// language on the wire.
				hostInvokeParams["bad"] = append(hostInvokeParams["bad"], sce.ToWireString(paramVal))
			} else {
				// W3C SCXML 5.7.1: the pair is dropped AND the failure is
				// reported.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> <param name='bad'> expr failed to evaluate"))
			}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "req",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "req2" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			p.ensureScriptEngine()
			if val, err := p.ScriptEngine.EvaluateExpression(p.SessionID, `("body:" .. _scxml_tostring(n))`); err == nil {
				hostInvokeContent = sce.ToWireString(val)
			} else {
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke contentexpr=''body:' + n'> could not be evaluated"))
				continue
			}
			hostInvokeParams := map[string][]string{}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "req2",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "req3" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			p.ensureScriptEngine()
			if val, err := p.ScriptEngine.EvaluateExpression(p.SessionID, `n.nope.deeper`); err == nil {
				hostInvokeSrc = sce.ToWireString(val)
			} else {
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke srcexpr='n.nope.deeper'> could not be evaluated"))
				continue
			}
			hostInvokeParams := map[string][]string{}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "req3",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "probe" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := "pane://turn"
			hostInvokeContent := ""
			hostInvokeParams := map[string][]string{}
			hostInvokeParams["within"] = append(hostInvokeParams["within"], "2500")
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "probe",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "probe2" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := "pane://other"
			hostInvokeContent := ""
			hostInvokeParams := map[string][]string{}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "probe2",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "locating._invoke_1" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			p.ensureScriptEngine()
			// W3C SCXML 6.4.1: the invocation's generated id goes to
			// `idlocation` when the element is evaluated. It is the id the host
			// is handed and `done.invoke.<id>` names, so the document can match
			// on what it stored. The write is the assignment `<assign>` makes, so
			// a location that cannot take the id starts nothing (W3C SCXML
			// 5.9.2), like any argument that cannot be evaluated.
			if err := p.storeIDInLocation(`slot.id`, "locating._invoke_1"); err != nil {
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke idlocation='slot.id'> could not take the invoke id"))
				continue
			}
			hostInvokeParams := map[string][]string{}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "locating._invoke_1",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "locating._invoke_2" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			p.ensureScriptEngine()
			// W3C SCXML 6.4.1: the invocation's generated id goes to
			// `idlocation` when the element is evaluated. It is the id the host
			// is handed and `done.invoke.<id>` names, so the document can match
			// on what it stored. The write is the assignment `<assign>` makes, so
			// a location that cannot take the id starts nothing (W3C SCXML
			// 5.9.2), like any argument that cannot be evaluated.
			if err := p.storeIDInLocation(`n.nope.deeper`, "locating._invoke_2"); err != nil {
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke idlocation='n.nope.deeper'> could not take the invoke id"))
				continue
			}
			hostInvokeParams := map[string][]string{}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "locating._invoke_2",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "slow" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			p.ensureScriptEngine()
			hostInvokeParams := map[string][]string{}
			if paramVal, paramErr := p.ScriptEngine.EvaluateExpression(p.SessionID, `50`); paramErr == nil {
				// W3C SCXML C.2: the value crosses as text, rendered by the
				// neutral helper — an engine literal would put this machine's
				// language on the wire.
				hostInvokeParams["_sce_deadline_ms"] = append(hostInvokeParams["_sce_deadline_ms"], sce.ToWireString(paramVal))
			} else {
				// W3C SCXML 5.7.1: the pair is dropped AND the failure is
				// reported.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> <param name='_sce_deadline_ms'> expr failed to evaluate"))
			}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "slow",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "undated" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			hostInvokeParams := map[string][]string{}
			hostInvokeParams["_sce_deadline_ms"] = append(hostInvokeParams["_sce_deadline_ms"], "soon")
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "undated",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
		if pending.DocumentID == "perm" {
			// W3C SCXML 6.4.1: the host declared this `type`, so START the
			// invocation rather than refusing it. The id handed over is the
			// DOCUMENT's, not the per-instance one the pending queue carries:
			// `done.invoke.<id>` is the name the author wrote a transition
			// for, so it is the name the host must answer on.
			// W3C SCXML 6.4.1: what the request says is evaluated now, when the
			// invocation starts, by the rules the other invoke kinds follow: an
			// attribute that cannot be evaluated starts nothing, and a `<param>`
			// that cannot is reported and left out (W3C SCXML 5.7.1).
			hostInvokeSrc := ""
			hostInvokeContent := ""
			p.ensureScriptEngine()
			hostInvokeParams := map[string][]string{}
			if paramVal, paramErr := p.ScriptEngine.EvaluateExpression(p.SessionID, `scope`); paramErr == nil {
				// W3C SCXML C.2: the value crosses as text, rendered by the
				// neutral helper — an engine literal would put this machine's
				// language on the wire.
				hostInvokeParams["scope"] = append(hostInvokeParams["scope"], sce.ToWireString(paramVal))
			} else {
				// W3C SCXML 5.7.1: the pair is dropped AND the failure is
				// reported.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> <param name='scope'> expr failed to evaluate"))
			}
			if paramVal, paramErr := p.ScriptEngine.EvaluateExpression(p.SessionID, `level`); paramErr == nil {
				// W3C SCXML C.2: the value crosses as text, rendered by the
				// neutral helper — an engine literal would put this machine's
				// language on the wire.
				hostInvokeParams["level"] = append(hostInvokeParams["level"], sce.ToWireString(paramVal))
			} else {
				// W3C SCXML 5.7.1: the pair is dropped AND the failure is
				// reported.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> <param name='level'> expr failed to evaluate"))
			}
if !engine.PerformHostInvoke(sce.HostInvokeRequest{
				ProcessorType: "x-sce-host",
				InvokeID:      "perm",
				Src:           hostInvokeSrc,
				Params:        hostInvokeParams,
				Content:       hostInvokeContent,
			}) {
				// W3C SCXML 6.4.1: declared but no invoker registered. The
				// document asked for a process to be run and none was, which
				// is the same fact as an unsupported type — so the same
				// event, rather than a silence that reads as started.
				engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<invoke> names an invoker the host declared but never registered"))
			}
			continue
		}
	}
}

// TickChildren ticks child state machines (W3C SCXML 6.4).
// Port of Rust do_tick_children().
func (p *StatechartHostInvokerPolicy) TickChildren(engine *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) {
}




// ── Child engine wrappers (implement sce.ChildEngine interface) ──



// ======================================================================
// StatePolicy interface implementation
// ======================================================================

// InitialState returns the initial state (W3C SCXML 3.3).
func (p *StatechartHostInvokerPolicy) InitialState() StatechartHostInvokerState {
	return StatechartHostInvokerStateInvoking
}

// IsFinalState returns true if state is a <final> state (W3C SCXML 3.7).
func (p *StatechartHostInvokerPolicy) IsFinalState(state StatechartHostInvokerState) bool {
	switch state {
	}
	return false
}

// GetParent returns the parent of state, or (zero, false) if root (W3C SCXML 3.3).
func (p *StatechartHostInvokerPolicy) GetParent(state StatechartHostInvokerState) (StatechartHostInvokerState, bool) {
	switch state {
	}
	return 0, false
}

// IsCompoundState returns true if state is a <state> with child states — exactly
// the states that have an initial transition. A <parallel> is not compound
// (W3C SCXML 3.3).
func (p *StatechartHostInvokerPolicy) IsCompoundState(state StatechartHostInvokerState) bool {
	return len(initialTargetsOfStatechartHostInvoker[state]) > 0
}

func (p *StatechartHostInvokerPolicy) IsParallelState(_ StatechartHostInvokerState) bool { return false }

// GetChildStates returns state's <state>, <parallel> and <final> children, in
// document order — for a <parallel>, its regions (§scxml-D-getChildStates).
func (p *StatechartHostInvokerPolicy) GetChildStates(state StatechartHostInvokerState) []StatechartHostInvokerState {
	return childStatesOfStatechartHostInvoker[state]
}

// GetInitialTargets returns a compound state's initial transition target, as
// written; the engine's entry procedures dereference a <history> among them
// (W3C SCXML 3.3).
func (p *StatechartHostInvokerPolicy) GetInitialTargets(state StatechartHostInvokerState) []StatechartHostInvokerTarget {
	return initialTargetsOfStatechartHostInvoker[state]
}

// GetDocumentInitialTargets returns the target of the document's own initial
// transition, as written (W3C SCXML 3.2).
func (p *StatechartHostInvokerPolicy) GetDocumentInitialTargets() []StatechartHostInvokerTarget {
	return documentInitialTargetsOfStatechartHostInvoker
}

// W3C SCXML 3.10: this document declares no <history>, so no target list names
// one and the engine never asks the two below; answering would mean inventing
// one.
func (p *StatechartHostInvokerPolicy) GetHistoryParent(history sce.HistoryID) StatechartHostInvokerState {
	panic(fmt.Sprintf("StatechartHostInvokerPolicy declares no <history>; asked for %d", history))
}
func (p *StatechartHostInvokerPolicy) GetHistoryDefaultTargets(history sce.HistoryID) []StatechartHostInvokerTarget {
	panic(fmt.Sprintf("StatechartHostInvokerPolicy declares no <history>; asked for %d", history))
}
func (p *StatechartHostInvokerPolicy) HistoryValue(_ sce.HistoryID) ([]StatechartHostInvokerState, bool) {
	return nil, false
}

// GetDocumentOrder returns the document order index (W3C SCXML Appendix D).
func (p *StatechartHostInvokerPolicy) GetDocumentOrder(state StatechartHostInvokerState) int {
	switch state {
	case StatechartHostInvokerStateDone:
		return 2
	case StatechartHostInvokerStateEvaluating:
		return 1
	case StatechartHostInvokerStateInvoking:
		return 0
	case StatechartHostInvokerStateLocating:
		return 3
	case StatechartHostInvokerStateTimed:
		return 4
	case StatechartHostInvokerStateTyped:
		return 5
	}
	return -1
}

// GetEventName returns the string name of an event (W3C SCXML 3.12).
func (p *StatechartHostInvokerPolicy) GetEventName(event StatechartHostInvokerEvent) string {
	return event.String()
}

// GetEventFromName looks up an event by name (W3C SCXML 3.12).
func (p *StatechartHostInvokerPolicy) GetEventFromName(name string) (StatechartHostInvokerEvent, bool) {
	switch name {
	case "again":
		return StatechartHostInvokerEventAgain, true
	case "done.invoke":
		return StatechartHostInvokerEventDoneInvoke, true
	case "done.invoke.perm":
		return StatechartHostInvokerEventDoneInvokePerm, true
	case "done.invoke.probe":
		return StatechartHostInvokerEventDoneInvokeProbe, true
	case "done.invoke.probe2":
		return StatechartHostInvokerEventDoneInvokeProbe2, true
	case "done.invoke.slow":
		return StatechartHostInvokerEventDoneInvokeSlow, true
	case "error.execution":
		return StatechartHostInvokerEventErrorExecution, true
	case "error.invoke":
		return StatechartHostInvokerEventErrorInvoke, true
	case "error.invoke.slow":
		return StatechartHostInvokerEventErrorInvokeSlow, true
	case "evaluate":
		return StatechartHostInvokerEventEvaluate, true
	case "forget":
		return StatechartHostInvokerEventForget, true
	case "leak":
		return StatechartHostInvokerEventLeak, true
	case "leave":
		return StatechartHostInvokerEventLeave, true
	case "locate":
		return StatechartHostInvokerEventLocate, true
	case "ping":
		return StatechartHostInvokerEventPing, true
	case "retype":
		return StatechartHostInvokerEventRetype, true
	case "time":
		return StatechartHostInvokerEventTime, true
	case "type":
		return StatechartHostInvokerEventType, true
	}
	return StatechartHostInvokerEventNull, false
}

// HostInvokeIDs lists the invokes the host runs (sce.HostInvokePolicy). W3C
// SCXML 6.4: their `done.invoke` is accepted only through
// Engine.CompleteHostInvoke.
func (p *StatechartHostInvokerPolicy) HostInvokeIDs() []string {
	return []string{
		"probe",
		"probe2",
		"req",
		"req2",
		"req3",
		"done._invoke_0",
		"locating._invoke_1",
		"locating._invoke_2",
		"slow",
		"undated",
		"perm",
	}
}

// GetStateName returns the string name of a state (W3C SCXML 3.3).
func (p *StatechartHostInvokerPolicy) GetStateName(state StatechartHostInvokerState) string {
	return state.String()
}

// GetStateFromName reads a state id back into the state it names (W3C SCXML 3.3).
// The reverse of GetStateName, and what turns a host's recorded configuration
// back into the argument sce.Engine.EnterAt takes.
func (p *StatechartHostInvokerPolicy) GetStateFromName(name string) (StatechartHostInvokerState, bool) {
	return StatechartHostInvokerStateFromName(name)
}

// NullEvent returns the sentinel for eventless transition dispatch (W3C SCXML 3.13).
func (p *StatechartHostInvokerPolicy) NullEvent() StatechartHostInvokerEvent {
	return StatechartHostInvokerEventNull
}


// SetNextEventIsExternal sets the external event flag (W3C SCXML 5.10.1).
func (p *StatechartHostInvokerPolicy) SetNextEventIsExternal(value bool) {
	p.nextEventIsExternal = value
}

// HasParallelStates returns whether the SM has parallel states.
func (p *StatechartHostInvokerPolicy) HasParallelStates() bool {
	return false
}

// NeedsScriptEngine returns whether the SM needs a script engine.
func (p *StatechartHostInvokerPolicy) NeedsScriptEngine() bool {
	return true
}

// NeedsEventScheduler reports whether a host must drive this machine with
// Tick rather than Step alone.
//
// W3C SCXML 6.2: Step runs a macrostep and never drains the delayed-send
// scheduler nor ticks an invoked child, so a host that only ever calls it
// waits forever with nothing said. Same verdict the generate manifest
// publishes as needs_event_scheduler.
func (p *StatechartHostInvokerPolicy) NeedsEventScheduler() bool {
	return true
}

// NeedsDataModelInit returns whether the SM needs datamodel init.
func (p *StatechartHostInvokerPolicy) NeedsDataModelInit() bool {
	return true
}

// HasInvokeSupport returns true for invoke-capable SMs.
func (p *StatechartHostInvokerPolicy) HasInvokeSupport() bool { return true }
func (p *StatechartHostInvokerPolicy) HasChildTick() bool { return false }
func (p *StatechartHostInvokerPolicy) HasFinalize() bool { return false }
func (p *StatechartHostInvokerPolicy) HasAutoforward() bool { return false }
func (p *StatechartHostInvokerPolicy) HasActiveStates() bool { return false }
func (p *StatechartHostInvokerPolicy) GetActiveStates() []StatechartHostInvokerState { return nil }
// W3C SCXML 3.4: this machine keeps no active set — its configuration is the
// parent walk from the current state — so there is nothing for a restore to
// hand back here. sce.Engine.EnterAt reaches this only through HasActiveStates,
// which is false above; the method exists because the interface is one contract.
func (p *StatechartHostInvokerPolicy) SetActiveStates(_ []StatechartHostInvokerState) {}
func (p *StatechartHostInvokerPolicy) HasExternalEventFlag() bool { return true }
// ExecuteFinalizeForChildEvent is a no-op (no finalize invokes).
func (p *StatechartHostInvokerPolicy) ExecuteFinalizeForChildEvent(_ *sce.EventWithMetadata[StatechartHostInvokerEvent], _ *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) {}
// ForwardToAutoforwardChildren is a no-op (no autoforward invokes).
func (p *StatechartHostInvokerPolicy) ForwardToAutoforwardChildren(_ string, _ sce.EventMetadata, _ *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) {}

// PopulateEventMetadata stores pending event metadata (W3C SCXML 5.10).
// Note: event name is set separately via setCurrentEvent(), not from metadata.
func (p *StatechartHostInvokerPolicy) PopulateEventMetadata(meta *sce.EventMetadata) {
	// NL→IR Item C1 Path A: lift the dequeued event's typed payload from the
	// type-erased carrier into the typed policy field the native guards read
	// (twin of the C11 pop loop's `sm->pending_payload = evt.payload`).
	p.pendingPayloadTag = StatechartHostInvokerPayloadTagNone
	switch v := meta.TypedPayload.(type) {
	case StatechartHostInvokerDoneInvokePermPayload:
		p.pendingPayloadTag = StatechartHostInvokerPayloadTagDoneInvokePerm
		p.pendingDoneInvokePermPayload = v
	}

	p.pendingEventData = meta.Data
	p.pendingEventType = meta.EventType.String()
	p.pendingEventSendid = meta.SendID
	p.pendingEventOrigin = meta.Origin
	p.pendingEventOrigintype = meta.OriginType
	p.pendingEventInvokeid = meta.InvokeID
}

// LiftTypedPayload binds the dequeued event's typed `_event.data` view from
// the data it carries (NL→IR Item C1 Path A).
//
// The typed carrier PopulateEventMetadata reads is filled by one producer, the
// generated Raise<Event> inject seam. Every other producer — <send> with
// <param>, an invoke forwarding either way, autoforward, BasicHTTP, mesh —
// fills EventMetadata.Data, so the schema's fields are read out of that here.
// A refusal is returned for the engine to raise as error.execution, which is
// what W3C SCXML 3.13 gives for a guard that cannot be evaluated.
func (p *StatechartHostInvokerPolicy) LiftTypedPayload(event StatechartHostInvokerEvent, meta *sce.EventMetadata) error {
	// A typed carrier already bound the view, so the producer was the
	// inject seam and there is nothing to read out of `Data`.
	if p.pendingPayloadTag != StatechartHostInvokerPayloadTagNone {
		return nil
	}
	switch event {
	case StatechartHostInvokerEventDoneInvokePerm:
		fields, err := sce.LiftPayload(meta.Data)
		if err != nil {
			return err
		}
		var payload StatechartHostInvokerDoneInvokePermPayload
		if payload.granted, err = sce.PayloadBool(fields, "granted"); err != nil {
			return err
		}
		p.pendingDoneInvokePermPayload = payload
		p.pendingPayloadTag = StatechartHostInvokerPayloadTagDoneInvokePerm
	}
	return nil

}

// ClearEventMetadata resets pending event metadata (W3C SCXML 5.10).
func (p *StatechartHostInvokerPolicy) ClearEventMetadata() {
	// NL→IR Item C1 Path A: reset the typed-payload tag (PopulateEventMetadata
	// always re-establishes it for the next event; this keeps a stale payload
	// from being read between macrosteps).
	p.pendingPayloadTag = StatechartHostInvokerPayloadTagNone

	p.pendingEventName = ""
	p.pendingEventData = ""
	p.pendingEventType = ""
	p.pendingEventSendid = ""
	p.pendingEventOrigin = ""
	p.pendingEventOrigintype = ""
	p.pendingEventInvokeid = ""
}



// ExecuteEntryActions enters one state (W3C SCXML 3.8): adds it to the
// configuration, runs its <onentry>, and its <initial> transition's content when
// its initial state is entered by default.
//line statechart_host_invoker.scxml:106
func (p *StatechartHostInvokerPolicy) ExecuteEntryActions(state StatechartHostInvokerState, engine *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent], isDefaultEntry bool) {
	p.ensureScriptEngine()
	switch state {
	case StatechartHostInvokerStateDone:
		//line statechart_host_invoker.scxml:187
		// W3C SCXML 3.8: onentry block 0 (break on error stops subsequent actions)
		for actionBlock0 := 0; actionBlock0 < 1; actionBlock0++ {
			_ = actionBlock0

	// W3C SCXML 5.3: <assign location="ended" expr="ended + 1">
	if err := p.assignVariable(`ended`, `_scxml_add(ended, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'ended' failed"))
		break
	}

		}
		// W3C SCXML 6.4.1: `type` names no processor this platform implements.
		// Defer only — the error.execution raise happens in
		// ExecutePendingInvokes, so §scxml-6.4 ordering holds and
		// CancelInvokesForState drops the entry if the state exits first.
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.done._invoke_0", "done", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateDone,
				DocumentID: "done._invoke_0",
			})
		}
	case StatechartHostInvokerStateEvaluating:
		//line statechart_host_invoker.scxml:171
		// W3C SCXML 6.4.1: `type` names no processor this platform implements.
		// Defer only — the error.execution raise happens in
		// ExecutePendingInvokes, so §scxml-6.4 ordering holds and
		// CancelInvokesForState drops the entry if the state exits first.
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.req", "evaluating", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateEvaluating,
				DocumentID: "req",
			})
		}
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.req2", "evaluating", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateEvaluating,
				DocumentID: "req2",
			})
		}
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.req3", "evaluating", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateEvaluating,
				DocumentID: "req3",
			})
		}
	case StatechartHostInvokerStateInvoking:
		//line statechart_host_invoker.scxml:143
		// W3C SCXML 3.8: onentry block 0 (break on error stops subsequent actions)
		for actionBlock0 := 0; actionBlock0 < 1; actionBlock0++ {
			_ = actionBlock0

	// W3C SCXML 5.3: <assign location="entered" expr="entered + 1">
	if err := p.assignVariable(`entered`, `_scxml_add(entered, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'entered' failed"))
		break
	}

		}
		// W3C SCXML 6.4.1: `type` names no processor this platform implements.
		// Defer only — the error.execution raise happens in
		// ExecutePendingInvokes, so §scxml-6.4 ordering holds and
		// CancelInvokesForState drops the entry if the state exits first.
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.probe", "invoking", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateInvoking,
				DocumentID: "probe",
			})
		}
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.probe2", "invoking", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateInvoking,
				DocumentID: "probe2",
			})
		}
	case StatechartHostInvokerStateLocating:
		//line statechart_host_invoker.scxml:198
		// W3C SCXML 3.8: onentry block 0 (break on error stops subsequent actions)
		for actionBlock0 := 0; actionBlock0 < 1; actionBlock0++ {
			_ = actionBlock0

	// W3C SCXML 6.2: send id="__send_0"
	// W3C SCXML 6.2.4: the send id goes to `idlocation`, through the
	// assignment `<assign>` makes. A location that cannot take it is an
	// argument that cannot be evaluated, so the message is discarded and
	// error.execution raised (W3C SCXML 6.2, 5.9.2).
	if err := p.storeIDInLocation(`slot.sid`, "__send_0"); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<send idlocation='slot.sid'> could not take the send id"))
	} else {
	{
		eventDataStr := ""
		_ = eventDataStr
	// W3C SCXML 6.2: External send
	{
		meta := sce.NewEventWithMetadata(StatechartHostInvokerEventPing)
		meta.Metadata = sce.ExternalMetadata("__send_0", "")
		meta.Metadata.Data = eventDataStr
		engine.RaiseExternalWithMeta(meta)
	}
	}
	}


	// W3C SCXML 6.2: send id="__send_1"
	// W3C SCXML 6.2.4: the send id goes to `idlocation`, through the
	// assignment `<assign>` makes. A location that cannot take it is an
	// argument that cannot be evaluated, so the message is discarded and
	// error.execution raised (W3C SCXML 6.2, 5.9.2).
	if err := p.storeIDInLocation(`n.nope.deeper`, "__send_1"); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<send idlocation='n.nope.deeper'> could not take the send id"))
	} else {
	{
		eventDataStr := ""
		_ = eventDataStr
	// W3C SCXML 6.2: External send
	{
		meta := sce.NewEventWithMetadata(StatechartHostInvokerEventLeak)
		meta.Metadata = sce.ExternalMetadata("__send_1", "")
		meta.Metadata.Data = eventDataStr
		engine.RaiseExternalWithMeta(meta)
	}
	}
	}

		}
		// W3C SCXML 6.4.1: `type` names no processor this platform implements.
		// Defer only — the error.execution raise happens in
		// ExecutePendingInvokes, so §scxml-6.4 ordering holds and
		// CancelInvokesForState drops the entry if the state exits first.
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.locating._invoke_1", "locating", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateLocating,
				DocumentID: "locating._invoke_1",
			})
		}
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.locating._invoke_2", "locating", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateLocating,
				DocumentID: "locating._invoke_2",
			})
		}
	case StatechartHostInvokerStateTimed:
		//line statechart_host_invoker.scxml:220
		// W3C SCXML 6.4.1: `type` names no processor this platform implements.
		// Defer only — the error.execution raise happens in
		// ExecutePendingInvokes, so §scxml-6.4 ordering holds and
		// CancelInvokesForState drops the entry if the state exits first.
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.slow", "timed", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateTimed,
				DocumentID: "slow",
			})
		}
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.undated", "timed", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateTimed,
				DocumentID: "undated",
			})
		}
	case StatechartHostInvokerStateTyped:
		//line statechart_host_invoker.scxml:254
		// W3C SCXML 6.4.1: `type` names no processor this platform implements.
		// Defer only — the error.execution raise happens in
		// ExecutePendingInvokes, so §scxml-6.4 ordering holds and
		// CancelInvokesForState drops the entry if the state exits first.
		{
			generatedInvokeID := fmt.Sprintf("%s.%d.perm", "typed", sce.NextInvokeCounter())
			sce.DeferInvoke(&p.pendingInvokes, sce.PendingInvoke[StatechartHostInvokerState]{
				InvokeID:   generatedInvokeID,
				State:      StatechartHostInvokerStateTyped,
				DocumentID: "perm",
			})
		}
	default:
		// No entry actions
	}
}

// ExecuteHistoryDefaultContent runs a <history>'s default transition content
// (W3C SCXML 3.10.2), after its parent's onentry (and after the parent's own
// <initial> content) when the history was taken with nothing recorded. The
// engine asks for it by the entry set's defaultHistoryContent answer; a history
// that restored what it recorded runs nothing.
//line statechart_host_invoker.scxml:106
func (p *StatechartHostInvokerPolicy) ExecuteHistoryDefaultContent(history sce.HistoryID, engine *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) {
	// W3C SCXML 3.10.2: no <history> in this document has default content.
}

// ExecuteExitActions exits one state (W3C SCXML 3.9): records its histories,
// runs its <onexit>, cancels its invocations and removes it from the
// configuration — §scxml-D-exitStates's order.
//line statechart_host_invoker.scxml:106
func (p *StatechartHostInvokerPolicy) ExecuteExitActions(state StatechartHostInvokerState, engine *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent], configurationBeforeExit []StatechartHostInvokerState) {
	p.ensureScriptEngine()
	// §scxml-D-exitStates orders one state's exit as onexit, then
	// cancelInvoke, then configuration.delete(s), so `In(s)` inside s's own
	// handler is true: the <onexit> switch comes first and the removal from
	// the configuration last.
	switch state {
	default:
		// No exit actions
	}
	// W3C SCXML 6.4: Cancel pending invokes and cleanup active children on state exit
	switch state {
	case StatechartHostInvokerStateDone:
		sce.CancelInvokesForState(&p.pendingInvokes, StatechartHostInvokerStateDone)
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "done._invoke_0")
	case StatechartHostInvokerStateEvaluating:
		sce.CancelInvokesForState(&p.pendingInvokes, StatechartHostInvokerStateEvaluating)
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "req")
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "req2")
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "req3")
	case StatechartHostInvokerStateInvoking:
		sce.CancelInvokesForState(&p.pendingInvokes, StatechartHostInvokerStateInvoking)
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "probe")
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "probe2")
	case StatechartHostInvokerStateLocating:
		sce.CancelInvokesForState(&p.pendingInvokes, StatechartHostInvokerStateLocating)
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "locating._invoke_1")
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "locating._invoke_2")
	case StatechartHostInvokerStateTimed:
		sce.CancelInvokesForState(&p.pendingInvokes, StatechartHostInvokerStateTimed)
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "slow")
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "undated")
	case StatechartHostInvokerStateTyped:
		sce.CancelInvokesForState(&p.pendingInvokes, StatechartHostInvokerStateTyped)
		// W3C SCXML 6.4: the host's invocation ends with the state that
		// started it. Unconditional here: the engine knows whether this one
		// ever started and stays silent when it did not, so the emitted chain
		// does not need its own bookkeeping.
		engine.CancelHostInvoke("x-sce-host", "perm")
	default:
	}
}



// BindCurrentEvent binds the event whose transitions are about to be selected as
// the _event their guards read (W3C SCXML 5.10) — before the first guard runs,
// and not for an eventless selection, which has no event of its own.
//line statechart_host_invoker.scxml:106
func (p *StatechartHostInvokerPolicy) BindCurrentEvent(event StatechartHostInvokerEvent, engine *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) {
	if event != StatechartHostInvokerEventNull {
		// §scxml-B-2-8-1: the rung the payload got, handed to the engine
		// rather than dropped. This is the only frame that has both the
		// reading and the event it belongs to.
		engine.NotePayloadReading(event, p.setCurrentEvent(p.GetEventName(event)))
	}
}

// FirstEnabledTransition is Appendix D selectTransitions, the half only this
// document can answer: the first of state's own transitions, in document order,
// that event enables and whose guard holds. The engine walks the atomic states
// and their ancestors and keeps the ordered set; the null event asks for
// eventless transitions.
//line statechart_host_invoker.scxml:106
func (p *StatechartHostInvokerPolicy) FirstEnabledTransition(state StatechartHostInvokerState, event StatechartHostInvokerEvent, engine *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) (sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID], bool) {
	switch state {
	case StatechartHostInvokerStateDone:
		if event == StatechartHostInvokerEventDoneInvoke || event == StatechartHostInvokerEventDoneInvokePerm || event == StatechartHostInvokerEventDoneInvokeProbe || event == StatechartHostInvokerEventDoneInvokeProbe2 || event == StatechartHostInvokerEventDoneInvokeSlow {
			if p.evaluateGuard(`(_event.invokeid == doneId)`, engine) {
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 0,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventAgain {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					Targets:         transitionTargetsOfStatechartHostInvoker[state][1],
					TransitionIndex: 1,
					HasActions:      false,
					IsInternal:      false,
				}, true
			}
		}
	case StatechartHostInvokerStateEvaluating:
		if event == StatechartHostInvokerEventErrorExecution {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 0,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
	case StatechartHostInvokerStateInvoking:
		if event == StatechartHostInvokerEventDoneInvokeProbe {
			if p.evaluateGuard(`(_event.invokeid == "probe")`, engine) {
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 0,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventDoneInvokeProbe2 {
			if p.evaluateGuard(`(_event.invokeid == "probe2")`, engine) {
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 1,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventErrorExecution {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 2,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventLeave {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					Targets:         transitionTargetsOfStatechartHostInvoker[state][3],
					TransitionIndex: 3,
					HasActions:      false,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventEvaluate {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					Targets:         transitionTargetsOfStatechartHostInvoker[state][4],
					TransitionIndex: 4,
					HasActions:      false,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventLocate {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					Targets:         transitionTargetsOfStatechartHostInvoker[state][5],
					TransitionIndex: 5,
					HasActions:      false,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventTime {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					Targets:         transitionTargetsOfStatechartHostInvoker[state][6],
					TransitionIndex: 6,
					HasActions:      false,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventType {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					Targets:         transitionTargetsOfStatechartHostInvoker[state][7],
					TransitionIndex: 7,
					HasActions:      false,
					IsInternal:      false,
				}, true
			}
		}
	case StatechartHostInvokerStateLocating:
		if event == StatechartHostInvokerEventDoneInvoke || event == StatechartHostInvokerEventDoneInvokePerm || event == StatechartHostInvokerEventDoneInvokeProbe || event == StatechartHostInvokerEventDoneInvokeProbe2 || event == StatechartHostInvokerEventDoneInvokeSlow {
			if p.evaluateGuard(`(_event.invokeid == slot.id)`, engine) {
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 0,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventPing {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 1,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventLeak {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 2,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventErrorExecution {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 3,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
	case StatechartHostInvokerStateTimed:
		if event == StatechartHostInvokerEventErrorExecution {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 0,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventForget {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 1,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventLeave {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					Targets:         transitionTargetsOfStatechartHostInvoker[state][2],
					TransitionIndex: 2,
					HasActions:      false,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventErrorInvokeSlow {
			if p.evaluateGuard(`((_event.invokeid == "slow") and (_event.data == "deadline"))`, engine) {
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 3,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventDoneInvokeSlow {
			if p.evaluateGuard(`(_event.invokeid == "slow")`, engine) {
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 4,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
	case StatechartHostInvokerStateTyped:
		if event == StatechartHostInvokerEventDoneInvokePerm {
			// NL→IR Item C1 Path A: native typed `_event.data` guard — cond
			// "_event.data.granted === true" lowered to a tag-checked field comparison on
			// the lifted `pending<Event>Payload` (no script engine).
			if p.pendingPayloadTag == StatechartHostInvokerPayloadTagDoneInvokePerm && (p.pendingDoneInvokePermPayload.granted == true) {
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 0,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventDoneInvokePerm {
			// NL→IR Item C1 Path A: native typed `_event.data` guard — cond
			// "_event.data.granted === false" lowered to a tag-checked field comparison on
			// the lifted `pending<Event>Payload` (no script engine).
			if p.pendingPayloadTag == StatechartHostInvokerPayloadTagDoneInvokePerm && (p.pendingDoneInvokePermPayload.granted == false) {
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 1,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventErrorExecution {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					TransitionIndex: 2,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
		if event == StatechartHostInvokerEventRetype {
			{
				return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{
					Source:          state,
					Targets:         transitionTargetsOfStatechartHostInvoker[state][3],
					TransitionIndex: 3,
					HasActions:      true,
					IsInternal:      false,
				}, true
			}
		}
	}
	return sce.EnabledTransition[StatechartHostInvokerState, sce.HistoryID]{}, false
}

// ExecuteTransitionContent runs one transition's executable content (W3C SCXML
// 3.13), between the microstep's exits and its entries.
//line statechart_host_invoker.scxml:106
func (p *StatechartHostInvokerPolicy) ExecuteTransitionContent(source StatechartHostInvokerState, transitionIndex int, engine *sce.Engine[StatechartHostInvokerState, StatechartHostInvokerEvent]) {
	p.ensureScriptEngine()
	switch source {
	case StatechartHostInvokerStateDone:
		switch transitionIndex {
		case 0:
			//line statechart_host_invoker.scxml:192

	// W3C SCXML 5.3: <assign location="matched" expr="matched + 1">
	if err := p.assignVariable(`matched`, `_scxml_add(matched, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'matched' failed"))
	}

		}
	case StatechartHostInvokerStateEvaluating:
		switch transitionIndex {
		case 0:
			//line statechart_host_invoker.scxml:182

	// W3C SCXML 5.3: <assign location="dropped" expr="dropped + 1">
	if err := p.assignVariable(`dropped`, `_scxml_add(dropped, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'dropped' failed"))
	}

		}
	case StatechartHostInvokerStateInvoking:
		switch transitionIndex {
		case 0:
			//line statechart_host_invoker.scxml:153

	// W3C SCXML 5.3: <assign location="started" expr="started + 1">
	if err := p.assignVariable(`started`, `_scxml_add(started, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'started' failed"))
	}

		case 1:
			//line statechart_host_invoker.scxml:156

	// W3C SCXML 5.3: <assign location="started2" expr="started2 + 1">
	if err := p.assignVariable(`started2`, `_scxml_add(started2, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'started2' failed"))
	}

		case 2:
			//line statechart_host_invoker.scxml:159

	// W3C SCXML 5.3: <assign location="refused" expr="refused + 1">
	if err := p.assignVariable(`refused`, `_scxml_add(refused, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'refused' failed"))
	}

		}
	case StatechartHostInvokerStateLocating:
		switch transitionIndex {
		case 0:
			//line statechart_host_invoker.scxml:206

	// W3C SCXML 5.3: <assign location="slotted" expr="slotted + 1">
	if err := p.assignVariable(`slotted`, `_scxml_add(slotted, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'slotted' failed"))
	}

		case 1:
			//line statechart_host_invoker.scxml:209

	// W3C SCXML 5.3: <assign location="pinged" expr="pinged + 1">
	if err := p.assignVariable(`pinged`, `_scxml_add(pinged, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'pinged' failed"))
	}

		case 2:
			//line statechart_host_invoker.scxml:212

	// W3C SCXML 5.3: <assign location="leaked" expr="leaked + 1">
	if err := p.assignVariable(`leaked`, `_scxml_add(leaked, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'leaked' failed"))
	}

		case 3:
			//line statechart_host_invoker.scxml:215

	// W3C SCXML 5.3: <assign location="lost" expr="lost + 1">
	if err := p.assignVariable(`lost`, `_scxml_add(lost, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'lost' failed"))
	}

		}
	case StatechartHostInvokerStateTimed:
		switch transitionIndex {
		case 0:
			//line statechart_host_invoker.scxml:227

	// W3C SCXML 5.3: <assign location="misdated" expr="misdated + 1">
	if err := p.assignVariable(`misdated`, `_scxml_add(misdated, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'misdated' failed"))
	}

		case 1:
			//line statechart_host_invoker.scxml:233

	{
		cancelSendIDVal, cancelErr := p.ScriptEngine.EvaluateExpression(p.SessionID, `""`)
		if cancelErr == nil && cancelSendIDVal != nil {
			engine.CancelEvent(fmt.Sprintf("%v", cancelSendIDVal))
		}
	}

		case 3:
			//line statechart_host_invoker.scxml:240

	// W3C SCXML 5.3: <assign location="expired" expr="expired + 1">
	if err := p.assignVariable(`expired`, `_scxml_add(expired, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'expired' failed"))
	}

		case 4:
			//line statechart_host_invoker.scxml:244

	// W3C SCXML 5.3: <assign location="finished" expr="finished + 1">
	if err := p.assignVariable(`finished`, `_scxml_add(finished, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'finished' failed"))
	}

		}
	case StatechartHostInvokerStateTyped:
		switch transitionIndex {
		case 0:
			//line statechart_host_invoker.scxml:259

	// W3C SCXML 5.3: <assign location="granted" expr="granted + 1">
	if err := p.assignVariable(`granted`, `_scxml_add(granted, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'granted' failed"))
	}

		case 1:
			//line statechart_host_invoker.scxml:262

	// W3C SCXML 5.3: <assign location="denied" expr="denied + 1">
	if err := p.assignVariable(`denied`, `_scxml_add(denied, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'denied' failed"))
	}

		case 2:
			//line statechart_host_invoker.scxml:265

	// W3C SCXML 5.3: <assign location="unreadable" expr="unreadable + 1">
	if err := p.assignVariable(`unreadable`, `_scxml_add(unreadable, 1)`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'unreadable' failed"))
	}

		case 3:
			//line statechart_host_invoker.scxml:268

	// W3C SCXML 5.3: <assign location="level" expr="'high'">
	if err := p.assignVariable(`level`, `"high"`); err != nil {
		engine.Raise(sce.NewPlatformError(StatechartHostInvokerEventErrorExecution, "<assign> to 'level' failed"))
	}

		}
	}
}
