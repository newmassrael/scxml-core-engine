...
"""W3C SCXML Appendix D: the invoke-before-dequeue order holds mid-run — Python AOT path.
...
Regeneration (after fixture or template edit):
  ``scripts/regen_invoke_precedes_dequeue_midrun_python.sh`` (local)
  ``sce-codegen generate-integration -l python --stem invoke_precedes_dequeue_midrun`` (CI)
"""
...
import invoke_precedes_dequeue_midrun_sm as _sm  # noqa: E402 — path inserted above


def test_invoke_precedes_dequeue_midrun_aot() -> None:
    ...
    assert engine.reached_final, (
        "invoke_precedes_dequeue_midrun did not reach a top-level <final> within "
