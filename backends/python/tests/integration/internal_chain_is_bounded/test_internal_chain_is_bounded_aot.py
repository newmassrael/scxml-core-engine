...
def test_a_dequeue_that_selected_nothing_spends_no_budget() -> None:
    ...
    that matched none is the loop turn the clause does not count. The
    fixture's `unanswered` chain is `bounded` with one unmatched event added
    per link, so the two differ in exactly that and must cost the same.
    ...
    engine.send_event(_Event.UNANSWERED)

    assert _counter(engine, se, "ignores") == MAX_MICROSTEPS, (
    ...
    assert engine.truncated_macrosteps() == 0, (
        ...
        "microsteps: an engine that counted the discards refuses this "
        "document at link five hundred and reports a runaway that is not one"
    )
    assert engine.last_truncated_macrostep_state() is None, (
        "and nothing names a state, because nothing was stopped"
    )
